...
// Находит или создает ссылку на справочник ESD_ВидыУпаковки и заполняет ее в структуру.
//
// Параметры:
...
Процедура ЗаполнитьВидУпаковки(СтруктураВидаУпаковки) Экспорт
	...
	Если ТипЗнч(ЗначениеМетаданных("ESD_ВидыУпаковки.ПустаяСсылка", "Справочник")) = Тип("Строка") Тогда
	...
	ОбъектВидУпаковки = МодульКода("Справочники.ESD_ВидыУпаковки").СоздатьЭлемент();
...
//
Процедура ДозаполнитьНовыеВидыУпаковки(РезультатЗагрузки, ОбщиеПараметрыЗагрузки) Экспорт
	
	Если РезультатЗагрузки.НовыеВидыУпаковки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ЗагрузитьВидыУпаковкиСОнлайна(ОбщиеПараметрыЗагрузки, РезультатЗагрузки.НовыеВидыУпаковки, РезультатЗагрузки);
...
// Параметры:
//  ОбщиеПараметрыЗагрузки - Структура - общие параметры загрузки
//  МассивВидовУпаковки - Массив - массив структур видов упаковки для загрузки, если не передан, то грузятся все.
//  РезультатЗагрузки - Структура - результат загрузки см. ESD_ТНОбщегоНазначенияКлиентСервер.РезультатВзаимодействияСоСБИС
//
// Возвращаемое значение:
//   Структура - результат загрузки
//
Функция ЗагрузитьВидыУпаковкиСОнлайна(ОбщиеПараметрыЗагрузки, МассивВидовУпаковки = Неопределено,
		РезультатЗагрузки = Неопределено) Экспорт
				
	Если МассивВидовУпаковки = Неопределено Тогда
		МассивСтруктурВидовУпаковки = Новый Массив;
	Иначе
		МассивСтруктурВидовУпаковки = МассивВидовУпаковки;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ОбщиеПараметрыЗагрузки.context_params) Тогда		
		...
		ДанныеУпаковкиДляЗагрузки = Новый Массив;		
		МассивОшибок.Добавить("Загрузка возможна только после авторизации");
		
	Иначе
		
		ЗагрузитьДанныеВидовУпаковки(МассивСтруктурВидовУпаковки, ОбщиеПараметрыЗагрузки, РезультатЗагрузки);
	...
	Для Каждого СтруктураДанных Из ДанныеУпаковкиДляЗагрузки Цикл
		...
		Иначе
			ОбъектСправочника = МодульКода("Справочники.ESD_ВидыУпаковки").СоздатьЭлемент();
		КонецЕсли;
	...
	Возврат СформироватьРезультатЗагрузкиВидовУпаковки(ДанныеУпаковкиДляЗагрузки, Загружено, МассивОшибок);
...
Процедура ЗаполнитьСсылкиВидовУпаковкиНовыйРедактор(МассивДанныхВидовУпаковки) Экспорт
	
	ДанныеУпаковки            = ДанныеУпаковки(МассивДанныхВидовУпаковки);
	ДанныеУпаковкиДляЗагрузки = ДанныеУпаковкиДляЗагрузки(ДанныеУпаковки);
	
	СозданныеВидыУпаковки = Новый Соответствие;
	
	Для Индекс = 0 По ДанныеУпаковкиДляЗагрузки.Количество() - 1 Цикл
		СтрокаВидаУпаковки = ДанныеУпаковкиДляЗагрузки[Индекс];
		ДанныеВидаУпаковки = МассивДанныхВидовУпаковки[Индекс];
		
		Если ЗначениеЗаполнено(ДанныеВидаУпаковки["Ссылка"]) Тогда
			Продолжить;
		КонецЕсли;
		
		СсылкаВидУпаковки = СтрокаВидаУпаковки.Ссылка;
		Если Не ЗначениеЗаполнено(СсылкаВидУпаковки) И ЗначениеЗаполнено(ДанныеВидаУпаковки) Тогда
			КодВидаУпаковки = ДанныеВидаУпаковки["Код"];
			СсылкаВидУпаковки = СозданныеВидыУпаковки.Получить(КодВидаУпаковки);
			Если СсылкаВидУпаковки = Неопределено Тогда
				СсылкаВидУпаковки = НоваяСсылкаНаУпаковку(ДанныеВидаУпаковки);
				СозданныеВидыУпаковки.Вставить(КодВидаУпаковки, СсылкаВидУпаковки);
			КонецЕсли;
		КонецЕсли;
		
		ДанныеВидаУпаковки.Вставить("Ссылка", СсылкаВидУпаковки);
...
Процедура ЗагрузитьДанныеВидовУпаковки(МассивСтруктурВидовУпаковки, ОбщиеПараметрыЗагрузки, РезультатЗагрузки)
	
	Если РезультатЗагрузки <> Неопределено И РезультатЗагрузки.ВидыУпаковкиСОнлайна <> Неопределено Тогда
		ДанныеСправочника = РезультатЗагрузки.ВидыУпаковкиСОнлайна;
	Иначе
...
Функция ДанныеУпаковки(МассивСтруктурВидовУпаковки = Неопределено)
	...
	Для Каждого СтруктураВидаУпаковки Из МассивСтруктурВидовУпаковки Цикл
		...
		СтрокаДанных.Идентификатор = МодульТНОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(
			СтруктураВидаУпаковки, "Идентификатор", "");
		СтрокаДанных.Наименование = МодульТНОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(
			СтруктураВидаУпаковки, "Наименование", "");
		СтрокаДанных.ТехническоеНаименование = МодульТНОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(
			СтруктураВидаУпаковки, "ТехническоеНаименование", "");
		СтрокаДанных.Код = МодульТНОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(СтруктураВидаУпаковки, "Код", "");
...
Функция ДанныеУпаковкиДляЗагрузки(ДанныеУпаковки)
	...
	|				И (ТаблицаДанныхУпаковки.Идентификатор <> """")
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		ТаблицаДанныхУпаковки КАК ТаблицаДанныхУпаковки
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ СправочникВидыУпаковки КАК СправочникВидыУпаковки
	|			ПО ТаблицаДанныхУпаковки.Код = СправочникВидыУпаковки.Код
	|				И (ТаблицаДанныхУпаковки.Код <> """")) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	...
	|			ТОГДА ЕСТЬNULL(СуществующиеСправочники.СсылкаПоИдентификатору, &ВидУпаковкиПустаяСсылка)
	|		КОГДА ЕСТЬNULL(СуществующиеСправочники.СсылкаПоКоду, &ВидУпаковкиПустаяСсылка) <> &ВидУпаковкиПустаяСсылка
	|			ТОГДА ЕСТЬNULL(СуществующиеСправочники.СсылкаПоКоду, &ВидУпаковкиПустаяСсылка)
	...
	|			И (СправочникВидыУпаковки.Ссылка ЕСТЬ NULL)
	...
	ЗапросДанных.УстановитьПараметр("ВидУпаковкиПустаяСсылка", ЗначениеМетаданных("ESD_ВидыУпаковки.ПустаяСсылка", "Справочник"));
...
Функция СформироватьРезультатЗагрузкиВидовУпаковки(ДанныеУпаковкиДляЗагрузки, Загружено, МассивОшибок)	
	...
	Если Загружено <> ДанныеУпаковкиДляЗагрузки.Количество() Тогда
		...
		Если Не ПустаяСтрока(ОписаниеОшибок) Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС + ОписаниеОшибок;	
		КонецЕсли; 
		МодульТНОбщегоНазначенияСервер.ЗаписатьОшибкуВЖурналРегистрации(
			ЗначениеМетаданных("ESD_ВидыУпаковки.ПустаяСсылка", "Справочник"),
			МодульТНОбщегоНазначенияСервер.ИмяСобытияЖурналаРегистрации_ЗагрузкаВидовУпаковки(),
			ТекстОшибки);
	КонецЕсли;	
...
Функция НоваяСсылкаНаУпаковку(ДанныеВидаУпаковки)
	
	РезультатФункции = МодульКода("Справочники.ESD_ВидыУпаковки").СоздатьЭлемент();
	МодульКода("ESD_ТНОбщегоНазначенияСервер").ЗаполнитьЗначенияИзСоответствия(РезультатФункции, ДанныеВидаУпаковки);
