...
Функция СформироватьРезультатЗагрузкиОпасныхГрузов(ДанныеОпасныхГрузовДляЗагрузки, Загружено, МассивОшибок)
	...
	Если Загружено <> ДанныеОпасныхГрузовДляЗагрузки.Количество() Тогда
		...
		Если Не ПустаяСтрока(ОписаниеОшибок) Тогда
			ТекстОшибки = ТекстРезультата +	Символы.ПС + ОписаниеОшибок;
		КонецЕсли;
		МодульТНОбщегоНазначенияСервер.ЗаписатьОшибкуВЖурналРегистрации(
			ЗначениеМетаданных("ESD_ОпасныеГрузы.ПустаяСсылка", "Справочник"),
			МодульТНОбщегоНазначенияСервер.ИмяСобытияЖурналаРегистрации_ЗагрузкаОпасныхГрузов(),
			ТекстОшибки);
	КонецЕсли;	
