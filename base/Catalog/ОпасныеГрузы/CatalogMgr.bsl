...
Процедура ЗаполнитьОпасныйГруз(СтруктураОпасногоГруза) Экспорт
	...
	Если Метаданные.Справочники.Найти("Saby_ОпасныеГрузы") = Неопределено Тогда
...
//
// Параметры:
//  НовыеОпасныеГрузы - Массив - массив структур см. ШаблонСтруктурыОпасногоГруза()
//
Процедура ДозаполнитьНовыеОпасныеГрузы(НовыеОпасныеГрузы) Экспорт
	
	Если НовыеОпасныеГрузы.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ЗагрузитьОпасныеГрузыСОнлайна(НовыеОпасныеГрузы);
...
//
Процедура ЗагрузитьОпасныеГрузыСОнлайна(МассивСтруктурОпасныхГрузов = Неопределено, Результат = Неопределено) Экспорт
	
	Если МассивСтруктурОпасныхГрузов = Неопределено Тогда
		МассивСтруктурОпасныхГрузов = Новый Массив;
	КонецЕсли;
	
	ЗагрузитьДанныеОпасныхГрузов(МассивСтруктурОпасныхГрузов);
	...
	СформироватьРезультатЗагрузки(ДанныеОпасныхГрузовДляЗагрузки, Загружено, МассивОшибок, Результат);
...
Процедура ЗагрузитьДанныеОпасныхГрузов(МассивСтруктурОпасныхГрузов)
	
	ДанныеСправочника = МодульКода("Saby_ТНОбщегоНазначенияСервер").СправочникСОнлайна("Справочник опасных грузов");
...
Процедура СформироватьРезультатЗагрузки(ДанныеОпасныхГрузовДляЗагрузки, Загружено, МассивОшибок, Результат)
