...
Процедура УстановитьНовыеДанные(Команда)
	...
	Если ЭтаФорма.РольСтрокой = "Составитель" И Не ДанныеЮрЛицаЗаполнены() Тогда
		...
		Сообщение.Текст = "Необходимо заполнить данные организации для отправки черновика.";
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
Функция ЗаполнитьДанныеЮрЛицИзОрганизацииРасширения(Знач Организация)
	...
	ИспользоватьНовыйРедактор = ЭтоУниверсальноеРасширение()
	...
	ИначеЕсли ИспользоватьНовыйРедактор Тогда
...
Процедура СформируемТаблицуЯщиков()
	...
	Если ЭтаФорма.ЭтоВО Тогда
		ТранспортВО = ТранспортВО(ЭтаФорма, context_params);
