...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИмяКонфигурации       = МодульКода("ESD_ТНОбщегоНазначенияСервер").ИмяКонфигурации();
...
Функция ЗаполнитьДанныеЮрЛицИзОрганизацииРасширения(Знач Организация)
	...
	ЗапросДанных.Текст =
	"ВЫБРАТЬ
	|	ESD_Организации.Наименование КАК НаименованиеОрганизации,
	|	*
	|ИЗ
	|	Справочник.ESD_Организации КАК ESD_Организации
	|ГДЕ
	|	ESD_Организации.Ссылка = &Ссылка";
	
	ЗапросДанных.УстановитьПараметр("Ссылка", Организация);
	...
	Если ВыборкаДанных.Следующий() Тогда
		...
		ЗаполнитьЗначенияСвойств(СтруктураЮрЛица, ВыборкаДанных, , "КонтактныеДанные");
		СтруктураЮрЛица.ЮрФизЛицоСтрокой = МодульКода("Перечисления.ESD_ЮрФизЛицо").ИмяВида(ВыборкаДанных.ЮрФизЛицо);
		СтруктураЮрЛица.СтруктураФИО = Новый Структура("Фамилия,Имя,Отчество");
		ЗаполнитьЗначенияСвойств(СтруктураЮрЛица.СтруктураФИО, ВыборкаДанных);
		...
		ВыборкаКонтактныхДанных = ВыборкаДанных.КонтактныеДанные.Выбрать();
		Пока ВыборкаКонтактныхДанных.Следующий() Цикл
			СтрокаКонтактныхДанных = ЭтаФорма.КонтактныеДанные.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаКонтактныхДанных, ВыборкаКонтактныхДанных);
