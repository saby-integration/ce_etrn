...
Функция ОбновитьДанныеОтветственногоЭПД()
	...
	ПараметрыРасчета = ПараметрыЗаполненияНаОснованииINI();
	...
	Если ЭтаФорма.ЭтоВО Тогда
		...
		РезультатРасчета = РезультатРасчетаINI(ПараметрыРасчета, ТранспортБлокли);
	КонецЕсли;
...
Процедура ОбработатьРезультатРаспознаванияВУНаСервере(Ответ) Экспорт
	...
	// ФИО
	ИмяОтчество = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").СтрРазделитьЭПД(ДанныеВУ["NamePatronymic"], " ");
