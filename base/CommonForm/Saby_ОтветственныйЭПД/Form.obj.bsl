...
Процедура УстановитьНовыеДанные(Команда)
	...
	ВУ  = Saby_ТНОбщегоНазначенияКлиентСервер.ПредставлениеВУ(ЭтаФорма);
...
Процедура УстановитьВидимостьДоступностьРеквизитов(Знач ДанныеЗаполнения)
	...
	Роли = ПризнакиРолей(Роль, ДанныеЗаполнения.ИмяМетаданных);
...
Функция ПризнакиРолей(Роль, ИмяМетаданных)
	...
	ИначеЕсли Роль = Перечисления.Saby_РолиОтветственных.Ответственный Тогда
		СтруктураРолей.ЭтоОтветственный = Истина;
		СтруктураРолей.ЭтоОтветственныйЗаПеревозку = ИмяМетаданных = "Saby_ЗаказНаПеревозку";
	Иначе
...
Функция ТелефоныЭлектроннаяПочтаСериализовать()
	...
	Если ЭтаФорма.ЭлектроннаяПочта.Количество() > 0 Тогда
		...
		ЭлектроннаяПочтаСтрокой = ЗначениеВСтрокуВнутр(МассивЭлектроннаяПочта);
	Иначе
		ЭлектроннаяПочтаСтрокой = "";
	КонецЕсли;
	
	Структура = Новый Структура;
	Структура.Вставить("Телефоны",         ТелефоныСтрокой);
	Структура.Вставить("ЭлектроннаяПочта", ЭлектроннаяПочтаСтрокой);
