...
Процедура ЗаполнениеРеквизитовФормы(Знач ДанныеЗаполнения)
	...
	СписокОшибокЗаполнения = Новый Массив;
	МодульESD_ОбщегоНазначенияСервер.ESD_ЗаполнитьРеквизитыФормы(ЭтаФорма, ДанныеЗаполнения, , СписокОшибокЗаполнения);
	Для Каждого ОписаниеОшибки Из СписокОшибокЗаполнения Цикл
		МодульESD_ОбщегоНазначенияСервер.СообщитьПользователюСервер(
			ОписаниеОшибки.РеквизитФормы.Заголовок + ": " + ОписаниеОшибки.Текст);
	КонецЦикла;
	...
	Если ЭтаФорма.ТипДокументаИсточника = ПутевойЛист() Тогда
		Если Роли.ЭтоВодитель Тогда
			ЭтаФорма.ИспользоватьСНИЛС = МодульESD_ОбщегоНазначенияСервер
				.ESD_ЗагрузитьНастройкиОбъекта("ИспользоватьСНИЛС", , "");
		КонецЕсли;

		ЭтаФорма.ИспользоватьТабельныйНомер = МодульESD_ОбщегоНазначенияСервер
...
Функция ПроверитьЗаполнениеФормы()
	ОшибокНеОбнаружено = Истина;
	Телефоны_ПроверитьЗаполнение(ОшибокНеОбнаружено);
...
Процедура ТелефоныПриИзменении(Элемент)
	ОчиститьСообщения();
	Телефоны_ПроверитьЗаполнение();
КонецПроцедуры

&НаКлиенте
Процедура Телефоны_ПроверитьЗаполнение(ОшибокНеОбнаружено = Неопределено)
	Если Телефоны.Количество() > 10 Тогда
		ТекстСообщения = "Превышено максимальное количество номеров телефона.";
		МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").СообщитьПользователю(ТекстСообщения);
		ОшибокНеОбнаружено = Ложь;
	КонецЕсли;
КонецПроцедуры   
