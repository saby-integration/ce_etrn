...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СтрНайти(ЭтаФорма.ИмяФормы, "ESD") <> 0 Тогда
...
Процедура УстановитьЗаголовокАвторизации(Знач Выйти = Ложь)
	...
	АвторизацияВыполнена = ЗначениеЗаполнено(СтруктураСессии);
	...
	Если АвторизацияВыполнена Тогда
		Элементы.ДекорацияПользователь.Заголовок = СтруктураСессии.user_fio;
		Элементы.ДекорацияСервис.Заголовок       = ПараметрыАвторизации.api_url;
	Иначе 
		Элементы.ДекорацияПользователь.Заголовок	= "";
		Элементы.ДекорацияСервис.Заголовок			= "";
	КонецЕсли;
...
Процедура ВывестиТекущуюВерсию()
	...
	Версия = "00.0000.0";
	ОтборРасширения = Новый Структура("Имя", "ESD");
	НайденныеРасширения = РасширенияКонфигурации.Получить(ОтборРасширения);
	Если НайденныеРасширения.Количество() > 0 Тогда
		Расширение = НайденныеРасширения[НайденныеРасширения.ВГраница()];
		ЧастиВерсии = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").СтрРазделитьЭПД(Расширение.Версия, "_");
		Версия = ЧастиВерсии[ЧастиВерсии.ВГраница()];
	КонецЕсли;
...
Процедура ОбновитьЗначениеФункциональнойОпцииСервер(Знач ИмяОпции)
	...
	Если ОтключенВесьФункционал() Тогда
		...
		МодульКода("ESD_ТНОбщегоНазначенияСервер").СообщитьПользователю(ТекстСообщения, , "ГруппаИспользуемыйФункционал");
