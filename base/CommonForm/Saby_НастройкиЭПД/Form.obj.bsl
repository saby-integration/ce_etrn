...
Процедура ВыполнитьРегламентноеЗадание(Команда)
	...
	Если ПараметрыВыполнения.ЗапускВыполнен Тогда
		...
		Пояснение = Saby_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку(
			ПояснениеТекст,
			Наименование,
	...
	ИначеЕсли ПараметрыВыполнения.ПроцедураУжеВыполняется Тогда
		...
		Сообщение = Saby_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку(
			Текст,
			Наименование,
	...
	Если КоличествоОшибок > 0 Тогда
		...
		ЗаголовокТекстаПроОшибки = Saby_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку(
			Текст,
...
Процедура СообщитьОбОкончанииРучногоВыполненияРегламентногоЗадания()
	...
	Для каждого Оповещение Из ОповещенияОбОкончанииВыполнения Цикл
		...
		ТекстПояснения = Saby_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку(
			СтрокаДляПояснения,
...
Функция ПараметрыДляНастройкиОснований(Знач ИмяКоманды)
	...
	ИначеЕсли ИмяКоманды = "НастройкиДокументовОснованийЗнП" Тогда
		Ссылка = Документы.Saby_ЗаказНаПеревозку.ПустаяСсылка();
	Иначе 	
...
Функция РезультатЗагрузкиСправочника(Знач НаименованиеСправочника)
	...
	Если НаименованиеСправочника = "Saby_ВидыУпаковки" Тогда
		Результат = Справочники.Saby_ВидыУпаковки.ЗагрузитьВидыУпаковкиСОнлайна(ОбщиеПараметрыЗагрузки);
	Иначе
		Результат = Справочники.Saby_ОпасныеГрузы.ЗагрузитьОпасныеГрузыСОнлайна(ОбщиеПараметрыЗагрузки);
	КонецЕсли;
