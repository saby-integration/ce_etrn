...
Функция ОбновитьДанныеТранспортногоСредстваЭПД()
	...
	Результат = ЗаполнитьДанныеТранспортногоСредстваЭПД_НаОснованииСправочника(ЭтаФорма.ТранспортноеСредство);
...
&НаСервере
Функция ЗаполнитьДанныеТранспортногоСредстваЭПД_НаОснованииСправочника(Знач ТранспортноеСредство)
	
	ЗаполнитьИмяИНИИзНастроекСопоставления(ТранспортноеСредство);
	
	РезультатФункции = Новый Структура;
	РезультатФункции.Вставить("Ошибка",         Ложь);
	РезультатФункции.Вставить("ТекстСообщения", "");
	РезультатФункции.Вставить("СтруктураТС",    МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").СтруктураТранспортногоСредства());
	
	МодульТНОбщегоНазначенияСервер = МодульКода("ESD_ТНОбщегоНазначенияСервер");
	
	Если ТранспортноеСредство.Метаданные().Имя = "ESD_ТранспортныеСредства" Тогда
		ОбъектСправочника = ТранспортноеСредство.ПолучитьОбъект();
	ИначеЕсли ЭтоУниверсальноеРасширение() Тогда
		ОбъектСправочника = МодульКода("Справочники.ESD_ТранспортныеСредства").СоздатьЭлемент();
		МассивОснований = Новый Массив;
		МассивОснований.Добавить(ТранспортноеСредство);
		СтруктураЗаполнения = МодульТНОбщегоНазначенияСервер.СтруктураЗаполненияНаОснованииНовогоРедактора(
			МассивОснований);
		ОбъектСправочника.Заполнить(СтруктураЗаполнения);
	ИначеЕсли Не ЗначениеЗаполнено(ЭтаФорма.ИмяИНИ) Тогда
		МассивТС = Новый Массив;
		МассивТС.Добавить(ЭтаФорма.ТранспортноеСредство);
		РеквизитыТС = МодульТНОбщегоНазначенияСервер.РеквизитыТранспортныхСредств(МассивТС);
	...
	ЗаполнитьЗначенияСвойств(РезультатФункции.СтруктураТС, ОбъектСправочника);
	РезультатФункции.СтруктураТС.РегистрационныйНомер = ОбъектСправочника.Наименование;
	ДокументыВладения = Новый Массив;
	Для Каждого СтрокаДокументаВладения Из ОбъектСправочника.ДокументыВладения Цикл
		СтруктураДокументаВладения = Новый Структура("ДокументВладения", СтрокаДокументаВладения.ДокументВладения);
		ДокументыВладения.Добавить(СтруктураДокументаВладения);
	КонецЦикла;
	
	РезультатФункции.СтруктураТС.Вставить("ОснованияВладения", ОснованияВладенияВСтроку(ДокументыВладения));
