...
Функция ЗаполнитьДанныеТранспортногоСредстваЭПД_НаОснованииСправочника(Знач ТранспортноеСредство)
	...
	Если ТранспортноеСредство.Метаданные().Имя <> "ESD_ТранспортныеСредства" Тогда
		Если Не ЗначениеЗаполнено(ЭтаФорма.ИмяИНИ) Тогда
	...
	Иначе
		...
		ЗапросДанных.Текст =
		"ВЫБРАТЬ
		|	ESD_ТранспортныеСредства.Наименование КАК РегистрационныйНомер,
		|	*
		|ИЗ
		|	Справочник.ESD_ТранспортныеСредства КАК ESD_ТранспортныеСредства
		|ГДЕ
		|	ESD_ТранспортныеСредства.Ссылка = &Ссылка";
		
		ЗапросДанных.УстановитьПараметр("Ссылка", ТранспортноеСредство);
		
		ВыборкаДанных = ЗапросДанных.Выполнить().Выбрать();
		Если ВыборкаДанных.Следующий() Тогда
			ЗаполнитьЗначенияСвойств(РезультатФункции.СтруктураТС, ВыборкаДанных);
			
			ВыборкаДокументовВладения = ВыборкаДанных.ДокументыВладения.Выбрать();
			ДокументыВладения = Новый Массив;
			Пока ВыборкаДокументовВладения.Следующий() Цикл
				СтруктураДокументаВладения = Новый Структура("ДокументВладения", ВыборкаДокументовВладения.ДокументВладения);
