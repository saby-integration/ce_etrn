...
Процедура ЗаполнениеРеквизитовФормы(Знач ДанныеЗаполнения)
	...
	// временное г... для перехода
	// пока не отказались полностью от перечисления
	Для Каждого Строка Из КонтактныеДанные Цикл
		Если Строка.Тип = "Адрес электронной почты" Тогда
			Строка.Тип = "АдресЭлектроннойПочты";
		КонецЕсли;	
	КонецЦикла;	
...
Функция ОбновитьДанныеЮрЛицНаСервере()
	...
	Если ЭтаФорма.ДанныеЮрЛиц.Количество() Тогда
		...
		Структура = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").СтруктураДанныхЮрЛица();
...
Функция МассивДокументовЭПД(ТаблицаДокументовЭПД)
	...
	Если Не ЗначениеЗаполнено(ЭтаФорма.Основание)
		...
		Для Каждого СтрокаДокументов Из ТаблицаДокументовЭПД Цикл
			...
			СтруктураДанных = МодульКодаКлиент("Saby_ТНОбщегоНазначенияКлиентСервер").СтруктураДокументовЭПД();
...
Функция ЗаполнитьДанныеЮрЛицНаОснованииINI()
	...
	ПараметрыРасчета.Вставить("ТранспортБлокли",      Обработки.SABY.Создать());
	...
	Если РезультатРасчета.Результат.Ошибка Тогда 
		...
		РезультатРасчета.Результат.Вставить("Картинка", БиблиотекаКартинок.Saby_Ошибка32);
