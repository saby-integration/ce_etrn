...
Процедура ЗаполнитьПрошлыйТитулДляВложения(Вложение, Титул) Экспорт
	...
	Период        = Saby_ТНЗагрузкаСервер.ДатаВложения(Вложение, Титул);
	Идентификатор = Saby_ТНЗагрузкаСервер.ИдентификаторВложения(Вложение, Титул);
	
	МаксимальныйПериод = Дата(1, 1, 1);
	ДанныеТитула = Неопределено;
	
	Если Не ДополнительныеСвойства.Свойство("ТитулыДляПривязки") Тогда
		Вложение.Вставить(ИмяКлючаПрошлыйТитул, Перечисления.Saby_ТипТитулаЭтрН.ТитулОтказа(Неопределено));
		Возврат;
	КонецЕсли;
	...
	Если ДанныеТитула <> Неопределено Тогда
		Вложение.Вставить(ИмяКлючаПрошлыйТитул, Перечисления.Saby_ТипТитулаЭтрН.ТитулОтказа(ДанныеТитула.ТипТитула));
	Иначе
		Вложение.Вставить(ИмяКлючаПрошлыйТитул, Перечисления.Saby_ТипТитулаЭтрН.ТитулОтказа(Неопределено));
	КонецЕсли;
...
Процедура ЗаполнитьНаОснованииЗаказаНаПеревозку(ДанныеЗаполнения)
	...
	//ДанныеПервогоТитула = Документы.Saby_ПутевойЛист.ДанныеДокумента(
	//	ДанныеЗаполнения, Перечисления.Saby_ТипТитулаЭПЛ.ПервыйТитул());
	//	
	//Исключения = Новый Соответствие;
	//Исключения.Вставить("Ссылка",                             Истина);
	//Исключения.Вставить("Номер",                              Истина);
	//Исключения.Вставить("Дата",                               Истина);
	...
	//Исключения.Вставить("ОтветственныйТехосмотр",             Истина);
	//Исключения.Вставить("ДанныеЮрЛицМедосмотрВыезд",          Истина);
	//Исключения.Вставить("ДанныеЮрЛицМедосмотрЗаезд",          Истина);
	//
	//СоответствияТаблиц = Новый Соответствие;
	//СоответствияТаблиц.Вставить("ОтветственныйМедосмотрВыезд", "ОтветственныеЛица");
	//СоответствияТаблиц.Вставить("ОтветственныйМедосмотрЗаезд", "ОтветственныеЛица");
	//
	//Для Каждого КлючЗначение Из ДанныеПервогоТитула.Реквизиты Цикл
	//	Если Исключения[КлючЗначение.Ключ] <> Неопределено
	//		Или Не Saby_ТНОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойство(ЭтотОбъект, КлючЗначение.Ключ) Тогда
	//		Продолжить;
	//	КонецЕсли;
	//	ЭтотОбъект[КлючЗначение.Ключ] = КлючЗначение.Значение;
	//КонецЦикла;
	//
	//Для Каждого КлючЗначение Из ДанныеПервогоТитула Цикл
	//	Если КлючЗначение.Ключ = "Реквизиты" Или Исключения[КлючЗначение.Ключ] <> Неопределено Тогда
	//		Продолжить;
	//	КонецЕсли;
	//	
