...
Процедура ОбновитьСостояниеИзменений()
	...
	Результат = РегистрыСведений.Saby_ДанныеТитулов.КоличествоИзменений(Объект.Ссылка);
...
&НаСервере
Процедура ПолучитьЕщеИзмененияНаСервере(Знач СледующиеИзменения = Истина)
	...
	Отбор = Новый Структура;
	Отбор.Вставить("Основной", Ложь);
	
	МассивИзменений = РегистрыСведений.Saby_ДанныеТитулов.СледующиеИзменения(Объект.Ссылка, ПериодДо, Отбор);
...
&НаСервере
Процедура ЗаполнитьНаименованиеТитула(СтрокаСобытия) Экспорт
	
	ПредставленияТитула = Перечисления.Saby_ТипТитулаЭтрН.ПредставленияТитула(СтрокаСобытия.ТипТитула);
...
Функция ПараметрыФормыРасшифровки(ИдентификаторТитула, НаименованиеТитула)
	...
	РезультатФункции = Новый Структура("ИмяФормы,СсылкаНаДокумент,ИдентификаторТитула");
	
	Если НаименованиеТитула = "Переадресовка" Или НаименованиеТитула = "ПереадресовкаУведомление" Тогда
		
		РезультатФункции.ИмяФормы            = "ФормаПереадресовка";
	...
	ИначеЕсли НаименованиеТитула = "ЗаменаВодителяТС" Тогда
		...
		РезультатФункции.ИмяФормы            = "ФормаЗаменыВодителяИлиТС";
		РезультатФункции.СсылкаНаДокумент    = Объект.Ссылка;
		РезультатФункции.ИдентификаторТитула = ИдентификаторТитула;
		
	ИначеЕсли НаименованиеТитула = "ИзменениеСтоимости" Или НаименованиеТитула = "СогласованиеСтоимости" Тогда
		
		РезультатФункции.ИмяФормы            = "ФормаИзмененияСогласованияСтоимости";
		РезультатФункции.СсылкаНаДокумент    = Объект.Ссылка;
		РезультатФункции.ИдентификаторТитула = ИдентификаторТитула;
...
&НаКлиенте
Процедура ОткрытьФормуТитула(ИдентификаторТитула, НаименованиеТитула, Описание) Экспорт
	...
	Если НаименованиеТитула = "УведомлениеОУточнении" Тогда
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("ИдентификаторТитула", ИдентификаторТитула);
		ПроцедураПослеПоказаЗначения = Новый ОписаниеОповещения("ПослеПоказаЗначения", ЭтотОбъект, ДополнительныеПараметры);
		ПоказатьЗначение(ПроцедураПослеПоказаЗначения, Описание);
	Иначе
		ОткрытьФорму(
...
&НаКлиенте
Процедура ПослеПоказаЗначения(ДополнительныеПарамеры) Экспорт
	
	ПослеПоказаЗначенияНаСервере(Объект.Ссылка, ДополнительныеПарамеры.ИдентификаторТитула);
	
	ПослеЗакрытияФормыТитула(ДополнительныеПарамеры.ИдентификаторТитула);
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ПослеПоказаЗначенияНаСервере(Знач СсылкаНаДокумент, Знач ИдентификаторТитула)
	РегистрыСведений.Saby_ДанныеТитулов.Просмотреть(СсылкаНаДокумент, ИдентификаторТитула);
КонецПроцедуры

&НаКлиенте
Процедура ПослеЗакрытияФормыТитула(ИдентификаторТитула) Экспорт
...
Процедура ПросмотретьВсеСобытияНаСервере()
	...
	РегистрыСведений.Saby_ДанныеТитулов.Просмотреть(МассивДокументов);
