...
Процедура ДоРейса_МедосмотрСторонняяОрганизацияПриИзменении(Элемент)
	...
	КонецЕсли;
	ОбновитьВидимостьИДоступностьЭлементов_Медосмотр_ПриИзменении();
КонецПроцедуры  
...
Процедура ПослеРейса_МедосмотрСторонняяОрганизацияПриИзменении(Элемент)
	ПриИзмененииВидаМедицинскогоРаботника("МедосмотрЗаезд", "ПослеРейса");
	ОбновитьВидимостьИДоступностьЭлементов_ПослерейсовыйМедосмотр_ПриИзменении();
КонецПроцедуры 
...
#Область include_etrn_base_Document_ПутевойЛист_DocumentForm_ФормаДокумента_ДополнительныеКомандыSaby
#КонецОбласти // include_etrn_base_Document_ПутевойЛист_DocumentForm_ФормаДокумента_ДополнительныеКомандыSaby
...
Процедура ПриИзмененииВидаМедицинскогоРаботника(РольСтрокой, ВидМедосмотра)
	...
	МодульТНОбщегоНазначенияКлиентСервер.УдалитьОтветственноеЛицоПоРоли(Объект.ОтветственныеЛица, РольСтрокой);
...
Процедура ПриИзмененииСтороннейОрганизации(РольСтрокой, ВидМедосмотра) 
	...
	МодульESD_ОбщегоНазначенияКлиентСервер.УдалитьЮрЛицоПоРоли(Объект.ДанныеЮрЛиц, Неопределено,
		РольСтрокой);
	МодульESD_ОбщегоНазначенияКлиентСервер.УдалитьКонтактныеДанныеПоРоли(Объект.КонтактныеДанные, 
		РольСтрокой);
	...
	Если Не ЗначениеЗаполнено(ТекущаяМедицинскаяОрганизация) Тогда
		ПредставлениеМедицинскойОрганизации = МодульESD_ОбщегоНазначенияКлиентСервер.ПредставлениеЮрЛицаПоРоли(
...
Процедура ОбновитьВидимостьИДоступностьЭлементов()
	...
	// Признак доступности редактирования
	ДоступноРедактированиеОбъекта_ = ДоступноРедактированиеОбъекта();

	// Базовые команды Saby
	Элементы.ФормаОбновитьДокументИзСбис.Видимость = ДоступноРедактированиеОбъекта_;
	Элементы.ФормаЗагрузитьВложенияВ1С.Видимость = ДоступноРедактированиеОбъекта_;

	// Признаки документооборота ЭПД
	Признаки = ПризнакиДокумента();
	
	// Определим и установим признак запрета интерактивного изменения формы
	ЭтаФорма.ТолькоПросмотр = Не (ДоступноРедактированиеЭПД(Признаки) И ДоступноРедактированиеОбъекта_);
	...
	НайденныеДоступныеЭтапы = ESD_ТНОбщегоНазначенияКлиентСервер
		.НайтиСтрокиУниверсально(Объект.ТекущиеЭтапы, ОтборДоступных);
...
&НаСервере
Функция ДоступноРедактированиеЭПД(Знач Признаки = Неопределено)
	Если Признаки = Неопределено Тогда
		Признаки = ПризнакиДокумента();
	КонецЕсли;
	
	Возврат Не Признаки.Этапы.ДОЗавершен;
КонецФункции

&НаСервере
Функция ДоступноРедактированиеОбъекта()
	Возврат ПравоДоступа("ИнтерактивноеДобавление", Метаданные.Документы.ESD_ПутевойЛист);
КонецФункции
...
Процедура ОбновитьВидимостьПоЭтапуИУчастнику()
	...
	ПризнакВыпускДоступен = Признаки.Этапы.ВыпускДоступен И Не ЭтаФорма.ТолькоПросмотр;

	Элементы.ГруппаДокументОснование.Видимость = Признаки.Исходящий;
	Элементы.НастройкаДокументовОснований.Видимость = Ложь;
	Элементы.ГруппаКомандыДокументаОснования.Видимость = ПризнакВыпускДоступен;
	Элементы.ДекорацияДокументОснование.Видимость = ПризнакВыпускДоступен
	...
	МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ИзменитьСвойствоЭлементовФормы(
		ЭтаФорма, "ТолькоПросмотр", Не ПризнакВыпускДоступен, МассивЭлементов);
	...
	МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ИзменитьСвойствоЭлементовФормы(
		ЭтаФорма, "Доступность", ПризнакВыпускДоступен, МассивЭлементов);
		
	// Обновление доступности полей пред- и после- рейсового медосмотра
	ОбновитьВидимостьИДоступностьЭлементов_Медосмотр_ПриЧтенииНаСервере(Признаки);
	ОбновитьВидимостьИДоступностьЭлементов_Медосмотр_ПриИзменении(Признаки);
	
	// Обновление доступности полей техосмотра
...
&НаСервере
Процедура ОбновитьВидимостьИДоступностьЭлементов_Медосмотр_ПриЧтенииНаСервере(Признаки)
...
&НаСервере
Процедура ОбновитьВидимостьИДоступностьЭлементов_Медосмотр_ПриИзменении(Знач Признаки = Неопределено)
	ОбновитьВидимостьИДоступностьЭлементов_ПредрейсовыйМедосмотр_ПриИзменении(Признаки);
	ОбновитьВидимостьИДоступностьЭлементов_ПослерейсовыйМедосмотр_ПриИзменении(Признаки);
КонецПроцедуры

&НаСервере
Процедура ОбновитьВидимостьИДоступностьЭлементов_ПредрейсовыйМедосмотр_ПриИзменении(Признаки = Неопределено)
	Если Признаки = Неопределено Тогда
		Признаки = ПризнакиДокумента();
	КонецЕсли;
	...
	ОтобразитьМедРаботника_ДоРейса = Не (Объект.ДоРейса_МедосмотрСторонняяОрганизация И Признаки.Этапы.ВыпускДоступен 
		Или ЗначениеЗаполнено(Объект.ДоРейса_МедицинскаяОрганизация));	
	Элементы.МедосмотрВыездСтрокой.Видимость  = ОтобразитьМедРаботника_ДоРейса; 
	Элементы.ГруппаДоРейса_Лицензия.Видимость = ОтобразитьМедРаботника_ДоРейса;
КонецПроцедуры

&НаСервере
Процедура ОбновитьВидимостьИДоступностьЭлементов_ПослерейсовыйМедосмотр_ПриИзменении(Признаки = Неопределено)
	Если Признаки = Неопределено Тогда
		Признаки = ПризнакиДокумента();
	КонецЕсли;
	
	Элементы.ГруппаМедосмотрПослеРейса.Видимость = Объект.ПослерейсовыйМедосмотрТребуется;
	Если Не Элементы.ГруппаМедосмотрПослеРейса.Видимость Тогда
		Возврат;
	КонецЕсли;
	
	Элементы.ПослеРейса_МедицинскаяОрганизация.ТолькоПросмотр = Истина;
	Объект.ПослеРейса_МедицинскаяОрганизация = Объект.ДоРейса_МедицинскаяОрганизация;
	Если ЗначениеЗаполнено(Объект.ДоРейса_МедицинскаяОрганизация) И Объект.ДоРейса_МедосмотрСторонняяОрганизация Тогда
		Элементы.ПослеРейса_МедосмотрСторонняяОрганизация.ТолькоПросмотр = Истина;
		Объект.ПослеРейса_МедосмотрСторонняяОрганизация = Объект.ДоРейса_МедосмотрСторонняяОрганизация;
	Иначе
		Элементы.ПослеРейса_МедосмотрСторонняяОрганизация.ТолькоПросмотр = Ложь;
	КонецЕсли;
	...
	ОтобразитьМедРаботника_ПослеРейса = Не (Объект.ПослеРейса_МедосмотрСторонняяОрганизация И Признаки.Этапы.ВыпускДоступен 
		Или ЗначениеЗаполнено(Объект.ПослеРейса_МедицинскаяОрганизация));
	Элементы.МедосмотрЗаездСтрокой.Видимость     = ОтобразитьМедРаботника_ПослеРейса;
	Элементы.ГруппаПослеРейса_Лицензия.Видимость = ОтобразитьМедРаботника_ПослеРейса;	
КонецПроцедуры
...
Процедура ОбновитьЛентуНаСервере() Экспорт
	...
	МодульКода("ESD_ТНОбщегоНазначенияСервер").ЛентаТитулов_Обновить(ЭтаФорма, Истина);
...
Процедура ПослерейсовыйМедосмотрТребуетсяПриИзмененииНаСервере()
	...
	ОбновитьВидимостьИДоступностьЭлементов_ПослерейсовыйМедосмотр_ПриИзменении(ПризнакиДокумента());
КонецПроцедуры
...
#Область include_etrn_base_Document_ПутевойЛист_DocumentForm_ФормаДокумента_ЗаполнениеНаОсновании
#КонецОбласти // include_etrn_base_Document_ПутевойЛист_DocumentForm_ФормаДокумента_ЗаполнениеНаОсновании
