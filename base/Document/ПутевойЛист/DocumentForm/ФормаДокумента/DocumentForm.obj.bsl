...
Процедура ОбновитьВидимостьИДоступностьЭлементов()
	...
	ЭтаФорма.ТолькоПросмотр = ЗапрещеноИнтерактивноеИзменение(Признаки);
	...
	Если ЭтоУниверсальноеРасширение() Тогда
		...
		Элементы.ФормаОбновитьДокументИзСбис.Видимость = Не ЭтаФорма.ТолькоПросмотр;
		Элементы.ФормаЗагрузитьВложенияВ1С.Видимость = Не ЭтаФорма.ТолькоПросмотр;
	КонецЕсли;
КонецПроцедуры

&НаСервере
Функция ЗапрещеноИнтерактивноеИзменение(Знач Признаки = Неопределено)
	Если Признаки = Неопределено Тогда
		Признаки = ПризнакиДокумента();
	КонецЕсли;
	
	Возврат Признаки.Этапы.ДОЗавершен 
		Или Не ПравоДоступа("ИнтерактивноеДобавление", Метаданные.Документы.ESD_ПутевойЛист);
КонецФункции
...
Процедура ОбновитьВидимостьИДоступностьЭлементов_ПослерейсовыйМедосмотр_ПриИзменении(Признаки = Неопределено)
	...
	Иначе
		...
		Элементы.ПослеРейса_МедицинскаяОрганизация.ТолькоПросмотр = Ложь;
	КонецЕсли;
