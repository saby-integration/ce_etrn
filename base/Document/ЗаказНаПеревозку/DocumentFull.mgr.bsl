...
Функция ВыгрузкаДанныеОформление(СсылкаНаДокумент)
	...
	Таблицы.Добавить("Груз_АдресаПунктов");
...
Функция ТитулУтверждение(ДанныеДокумента)
	СтруктураТитула = Новый Структура;
	// Отказываемся закрывать второй титул для исходящего документа
	Если ДанныеДокумента.Направление = ЗначениеМетаданных("ESD_Направление.Исходящий") Тогда
		Возврат СтруктураТитула;
	КонецЕсли;
	...
	СтруктураТитула.ТранспортноеСредство.Вставить("Прицеп", 
		МодульТНВыгрузкаСервер.ПодстановкиПрицепов(ДанныеДокумента));
	СтруктураТитула.ТранспортноеСредство.Вставить("СпецУсловия", 
		МодульТНВыгрузкаСервер.СпецУсловия(ДанныеДокумента.СпецРазрешения));
