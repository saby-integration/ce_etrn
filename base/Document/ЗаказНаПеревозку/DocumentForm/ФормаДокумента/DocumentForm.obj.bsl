...
Процедура ОбновитьВидимостьПоЭтапуИУчастнику()
	...
	МодульТНОбщегоНазначенияКлиентСервер.ИзменитьСвойствоЭлементовФормы(
		ЭтаФорма, "Видимость", Признаки.Этапы.Номер >= НомерТитулаУтверждение, СписокЭлементов);
	МодульТНОбщегоНазначенияКлиентСервер.ИзменитьСвойствоЭлементовФормы(
		ЭтаФорма, "ТолькоПросмотр", Не Признаки.Этапы.УтверждениеДоступно Или ЭтаФорма.ТолькоПросмотр, СписокЭлементов, Истина);
		
	СписокЭлементов = Новый Массив;
	СписокЭлементов.Добавить("ПрицепыДобавить");
	СписокЭлементов.Добавить("ВодителиДобавить");
	МодульТНОбщегоНазначенияКлиентСервер.ИзменитьСвойствоЭлементовФормы(
		ЭтаФорма, "Доступность", Признаки.Этапы.УтверждениеДоступно, СписокЭлементов, Ложь);
КонецПроцедуры
...
Функция ПризнакиДокумента()
	...
	РезультатФункции.Вставить("Исходящий", Объект.Направление = ЗначениеМетаданных("ESD_Направление.Исходящий"));
