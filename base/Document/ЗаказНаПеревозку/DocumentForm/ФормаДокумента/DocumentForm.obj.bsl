...
&НаКлиенте
Процедура ПредзаполнитьДанныеТ2ПриИзменении(Элемент)
...
Функция ЗапрещеноИнтерактивноеИзменение(Знач Признаки = Неопределено)
	...
	Возврат Признаки.Этапы.ДОЗавершен 
		Или Не ПравоДоступа("ИнтерактивноеДобавление", Метаданные.Документы.ESD_ЗаказНаПеревозку);
КонецФункции
...
Процедура ОбновитьВидимостьПоЭтапуИУчастнику_Оформление(Признаки = Неопределено)
	...
	СписокЭлементов = Новый Массив;
	СписокЭлементов.Добавить("ПредзаполнитьДанныеТ2");
	МодульТНОбщегоНазначенияКлиентСервер.ИзменитьСвойствоЭлементовФормы(ЭтаФорма,
...
Процедура ОбновитьВидимостьПоЭтапуИУчастнику_Утверждение_Предзаполнение()
	...
	ПризнакДоступности = ЭтаФорма.ПредзаполнитьДанныеТ2 И Не ЭтаФорма.ТолькоПросмотр;
	СписокЭлементов = Новый Массив;
...
Процедура ОбменДаннымиЗнП(Результат, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ИмяКоманды = "ВыгрузитьВСбис" Тогда
		...
		ПараметрыВыгрузки.ДополнительныеПараметры.Вставить("ПредзаполнитьДанныеТ2", ЭтаФорма.ПредзаполнитьДанныеТ2);
