...
Процедура ПредставительЗаказчикаПриИзменении(Элемент)
	...
	Если Не Объект.ПредставительЗаказчика Тогда
		...
		ЭтаФорма.ОформительСтрокой = МодульESD_ОбщегоНазначенияКлиентСервер.ПредставлениеЮрЛицаПоРоли(
...
Процедура ОтветственныйЗаПеревозкуСтрокойНажатие(Элемент, СтандартнаяОбработка)
	...
	ПризнакТолькоПросмотр_ = Элементы.СтраницаВодителиИТС.ТолькоПросмотр 
		Или Не Элементы.ГруппаОтветственный.Доступность 
		Или Элементы.ГруппаОтветственный.ТолькоПросмотр;
	СтруктураОтветственного.Вставить("ТолькоПросмотр", ПризнакТолькоПросмотр_);
		
	СтруктураОтветственного.Вставить("ИмяМетаданных",
...
&НаКлиенте
Процедура ОформлениеПредзаполнятьДанныеПеревозчикаПриИзменении(Элемент)
	ОбновитьВидимостьПоЭтапуИУчастнику_Утверждение_Предзаполнение();
КонецПроцедуры
...
Процедура ГрузыПередУдалением(Элемент, Отказ)
	...
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер")
		.УдалитьСтрокиСвязанныхТаблиц(ЭтаФорма, СтруктураСвязанныхТаблиц);
КонецПроцедуры
...
Процедура ГрузыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		Грузы_УстановитьОтборПоСвязаннымТаблицам();
		Грузы_ЗаполнитьОбщиеЗначенияНаОсновании();
	КонецЕсли;
	
КонецПроцедуры  

&НаКлиенте
Процедура Грузы_ПослеУдаления(Элемент)
	Грузы_ЗаполнитьОбщиеЗначенияНаОсновании();
КонецПроцедуры

&НаКлиенте
Процедура Грузы_МассаБруттоПриИзменении(Элемент)
	Объект.ГрузоподъемностьТС = Грузы_АгрегироватьЧисленныеЗначения().ГрузоподъемностьТС;
КонецПроцедуры 

&НаКлиенте
Процедура Грузы_ОбъемПриИзменении(Элемент)
	Объект.ВместимостьТС = Грузы_АгрегироватьЧисленныеЗначения().ВместимостьТС;
КонецПроцедуры
...
#Область include_etrn_base_Document_ЗаказНаПеревозку_DocumentForm_ФормаДокумента_ДополнительныеКомандыSaby
#КонецОбласти // include_etrn_base_Document_ЗаказНаПеревозку_DocumentForm_ФормаДокумента_ДополнительныеКомандыSaby
...
&НаСервере
Процедура ПриЧтенииСозданииПослеЗаписиНаСервере(ПовторныйВызов = Ложь) Экспорт
	ЗаполнитьРасчетныеРеквизиты(ПовторныйВызов);
...
&НаСервере
Процедура ЗаполнитьРасчетныеРеквизиты(ПовторныйВызов = Ложь)
	...
	Если Не ПовторныйВызов Тогда
		// Загрузим актуальные настройки ЭЗЗ
		НастройкиЭЗЗ_ = МодульТНОбщегоНазначенияСервер.НастройкиЭЗЗ_Загрузить();
		ЗаполнитьЗначенияСвойств(ЭтаФорма, НастройкиЭЗЗ_);
	КонецЕсли;
...
Процедура ОбновитьВидимостьИДоступностьЭлементов()
	ДоступноРедактирование_ = ПравоДоступа("ИнтерактивноеДобавление", Метаданные.Документы.ESD_ЗаказНаПеревозку);

	Признаки = ПризнакиДокумента();
	
	// Определим и установим признак запрета интерактивного изменения формы
	ЭтаФорма.ТолькоПросмотр = Признаки.Этапы.ДОЗавершен Или Не ДоступноРедактирование_;

	// Настроим доступность команд
	Элементы.ФормаВыгрузитьВСбис.Видимость = Не ЭтаФорма.ТолькоПросмотр;
	Элементы.ФормаПрочитатьДокумент.Видимость = ДоступноРедактирование_;
	Элементы.ФормаЗагрузитьВложенияВ1С.Видимость = ДоступноРедактирование_;
...
Процедура ОбновитьВидимостьПоЭтапуИУчастнику()
	...
	ОбновитьВидимостьПоЭтапуИУчастнику_Оформление(Признаки);
	ОбновитьВидимостьПоЭтапуИУчастнику_Утверждение(Признаки);
	
	Если Признаки.Этапы.ОформлениеДоступно Тогда
		ОбновитьВидимостьПоЭтапуИУчастнику_Утверждение_Предзаполнение();
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура ОбновитьВидимостьПоЭтапуИУчастнику_Оформление(Признаки = Неопределено)
	МодульТНОбщегоНазначенияКлиентСервер = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер");
	Если Признаки = Неопределено Тогда 
		Признаки = ПризнакиДокумента();
	КонецЕсли;
	
	ПризнакДоступноОформление = Признаки.Этапы.ОформлениеДоступно И Не Этаформа.ТолькоПросмотр;

	Элементы.ГруппаДокументОснование.Видимость = Признаки.Исходящий;
	Элементы.НастройкаДокументовОснований.Видимость = Ложь;
	Элементы.ГруппаКомандыДокументаОснования.Видимость = ПризнакДоступноОформление;
	Элементы.ДекорацияДокументОснование.Видимость = ПризнакДоступноОформление 
	...
	МодульТНОбщегоНазначенияКлиентСервер.ИзменитьСвойствоЭлементовФормы(ЭтаФорма,
		"ТолькоПросмотр", Не ПризнакДоступноОформление, СписокЭлементов, Истина);
	
	СписокЭлементов = Новый Массив;
	СписокЭлементов.Добавить("ОформлениеПредзаполнятьДанныеПеревозчика");
	МодульТНОбщегоНазначенияКлиентСервер.ИзменитьСвойствоЭлементовФормы(ЭтаФорма,
		"Видимость", ПризнакДоступноОформление, СписокЭлементов, Ложь);
КонецПроцедуры

&НаСервере
Процедура ОбновитьВидимостьПоЭтапуИУчастнику_Утверждение_Предзаполнение()
	МодульТНОбщегоНазначенияКлиентСервер = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер");
	
	// Для "оформления" при предзаполнении "утверждения" (отображение и заполнение)
	ПризнакДоступности = ЭтаФорма.ОформлениеПредзаполнятьДанныеПеревозчика И Не ЭтаФорма.ТолькоПросмотр;
	СписокЭлементов = Новый Массив;
	СписокЭлементов.Добавить("ГруппаОплата");
	СписокЭлементов.Добавить("СтраницаВодителиИТС");
	МодульТНОбщегоНазначенияКлиентСервер.ИзменитьСвойствоЭлементовФормы(ЭтаФорма,
		"Видимость", ПризнакДоступности, СписокЭлементов, ПризнакДоступности);
	МодульТНОбщегоНазначенияКлиентСервер.ИзменитьСвойствоЭлементовФормы(ЭтаФорма,
		"ТолькоПросмотр", Не ПризнакДоступности, СписокЭлементов, Не ПризнакДоступности);
	
	// Для "оформления" при предзаполнении "утверждения" (только отображение)
	СписокЭлементов = Новый Массив;
	...
	МодульТНОбщегоНазначенияКлиентСервер.ИзменитьСвойствоЭлементовФормы(ЭтаФорма,
		"Доступность", ПризнакДоступности, СписокЭлементов, ПризнакДоступности);	
КонецПроцедуры

&НаСервере
Процедура ОбновитьВидимостьПоЭтапуИУчастнику_Утверждение(Признаки = Неопределено)
	МодульТНОбщегоНазначенияКлиентСервер = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер");
	Если Признаки = Неопределено Тогда 
		Признаки = ПризнакиДокумента();
	КонецЕсли;
	
	// Только для "утверждения"
	ПризнакТолькоПросмотр = Не Признаки.Этапы.УтверждениеДоступно Или ЭтаФорма.ТолькоПросмотр;
	СписокЭлементов = Новый Массив;
	СписокЭлементов.Добавить("ГруппаОплата");
	СписокЭлементов.Добавить("СтраницаВодителиИТС");
	СписокЭлементов.Добавить("ГруппаУполномоченныеЛицаПеревозчика");
	СписокЭлементов.Добавить("ГруппаОтветственный");
	МодульТНОбщегоНазначенияКлиентСервер.ИзменитьСвойствоЭлементовФормы(ЭтаФорма,
		"ТолькоПросмотр", ПризнакТолькоПросмотр, СписокЭлементов, ПризнакТолькоПросмотр);
	СписокЭлементов = Новый Массив;
	СписокЭлементов.Добавить("ПрицепыДобавить");
	СписокЭлементов.Добавить("ВодителиДобавить");
	МодульТНОбщегоНазначенияКлиентСервер.ИзменитьСвойствоЭлементовФормы(ЭтаФорма,
		"Доступность", Не ПризнакТолькоПросмотр, СписокЭлементов, Не ПризнакТолькоПросмотр);
КонецПроцедуры

&НаКлиенте
Функция Грузы_АгрегироватьЧисленныеЗначения()
	РезультатФункции = Новый Структура();	
	ГрузоподъемностьТС_ = 0;
	ВместимостьТС_ = 0;
	
	Для Каждого Строка Из Объект.Грузы Цикл
		ГрузоподъемностьТС_ = ГрузоподъемностьТС_ + Строка.МассаБрутто;
		ВместимостьТС_ = ВместимостьТС_ + Строка.Объем;
	КонецЦикла; 
	
	РезультатФункции.Вставить("ГрузоподъемностьТС", Окр((ГрузоподъемностьТС_/1000)+0.5,0,0));
	РезультатФункции.Вставить("ВместимостьТС", Окр(ВместимостьТС_+0.5,0,0)); 
	
	Возврат РезультатФункции;
КонецФункции

&НаКлиенте
Процедура Грузы_ЗаполнитьОбщиеЗначенияНаОсновании(ДанныеГрузов = Неопределено, ПринудительноУстанваливать = Ложь)
	Если ДанныеГрузов = Неопределено Тогда
		ДанныеГрузов = Грузы_АгрегироватьЧисленныеЗначения();	
	КонецЕсли; 
	
	Для Каждого КлючЗначение Из ДанныеГрузов Цикл
		Если ПринудительноУстанваливать Или ЗначениеЗаполнено(КлючЗначение.Значение) Тогда
...
Функция ПризнакиДокумента()
	РезультатФункции = ПризнакиДокумента_СтруктураОписания();
	РезультатФункции.Исходящий = (Объект.Направление <> ЗначениеМетаданных("ESD_Направление.Входящий"));
		
	РезультатФункции.Этапы.ДОЗавершен = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер")
		.ДокументооборотЗавершен(ЭтаФорма.ИндексКартинки);
	
	МодульESD_ОбщегоНазначенияСервер = МодульКода("ESD_ТНОбщегоНазначенияСервер");
	РезультатФункции.Этапы.ОформлениеДоступно = МодульESD_ОбщегоНазначенияСервер.ТипТитулаДоступенУчастнику(ЭтаФорма, 
		Объект.ТекущиеЭтапы, ЗначениеМетаданных("ESD_ТипТитулаЗнП.Оформление"), РезультатФункции.Этапы.ДОЗавершен);
		
	РезультатФункции.Этапы.УтверждениеДоступно = МодульESD_ОбщегоНазначенияСервер.ТипТитулаДоступенУчастнику(ЭтаФорма,
	    Объект.ТекущиеЭтапы, ЗначениеМетаданных("ESD_ТипТитулаЗнП.Утверждение"), РезультатФункции.Этапы.ДОЗавершен);
	
	Если РезультатФункции.Этапы.ОформлениеДоступно Тогда
		РезультатФункции.Этапы.Номер = 1;
	ИначеЕсли РезультатФункции.Этапы.УтверждениеДоступно Тогда
		РезультатФункции.Этапы.Номер = 2;
	ИначеЕсли РезультатФункции.Этапы.ДОЗавершен Тогда
		РезультатФункции.Этапы.Номер = 3;
	Иначе
		РезультатФункции.Этапы.Номер = 4;
	КонецЕсли;
...
Функция ПризнакиДокумента_СтруктураОписания()
	РезультатФункции = Новый Структура("Исходящий", Истина);
	
	Этапы_ = Новый Структура("УтверждениеДоступно, ОформлениеДоступно", Ложь, Ложь);
	Этапы_.Вставить("ДОЗавершен", Ложь);
	Этапы_.Вставить("Номер", 0);
	РезультатФункции.Вставить("Этапы", Этапы_);
...
Процедура ОбменДаннымиЗнП(Результат, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ИмяКоманды = "ВыгрузитьВСбис" Тогда
		ПараметрыВыгрузки = МодульESD_ОбщегоНазначенияКлиентСервер.ПараметрыВыгрузки(Объект.Ссылка);
		ПараметрыВыгрузки.ДополнительныеПараметры = ВыгрузкаДанных_ДополнительныеПараметры();
		ДанныеДляОбработки = Новый Массив;
		ДанныеДляОбработки.Добавить(ПараметрыВыгрузки);
	Иначе
		ПараметрыЗагрузки = МодульESD_ОбщегоНазначенияКлиентСервер.ПараметрыЗагрузки(Объект.Ссылка);
	...
	ДополнительныеПараметрыОбмена = Новый Структура("ДвоичныеДанныеОбработки", Неопределено);
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ВыполнитьКомандуСбис(ЭтаФорма,
		ИмяКоманды, ДанныеДляОбработки, ДополнительныеПараметрыОбмена);	
КонецПроцедуры

&НаКлиенте
Функция ВыгрузкаДанных_ДополнительныеПараметры() Экспорт
	РезультатФункции = Новый Структура;
	РезультатФункции.Вставить("ОформлениеПредзаполнятьДанныеПеревозчика", ЭтаФорма.ОформлениеПредзаполнятьДанныеПеревозчика);
...
Функция СвязанныеСГрузомТаблицы()
	РезультатФункции = Новый Структура("Грузы", Новый Массив);
	РезультатФункции.Грузы.Добавить("Груз_АдресаПунктов");
	
	СвязьМеждуГрузомИДаннымиЮрЛиц = ESD_ТНОбщегоНазначенияКлиентСервер
		.ОписаниеСвязиТаблицы("ДанныеЮрЛиц", "КлючСтроки_ДанныеЮрЛиц", "КлючСтроки");
	СвязьМеждуГрузомИДаннымиЮрЛиц.ОписаниеСвязейТаблицы.Добавить("КонтактныеДанные");
	РезультатФункции.Грузы.Добавить(СвязьМеждуГрузомИДаннымиЮрЛиц);
...
Функция СоответствиеТаблицФормыИОбъекта()
	...
	СоответствиеТаблицФормыИОбъекта.Вставить("АдресаПунктовВыгрузки", 		"ДанныеЮрЛиц"); 
...
#Область include_etrn_base_Document_ЗаказНаПеревозку_DocumentForm_ФормаДокумента_ЗаполнениеНаОсновании
#КонецОбласти // include_etrn_base_Document_ЗаказНаПеревозку_DocumentForm_ФормаДокумента_ЗаполнениеНаОсновании
