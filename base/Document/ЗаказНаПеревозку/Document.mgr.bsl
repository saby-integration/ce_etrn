...
Функция ИмяТЧДокумента(ИмяТабличнойЧасти, СтрокаТабличнойЧасти, ДанныеДокумента) Экспорт
	...
	Если ИмяТабличнойЧасти = "ДанныеЮрЛиц"
		И СтрокаТабличнойЧасти.Роль = ЗначениеМетаданных("ESD_РолиКонтрагентов.УполномоченноеЛицо") Тогда
...
Функция ПроверятьРеквизитТЧДокумента(СтрокаТЧ, ИмяТЧ, ИмяРеквизита, ДанныеДокумента) Экспорт
	...
	Если ИмяТЧ = "ДанныеЮрЛиц" Тогда
		Возврат СтрокаТЧ.Роль = ЗначениеМетаданных("ESD_РолиКонтрагентов.УполномоченноеЛицоПеревозчика")
	...
	ИначеЕсли ИмяТЧ = "УполномоченныеЛицаЗаказчика" Тогда
		Возврат МодульКода("ESD_ТНОбщегоНазначенияСервер").РезультатФункцииТитулов(
...
Функция ТипыПоУмолчаниюДляЗаполненияНаОсновании()
	...
	Если Метаданные.Синоним <> ДоступныеКонфигурации.УАТ 
		И Метаданные.Синоним <> ДоступныеКонфигурации.УАТПроф Тогда
...
Процедура ДопПроверкиФЛКУтверждение(ДанныеДокумента, ПараметрыВыгрузки, Ошибки)
	...
	МодульКода("ESD_ТНОбщегоНазначенияСервер").ПроверитьЗаполнениеТипаВладения(ДанныеДокумента, Ошибки, СоответствиеОшибок);
...
Функция ВодителиДляВыгрузки(ДанныеДокумента)
	...
	Для Каждого СтрокаВодитель Из СтрокиВодителей Цикл
		...
		Водитель = МодульКода("ESD_ТНОбщегоНазначенияСервер").СтруктураОтветственного(
...
Функция ДанныеУтверждение(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Для Каждого Вложение Из ВложенияДляЗагрузки Цикл
		...
		МодульТНЗагрузкаСервер.ЗагрузитьОтветственного(
			...
			МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(Вложение, "Грузоперевозчик.Ответственный"));
		...
		УполномоченноеЛицо = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(
...
Процедура ЗагрузитьМаршрут(ДляДокумента, Маршрут)
	...
	ДляДокумента.Вставить(
		"КонечнаяТочкаАдрес",
		МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Маршрут, "КонечныйПункт.Название"));
	
	// Отправление
	Отправление = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Маршрут, "Отправление");
	
	ДляДокумента.Вставить(
		"ПодачаАдрес",
		МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Отправление, "АдресТекст"));
	
	// Подача ТС
	ПодачаДата = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Отправление, "ПодачаТС.ДатаВремя");
	...
	Интервал = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
...
Процедура ЗагрузитьПунктыВыгрузки(ДляДокумента, Маршрут)
	...
	Для Каждого Пункт Из Пункты Цикл
		...
		// Операция
		ДатаВремяСтрокой = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Пункт, "Операция.ДатаВремя");
		СтруктураПункта.Вставить(
			"ДатаВремя",
			МодульКода("ESD_ТНОбщегоНазначенияСервер").ПреобразоватьСтрокуВДатуЭПД(ДатаВремяСтрокой));
		
		ИнтервалСтрокой = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
		...
		Тип = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Пункт, "Операция.Тип");
		СтруктураПункта.Вставить("ЭтоПогрузка", Тип = "Погрузка");
		
		Если СтруктураПункта.ЭтоПогрузка Тогда
			СтруктураПункта.Вставить(
				"ОрганизацияСтрока", 
				МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Пункт, "Организация.Название"));
			
			СтруктураПункта.Вставить(
				"ОрганизацияИНН",
				МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Пункт, "Организация.ИНН"));
...
Процедура ЗагрузитьУсловияПеревозки(ДляДокумента, УсловияПеревозки)
	...
	// УсловияПеревозки
	СпецУсловия    = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(УсловияПеревозки, "УказанияНорм");
	ПредельныйСрок = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(УсловияПеревозки, "ПредельныйСрок");
	ИныеУсловия    = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(УсловияПеревозки, "Иные");
	ТребованияКТС  = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
	...
	ВлажностьОт   = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(КлиматическийРежим, "Влажность.Минимальная");
	ВлажностьДо   = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(КлиматическийРежим, "Влажность.Максимальная");
	ТемператураОт = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(КлиматическийРежим, "Температура.Минимальная");
	ТемператураДо = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(КлиматическийРежим, "Температура.Максимальная");
...
Процедура ЗагрузитьПараметрыТС(ДляДокумента, ПараметрыТС)
	...
	ТипТС      = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(ПараметрыТС, "Тип");
	КузовТС    = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(ПараметрыТС, "ТипКузова");
	ПогрузкаТС = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(ПараметрыТС, "ТипПогрузки");
	
	ДляДокумента.Вставить("ТипТС",      ТипТС);
	ДляДокумента.Вставить("КузовТС",    КузовТС);
	ДляДокумента.Вставить("ПогрузкаТС", ПогрузкаТС);
									
	ГрузоподъемностьТС = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(ПараметрыТС, "Грузоподъемность");
	ВместимостьТС      = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(ПараметрыТС, "Вместимость");
...
Процедура ЗагрузитьДанныеДокументовЭПД(ДляДокумента, ДанныеДокументов)
	...
	Договор = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(ДанныеДокументов, "Договор");
	...
	СоставительОснование = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(
...
Процедура ЗаполнитьДанныеЮрЛиц(ДляДокумента, Вложение)
	...
	УполномоченныеЛица = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(
	...
	Если ЗначениеЗаполнено(Основание) Тогда 
		...
		Составитель = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Вложение, "Файл.Составитель.Наименование");
...
Процедура ЗагрузитьДанныеПоСтоимости(ДляДокумента, СтоимостьПеревозки)
	...
	ДляДокумента.Вставить("ВалютаКод", МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(СтоимостьПеревозки, "Валюта.Код"));
	ДляДокумента.Вставить(
		"ВалютаНаименование", МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(СтоимостьПеревозки, "Валюта.Наименование"));
	
	СтавкаНДС = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(СтоимостьПеревозки, "Налог.Ставка");
...
Процедура ЗагрузитьКомментарийОтрицательногоЗавершения(ДляДокумента, Вложение)
	...
	ОтветПеревозчика = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(Вложение, "Документ.Ответ.Значение");
	ОтказаноВПриемке = ОтветПеревозчика = "2";
	
	Если ОтказаноВПриемке Тогда
		ДляДокумента.Вставить(
			"КомментарийУведомление",
			МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(Вложение, "Документ.Ответ.Причина"));
	КонецЕсли;
