...
Процедура ДобавитьОснованияДляЗаполнения(ОснованияДляЗаполнения, РезультатФункции) Экспорт
	...
	Для Каждого НовоеОснование Из РезультатФункции.НовыеОснования Цикл
		...
		СтруктураОснования.ini_name        = "Blockly_ДанныеКонтрагентаОрганизации_ЭПЛ_read";
...
Процедура ЗагрузкаТитулаПослерейсовыйМедосмотр(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Если Не ДопПараметры.ТолькоАктивныйЭтап Тогда
		...
		ЗаполнитьОтветственныхЛиц(
			ОбъектДок, ДанныеТитула.ДляДокумента.ОтветственныеЗаМедосмотр, Перечисления.Saby_РолиОтветственных.МедосмотрЗаезд);
