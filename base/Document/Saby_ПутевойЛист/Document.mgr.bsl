...
Функция ТекущиеЭтапыДокумента(Форма) Экспорт
	...
	Если РезультатФункции.Количество() = 0 Тогда
		РезультатФункции.Добавить(Перечисления.Saby_ТипТитулаЭПЛ.ПервыйТитул());
	КонецЕсли;
...
Функция ДанныеВыпуск(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Для Каждого Вложение Из ВложенияДляЗагрузки Цикл
		...
		ЗагрузитьСтороныВыпуск(ДляДокумента, ДопПараметры.Стороны);
...
Функция ДанныеПослерейсовыйМедосмотр(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Для Каждого Вложение Из ВложенияДляЗагрузки Цикл
		...
		ЗагрузитьМедосмотр(ДляДокумента, Вложение["ПостМедосмотр"], Ложь);
...
Функция СтруктураДанныхДляРегистра(ОбъектДок, Вложение, Титул, ИмяМетаданных, СохранитьДанные = Ложь)
	...
	Основной = Не Перечисления.Saby_ТипТитулаЭПЛ.ЭтоДинамическийТитул(Титул);
...
Процедура ЗаполнитьИдентификаторИДатуТитулаОтказа(Вложение, СобытияСОнлайна, Титул)
	...
	Для Каждого СобытиеСоответствие Из СобытияСОнлайна Цикл
		...
		ТитулСобытия = Перечисления.Saby_ТипТитулаЭПЛ.ЗначениеПоПредставлению(СобытиеСоответствие["Название"]);
	...
	ДатаВремя = Saby_ТНОбщегоНазначенияСервер.ПреобразоватьСтрокуВДату(СтруктураСобытия["ДатаВремя"]);
...
Процедура ЗагрузитьДанныеДокумента(ДляДокумента, ДокументСОнлайна, ДопПараметры)
	...
	Если ЗначениеЗаполнено(ДопПараметры.СторонняяОрганизация) Тогда
		Saby_ТНЗагрузкаСервер.ЗагрузитьДанныеОрганизации(
			ДляДокумента, ДопПараметры.СторонняяОрганизация, РольОформитель, , Истина);
	Иначе
		Saby_ТНЗагрузкаСервер.ЗагрузитьДанныеОрганизации(ДляДокумента, ДокументСОнлайна["Оформитель"], РольОформитель);
	КонецЕсли;
...
Процедура ЗагрузитьДанныеМаршрутов(ДляДокумента, МаршрутСОнлайна)
	...
	Для Каждого Грузоотправитель Из Грузоотправители Цикл
		...
		Saby_ТНЗагрузкаСервер.ЗагрузитьДанныеОрганизации(ДляДокумента, Грузоотправитель, РольКонтрагента, КлючСтроки);
...
Процедура ЗагрузитьМедосмотр(ДляДокумента, МедосмотрСОнлайна, ЭтоПредрейсовыйМедосмотр)
	...
	Если Не ШтатныйСотрудник Тогда
		...
		Saby_ТНЗагрузкаСервер.ЗагрузитьДанныеОрганизации(ДляДокумента, МедосмотрСОнлайна["Организация"], Роль, , Истина);
...
Процедура ЗагрузитьСтороныВыпуск(ДляДокумента, Стороны)
	
	Медосмотр = Saby_ТНОбщегоНазначенияСервер.ДанныеСоответствия(Стороны, "Медосмотр");
	ЗагрузитьМедосмотр(ДляДокумента, Медосмотр, Истина);
	
	ПостМедосмотр = Saby_ТНОбщегоНазначенияСервер.ДанныеСоответствия(Стороны, "ПостМедосмотр");
	ЗагрузитьМедосмотр(ДляДокумента, ПостМедосмотр, Ложь);
	
	Техосмотр = Saby_ТНОбщегоНазначенияСервер.ДанныеСоответствия(Стороны, "Техосмотр");
