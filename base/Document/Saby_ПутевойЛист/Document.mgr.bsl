...
Процедура СформироватьНовыеОснования(СтруктураРезультата, ДанныеИзИНИ) Экспорт
	...
	ОформительСсылка = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(ДанныеИзИНИ, "Документ.ОформительСсылка");
	Если ЗначениеЗаполнено(ОформительСсылка) Тогда
		
		СтруктураОснования = Saby_ТНОбщегоНазначенияСервер.СтруктураОснования(
			ОформительСсылка, "", "Оформитель");
...
Функция ФЛК(РеквизитОбъектаЭПЛ, ЭлементФормыЭПЛ, ОшибкаЭПЛ)
	...
	Возврат Saby_ТНОбщегоНазначенияСервер.СтруктураФЛК(РеквизитОбъектаЭПЛ, ЭлементФормыЭПЛ, ОшибкаЭПЛ);
...
Процедура ДополнитьПроверяемыеРеквизитыЭтапаВыпуск(ДанныеДокумента, ПроверяемыеРеквизиты)
	...
	Если ДанныеДокумента.ВидПеревозки = Перечисления.Saby_ВидПеревозки.КоммерческиеПеревозки Тогда
		ПроверяемыеРеквизиты.Добавить(
...
Процедура ДопПроверкиФЛКВыпуск(ДанныеДокумента, ПараметрыВыгрузки, Ошибки)
	...
	Saby_ТНОбщегоНазначенияСервер.ПроверитьЗаполнениеТранспортногоСредства(ДанныеДокумента, Ошибки, СтруктураОшибки);
...
Функция ДанныеИзБД(СсылкаНаДокумент, Реквизиты, Таблицы)
	...
	РезультатФункции.Вставить("ИмяМетаданных", "Saby_ПутевойЛист");
...
Функция ТитулВыпуск(ДанныеДокумента)
	...
	Дата               = Saby_ТНВыгрузкаСервер.ЗначениеДатыВремяДляВыгрузки(ДанныеДокумента.Дата, Истина);
	ПризнакНачалаРейса = Перечисления.Saby_ПризнакНачалаРейса.КодПоЗначению(ДанныеДокумента.ПризнакНачалаРейса);
	ВидПеревозки       = Перечисления.Saby_ВидПеревозки.КодПоЗначению(ДанныеДокумента.ВидПеревозки);
	...
	СтруктураТитула.Вставить(
		...
		Saby_ТНВыгрузкаСервер.ПодстановкиТранспортногоСредства(ДанныеДокумента, , Истина));
		
	СтруктураТитула.ТранспортноеСредство.Вставить(
		"Прицеп", 
		Saby_ТНВыгрузкаСервер.ПодстановкиПрицепов(ДанныеДокумента, Истина));
	
	СтруктураТитула.Вставить(
		"Водители", 
		Saby_ТНВыгрузкаСервер.ПодстановкиВодителя(ДанныеДокумента.Водители, Новый Массив, , Истина));
...
Функция ТитулВыпускающий(ДанныеДокумента)
	...
	Выезд.Вставить(
		"ДатаВремя",
		Saby_ТНВыгрузкаСервер.ЗначениеДатыВремяДляВыгрузки(ДанныеДокумента.Выезд_ДатаВремя));
...
Функция ТитулПринимающий(ДанныеДокумента)
	...
	Заезд.Вставить(
		"ДатаВремя",
		Saby_ТНВыгрузкаСервер.ЗначениеДатыВремяДляВыгрузки(ДанныеДокумента.Заезд_ДатаВремя));
...
Функция ДанныеУзлаМаршрут(ДанныеДокумента)
	...
	Для Каждого СтрокаДанныхЮрЛиц Из ДанныеДокумента.ДанныеЮрЛиц Цикл
		...
		СтруктураДанных = Saby_ТНВыгрузкаСервер.ОбщийШаблонЮрЛица(ДанныеДокумента.ИмяМетаданных);
...
Процедура ДобавитьМедосмотрДляОнлайна(СтруктураДанных, ДанныеДокумента, ЭтоПредрейсовыйМедосмотр)
	...
	Если ШтатныйСотрудник И ДанныеДокумента.Направление = Перечисления.Saby_Направление.Исходящий Тогда
...
Процедура ДобавитьТехосмотрДляОнлайна(СтруктураДанных, ДанныеДокумента)
	...
	Если ДанныеДокумента.Свойство("ТО_НеРазрешенВыпускПосле")
		И Не ДанныеДокумента.ТО_НеРазрешенВыпускПосле Тогда
		
		Техосмотр.Вставить("ВыпускНаЛинию", Новый Структура);
		Техосмотр.ВыпускНаЛинию.Вставить(
			"ДатаВремя",
			Saby_ТНВыгрузкаСервер.ЗначениеДатыВремяДляВыгрузки(ДанныеДокумента.ТО_ДатаВремяВыпускаВРейс));
	КонецЕсли;
...
Процедура ЗагрузкаТитулаВыпуск(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Если Не ДопПараметры.ТолькоАктивныйЭтап Тогда
		...
		Saby_ТНЗагрузкаСервер.ЗаполнитьДанныеЮрЛицИКонтактнуюИнформацию(
			ОбъектДок, ДанныеТитула.ДляДокумента, Перечисления.Saby_РолиКонтрагентов.Оформитель, ДопПараметры);
		...
		Saby_ТНЗагрузкаСервер.ЗаполнитьОтветственныхЛиц(
			ОбъектДок, ДанныеТитула.ДляДокумента.ОтветственныеЛица, Перечисления.Saby_РолиОтветственных.Водитель);
		...
		Saby_ТНЗагрузкаСервер.ЗаполнитьДанныеЮрЛицИКонтактнуюИнформацию(
			ОбъектДок, ДанныеТитула.ДляДокумента, Перечисления.Saby_РолиКонтрагентов.МедосмотрВыезд, ДопПараметры);
		...
		Saby_ТНЗагрузкаСервер.ЗаполнитьДанныеЮрЛицИКонтактнуюИнформацию(
			ОбъектДок, ДанныеТитула.ДляДокумента, Перечисления.Saby_РолиКонтрагентов.МедосмотрЗаезд, ДопПараметры);
...
Функция ДанныеВыпуск(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Для Каждого Вложение Из ВложенияДляЗагрузки Цикл
		...
		Saby_ТНЗагрузкаСервер.ДанныеТСПрицеповСпецРазрешений(ОбъектДок, ДляДокумента, Вложение["ТранспортноеСредство"]);
		
		Saby_ТНЗагрузкаСервер.ДанныеВодителей(ОбъектДок, ДляДокумента, Вложение["Водители"], ДопПараметры);
...
Функция ДанныеВыпускающий(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Для Каждого Вложение Из ВложенияДляЗагрузки Цикл
		...
		ПризнакНачалаРейса = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Вложение, "Документ.ПризнакНачалаРейса");
		...
		ДатаВремя = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Вложение, ПутьДатаВремя);
		ДатаВремя = Saby_ТНОбщегоНазначенияСервер.ПреобразоватьСтрокуВДатуЭПД(ДатаВремя);
		
		Одометр = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Вложение, ПутьОдометр);
		...
		Ответственный = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Вложение, "Выпуск.Ответственный");
...
Функция ДанныеПринимающий(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Для Каждого Вложение Из ВложенияДляЗагрузки Цикл
		...
		ПризнакОкончанияРейса = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Вложение, "Документ.ПризнакОкончанияРейса");
		ДляДокумента.Вставить(
		...
		Ответственный = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Вложение, "Приемка.Ответственный");
...
Процедура ЗаполнитьДанныеОтрицательногоЗакрытия(Вложение, ДляДокумента, СобытияСОнлайна, Титул)
	...
	Для Каждого СобытиеСоответствие Из СобытияСОнлайна Цикл
		...
		ТитулСобытия = Saby_ТНОбщегоНазначенияСервер.РезультатФункцииТитулов(
			"Saby_ПутевойЛист", "ЗначениеПоПредставлению", НазваниеСобытия);
...
Процедура ЗагрузитьДанныеДокумента(ДляДокумента, ДокументСОнлайна, ДопПараметры)
	...
	ПризнакНачалаРейса = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(ДокументСОнлайна, "ПризнакНачалаРейса", "1");
	ДляДокумента.Вставить("ПризнакНачалаРейса", Перечисления.Saby_ПризнакНачалаРейса.ЗначениеПоКоду(ПризнакНачалаРейса));
	...
	Если ЗначениеЗаполнено(ДопПараметры.СторонняяОрганизация) Тогда
		...
		КонтактыОформителя = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(ДокументСОнлайна, "Оформитель.Контакты");
		ПодстановкиЮрЛиц.Вставить("Контакты", КонтактыОформителя);
		АдресОформителя = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(ДокументСОнлайна, "Оформитель.Адрес");
...
Процедура ЗагрузитьДанныеМаршрутов(ДляДокумента, МаршрутСОнлайна)
	...
	ДляДокумента.Вставить(
		"МестоОтправления",
		Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(МаршрутСОнлайна, "Отправление.АдресТекст"));
...
Процедура ЗагрузитьМедосмотр(ДляДокумента, МедосмотрСОнлайна, ЭтоПредрейсовыйМедосмотр, ДопПараметры)
	...
	Если СтруктураМедика.Имя <> Неопределено Тогда
		Лицензия      = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Ответственный, "Лицензия");
	...
	ДатаВремя = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(МедосмотрСОнлайна, "Результат.ДатаВремя");
	ДатаВремя = Saby_ТНОбщегоНазначенияСервер.ПреобразоватьСтрокуВДатуЭПД(ДатаВремя);
	Отметка   = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(МедосмотрСОнлайна, "Результат.Отметка");
	Допущен   = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(МедосмотрСОнлайна, "Результат.Допуск");
...
Функция ЭтоШтатныйМедик(МедосмотрСОнлайна, ДопПараметры)
	...
	Если Не ДопПараметры.ЗагрузкаСОнлайна Тогда
		Возврат Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(МедосмотрСОнлайна, "ШтатныйМедик", Ложь);
	КонецЕсли;
	...
	Иначе
		СтруктураЮрЛицаМедосмотр = Saby_ТНОбщегоНазначенияКлиентСервер.ШаблонДанныхЮрЛица();
	КонецЕсли;
...
Процедура ЗагрузитьТехосмотр(ДляДокумента, ТехосмотрСОнлайна, ТолькоОтветственный = Ложь)
	...
	Если Не ТолькоОтветственный Тогда
		...
		ДатаВремя = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(ТехосмотрСОнлайна, "Результат.ДатаВремя");
		ДатаВремя = Saby_ТНОбщегоНазначенияСервер.ПреобразоватьСтрокуВДатуЭПД(ДатаВремя);
		Отметка   = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(ТехосмотрСОнлайна, "Результат.Отметка");
		
		ДляДокумента.Вставить("ТО_ДатаВремя",   ДатаВремя);
		ДляДокумента.Вставить("ТО_Комментарий", "");
		
		ДатаВыпускаВРейс = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(ТехосмотрСОнлайна, "ВыпускНаЛинию.ДатаВремя");
...
Процедура ЗагрузитьСтороныВыпуск(ДляДокумента, ДопПараметры)
	...
	Медосмотр = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(ДопПараметры.Стороны, "Медосмотр");
	ЗагрузитьМедосмотр(ДляДокумента, Медосмотр, Истина, ДопПараметры);
	
	ПостМедосмотр = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(ДопПараметры.Стороны, "ПостМедосмотр");
	ЗагрузитьМедосмотр(ДляДокумента, ПостМедосмотр, Ложь, ДопПараметры);
	
	Техосмотр = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(ДопПараметры.Стороны, "Техосмотр");
...
Процедура ЗагрузитьСтороныВыпускающий(ДляДокумента, Стороны)
	...
	Ответственный = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Стороны, "Приемка.Ответственный");
...
Процедура ЗагрузитьЗаменуВодителя(ДляДокумента, ДопПараметры)
	...
	ПЛПредыдущий = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(ДопПараметры.ЗаменаВодителя, "ПЛПредыдущий.Идентификатор");
	ПЛСледующий  = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(ДопПараметры.ЗаменаВодителя, "ПЛСледующий.Идентификатор");
