...
Процедура ВодителиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	СтруктураОтветственного = МодульКодаКлиент("ESD_ТНОбщегоНазначенияСервер").СтруктураОтветственного(
...
Процедура ДобавитьВодителя(Команда)
	...
	НоваяСтрока.КлючСтроки = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").НовыйКлючСтроки(ЭтаФорма, "ОтветственныеЛица");
	...
	СтруктураОтветственного = ВыполнитьНаСервере("ESD_ТНОбщегоНазначенияСервер.СтруктураОтветственного",
...
&НаСервере
Процедура ПриЧтенииСозданииПослеЗаписиНаСервере(Знач ПрочитатьИзСтруктуры = Ложь)
...
Процедура ОбновитьДоступностьПолейВвода()
	...
	ДоступныеРоли   = МодульКода("Документы." + ТранспортнаяНакладная()).РолиОрганизации(ИсточникДанных().ДанныеЮрЛиц);
...
Процедура ОбработатьОповещениеЗавершениеВыполнитьДействиеНаФорме(Параметр) Экспорт
	...
	Если Параметр.status = "error" Тогда
		...
		МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ОбработатьОшибкиИзФормыВыполнитьДействие(
