...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ЭтаФорма.ЭтоВО И ЭтаФорма.Открыта() Тогда 
		...
		Иначе
			...
			Если ИмяСобытия = "ESD_ВыполнитьПереходыПоДокументам" И Параметр.Количество() > 0 Тогда
				ЭтаФорма.Ссылка.Идентификатор = Параметр[0].Идентификатор;
			КонецЕсли;
...
Функция ЗаполнениеНаОснованииINIНаСервере()
	...
	Возврат МодульКода("ESD_ТНОбщегоНазначенияСервер").ЗаполнитьНаОснованииINI(
		ЭтаФорма, ДокументОснование, ТранспортнаяНакладная());
...
Процедура ОбменДаннымиСБИС(Результат, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ИмяКоманды = "ВыгрузитьВСбис" Тогда
		...
		ПараметрыВыгрузки = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ПараметрыВыгрузки(ИсточникДанных.Ссылка);
	...
	Иначе
		...
		ДанныеДляОбработки = Новый Массив;
		ПараметрыЗагрузки = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ПараметрыЗагрузки(ИсточникДанных.Ссылка);
