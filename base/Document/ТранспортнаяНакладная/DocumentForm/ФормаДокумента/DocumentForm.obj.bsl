...
#Область include_etrn_src_vo2_Document_ТранспортнаяНакладная_DocumentForm_ФормаДокумента_Переменные
#КонецОбласти // include_etrn_src_vo2_Document_ТранспортнаяНакладная_DocumentForm_ФормаДокумента_Переменные
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		МодульКода("ESD_ТНОбщегоНазначенияСервер").УбратьПризнакНеПросмотрен(ИсточникДанных.Ссылка);
	КонецЕсли;
	...
	Если Не ЭтоУниверсальноеРасширение() И Не ЭтаФорма.ЭтоВО И Не ЗначениеЗаполнено(ИсточникДанных.НомерСбис) Тогда
		ИсточникДанных.НомерСбис = ИсточникДанных.Номер;
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ЭтаФорма.ЭтоВО И ЭтаФорма.Открыта() Тогда 
		...
		// закрыть форму
		Если ИмяСобытия = "ЗакрытьСБИС" Или ИмяСобытия = "ПереоткрытьФормуЭПД" Тогда
			ЭтаФорма.Закрыть();
		Иначе
			
			// фиксируем идентификатор выгруженного документа
			Если ИмяСобытия = "ESD_ВыполнитьПереходыПоДокументам" И Параметр.Документы.Количество() > 0 Тогда
				ЭтаФорма.Ссылка.Идентификатор = Параметр.Документы[0].Идентификатор;
			КонецЕсли;
			
			Если ИмяСобытия = "ESD_ОбновитьАктивныйЭтап" Тогда
				ЭтаФорма.Ссылка.Идентификатор = Параметр.Документы[0].Идентификатор;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	ЗаполнитьЮрЛицоКопированиемНаСервере("ВладелецОбъекта", "Отправитель");
...
Процедура ОбменСбис(Команда)
	...
	ЗапретитьОбновление = ЭтаФорма.ЭтоВО
		И ИмяКоманды = "ПрочитатьДокумент"
		И Не ЗначениеЗаполнено(ЭтаФорма.Идентификатор);
		
	Если ЗапретитьОбновление Тогда
		
		ТекстСообщения = "Обновление невозможно. Документ не выгружен в SABY";
		МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").СообщитьПользователю(ТекстСообщения);
		Возврат;
	...
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ПроверитьМодифицированностьИПродолжитьВыполнение(ПроцедураОбменДаннымиСБИС);
...
Процедура ЗаполнитьВыгрузкуПоДаннымГрузоотправителя(Команда)
	...
	ИсточникДанных.Выгрузка_Адрес             = ИсточникДанных.Отправитель_АдресДоставки;
	ИсточникДанных.Выгрузка_Адрес_GLN         = ИсточникДанных.Отправитель_АдресДоставки_GLN;
	
	Выгрузка_МассаБрутто = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ЧислоИзСтроки(ИсточникДанных.Погрузка_Масса);
...
Процедура Сброс(Команда)
	...
	ЭтаФорма.Идентификатор       = "";
	ЭтаФорма.АктивныйЭтапСтрокой = "";
	ЭтаФорма.ТекущийЭтап         = "";
	ЭтаФорма.Состояние           = Неопределено;
	
	ЭтаФорма.ТекущиеЭтапы.Очистить();
...
Процедура ИныеПолучателиДокумента(Команда)
	...
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ОткрытьИныхПолучателейИзФормы(ЭтаФорма, Элементы.Страница_Основное.ТолькоПросмотр,
																					Ложь);
КонецПроцедуры
...
Процедура ОтправитьЧерновик(Команда)
	...
	ПараметрыРеквизитаФормы = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ПараметрыРеквизитаФормы(
		Неопределено, Неопределено, , ЭтаФорма.ЭтоВО);
		
	ПроцедураПослеУказанияСоставителяЧерновика = Новый ОписаниеОповещения("ПослеУказанияСоставителяЧерновика", ЭтаФорма);
		
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ЮрЛицоНажатие(
		ЭтаФорма,
		"Составитель",
		ПараметрыРеквизитаФормы,
		ПроцедураПослеУказанияСоставителяЧерновика);
	
	КонецПроцедуры
	
&НаКлиенте
Процедура НастройкиГруппировки(Команда)
	ИмяФормыНастроекГруппировкиНоменклатуры = "НастройкиГруппировкиНоменклатуры";
	ПараметрыОткрытия = ПараметрыОткрытияФормыОбработки(ИмяФормыНастроекГруппировкиНоменклатуры, , ЭтаФорма);
	ОткрытьФормуОбработки(ПараметрыОткрытия);
КонецПроцедуры

&НаКлиенте
Процедура ПодробныеСведения(Команда)
	Элементы.Группа_ГрузДетализация.Видимость = Не Элементы.Группа_ГрузДетализация.Видимость;
КонецПроцедуры
...
Процедура ДекорацияДокументОснование(Элемент)
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ДокументОснованиеНажатие(ЭтаФорма, 
		"Документ.ESD_ТранспортнаяНакладная");
КонецПроцедуры
...
Процедура НастройкаДокументовОснований(Команда)
	...
	ПараметрыОткрытияФормыОбработки = ПараметрыОткрытияФормыОбработки("ОбщаяФорма.ESD_ВыборТиповЭПД", ПараметрыФормы, ЭтаФорма);
...
Процедура ПослеИзмененияОтправителя(Результат, ДополнительныеПараметры) Экспорт
	Если ЗначениеЗаполнено(ИсточникДанныхКлиент().Погрузка_Адрес) Тогда
		ТекстВопроса = НСтр("ru = 'Обновить адрес погрузки?'");
		...
		ОбновитьАдресПогрузки_ПослеОтветаНаВопрос(Ответ);
		#Иначе
		Оповещение = Новый ОписаниеОповещения("ОбновитьАдресПогрузки_ПослеОтветаНаВопрос", ЭтаФорма);
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет, 0, КодВозвратаДиалога.Да);
		#КонецЕсли
	Иначе
		ОбновитьАдресПогрузки_ПослеОтветаНаВопрос(КодВозвратаДиалога.Да);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьАдресПогрузки_ПослеОтветаНаВопрос(Результат, ДополнительныеПараметры = Неопределено) Экспорт
	ПараметрыЗаполненияДанныхЮрЛица = ПараметрыЗаполненияДанныхЮрЛица();
	ПараметрыЗаполненияДанныхЮрЛица.Адрес.ИгнорироватьЗаполненноеЗначение = (Результат = КодВозвратаДиалога.Нет);
	
	ПослеИзмененияОтправителяНаСервере(ПараметрыЗаполненияДанныхЮрЛица);
КонецПроцедуры
...
Процедура Отправитель_ТелефонПриИзменении(Элемент)
	...
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ТелефонПриИзменении(ЭтаФорма, Отправитель_Телефон, РолиЮрЛиц, Истина);
...
Процедура ПослеИзмененияСоставителя(Результат, ДополнительныеПараметры) Экспорт
	ПослеИзмененияСоставителяНаСервере();
КонецПроцедуры
...
Процедура ПослеИзмененияПолучателя(Результат, ДополнительныеПараметры) Экспорт
	Если ЗначениеЗаполнено(ИсточникДанныхКлиент().Отправитель_АдресДоставки) Тогда
		ТекстВопроса = НСтр("ru = 'Обновить адрес доставки?'");
		#Если ТолстыйКлиентОбычноеПриложение Или ТолстыйКлиентУправляемоеПриложение Тогда
		// BSLLS:UsingSynchronousCalls-off
		Ответ = Вопрос(ТекстВопроса, РежимДиалогаВопрос.ДаНет, 0,
			КодВозвратаДиалога.Да);
		// BSLLS:UsingSynchronousCalls-on
		ОбновитьАдресДоставки_ПослеОтветаНаВопрос(Ответ);
		#Иначе
		Оповещение = Новый ОписаниеОповещения("ОбновитьАдресДоставки_ПослеОтветаНаВопрос", ЭтаФорма);
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет, 0, КодВозвратаДиалога.Да);
		#КонецЕсли
	Иначе
		ОбновитьАдресДоставки_ПослеОтветаНаВопрос(КодВозвратаДиалога.Да);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьАдресДоставки_ПослеОтветаНаВопрос(Результат, ДополнительныеПараметры = Неопределено) Экспорт
	ПараметрыЗаполненияДанныхЮрЛица = ПараметрыЗаполненияДанныхЮрЛица();
	ПараметрыЗаполненияДанныхЮрЛица.Адрес.ИгнорироватьЗаполненноеЗначение = (Результат = КодВозвратаДиалога.Нет);
	
	ПослеИзмененияПолучателяНаСервере(ПараметрыЗаполненияДанныхЮрЛица);
КонецПроцедуры

&НаКлиенте
Процедура Получатель_ТелефонПриИзменении(Элемент)
	
	РолиЮрЛиц = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").РолиЮрЛиц(ЭтаФорма, "Получатель");
	
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ТелефонПриИзменении(ЭтаФорма, Получатель_Телефон, РолиЮрЛиц, Истина);
...
Процедура Перевозчик_ТелефонПриИзменении(Элемент)
	...
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ТелефонПриИзменении(ЭтаФорма, Перевозчик_Телефон, РолиЮрЛиц, Истина);
...
Процедура ЗаказчикСтрокойНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыРеквизитаФормы.ТолькоПросмотр = Элементы.Страница_Основное.ТолькоПросмотр;
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ЮрЛицоНажатие(ЭтаФорма, "Заказчик", ПараметрыРеквизитаФормы);
...
Процедура ВладелецОбъектаСтрокойНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыРеквизитаФормы.ТолькоПросмотр = Элементы.Страница_Основное.ТолькоПросмотр;
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ЮрЛицоНажатие(ЭтаФорма, "ВладелецОбъекта", ПараметрыРеквизитаФормы);
...
Процедура Владелец_ТелефонПриИзменении(Элемент)
	...
	РолиЮрЛиц = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").РолиЮрЛиц(ЭтаФорма, "ВладелецОбъекта");
	
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ТелефонПриИзменении(ЭтаФорма, ВладелецОбъекта_Телефон, РолиЮрЛиц, Истина);
...
Процедура ОтгрузчикСтрокойНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыРеквизитаФормы.ТолькоПросмотр = Элементы.ГруппаПогрузкаШапка.ТолькоПросмотр
		Или Элементы.Страница_Погрузка.ТолькоПросмотр
		Или Не Элементы.Группа_ТелОтгрузка.Доступность;
		
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ЮрЛицоНажатие(ЭтаФорма, "Отгрузчик", ПараметрыРеквизитаФормы);
...
Процедура Отгрузка_ТелефонПриИзменении(Элемент)
	...
	РолиЮрЛиц = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").РолиЮрЛиц(ЭтаФорма, "Отгрузчик");
	
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ТелефонПриИзменении(ЭтаФорма, Отгрузчик_Телефон, РолиЮрЛиц, Истина);
...
Процедура Грузоотправитель_ГрузополучательПриИзменении(Элемент)
	ЭтаФорма.Модифицированность = Истина;
	Грузоотправитель_ГрузополучательПриИзмененииНаСервере();
КонецПроцедуры
...
Процедура Грузоотправитель_ОсуществляетОтгрузкуПриИзменении(Элемент)
	...
	ИсточникДанных_ = ИсточникДанныхКлиент();
	ИсточникДанных_.Отгрузка_ОтветственныйНаОсновании = Неопределено;
	ИсточникДанных_.Отгрузка_НаОснованииДокумент = Неопределено;
	
	Грузоотправитель_ОсуществляетОтгрузкуПриИзмененииНаСервере();
...
Процедура Грузоотправитель_НеЯвляетсяОтправителемПриИзменении(Элемент)
	...
	ЭтаФорма.Модифицированность = Истина;
	Грузоотправитель_НеЯвляетсяОтправителемПриИзмененииНаСервере();	
КонецПроцедуры

&НаКлиенте
Процедура Владелец_ТипПриИзменении(Элемент)
	Владелец_ТипПриИзмененииНаСервере();
КонецПроцедуры

&НаКлиенте
Процедура Грузоотправитель_ЭкспедиторПриИзменении(Элемент)
	Грузоотправитель_ЭкспедиторПриИзмененииНаСервере();
КонецПроцедуры
...
Процедура ОтборИнформацииПоКонтактнымДанным(ИмяТЧ, Значение)
	...
	Иначе	
		Роль = ЗначениеМетаданныхКлиент("ESD_РолиКонтрагентов." + Значение);	
	КонецЕсли;
...
Процедура ОбработатьНовуюСтрокуКонтактнойИнформации(ИмяТЧ, Роль)
	...
	Для Каждого Строка Из НайденныеСтроки Цикл
		...
		Иначе
			...
			ФормаОбъект             = МодульТНОбщегоНазначенияКлиентСервер.ФормаОбъект(ЭтаФорма);
			ПараметрыРеквизитаФормы = МодульТНОбщегоНазначенияКлиентСервер.ПараметрыРеквизитаФормы(
				"КонтактныеДанные", ИмяТЧ);
			
			СтрокаЮрЛиц = МодульТНОбщегоНазначенияКлиентСервер.СтрокаЮрЛиц(ПараметрыРеквизитаФормы, ФормаОбъект, Роль);
...
Процедура ДобавитьПрицеп(Команда)
	...
	//		МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").НовыйКлючСтроки(ЭтаФорма, "ТранспортныеСредства");
	//		
	//КонецЕсли;
...
Процедура ДобавитьВодителя(Команда)
	...
	НоваяСтрока.КлючСтроки = МодульТНОбщегоНазначенияКлиент.НовыйКлючСтроки(ЭтаФорма, "ОтветственныеЛица");
	
	// Переустановим отбор
	Водители_УстановитьОтборПоВсемСвязаннымТаблицам();
	
	Для Каждого СтруктураОтбора Из Элементы.Водители.ОтборСтрок Цикл
		НоваяСтрока[СтруктураОтбора.Ключ] = СтруктураОтбора.Значение;
	КонецЦикла;
	
	СтруктураОтветственного = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").СтруктураОтветственного(
		ИсточникДанныхКлиент().ОтветственныеЛица, "", ИдентификаторСтроки);
	
	МодульТНОбщегоНазначенияКлиент.НачатьРедактированиеОтветственногоЛица(
		ЭтаФорма, СтруктураОтветственного, "Водитель");
...
Процедура ВодителиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		ТекДанные.КлючСтроки = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").НовыйКлючСтроки(ЭтаФорма, "ОтветственныеЛица");
...
#Область include_etrn_base_Document_ТранспортнаяНакладная_DocumentForm_ФормаДокумента_ПутевыеЛисты 
#КонецОбласти
...
Процедура ОтправительКонтактныеДанныеПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	СтруктураДанныхЮрЛица = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеЮрЛицаПоРоли(
...
Процедура ВладелецОбъектаКонтактныеДанныеЗначениеПриИзменении(Элемент)
	...
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ЗначениеКонтактныхДанныхПриИзменении(
		ЭтаФорма, Элементы.ВладелецОбъектаКонтактныеДанные.ТекущиеДанные, РолиЮрЛиц);
...
Процедура ОтгрузчикКонтактныеДанныеЗначениеПриИзменении(Элемент)
	...
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ЗначениеКонтактныхДанныхПриИзменении(
		ЭтаФорма, Элементы.ОтгрузчикКонтактныеДанные.ТекущиеДанные, РолиЮрЛиц);
...
Процедура МаркировкиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если Не Копирование Тогда
		...
		ПараметрыСвязи = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ПараметрыКлючаСвязиСтрок(
			Элемент.Имя, "Грузы", Элемент.ТекущаяСтрока, НоваяСтрока);
		
		МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ДобавитьКлючСвязиСтроки(ЭтаФорма, ПараметрыСвязи);
...
Процедура ГрузыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		ТекДанные.КлючСтроки = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").НовыйКлючСтроки(ЭтаФорма, "Грузы");
		Если Не Копирование Тогда
			ТекДанные.Тип = ЗначениеМетаданныхКлиент("ESD_ТипГруза.Груз");
		КонецЕсли;
...
Функция ТипТитула(Титул)
	...
	Возврат ЗначениеМетаданныхКлиент("ESD_ТипТитулаЭтрН." + Титул);
...
Процедура ОчисткаОтметок(Этап)
	...
	Титул = ЗначениеМетаданных("ESD_ТипТитулаЭтрН." + Этап);
	Роль  = ЗначениеМетаданных("ESD_РолиКонтрагентов.ПустаяСсылка");
	
	МодульКода("ESD_ТНОбщегоНазначенияСервер").ЗачисткаОтметокШтрафовАктов(Объект, Титул, Роль);
...
Процедура Грузоотправитель_ИнициаторПереадресацииПриИзменении(Элемент)
	...
	РольОтправитель = ЗначениеМетаданныхКлиент("ESD_РолиКонтрагентов.Отправитель");
...
#Область include_etrn_base_Document_ТранспортнаяНакладная_DocumentForm_ФормаДокумента_СопроводительныеДокументы
#КонецОбласти // include_etrn_base_Document_ТранспортнаяНакладная_DocumentForm_ФормаДокумента_СопроводительныеДокументы
...
Процедура Отгрузка_ОтветственныйНаОснованииПриИзменении(Элемент)
	...
	ЭлементСписка = Элементы.Отгрузка_ОтветственныйНаОсновании.СписокВыбора.НайтиПоЗначению(
		ИсточникДанныхКлиент().Отгрузка_ОтветственныйНаОсновании);
	
	Если ЭлементСписка <> Неопределено И ЭлементСписка.Пометка Тогда
		
		ОткрытьВыборДокументаОтветственногоНаОсновании();
...
Процедура Отгрузка_ОтветственныйНаОснованииОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Не ЭтаФорма.ЭтоВО И ВыбранноеЗначение <> "Должностные обязанности" Тогда
...
Процедура ОбработатьВыбранноеЗначение(ВыбранноеЗначение)
	...
	ЭлементСписка = Элементы.Отгрузка_ОтветственныйНаОсновании.СписокВыбора[
		Элементы.Отгрузка_ОтветственныйНаОсновании.СписокВыбора.Количество() - 1];
	...
	Если ЭлементСписка.Значение = ВыбранноеЗначение Тогда
		...
		КонецЕсли;
	Иначе
		ЭлементСписка.Значение = ВыбранноеЗначение;
		ЭлементСписка.Пометка  = Истина;
		Если Элементы.Отгрузка_ОтветственныйНаОсновании.РежимВыбораИзСписка Тогда
...
&НаКлиенте
Процедура ОткрытьВыборДокументаОтветственногоНаОсновании()
	...
	ТипДляОтбора = ЗначениеМетаданныхКлиент("ESD_ТипыДокумента.ОтветственныйНаОсновании");
	
	ПараметрыФормы.Отбор.Вставить("Тип", ТипДляОтбора);
	
	ПараметрыОткрытияФормыОбработки = ПараметрыОткрытияФормыОбработки(
		"Справочник.ESD_ДокументыЭПД.ФормаВыбора", ПараметрыФормы, Элементы.Отгрузка_ОтветственныйНаОсновании);
...
#Область include_etrn_base_Document_ТранспортнаяНакладная_DocumentForm_ФормаДокумента_ДополнительныеКомандыSaby
#КонецОбласти // include_etrn_base_Document_ТранспортнаяНакладная_DocumentForm_ФормаДокумента_ДополнительныеКомандыSaby
...
&НаСервере
Процедура ПриЧтенииСозданииПослеЗаписиНаСервере(Знач ПрочитатьИзСтруктуры = Ложь) Экспорт
	
	Если ЭтаФорма.ЭтоВО И ПрочитатьИзСтруктуры Тогда
		МодульКода("ЭтаФорма").ПрочитатьИзСтруктуры(ЭтаФорма.Ссылка);
	КонецЕсли;
	...
	Если ЭтоУниверсальноеРасширение() Тогда
		ЭтаФорма.АвтоЗаголовок = Не МодульКода("ESD_ТНОбщегоНазначенияСервер").ЭтоCF();
		Если Не ЭтаФорма.АвтоЗаголовок Тогда
			ЭтаФорма.Заголовок = МодульКода("ESD_ТНОбщегоНазначенияСервер").ПредставлениеДокумента(ЭтаФорма.Объект);
		КонецЕсли;
	КонецЕсли;
...
#Область include_etrn_base_Document_ТранспортнаяНакладная_DocumentForm_ФормаДокумента_ПриЧтенииСозданииПослеЗаписиНаСервере
#КонецОбласти // include_etrn_base_Document_ТранспортнаяНакладная_DocumentForm_ФормаДокумента_ПриЧтенииСозданииПослеЗаписиНаСервере

#Область include_etrn_base_Document_ТранспортнаяНакладная_DocumentForm_ФормаДокумента_ПриСозданииНаСервере
#КонецОбласти // include_etrn_base_Document_ТранспортнаяНакладная_DocumentForm_ФормаДокумента_ПриСозданииНаСервере

#Область include_etrn_base_Document_ТранспортнаяНакладная_DocumentForm_ФормаДокумента_ПодключаемыйФункционал
#КонецОбласти // include_etrn_base_Document_ТранспортнаяНакладная_DocumentForm_ФормаДокумента_ПодключаемыйФункционал

#Область include_etrn_base_Document_ТранспортнаяНакладная_DocumentForm_ФормаДокумента_ПриОткрытии
#КонецОбласти // include_etrn_base_Document_ТранспортнаяНакладная_DocumentForm_ФормаДокумента_ПриОткрытии
...
Процедура УстановкаВидимостиИРасчетныхРеквизитов(Знач ОбновитьДанныеПоОшибкам = Ложь)
	...
	МодульТНОбщегоНазначенияСервер.ЛентаТитулов_Обновить(ЭтаФорма, Истина);
	...
	МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ОбновитьЭлементыИныхПолучателей(ЭтаФорма);
КонецПроцедуры
...
Процедура ЗаполнитьРасчетныеРеквизиты()
	...
	ОбработатьДанныеГрузоотправителяГрузополучателяНаСервере(СтруктураОтправителя, СтруктураПолучателя);
	...
	ЭтаФорма.Грузоотправитель_ОсуществляетОтгрузку = (МодульТНОбщегоНазначенияКлиентСервер.ЮрЛицаСовпадают(
			СтруктураОтправителя, СтруктураОтгрузчика) Или Не СтруктураОтгрузчика.Заполнена)
			И Не ЭтаФорма.Грузоотправитель_НеЯвляетсяОтправителем; 
			
	Если ЭтаФорма.Грузоотправитель_НеЯвляетсяОтправителем 
		И СтруктураОформителя.Заполнена 
		И Не СтруктураОтгрузчика.Заполнена Тогда
		ЗаполнитьЮрЛицоКопированиемНаСервере("Отгрузчик", "Оформитель");
		СтруктураОтгрузчика = МодульТНОбщегоНазначенияКлиентСервер.ДанныеЮрЛицаПоРоли(
			ИсточникДанных.ДанныеЮрЛиц, "Отгрузчик");
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ЭтаФорма.ТипОтметок_Погрузка) Тогда
		ЭтаФорма.ТипОтметок_Погрузка = ЗначениеМетаданных("ESD_РолиКонтрагентов.Отправитель");
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ЭтаФорма.ТипОтметок_Выгрузка) Тогда
		ЭтаФорма.ТипОтметок_Выгрузка = ЗначениеМетаданных("ESD_РолиКонтрагентов.Получатель");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ИсточникДанных.ДокументОснование_Идентификатор) Тогда
		ДокументОснование = МодульТНОбщегоНазначенияСервер.СсылкаНаОбъектОснование(
			ИсточникДанных.ДокументОснование_ОбъектМетаданных, ИсточникДанных.ДокументОснование_Идентификатор);
...
Функция ПараметрыАктивныхЭтапов()
	...
	ТекущийЭтап =  МодульКода("ESD_ТНОбщегоНазначенияСервер").РезультатФункцииТитулов(
		ТранспортнаяНакладная(), "ЗначениеПоПредставлению", АктивныйЭтапСтрокой);
	
	РезультатФункции.ЭтоДинамическийТитул = МодульКода("ESD_ТНОбщегоНазначенияСервер").РезультатФункцииТитулов(
		ТранспортнаяНакладная(), "ЭтоДинамическийТитул", ТекущийЭтап);
	
	Если Не РезультатФункции.Завершен Тогда
		
		Погрузка = (ТекущийЭтап = ЗначениеМетаданных("ESD_ТипТитулаЭтрН.Погрузка"));
		ЭтоСогласованиеСтоимости = ТекущийЭтап = ЗначениеМетаданных("ESD_ТипТитулаЭтрН.СогласованиеСтоимости");
		
		РезультатФункции.ПолучениеГруза = (ТекущийЭтап = ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ПолучениеГруза"));
		РезультатФункции.ПриемкаГруза   = (ТекущийЭтап = ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ПриемкаГруза"));
		РезультатФункции.ВыдачаГруза    = (ТекущийЭтап = ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ВыдачаГруза"));
...
Процедура ОграничитьТипыАктов(АктивныеЭтапы = Неопределено)
	...
	ЗначениеАктВзвешивания  = ЗначениеМетаданных("ESD_ТипыДокумента.АктВзвешивания");
	ЗначениеАктКоммерческий = ЗначениеМетаданных("ESD_ТипыДокумента.АктКоммерческий");
...
Процедура ОбновитьДоступностьПолейВвода()
	...
	ДоступныеРоли   = МодульКода("Документы." + ТранспортнаяНакладная()).РолиОрганизации(
		ИсточникДанных().ДанныеЮрЛиц, ИсточникДанных().ТекущиеЭтапы);
	...
	ЭтоПриемкаИлиВыдачаТолькоПросмотр = ПараметрыЭтапов.ПриемкаГруза Или ПараметрыЭтапов.ВыдачаГруза Или ЭтаФорма.ТолькоПросмотр;
	...
	ЭтоНеЧерновикТолькоПросмотр = ЭтоНеЧерновик Или ЭтаФорма.ТолькоПросмотр;
	...
	Элементы.Группа_ОговоркиПеревозчика.Видимость = ПараметрыЭтапов.ВыдачаГруза 
		Или ПараметрыЭтапов.Завершен Или ПараметрыЭтапов.СогласованиеСтоимости;
	
	// Только просмотр
	Элементы.Группа_СтандартныеРеквизиты.ТолькоПросмотр = ЭтоНеЧерновикТолькоПросмотр;
	Элементы.Группа_Шапка.ТолькоПросмотр                = ЭтоНеЧерновикТолькоПросмотр;
	Элементы.Страница_Основное.ТолькоПросмотр           = ЭтоНеЧерновикТолькоПросмотр;
	Элементы.Страница_Груз.ТолькоПросмотр               = ЭтоНеЧерновикТолькоПросмотр;
	ОбновитьДоступностьКоманд_Грузы(Не ЭтоНеЧерновикТолькоПросмотр);
	Элементы.ГруппаПогрузкаШапка.ТолькоПросмотр         = ЭтоНеЧерновикТолькоПросмотр;
	Элементы.ГруппаАдресПогрузки.ТолькоПросмотр         = ЭтоНеЧерновикТолькоПросмотр;
	Элементы.ДекорацияЗаполнитьАдресПогрузки.Видимость  = Не ЭтоНеЧерновикТолькоПросмотр;
	Элементы.ГруппаАдресДоставки.ТолькоПросмотр         = ЭтоНеЧерновикТолькоПросмотр;
	Элементы.ДекорацияЗаполнитьАдресДоставки.Видимость  = Не ЭтоНеЧерновикТолькоПросмотр;
	Если Элементы.Найти("ПрицепыДобавитьПрицеп") <> Неопределено Тогда
		Элементы.ПрицепыДобавитьПрицеп.Доступность      = Не ЭтоНеЧерновикТолькоПросмотр;
		Элементы.ВодителиДобавитьВодителя.Доступность   = Не ЭтоНеЧерновикТолькоПросмотр;
	КонецЕсли;
	
	Элементы.ДекорацияДокументОснование.Доступность = Не ЭтоНеЧерновикТолькоПросмотр 
		Или ЗначениеЗаполнено(ДокументОснование);
	Элементы.ГруппаКомандыДокументаОснования.Видимость = Не ЭтоНеЧерновикТолькоПросмотр;
	...
	Элементы.Группа_ОтметкиВыгрузкаТЧ.ТолькоПросмотр    = ПараметрыЭтапов.Завершен Или ЭтаФорма.ТолькоПросмотр;
	Элементы.Группа_ОтметкиПогрузка.ТолькоПросмотр      = ЭтоПриемкаИлиВыдачаТолькоПросмотр;
	Элементы.Группа_ПогрузкаОговорки.ТолькоПросмотр     = ЭтоПриемкаИлиВыдачаТолькоПросмотр Или Не ДоступныеРоли.Перевозчик;
	
	ОбновитьДоступностьПолейВводаДополнительно(ДоступныеРоли, ПараметрыЭтапов, ЭтоНеЧерновикТолькоПросмотр);
КонецПроцедуры

&НаСервере
Процедура ОбновитьДоступностьПолейВводаДополнительно(ДоступныеРоли, ПараметрыЭтапов, ЭтоНеЧерновикТолькоПросмотр)
	Элементы.Группа_ОговоркиПеревозчика.ТолькоПросмотр = ПараметрыЭтапов.Завершен 
		Или ЭтаФорма.ТолькоПросмотр
		Или Не ПараметрыЭтапов.ВыдачаГруза 
	...
	// Доступность
	Элементы.НастройкаДокументовОснований.Доступность = Не ЭтоНеЧерновикТолькоПросмотр;
КонецПроцедуры

&НаСервере
Процедура ОбновитьДоступностьКоманд_Грузы(Знач ПризнакДоступности = Истина)
	Если Элементы.Найти("Группа_ГрузКомандыОсновные") = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Элементы.ГрузыДобавить.Доступность = ПризнакДоступности;
	Элементы.ГрузыУдалить.Доступность = ПризнакДоступности;
	Элементы.ГрузыСкопировать.Доступность = ПризнакДоступности;
КонецПроцедуры
...
Процедура ОбработатьОповещениеВыполнитьПереходНаФорме(ПараметрыПерехода = Неопределено) Экспорт
	...
	ЭтоЗагрузкаЧерновика = ПараметрыПерехода <> Неопределено И ПараметрыПерехода.ЭтоВыгрузкаЧерновика;
	
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ИмяКоманды",           "ОбновитьАктивныйЭтап");
	ДополнительныеПараметры.Вставить("ЭтоЗагрузкаЧерновика", ЭтоЗагрузкаЧерновика);
	
	ОбменДаннымиСБИС(Неопределено, ДополнительныеПараметры);
...
Процедура ОбновитьВидимостьЭлементов(ИсточникДанных)
	...
	Неизвестен = ЗначениеМетаданных("ESD_ВладелецОбъектаОтгрузки.Неизвестен");
	...
	// Владелец
	Если ИсточникДанных.Владелец_Тип = ЗначениеМетаданных("ESD_ВладелецОбъектаОтгрузки.Отправитель") Тогда	
		Элементы.Группа_Владелец.Видимость            = Ложь;
		Элементы.Владелец_НеизвестенПричина.Видимость = Ложь;	
	ИначеЕсли ИсточникДанных.Владелец_Тип = ЗначениеМетаданных("ESD_ВладелецОбъектаОтгрузки.ДругоеЮрЛицо") Тогда	
	...
	ЭтоИсходящий = (ИсточникДанных.Направление <> ЗначениеМетаданных("ESD_Направление.Входящий"));
	ЭтоЧерновик  = ЭтоИсходящий И МодульESDОбщегоНазначенияСервер.КодСостоянияДокументаРасширения(ИсточникДанных.Ссылка) = 0;
	...
	ОбновитьВидимостьЭлементов_АдресаПогрузкиДоставки(ИсточникДанных);

	// комментарии
	ЕстьКомментарийУведомления = ЗначениеЗаполнено(ИсточникДанных.КомментарийУведомление);
	
	Элементы.КомментарийУведомление.Видимость = ЕстьКомментарийУведомления;
	Элементы.Комментарий.Видимость            = Не ЕстьКомментарийУведомления;
	
	Элементы.ФормаОтправитьЧерновик.Доступность = ЭтоЧерновик;
КонецПроцедуры
...
Функция ДОЗавершен() Экспорт
	...
	Завершен = ИндексКартинки = ДОЗавершенОтрицательно
		...
		// Временно считаем "завершенным"
		Или ИндексКартинки = ДОПриостановлен;
...
Функция БылаПриемкаИлиВыдача()
	...
	ОтборСтрок = Новый Структура("ТипТитула", ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ПриемкаГруза"));
	...
	ОтборСтрок = Новый Структура("ТипТитула", ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ВыдачаГруза"));
	НайденныеСтроки = Лента.НайтиСтроки(ОтборСтрок);
	Для Каждого СтрокаЛенты Из НайденныеСтроки Цикл
		РезультатФункции = РезультатФункции Или СтрокаЛенты.Период <> Дата(1, 1, 1, 0, 0, 0);
	КонецЦикла;
...
Процедура ТипОтметок_ПогрузкаТолькоПросмотр(ПараметрыЭтапов = Неопределено, ДоступныеРоли = Неопределено)
	...
	РольОтправитель = ЗначениеМетаданных("ESD_РолиКонтрагентов.Отправитель");
	
	Если ТипОтметок_Погрузка = РольОтправитель Тогда
		Элементы.Группа_ОтметкиПогрузка.ТолькоПросмотр = Истина;
	Иначе
		Элементы.Группа_ОтметкиПогрузка.ТолькоПросмотр = ПараметрыЭтапов.Завершен Или ЭтаФорма.ТолькоПросмотр 
																						Или Не ДоступныеРоли.Перевозчик;
	КонецЕсли;
...
Процедура ТипОтметок_ВыгрузкаТолькоПросмотр(ПараметрыЭтапов = Неопределено, ДоступныеРоли = Неопределено)
	...
	Элементы.Группа_ОтметкиВыгрузкаТЧ.ТолькоПросмотр = ПросмотрОтметок Или ЭтаФорма.ТолькоПросмотр;
...
Процедура ОбновитьДоступностьДополнительныхКоманд()
	...
	ДоступностьКоманд = ДоступностьКомандДинамическихТитулов(ИсточникДанных.ДоступныеДействия, ИсточникДанных.ДанныеЮрЛиц);
	
	// Доступность кнопок
	Элементы.ФормаПереадресовка.Доступность            = ДоступностьКоманд.ПереадресовкаДоступно;
	Элементы.ФормаЗаменаВодителя.Доступность           = ДоступностьКоманд.ЗаменаВодителяДоступно;
	Элементы.ФормаИзменитьСтоимость.Доступность        = ДоступностьКоманд.ИзменитьСтоимостьДоступно;
	Элементы.ФормаСогласоватьСтоимость.Доступность     = ДоступностьКоманд.СогласоватьСтоимостьДоступно;
	Элементы.ФормаПереадресовкаУведомление.Доступность = ДоступностьКоманд.УведомлениеОПереадресовкеДоступно;
	
КонецПроцедуры  

Функция ДоступностьКомандДинамическихТитулов(ДоступныеДействия, ДанныеЮрЛиц) Экспорт
	...
	Для Каждого Строка Из ДоступныеДействия Цикл
	...
	Если ДопДействия.Количество() Тогда
		...
		ПереадресовкаДоступно = ДействиеДоступноНаТекущемЭтапе(
			ДопДействия, ЗначениеМетаданных("ESD_ДоступныеДействия.Переадресовка"));
		
		ЗаменаВодителяДоступно = ДействиеДоступноНаТекущемЭтапе(
			ДопДействия, ЗначениеМетаданных("ESD_ДоступныеДействия.ЗаменаВодителяТС"));
		
		УведомлениеОПереадресовкеДоступно = ДействиеДоступноНаТекущемЭтапе(
			ДопДействия, ЗначениеМетаданных("ESD_ДоступныеДействия.УведомлениеОПереадресовке"));
		
		ИзменитьСтоимостьДоступно = ДействиеДоступноНаТекущемЭтапе(
			ДопДействия, ЗначениеМетаданных("ESD_ДоступныеДействия.ИзменитьСтоимость")) И НЕ БылоИзменениеСтоимости();
		
	КонецЕсли;
	
	СтруктураОтправителя = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеЮрЛицаПоРоли(
		ДанныеЮрЛиц, "Отправитель");
	...
	ЭтоОтправитель           = СтруктураОтправителя.Заполнена
		И МодульКода("ESD_ТНОбщегоНазначенияСервер").ОрганизацияПрисутствуетВБазе(
			СтруктураОтправителя.ИНН, СтруктураОтправителя.КПП);
		
	СогласоватьСтоимостьДоступно = ЭтоСогласованиеСтоимости И ЭтоОтправитель;
	
	ДоступностьКоманд = Новый Структура;
	ДоступностьКоманд.Вставить("ИзменитьСтоимостьДоступно", ИзменитьСтоимостьДоступно);
	ДоступностьКоманд.Вставить("СогласоватьСтоимостьДоступно", СогласоватьСтоимостьДоступно);
	ДоступностьКоманд.Вставить("ПереадресовкаДоступно", ПереадресовкаДоступно);
	ДоступностьКоманд.Вставить("ЗаменаВодителяДоступно", ЗаменаВодителяДоступно);
	ДоступностьКоманд.Вставить("УведомлениеОПереадресовкеДоступно", УведомлениеОПереадресовкеДоступно);
...
Функция БылоИзменениеСтоимости()
	...
	ОтборСтрок = Новый Структура("ТипТитула", ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ИзменениеСтоимости"));
	НайденныеСтроки = Лента.НайтиСтроки(ОтборСтрок);
...
Процедура ОбменДаннымиСБИС(Результат, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ИмяКоманды = "ВыгрузитьВСбис" Тогда
		...
		ПараметрыВыгрузки = МодульТНОбщегоНазначенияКлиентСервер.ПараметрыВыгрузки(ИсточникДанных.Ссылка);
		...
		ПараметрыВыгрузки.ЭтоВыгрузкаЧерновика = МодульТНОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(
			ДополнительныеПараметры, "ЭтоВыгрузкаЧерновика", Ложь);
	...
	Иначе
		...
		ДанныеДляОбработки = Новый Массив;
		ПараметрыЗагрузки = МодульТНОбщегоНазначенияКлиентСервер.ПараметрыЗагрузки(ИсточникДанных.Ссылка);
		...
		ПараметрыЗагрузки.ИмяМетаданных = ТранспортнаяНакладнаяКлиент();
		ПараметрыЗагрузки.ЗагрузкаНаКлиенте = ЭтаФорма.ЭтоВО;
		
		ПараметрыЗагрузки.ЭтоЗагрузкаЧерновика = МодульТНОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(
			ДополнительныеПараметры, "ЭтоЗагрузкаЧерновика", Ложь);
...
Процедура ОбновитьДанныеПоОшибкамНаКлиенте(Результат, ДополнительныеПараметры) Экспорт
	...
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").РасшифровкаЗаписиТаблицыОшибок(ЭтаФорма);
...
Процедура ЗаполнитьСписокВыбораОтветственногоНаОсновании(Знач НовоеЗначение = Неопределено) Экспорт
	...
	Иначе
		Если НовоеЗначение = Неопределено Тогда
			ЗаполненноеЗначение = ИсточникДанных().Отгрузка_ОтветственныйНаОсновании;
		Иначе
	...
	ДобавитьЗначениеДокумента(ЗаполненноеЗначение);
	
	// дефолтное значение
	Если ЭтаФорма.ЭтоВО Тогда
		Если ЗначениеЗаполнено(ЗаполненноеЗначение) Тогда
...
&НаСервере
Процедура ДобавитьЗначениеДокумента(ЗаполненноеЗначение)
	Если ТипЗнч(ЗаполненноеЗначение) = Тип("Строка") И ЗаполненноеЗначение = "Должностные обязанности" 
		Или ЗаполненноеЗначение = Неопределено
	Тогда
		Элементы.Отгрузка_ОтветственныйНаОсновании.СписокВыбора.Добавить(
			ЗначениеМетаданных("ESD_ДокументыЭПД.ПустаяСсылка", "Справочник"), "Документ", Истина);
	Иначе
		ПредставлениеДокумента = "Документ";
		Если Не ЭтаФорма.ЭтоВО
			И Элементы.Отгрузка_ОтветственныйНаОсновании.РежимВыбораИзСписка 
		Тогда	
			ПредставлениеДокумента = Строка(ЗаполненноеЗначение);	
		КонецЕсли;
		
		Элементы.Отгрузка_ОтветственныйНаОсновании.СписокВыбора.Добавить(ЗаполненноеЗначение, ПредставлениеДокумента, Истина);
	КонецЕсли;	
КонецПроцедуры
...
Функция ПараметрыФормыРасшифровки(ИдентификаторТитула, НаименованиеТитула)
	...
	Если НаименованиеТитула = "ИзменениеСтоимости" Или НаименованиеТитула = "СогласованиеСтоимости" Тогда
		...
		РезультатФункции.Вставить(
			...
			МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеЮрЛицаПоРоли(ИсточникДанных.ДанныеЮрЛиц, "Отправитель"));
			
		РезультатФункции.Вставить(
			"СтруктураПеревозчика",
			МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеЮрЛицаПоРоли(ИсточникДанных.ДанныеЮрЛиц, "Перевозчик"));
...
Функция ШаблонРезультата()
	...
	РезультатФункции.Вставить("ЭтапДляПерехода",         Неопределено);
...
Процедура ОткрытьФормуТитула(НаименованиеТитула, ИдентификаторТитула = "", Описание = "") Экспорт
	...
	Иначе
		...
		ПараметрыОткрытияФормыОбработки = ПараметрыОткрытияФормыОбработки(
			"Документ.ESD_ТранспортнаяНакладная.Форма.ФормаДинамическихТитулов", ПараметрыФормы, ЭтаФорма);
		ПараметрыОткрытияФормыОбработки.РежимОткрытияОкна = РежимОткрытияОкнаФормы.БлокироватьОкноВладельца;
		ПараметрыОткрытияФормыОбработки.ИмяФормы = СтрЗаменить(
			ПараметрыОткрытияФормыОбработки.ИмяФормы, "ESD_ТранспортнаяНакладная", ТранспортнаяНакладная());
...
Функция ТитулОтрицательноЗавершен(СтрокаЛенты) Экспорт
	МодульМенеджера = МодульКода("Документы." + ТранспортнаяНакладная());
	
	Если СтрокаЛенты.ТипТитула = ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ПриемкаГруза") Тогда
		РезультатФункции = ЗначениеЗаполнено(СтрокаЛенты.Описание)
			И СтрокаЛенты.Представление = МодульМенеджера.ПриемкаГруза_НеПринят();
	ИначеЕсли СтрокаЛенты.ТипТитула = ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ВыдачаГруза") Тогда
		РезультатФункции = ЗначениеЗаполнено(СтрокаЛенты.Описание)
			И СтрокаЛенты.Представление = МодульМенеджера.ВыдачаГруза_НеВыдан();
	Иначе
...
Процедура СдвинутьЛентуНаСервере(Знач Вправо)
	МодульКода("ESD_ТНОбщегоНазначенияСервер").ЛентаТитулов_Сдвинуть(ЭтаФорма, Вправо);
КонецПроцедуры
...
Процедура ПослеОбновленияДанныхТранспортныхСредствНаСервере()
	...
	МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ОбновитьТранспортныеСредстваНаФорме(ЭтаФорма);
...
Процедура ОбновитьКодФилиалаНаФорме(Знач КодФилиала = Неопределено)
	...
	Отбор = Новый Структура;
	Отбор.Вставить("Роль", ЗначениеМетаданных("ESD_РолиКонтрагентов.Отправитель"));
...
&НаСервере
Процедура Владелец_ТипПриИзмененииНаСервере()
	ИсточникДанных = ИсточникДанных();
	...
	Если СокрЛП(ИсточникДанных.Владелец_Тип) = "Отправитель" Тогда
		ЗаполнитьЮрЛицоКопированиемНаСервере("ВладелецОбъекта", "Отправитель");
	Иначе
		ОчиститьЮрЛицоНаСервере("ВладелецОбъекта");
	КонецЕсли;
...
&НаСервере
Процедура Грузоотправитель_ГрузополучательПриИзмененииНаСервере()
	ЗаполнитьДанныеГрузополучателяДаннымиГрузоотправителяНаСервере(); 
	ЗаполнитьИныхПолучателейНаСервере();
	ОбновитьВидимостьИДоступностьЭлементов();	
КонецПроцедуры

&НаСервере
Процедура Грузоотправитель_ОсуществляетОтгрузкуПриИзмененииНаСервере()
	ОчиститьЮрЛицоНаСервере("Отгрузчик");
	Если Не ЭтаФорма.Грузоотправитель_НеЯвляетсяОтправителем Тогда
		Если ЭтаФорма.Грузоотправитель_ОсуществляетОтгрузку Тогда
			ЗаполнитьЮрЛицоКопированиемНаСервере("Отгрузчик", "Отправитель");
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура Грузоотправитель_ЭкспедиторПриИзмененииНаСервере()
	ИсточникДанных = ИсточникДанных();
	Если Не ИсточникДанных.Отправитель_Экспедитор Тогда
		ОчиститьЮрЛицоНаСервере("Заказчик");
...
&НаСервере
Процедура Грузоотправитель_НеЯвляетсяОтправителемПриИзмененииНаСервере()
	Если Не Грузоотправитель_НеЯвляетсяОтправителем Тогда	
		ОчиститьЮрЛицоНаСервере("Оформитель");
...
&НаСервере
Процедура ПослеИзмененияСоставителяНаСервере()
	ЗаполнитьЮрЛицоКопированиемНаСервере("Отгрузчик", "Оформитель");
КонецПроцедуры

&НаСервере
Процедура ПослеИзмененияОтправителяНаСервере(Знач ПараметрыЗаполненияДанныхЮрЛица = Неопределено)
	Если ЭтаФорма.Грузоотправитель_ОсуществляетОтгрузку Тогда
		ЗаполнитьЮрЛицоКопированиемНаСервере("Отгрузчик", "Отправитель");
	КонецЕсли;
	
	ЗаполнитьДополнительныеДанныеЮрЛица("Отправитель", ПараметрыЗаполненияДанныхЮрЛица);
	ЗаполнитьИныхПолучателейНаСервере();
КонецПроцедуры

&НаСервере
Процедура ПослеИзмененияПолучателяНаСервере(Знач ПараметрыЗаполненияДанныхЮрЛица = Неопределено)
	ОбработатьДанныеГрузоотправителяГрузополучателяНаСервере();
	ЗаполнитьДополнительныеДанныеЮрЛица("Получатель", ПараметрыЗаполненияДанныхЮрЛица);
	ЗаполнитьИныхПолучателейНаСервере();
КонецПроцедуры

&НаСервере
Процедура ОбработатьДанныеГрузоотправителяГрузополучателяНаСервере(СтруктураОтправителя = Неопределено, СтруктураПолучателя = Неопределено)
	МодульESD_ОбщегоНазначенияКлиентСервер = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер");
	Если СтруктураОтправителя = Неопределено Тогда
		СтруктураОтправителя = МодульESD_ОбщегоНазначенияКлиентСервер.ДанныеЮрЛицаПоРоли(
			ИсточникДанных().ДанныеЮрЛиц, "Отправитель");
	КонецЕсли;
	
	Если СтруктураПолучателя = Неопределено Тогда	
		СтруктураПолучателя = МодульESD_ОбщегоНазначенияКлиентСервер.ДанныеЮрЛицаПоРоли(
			ИсточникДанных().ДанныеЮрЛиц, "Получатель");
	КонецЕсли;
	
	Если Не СтруктураОтправителя.Заполнена Тогда 
		Возврат;
	КонецЕсли;
	
	ЭтаФорма.Грузоотправитель_Грузополучатель = МодульESD_ОбщегоНазначенияКлиентСервер.ЮрЛицаСовпадают(
		СтруктураПолучателя, СтруктураОтправителя);
	Если ЭтаФорма.Грузоотправитель_Грузополучатель Тогда
		ЗаполнитьДанныеГрузополучателяДаннымиГрузоотправителяНаСервере();
		СтруктураПолучателя = МодульESD_ОбщегоНазначенияКлиентСервер.ДанныеЮрЛицаПоРоли(
			ИсточникДанных().ДанныеЮрЛиц, "Получатель");
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьДанныеГрузополучателяДаннымиГрузоотправителяНаСервере()
	ИмяРолиПолучатель = "Получатель";
	Если Грузоотправитель_Грузополучатель Тогда
		ЗаполнитьЮрЛицоКопированиемНаСервере(ИмяРолиПолучатель, "Отправитель");
	Иначе
		ОчиститьЮрЛицоНаСервере(ИмяРолиПолучатель);
	КонецЕсли;
	
	ЗаполнитьДополнительныеДанныеЮрЛица(ИмяРолиПолучатель);
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьЮрЛицоКопированиемНаСервере(РольСтрокойПриемник, РольСтрокойИсточник)
	ИсточникДанных = ИсточникДанных();
	...
	РольОтправитель = ЗначениеМетаданных("ESD_ВладелецОбъектаОтгрузки.Отправитель");
		
	ЗаполнениеРазрешено = Истина;
	Если РольСтрокойПриемник = "ВладелецОбъекта" Тогда
		ЗаполнениеРазрешено = СтруктураИсточник.Заполнена И Не СтруктураПриемник.Заполнена
	...
	ОчиститьЮрЛицоНаСервере(РольСтрокойПриемник);
...
&НаСервере
Процедура ОчиститьЮрЛицоНаСервере(РольСтрокой)
	ИсточникДанных = ИсточникДанных();
	
	МодульТНОбщегоНазначенияКлиентСервер = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер");
	
	МодульТНОбщегоНазначенияКлиентСервер.УдалитьЮрЛицоПоРоли(ИсточникДанных.ДанныеЮрЛиц, Неопределено, 
		РольСтрокой);
	МодульТНОбщегоНазначенияКлиентСервер.УдалитьКонтактныеДанныеПоРоли(ИсточникДанных.КонтактныеДанные, 
		РольСтрокой);
...
#Область include_etrn_base_Document_ТранспортнаяНакладная_DocumentForm_ФормаДокумента_ПереопределяемыйФункционал_ДанныеЮрЛиц
#КонецОбласти // include_etrn_base_Document_ТранспортнаяНакладная_DocumentForm_ФормаДокумента_ПереопределяемыйФункционал_ДанныеЮрЛиц
...
Процедура ПослеРедактированияОтветственногоЛицаНаФорме(СтруктураОтветственногоЛица) Экспорт
	...
	Если Роль = ЗначениеМетаданныхКлиент("ESD_РолиОтветственных.Ответственный") Тогда
		ЭтаФорма.ОтветственныйОтгрузка = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ПредставлениеФИО(
...
&НаСервере
Процедура ОбновитьАдресаПогрузкиВыгрузкиНаСервере(РольСтрокой, ПараметрыЗаполненияДанныхЮрЛица = Неопределено)
	...
	Если ПараметрыЗаполненияДанныхЮрЛица <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ДопПараметры, ПараметрыЗаполненияДанныхЮрЛица.Адрес);
	Иначе
		ДопПараметры.ИгнорироватьЗаполненноеЗначение = Истина;
	КонецЕсли;
...
&НаСервере
Функция ПараметрыЗаполненияДанныхЮрЛица()
	РезультатФункции = Новый Структура;
	
	ПараметрыЗаполненияАдреса = Новый Структура("ИгнорироватьЗаполненноеЗначение", Истина);
	РезультатФункции.Вставить("Адрес", ПараметрыЗаполненияАдреса);
...
Функция СоответствиеТаблицФормыИОбъекта()
	...
	Если ЭтаФорма.ЭтоВО Тогда
		СоответствиеТаблицФормыИОбъекта.Вставить("ПутевыеЛисты",              "ДокументыЭПД");
		СоответствиеТаблицФормыИОбъекта.Вставить("СопроводительныеДокументы", "ДокументыЭПД");
	КонецЕсли;
...
Процедура ПоказатьОшибкуЗаполнения()
	...
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").РасшифровкаЗаписиТаблицыОшибок(
		ЭтаФорма, СоответствиеТаблицФормыИОбъекта);
КонецПроцедуры
...
&НаКлиенте
Процедура ПослеУказанияСоставителяЧерновика(Результат, ДополнительныеПараметры) Экспорт
	...
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ПроверитьМодифицированностьИПродолжитьВыполнение(
		ПроцедураОбменДаннымиСБИС, Истина);
	
КонецПроцедуры

#Область include_etrn_base_Document_ТранспортнаяНакладная_DocumentForm_ФормаДокумента_ЗаполнениеНаОсновании
#КонецОбласти // include_etrn_base_Document_ТранспортнаяНакладная_DocumentForm_ФормаДокумента_ЗаполнениеНаОсновании
