	...
	Если ЭтаФорма.ЭтоВО Тогда
		...
		Иначе
			...
			Если ЗначениеЗаполнено(ЭтаФорма.ДокументОснование) Тогда
				...
				ДопПараметры.Вставить("ИспользоватьНовыйРедактор", Ложь);
...
Процедура ЗаполнитьВыгрузкуПоДаннымГрузоотправителя(Команда)
	...
	Выгрузка_МассаБрутто = МодульКодаКлиент("ESD_ТНОбщегоНазначенияСервер").ЧислоИзСтроки(ИсточникДанных.Погрузка_Масса);
...
Процедура ИныеПолучателиДокумента(Команда)
	...
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ОткрытьИныхПолучателей(ЭтаФорма, Элементы.Страница_Основное.ТолькоПросмотр,
																															Ложь);
...
Процедура ОбработатьНовуюСтрокуКонтактнойИнформации(ИмяТЧ, Роль)
	...
	Для Каждого Строка Из НайденныеСтроки Цикл
		...
		Иначе
			...
			СтрокаЮрЛиц = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").СтрокаЮрЛиц(ПараметрыРеквизитаФормы, ФормаОбъект, Роль);
...
Процедура ОбновитьВидимостьЭлементов(ИсточникДанных)
	...
	КонецЕсли;
	Элементы.НастройкаДокументовОснований.Видимость = Не МодульКода("ESD_ТНОбщегоНазначенияСервер").ЗначениеКонстанты(
