...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЭтаФорма.ЭтоВО И Параметры.ОбновитьОшибки Тогда
...
Процедура ПриОткрытии(Отказ) 
	...
	Если ЭтаФорма.ЭтоВО Тогда
		...
		ОграничитьДоступныеВидыУпаковок(ЭтаФорма);
...
Процедура Группа_СтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если Элементы.Группа_Страницы.ТекущаяСтраница.Заголовок = "События" Тогда	
		// Запишем статистику открытия страницы "События"
		МодульКодаКлиент("ESD_ТНЗагрузкаСервер").ЗаписатьСтатистикуВызоваВОблако("ПросмотрСобытийДокумента", "Расширение 1С ESD");	
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура ГрузыВидТарыАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ДанныеВыбора = ДанныеВыбораТары(Элемент.СписокВыбора, Текст);
КонецПроцедуры

&НаСервереБезКонтекста
Функция ДанныеВыбораТары(Знач ДоступныеТары, Знач СтрокаПоиска)
    
	НайденныеСчета = Новый СписокЗначений;
	Для Каждого ЭлементТара Из ДоступныеТары Цикл
		Если Найти(Нрег(ЭлементТара.Представление), Нрег(СтрокаПоиска)) Тогда
			НайденныеСчета.Добавить(ЭлементТара.Значение, ЭлементТара.Представление);
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбновитьДоступностьПолейВвода()
	...
	ЭтаФорма.ТолькоПросмотр = ПараметрыЭтапов.Завершен Или ПараметрыЭтапов.ЭтоДинамическийТитул Или ЭтаФорма.ТолькоПросмотр;
	...
	Элементы.Погрузка_Адрес.Доступность                 = Не ЭтоНеЧерновикТолькоПросмотр;
	Элементы.АдресДоставки.Доступность                  = Не ЭтоНеЧерновикТолькоПросмотр;
	...
	Элементы.ГруппаДокументОснование.ТолькоПросмотр     = ЭтоНеЧерновикТолькоПросмотр;
	
	Элементы.ДекорацияДокументОснование.Доступность = ЗначениеЗаполнено(ДокументОснование) Или Не ЭтаФорма.ТолькоПросмотр;

	Элементы.ГруппаКомандыДокументаОснования.Видимость = Не Этаформа.ТолькоПросмотр;
...
Процедура ОбновитьДоступностьПолейВводаДополнительно(ДоступныеРоли, ПараметрыЭтапов, ЭтоНеЧерновикТолькоПросмотр)
	Элементы.Группа_ОговоркиПеревозчика.ТолькоПросмотр  = ПараметрыЭтапов.Завершен Или ЭтаФорма.ТолькоПросмотр
...
Процедура ОбновитьВидимостьЭлементов(ИсточникДанных)
	...
	ЭтоИсходящий = (ИсточникДанных.Направление = ЗначениеМетаданных("ESD_Направление.Исходящий"));
...
Функция ТитулОтрицательноЗавершен(СтрокаЛенты) Экспорт
	...
	Если СтрокаЛенты.ТипТитула = ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ПриемкаГруза") Тогда
		РезультатФункции = ЗначениеЗаполнено(СтрокаЛенты.Описание)
			И СтрокаЛенты.Представление = МодульМенеджера.ПриемкаГруза_НеПолучен();
...
Процедура ЗаполнитьЮрЛицоКопированием(РольСтрокойПриемник, РольСтрокойИсточник)
	...
	РольОтправитель = ЗначениеМетаданныхКлиент("ESD_ВладелецОбъектаОтгрузки.Отправитель");
...
&НаКлиенте
Процедура ОграничитьДоступныеВидыУпаковок(Форма)

	ОбщиеПараметрыЗагрузки = МодульКодаКлиент("ESD_ТНЗагрузкаСервер").ОбщиеПараметрыЗагрузки(Форма); 
	
	МассивВидовУпаковокСОнлайна =  МодульКодаКлиент("ESD_ТНЗагрузкаСервер").СправочникСОнлайна(
		"Вид упаковки Меркурия", ОбщиеПараметрыЗагрузки);
	
	Для Каждого ЭлВидУпаковки Из МассивВидовУпаковокСОнлайна Цикл              
		Представление = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").СтрШаблонЭПД("%1 (%2)", ЭлВидУпаковки[2], ЭлВидУпаковки[4]);
		Форма.Элементы.ГрузыВидТары.СписокВыбора.Добавить(ЭлВидУпаковки[4], Представление); 
	КонецЦикла;
