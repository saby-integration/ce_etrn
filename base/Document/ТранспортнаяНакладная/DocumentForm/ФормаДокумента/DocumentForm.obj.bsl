...
Процедура НастройкиГруппировки(Команда)
	...
	ПараметрыОткрытия = ПараметрыОткрытияФормыОбработки(ПрефиксВнешнейОбработки + ИмяФормыНастроекГруппировкиНоменклатуры, , ЭтаФорма);
...
Процедура ОбновитьДоступностьПолейВвода()
	...
	Если Элементы.Найти("ПрицепыДобавитьПрицеп") <> Неопределено Тогда
		Элементы.ПрицепыДобавитьПрицеп.Доступность      = Не ЭтоНеЧерновик;
		Элементы.ВодителиДобавитьВодителя.Доступность   = Не ЭтоНеЧерновик;
	КонецЕсли;
...
Процедура ДобавитьЗначениеДокумента(ЗаполненноеЗначение)
	...
	Иначе
		...
		Если Не ЭтаФорма.ЭтоВО
			И Элементы.Отгрузка_ОтветственныйНаОсновании.РежимВыбораИзСписка Тогда
...
Процедура ЗаполнитьИныхПолучателейНаСервере()
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ESD_ТранспортнаяНакладнаяДанныеЮрЛиц.Ссылка.МоментВремени КАК МоментВремени,
	|	ESD_ТранспортнаяНакладнаяДанныеЮрЛиц.Ссылка КАК Ссылка,
	|	ESD_ТранспортнаяНакладнаяДанныеЮрЛиц.Ссылка.Дата КАК Дата
	...
	|	И (ESD_ТранспортнаяНакладнаяДанныеЮрЛиц.Роль, ESD_ТранспортнаяНакладнаяДанныеЮрЛиц.ИНН, ESD_ТранспортнаяНакладнаяДанныеЮрЛиц.КПП) В
	|			(ВЫБРАТЬ
	...
	|			ГДЕ
	|				ДанныеЮрЛицДокумента.Роль = ЗНАЧЕНИЕ(Перечисление.ESD_РолиКонтрагентов.Отправитель))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|				ДанныеЮрЛицДокумента КАК ДанныеЮрЛицДокумента
	|			ГДЕ
	|				ДанныеЮрЛицДокумента.Роль = ЗНАЧЕНИЕ(Перечисление.ESD_РолиКонтрагентов.Получатель))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
