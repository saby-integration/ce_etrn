...
Процедура ПриОткрытии(Отказ) 
	...
	Если ЭтаФорма.ЭтоВО И ЗначениеЗаполнено(ЭтаФорма.Идентификатор) Тогда
		ОбменДаннымиСБИС("ПрочитатьДокумент");
	КонецЕсли;
	
	Если ЭтаФорма.ЭтоВО И ЗначениеЗаполнено(ЭтаФорма.ДокументОснование) Тогда
...
Процедура Печать(Команда)
	...
	Если Не ЭтаФорма.ЭтоВО И ИсточникДанныхКлиент().Ссылка.Пустая() Тогда
...
Процедура ОбработатьНовуюСтрокуКонтактнойИнформации(ИмяТЧ, Роль)
	...
	Для Каждого Строка Из НайденныеСтроки Цикл 
		...
		Иначе
			...
			ПараметрыРеквизитаФормы = ПараметрыРеквизитаФормы("КонтактныеДанные", ИмяТЧ);
			
			СтрокаЮрЛиц = СтрокаЮрЛиц(ПараметрыРеквизитаФормы, ФормаОбъект, Роль);
...
Процедура ОпасныеГрузыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЭтотОбъект.ЭтоВО 
		И Элементы.Страница_Груз.ТолькоПросмотр Тогда
		МодульКодаКлиент("ЭтаФорма").ОткрытьОпасныйГруз();
	КонецЕсли;
...
Процедура АктыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если ЭтаФорма.ЭтоВО Тогда
		МодульКодаКлиент("ЭтотОбъект").АктыОткрыть(Элемент.ТекущийЭлемент.Имя, Неопределено);
	КонецЕсли;
...
Процедура Акты_ВыгрузкаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЭтаФорма.ЭтоВО
		И Элементы.ГруппаОтметки_Выгрузка.ТолькоПросмотр Тогда
		МодульКодаКлиент("ЭтотОбъект").АктыОткрыть(Элемент.ТекущийЭлемент.Имя, Неопределено);
	КонецЕсли; 
...
Процедура АктыДокументНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	Если ЭтаФорма.ЭтоВО Тогда
		МодульКодаКлиент("ЭтотОбъект").АктыОткрыть(Элемент.Имя, СтандартнаяОбработка);
	КонецЕсли;	
...
Процедура ИнициаторПереадресацииПодтверждениеОткрытие(Элемент, СтандартнаяОбработка)
	...
	Если ЭтаФорма.ЭтоВО
		И ЭтаФорма.Отгрузка_ОтветственныйНаОсновании <> "Должностные обязанности" Тогда 
...
Процедура СопроводительныеДокументыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		ТекущиеДанные.КлючСтроки = МодульКодаКлиент("Saby_ТНОбщегоНазначенияКлиент").НовыйКлючСтроки(
...
Процедура Отгрузка_ОтветственныйНаОснованииОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
		...
		Если ЭтаФорма.ЭтоВО
			И ВыбранноеЗначение = "Должностные обязанности" Тогда
...
Процедура ОбновитьДоступностьПолейВвода()
	...
	Элементы.Группа_ОговоркиПеревозчика.ТолькоПросмотр  = ПараметрыЭтапов.Завершен Или Не ПараметрыЭтапов.ВыдачаГруза 
...
Функция ЗаполнениеНаОснованииINI() Экспорт
	...
	ФормаОбъект = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").ФормаОбъект(ЭтаФорма);
	
	Результат = МодульКода("Saby_ТНОбщегоНазначенияСервер").ЗаполнитьНаОснованииINI(ФормаОбъект, ДокументОснование);
...
Процедура ОткрытьПечатнуюФорму(Результат, ДополнительныеПараметры) Экспорт
	...
	МодульКодаКлиент("Saby_ТНОбщегоНазначенияКлиент").СоздатьИОткрытьПечатнуюФорму(СсылкаНаДокумент, УникальныйИдентификатор);
	
	Если ЭтаФорма.ЭтоВО И ЭтаФорма.КоличествоОшибок = 0 Тогда
		ЭтаФорма.Ссылка = СсылкаНаДокумент;
		ЭтаФорма.Прочитать();
	КонецЕсли;
...
Процедура ПрочитатьИзСтруктуры(СтруктураДанных)
	...
	ЭтаФорма.МаксимальныйКлючСтроки_ДокументыЭПД = 
		ТНКлиентСервер.НовыйКлючОсновнойСтроки(ЭтаФорма, "ДокументыЭПД") - 1; 
	
    ЭтаФорма.МаксимальныйКлючСтроки_Отметки = 
		ТНКлиентСервер.НовыйКлючОсновнойСтроки(ЭтаФорма, "Отметки") - 1; 
		
	ЭтаФорма.МаксимальныйКлючСтроки_ТранспортныеСредства = 
		ТНКлиентСервер.НовыйКлючОсновнойСтроки(ЭтаФорма, "ТранспортныеСредства") - 1; 
