...
Процедура Акты_ВыгрузкаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЭтаФорма.ЭтоВО
		И Элементы.Группа_ОтметкиВыгрузкаТЧ.ТолькоПросмотр Тогда
		МодульКодаКлиент("ЭтотОбъект").АктыОткрыть(Элемент.ТекущийЭлемент.Имя, Неопределено);
	КонецЕсли; 
...
Функция ЗаполнениеНаОснованииINI() Экспорт
	...
	Если ЭтаФорма.ЭтоВО Тогда
		ФормаОбъект = МодульКодаКлиент("Saby_ТНОбщегоНазначенияКлиентСервер").ФормаОбъект(ЭтаФорма);
		Результат = ЗаполнитьНаОснованииINI(ФормаОбъект, ДокументОснование, "Saby_ТранспортнаяНакладная");
	Иначе
...
Процедура ЗаполнитьСписокВыбораОтветственногоНаОсновании(Знач НовоеЗначение = Неопределено) Экспорт
	...
	Иначе
		Элементы.Отгрузка_ОтветственныйНаОсновании.СписокВыбора.Добавить(НовоеЗначение, "Документ");
	КонецЕсли;
	
	// дефолтное значение
	Если ЭтаФорма.ЭтоВО Тогда
		Если НовоеЗначение <> Неопределено Тогда
			ЭтаФорма.Отгрузка_ОтветственныйНаОсновании = НовоеЗначение;
		Иначе
...
Процедура ОткрытьПечатнуюФорму(Результат, ДополнительныеПараметры) Экспорт
	...
	МодульКодаКлиент("Saby_ТНОбщегоНазначенияКлиент").СоздатьИОткрытьПечатнуюФорму(
		СсылкаНаДокумент, УникальныйИдентификатор, ЭтаФорма.ЭтоВО);
