...
Функция РасчетныйСчетИзСправочникаБанковскихСчетов(ЮрЛицо)
	...
	Если ИмяКонфигурации = "УАТ" Тогда
...
Функция РасчетныйСчетИзРазделенныхСправочников(ЮрЛицо)
	Если ТипЗнч(ЮрЛицо) = Тип("СправочникСсылка.Организации") Тогда
		РасчетныйСчет = БанковскийСчетОрганизацииИзТиповогоСправочника(ЮрЛицо);
	Иначе
		РасчетныйСчет = БанковскийСчетКонтрагентаИзТиповогоСправочника(ЮрЛицо);
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция БанковскийСчетОрганизацииИзТиповогоСправочника(ЮрЛицо)
	...
	Попытка
		РезультатФункции = Справочники[ИмяСправочника].ПолучитьБанковскийСчетОрганизацииПоУмолчанию(ЮрЛицо);
	Исключение
...
&НаСервереБезКонтекста
Функция БанковскийСчетКонтрагентаИзТиповогоСправочника(Контрагент)
	Перем РезультатФункции;
	...
	Попытка
		РезультатФункции = Справочники[ИмяСправочника].ПолучитьБанковскийСчетПоУмолчанию(Контрагент);
	Исключение
		РезультатФункции = Неопределено;
	КонецПопытки;
...
Процедура УстановкаПараметровВыбораСчетов()
	...
	Если РезультатПеревозчик.Ссылка <> Неопределено Тогда
		...
		ПараметрыВыбора.Добавить(Новый ПараметрВыбора("Отбор.Владелец", РезультатПеревозчик.Ссылка));
