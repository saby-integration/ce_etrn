...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Saby_ЗавершениеВыполнитьДействие" Тогда
		
		ОбработатьОповещениеЗавершениеВыполнитьДействиеНаФорме(Параметр);
		
	ИначеЕсли ИмяСобытия = "Saby_ВыполнитьПереход" Тогда
		
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("Параметр",          Параметр);
		ДополнительныеПараметры.Вставить("ВыгрузкаНаКлиенте", ЭтаФорма.ЭтоВО);
		
		ПроцедураПослеЗакрытияФормыВыполнитьДействие = Новый ОписаниеОповещения(
			"ПослеЗакрытияФормыВыполнитьДействие", ЭтаФорма, ДополнительныеПараметры);
		
		МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ОбработатьОповещениеВыполнитьПереход(
			ЭтаФорма, Параметр, ПроцедураПослеЗакрытияФормыВыполнитьДействие);
	...
	ИначеЕсли ИмяСобытия = "ESD_ВыполнитьПереходыПоДокументам" Тогда
		...
		МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ВыполнитьПереходыПоДокументам(
			ЭтаФорма, Параметр, ПроцедураПослеЗакрытияФормыВыполнитьДействие);
	...
	ИначеЕсли ИмяСобытия = "Saby_СписокДокументов" Тогда
		...
		Если Параметр.ЭтоДинамическийТитул Тогда
			ОбновитьДанныеПоОшибкамНаСервере(Неопределено, Ошибки);
		КонецЕсли;
	ИначеЕсли ИмяСобытия = "Saby_ОбновитьДокумент" Тогда
		Если ЭтаФорма.ВладелецФормы <> Неопределено Тогда
			ЭтаФорма.ВладелецФормы.ОбработатьОповещениеОбновитьДокумент(Параметр);
		КонецЕсли;
    Иначе
		Возврат;
	КонецЕсли;
...
Процедура ПроцедураПослеИзмененияПолучателя(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		ДопПараметры = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ДопПараметрыОбновленияКИ(
			"Получатель",
			ЭтаФорма.ДанныеЮрЛиц,
			ЭтаФорма.КонтактныеДанные);
		
		МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ОбновитьАдресаПогрузкиДоставки(ЭтаФорма, ДопПараметры);
...
Процедура Получатель_ТелефонНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыВыбораТелефона = Новый Структура("Роль, Значение");
	ПараметрыВыбораТелефона.Роль = РолиЮрЛиц;
	ПараметрыВыбораТелефона.Значение = Получатель_Телефон;
...
Процедура Получатель_ТелефонПриИзменении(Элемент)
	...
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ТелефонПриИзменении(
		ЭтаФорма, Получатель_Телефон, РолиЮрЛиц, Истина);
...
Процедура АдресДоставкиНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыВыбораАдреса.Вставить("Текст",    ЭтаФорма.Отправитель_АдресДоставки);
	ПараметрыВыбораАдреса.Вставить("Значение", ЭтаФорма.АдресДоставкиЗначение); 
...
Процедура ОформилНажатие(Элемент, СтандартнаяОбработка)
	...
	ИмяМетаданных = Модуль_ТНКлиентСервер.ИмяМетаданныхПоФорме(ЭтаФорма);
	
	СтруктураОтветственного = Модуль_ТНКлиентСервер.СтруктураОтветственного(
		ЭтаФорма.ОтветственныеЛица, "ОформительДокумента");
		
	СтруктураОтветственного.Вставить("ТолькоПросмотр", Элементы.ГруппаОформил.ТолькоПросмотр);
	СтруктураОтветственного.Вставить("ИмяМетаданных",  ИмяМетаданных);
	
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").НачатьРедактированиеОтветственногоЛица(
		ЭтаФорма, СтруктураОтветственного, "Оформитель");
...
Процедура Оформил_ТелефонНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДлинаТелефона = 25;	
	Оповещение = Новый ОписаниеОповещения("ВводТелефонаОформителяЗавершение", ЭтаФорма);
    ПоказатьВводСтроки(Оповещение, ЭтаФорма.Оформил_Телефон, "Введите телефон", ДлинаТелефона);
	
КонецПроцедуры

&НаКлиенте
Процедура Оформил_ТелефонПриИзменении(Элемент)
	ИзменениеТелефонаОформителя();		
КонецПроцедуры

&НаКлиенте
Процедура ВводТелефонаОформителяЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат <> Неопределено Тогда
		ЭтаФорма.Оформил_Телефон = Результат;
		ИзменениеТелефонаОформителя();
	КонецЕсли;	
...
&НаКлиенте
Процедура ИзменениеТелефонаОформителя()
	
	Отбор = Новый Структура;
	Отбор.Вставить("Роль", ЗначениеМетаданных("ESD_РолиОтветственных.ОформительДокумента"));
	
	СтрокаОформителя = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").НайтиСтрокиУниверсально(
							ЭтаФорма.ОтветственныеЛица, Отбор);
							
	Если СтрокаОформителя.Количество() Тогда
		
		ТелефоныСтрокой = СтрокаОформителя[0].Телефоны;
		ИзмененныйСписок = ЗаменитьТелефоныОформителя(ТелефоныСтрокой);
		
		СтрокаОформителя[0].Телефоны = ИзмененныйСписок;
				
	Иначе
		
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Поле  = "Оформил";
		Сообщение.Текст = "Не заполнены данные оформителя документа! Выполните заполнение и повторите попытку";
		Сообщение.Сообщить();
...
Процедура ПослеИзмененияПлательщика(Результат, ДополнительныеПараметры) Экспорт
	...
	СтруктураПлательщика = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеЮрЛицаПоРоли(ДанныеЮрЛиц, ОтборЮрЛица);
...
Процедура ТаблицаОшибокПриАктивизацииСтроки(Элемент)
	...
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ОшибкиПриАктивизацииСтроки(ЭтаФорма);
	
	ПараметрыВыделенияДекорации = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ПараметрыВыделенияДекорации(ЭтаФорма);
...
Процедура ОбновитьДанныеПоОшибкам(Команда)
	ПроцедураОбновитьДанныеПоОшибкамНаКлиенте = Новый ОписаниеОповещения(
		...
		ЭтаФорма,
		Новый Структура("Форма", ЭтаФорма.ВладелецФормы));
...
Процедура НачальноеЗаполнениеПараметров()
	...
	ЭтаФорма.ТипТитула               = ЗначениеМетаданных("ESD_ТипТитулаЭтрН." + Параметры.ТипТитула);
...
Процедура ЗаполнениеДанныхПоТитулам()
	...
	Если ЭтаФорма.ЭтоРасшифровкаИзменений Тогда
		МодульКода("ESD_ТНЗагрузкаСервер").ЗагрузитьДанныеДинамическогоТитула(ЭтаФорма, ИдентификаторТитула);
	КонецЕсли;
	
	Если ЭтаФорма.ТипТитула = ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ЗаменаВодителяТС") Тогда
		ЗаполнитьДанныеТитула_ЗаменаВодителяИлиТС();
	ИначеЕсли ЭтаФорма.ТипТитула = ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ПереадресовкаУведомление")
		Или ЭтаФорма.ТипТитула = ЗначениеМетаданных("ESD_ТипТитулаЭтрН.Переадресовка") Тогда
		ЗаполнитьДанныеТитула_Переадресовка();
	ИначеЕсли ЭтаФорма.ТипТитула = ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ИзменениеСтоимости")
		Или ЭтаФорма.ТипТитула = ЗначениеМетаданных("ESD_ТипТитулаЭтрН.СогласованиеСтоимости") Тогда
...
Процедура ОбновитьВидимостьИДоступностьЭлементов(Знач ВызовПриСоздании = Ложь)
	...
	СвойстваЭлементовФормы = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ПараметрыСвойствЭлементовФормы();
	...
	МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ГрупповаяУстановкаСвойствЭлементовФормы(ЭтаФорма, СвойстваЭлементовФормы);
...
Процедура РасчетПризнаковЭтапов()
	...
	ЭтаФорма.ЭтоЗаменаВодителяТС   = ЭтаФорма.ТипТитула = ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ЗаменаВодителяТС");
	
	ЭтаФорма.ЭтоИзменениеСтоимости = ЭтаФорма.ТипТитула = ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ИзменениеСтоимости")
		Или ЭтаФорма.ТипТитула = ЗначениеМетаданных("ESD_ТипТитулаЭтрН.СогласованиеСтоимости");
	
	ЭтаФорма.ЭтоПереадресовка      = ЭтаФорма.ТипТитула = ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ПереадресовкаУведомление")
		Или ЭтаФорма.ТипТитула = ЗначениеМетаданных("ESD_ТипТитулаЭтрН.Переадресовка");
...
Процедура ВыполнитьОбмен(Результат, ДополнительныеПараметры) Экспорт
	...
	ПараметрыВыгрузки = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ПараметрыВыгрузки(ЭтаФорма.СсылкаНаЭТрН);
	...
	ДополнитьПараметрыВыгрузки(ПараметрыВыгрузки, ДополнительныеПараметры);
	...
	// закрываем кейс отправки уведомления: ГО = ГП, переадресует ГП
	// этап проходит параллельно с приемкой
	Если (ЭтоУниверсальноеРасширение() Или ЭтаФорма.ЭтоВО) 
		И ПараметрыВыгрузки.Этап = "Уведомление о переадресовке" Тогда
		
		ЭтапПерехода = Новый Структура;
		ЭтапПерехода.Вставить("Наименование",  ПараметрыВыгрузки.Этап);
		ЭтапПерехода.Вставить("Идентификатор", Неопределено);
		
		ДополнительныеПараметры.Вставить("ЭтапДляПерехода", ЭтапПерехода);
...
&НаКлиенте
Процедура ДополнитьПараметрыВыгрузки(ПараметрыВыгрузки, ДополнительныеПараметры)
	
	Если ЭтаФорма.ТипТитула = ЗначениеМетаданныхКлиент("ESD_ТипТитулаЭтрН.СогласованиеСтоимости") Тогда
	...
	Если ЭтаФорма.ЭтоВО Тогда
		...
		ДанныеДокументовЭПД = МодульКодаКлиент("ЭтаФорма").СформироватьДокументыЭПД(ЭтаФорма.ДокументыЭПД, ЭтаФорма.ДанныеЮрЛиц);
		ПараметрыВыгрузки.ДанныеДокумента.Вставить("ДокументыЭПД", ДанныеДокументовЭПД);
...
Функция РасчетЭтапа(Знач Титул)
	...
	Этап.Вставить(ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ЗаменаВодителяТС"),         "Замена водителей/ТС");
	Этап.Вставить(ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ИзменениеСтоимости"),       "Изменение стоимости");
	Этап.Вставить(ЗначениеМетаданных("ESD_ТипТитулаЭтрН.СогласованиеСтоимости"),    "Согласовать стоимость");
	Этап.Вставить(ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ПереадресовкаУведомление"), "Уведомление о переадресовке");
	Этап.Вставить(ЗначениеМетаданных("ESD_ТипТитулаЭтрН.Переадресовка"),            "Переадресовка");
...
Процедура ЗаполнитьДанныеТитула_ЗаменаВодителяИлиТС()
	...
	МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ОбновитьВодителейНаФорме(ЭтаФорма);
	...
	МодульКода("ESD_ТНОбщегоНазначенияСервер").УстановитьОтборТаблицыОтветственных(Элементы.Водители, "Водитель");
...
Процедура ЗаполнитьДанныеТитула_Переадресовка()
	...
	ЭтаФорма.ЭтоУведомление = ЭтаФорма.ТипТитула = ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ПереадресовкаУведомление");
	
	Если Не ЭтаФорма.ЭтоРасшифровкаИзменений Тогда
		
		Если ЭтаФорма.ЭтоУведомление Тогда
			Пользователь = МодульКода("ESD_ТНОбщегоНазначенияСервер").ТекущийПользователь();
			Если Пользователь <> Неопределено
				И МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ЕстьРеквизитИлиСвойство(Пользователь, "ФизическоеЛицо") Тогда
				ЭтаФорма.Оформил = МодульКода("ESD_ТНОбщегоНазначенияСервер").ЗначениеРеквизитаОбъекта(Пользователь, "ФизическоеЛицо");
			КонецЕсли;
		Иначе
...
Процедура ЗагрузитьДанныеУведомленияОПереадресовке()
	...
	ЗапросДанных.УстановитьПараметр("Переадресовка", ЗначениеМетаданных("ESD_ТипТитулаЭтрН.Переадресовка"));
	ЗапросДанных.УстановитьПараметр("Уведомление",   ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ПереадресовкаУведомление"));
	...
	Если ВыборкаПереадресовкаУведомление.Следующий() Тогда
		...
		МодульКода("ESD_ТНЗагрузкаСервер").ЗагрузитьДанныеДинамическогоТитула(
...
Процедура ОтборИнформацииПоКонтактнымДанным(ИмяТЧ, Значение)
	...
	Роль = ЗначениеМетаданных("ESD_РолиКонтрагентов." + Значение);
...
&НаСервере
Функция ЗаменитьТелефоныОформителя(Знач ТелефоныСтрокой)
	
	Попытка
		Телефоны = ЗначениеИзСтрокиВнутр(ТелефоныСтрокой);
	Исключение
	...
	Если Не ЗначениеЗаполнено(Телефоны) Тогда
		...
		Если ЗначениеЗаполнено(ЭтаФорма.Оформил_Телефон) Тогда
			Телефоны.Добавить(ЭтаФорма.Оформил_Телефон);		
		КонецЕсли;		
		
	Иначе 
		
		// удалим запись
		Если Не ЗначениеЗаполнено(ЭтаФорма.Оформил_Телефон) Тогда			
			Телефоны.Удалить(0);						
		Иначе
			// изменим основной телефон
            Телефоны[0] = ЭтаФорма.Оформил_Телефон;
		КонецЕсли;
		
	КонецЕсли;		
	
	Возврат ЗначениеВСтрокуВнутр(Телефоны);
...
&НаСервере
Процедура ОбновитьОформившего(СтруктураОтветственного = Неопределено)
	
	МодульТНОбщегоНазначенияСервер = МодульКода("ESD_ТНОбщегоНазначенияСервер");
	
	Если СтруктураОтветственного = Неопределено Тогда		
		СтруктураОтветственного = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").СтруктураОтветственного(
			ОтветственныеЛица, "ОформительДокумента");	
	КонецЕсли;
		
	Если ЗначениеЗаполнено(СтруктураОтветственного) Тогда
				
		Телефоны = МодульТНОбщегоНазначенияСервер.ДесериализоватьЗначение(СтруктураОтветственного.Телефоны);
	...
	МодульТНОбщегоНазначенияСервер.ОбновитьОтветственногоНаФорме(ЭтаФорма, "Оформил", СтруктураОтветственного);
...
Процедура ЗаполнитьДанныеТитула_Стоимости()
	...
	ЭтаФорма.ЭтоСогласование = (ЭтаФорма.ТипТитула = ЗначениеМетаданных("ESD_ТипТитулаЭтрН.СогласованиеСтоимости"));
	...
	Если ЭтаФорма.ЭтоСогласование Тогда
		...
		ЗаголовокКоманды = "Отправить результат согласования";
	...
	ЭтаФорма.ДругойПлательщикСтрокой = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеЮрЛицаПоРоли(
	...
	Иначе
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Параметры.СтруктураОтправителя) Тогда
				...
				СтрокаПлательщика.Роль = ЗначениеМетаданных("ESD_РолиКонтрагентов.ПустаяСсылка");
...
Функция ЗначениеСтавки(Ставка)
	...
	Значение = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").СтавкиНДС_ПреобразоватьВЧисло(Ставка);
...
Процедура ЗаполнитьТитулИзмененияСтоимости(Идентификатор)
	...
	Иначе
		ЗапросДанных.УстановитьПараметр("ИзменениеСтоимости", ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ИзменениеСтоимости"));
...
Процедура УстановкаСвойствЭлементов_ЗаменаВодителяИлиТС(СвойстваЭлементовФормы, ВызовПриСоздании)
	...
	Если ВызовПриСоздании Тогда
		...
		Если Не ЕстьСтрокиТС(ЗначениеМетаданных("ESD_ВидыТС.Автомобиль")) Тогда
			СкрытьВидимость.Добавить("ГруппаТС");
		КонецЕсли;
		
		Если Не ЕстьСтрокиТС(ЗначениеМетаданных("ESD_ВидыТС.Прицеп")) Тогда
	...
	Если ЭтаФорма.ЭтоРасшифровкаИзменений Тогда
		...
		СнятьДоступность.Добавить("ПрицепыДобавитьПрицеп");
		СнятьДоступность.Добавить("ВодителиДобавитьВодителя");
	...
	Если ЭтаФорма.ЭтоВО Тогда
		Элементы.ПутевыеЛистыДокумент.РедактированиеТекста = Ложь;
		Элементы.АктыДокумент.РедактированиеТекста         = Ложь;
	КонецЕсли;	
...
Процедура УстановкаСвойствЭлементов_Переадресовка(СвойстваЭлементовФормы, ВызовПриСоздании)
	...
	Если ЭтаФорма.ЭтоРасшифровкаИзменений Тогда
		...
		Если ЭтаФорма.ПолучательСтрокой = "Заполнить" Тогда
			СкрытьВидимость.Добавить("ГруппаЗаменаПолучателя");
		КонецЕсли;
...
Процедура УстановкаСвойствЭлементов_ИзменениеСтоимости(СвойстваЭлементовФормы, ВызовПриСоздании)
	...
	Если ЭтаФорма.ЭтоРасшифровкаИзменений Или ЭтоСогласованиеИОноНедоступно Тогда
		...
		СкрытьВидимость.Добавить("РасчетныйСчетПеревозчик");
		СкрытьВидимость.Добавить("РасчетныйСчет");
...
Функция ДанныеТитула()
	...
	ДобавитьДанныеДокументовЭПД(Результат);
...
Функция ДанныеТитула_Переадресовка()
	...
	ДанныеТитула.Вставить("Ссылка",                    ЭтаФорма.СсылкаНаЭТрН);
	ДанныеТитула.Вставить("ИмяМетаданных",             ТранспортнаяНакладная());
	...
	Если ЗначениеЗаполнено(ЭтаФорма.ПолучательСтрокой) Тогда
		...
		ДанныеПолучатель = МодульКода("ESD_ТНВыгрузкаСервер").ДанныеЮрЛица("Получатель", ДанныеТитула, Истина);
	...
	ПочтаОтветственного = Новый Массив; 	
	Если ЗначениеЗаполнено(ЭтаФорма.Оформил_ЭлектроннаяПочта) Тогда
		ПочтаОтветственного.Добавить(СокрЛП(ЭтаФорма.Оформил_ЭлектроннаяПочта));	
	КонецЕсли;	
	
	СтруктураОтветственного.Вставить("ЭлектроннаяПочта", ПочтаОтветственного);
	
	ДанныеТитула.Вставить("ОформилСтруктура", СтруктураОтветственного);
	
	Возврат ДанныеТитула;
	
КонецФункции

#Область include_etrn_base_Document_ТранспортнаяНакладная_DocumentForm_ФормаДинамическихТитулов_ДанныеДокументовЭПД
#КонецОбласти // include_etrn_base_Document_ТранспортнаяНакладная_DocumentForm_ФормаДинамическихТитулов_ДанныеДокументовЭПД

#КонецОбласти // ДанныеТитуловДляВыгрузки

#Область ОбработкаРезультатаВыполненияДействия

&НаКлиенте
Процедура ПослеЗакрытияФормыВыполнитьДействие(Результат, ДополнительныеПараметры) Экспорт
	Если ЭтаФорма.ТаблицаОшибок.Количество() > 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДействиеВыполненоУспешно = ОбработатьРезультатВыполненияДействия(Результат);
	ЭтаФорма.КоличествоОшибок = ЭтаФорма.ТаблицаОшибок.Количество();
	Если Элементы.ГруппаОшибки.Видимость <> ЭтаФорма.ТаблицаОшибок.Количество() Тогда
		ОбновитьВнешнийВидЭлементовОшибок();		
	КонецЕсли;
	
	Если Не ДействиеВыполненоУспешно Тогда
		Возврат;
	КонецЕсли;
	
	ОбработатьРезультатВыполненияДействияДокументаОснования(ДополнительныеПараметры);
	Закрыть();
КонецПроцедуры

&НаКлиенте
Функция ОбработатьРезультатВыполненияДействия(Результат)
	ВыполненоУспешно = Истина;
	Если Не ЗначениеЗаполнено(Результат) Тогда
		Возврат ВыполненоУспешно;
	КонецЕсли;

	Статус_ = Результат["Status"];
	Описание_ = Результат["Result"];
	Если Статус_ <> "error" Или Не ЗначениеЗаполнено(Описание_) Тогда
		Возврат ВыполненоУспешно;
	КонецЕсли;
	
	ВыполненоУспешно = Ложь;
	НоваяСтрокаОшибки = ЭтаФорма.ТаблицаОшибок.Добавить();
	НоваяСтрокаОшибки.Ошибка = Описание_["detail"];
	
	Возврат ВыполненоУспешно;
КонецФункции

&НаКлиенте
Процедура ОбработатьРезультатВыполненияДействияДокументаОснования(ПараметрыВыполнения)
	Если ЭтаФорма.ВладелецФормы <> Неопределено Тогда
		ЭтаФорма.ВладелецФормы.АктивныйЭтапСтрокой = РасчетЭтапа(ЭтаФорма.ТипТитула);
		Если ПараметрыВыполнения.ВыгрузкаНаКлиенте Тогда
			ЭтаФорма.ВладелецФормы.ТекущийЭтап = ЭтаФорма.ВладелецФормы.АктивныйЭтапСтрокой;
		КонецЕсли;
		
		ЭтаФорма.ВладелецФормы.ОбработатьОповещениеВыполнитьПереходНаФорме();
	Иначе
		// BSLLS:GetFormMethod-off
		ФормаДокумента = ПолучитьФорму("Документ.ESD_ТранспортнаяНакладная.Форма.ФормаДокумента", 
			Новый Структура("Ключ", Этаформа.СсылкаНаЭТрН));
		// BSLLS:GetFormMethod-on
		Если ФормаДокумента <> Неопределено Тогда
			ФормаДокумента.ОбработатьОповещениеВыполнитьПереходНаФорме();
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура АктыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если ЭтаФорма.ЭтоВО Тогда
		...
		Элемент.ТекущиеДанные.Тип = "АктКоммерческий";
...
Процедура АктыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЭтаФорма.ЭтоВО
		И Элементы.ГруппаОтметкиИАкты.ТолькоПросмотр Тогда
		МодульКодаКлиент("ЭтаФорма").АктыОткрыть(Элемент.ТекущийЭлемент.Имя, Неопределено);
	КонецЕсли;
...
Процедура ПутевыеЛистыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если Не Копирование Тогда
		...
		ПараметрыКлючаСвязиСтрок = ПараметрыКлючаСвязиСтрок("ПутевыеЛисты", "Водители", Элемент.ТекущаяСтрока, НоваяСтрока);
		ДобавитьКлючСвязиСтроки(ЭтаФорма, ПараметрыКлючаСвязиСтрок);
	...
	Если ЭтаФорма.ЭтоВО Тогда
		...
		МодульКодаКлиент("ЭтаФорма").ПутевыеЛистыОткрыть(Элемент.ТекущийЭлемент.Имя, Неопределено);
...
Процедура ПутевыеЛистыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЭтаФорма.ЭтоВО
		И Элементы.ГруппаВодители.ТолькоПросмотр Тогда
		МодульКодаКлиент("ЭтаФорма").ПутевыеЛистыОткрыть(Элемент.ТекущийЭлемент.Имя, Неопределено);
	КонецЕсли
...
Процедура ЗапускУдаленияСвязанныхСтрок(ТаблицаФормы, ТаблицаДанных, Отказ)
	...
	Для Каждого ИдСтроки Из ВыделенныеСтроки Цикл
		ТекДанные = ЭтаФорма[ТаблицаДанных].НайтиПоИдентификатору(ИдСтроки);
		Если ТекДанные <> Неопределено Тогда
			// Необходимо очистить дополнительные сведения
...
Функция ТаблицыДляОтправки()
	...
	ОтборПоРоли.Вставить("Роль", ЗначениеМетаданных("ESD_РолиОтветственных.Водитель"));
...
Процедура ОбновитьДанныеПоОшибкамНаКлиенте(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЭтаФорма.ЭтоВО Тогда
		ДанныеДокументовЭПД = МодульКодаКлиент("ЭтаФорма").СформироватьДокументыЭПД(ЭтаФорма.ДокументыЭПД, ЭтаФорма.ДанныеЮрЛиц);
	Иначе
		ДанныеДокументовЭПД = Новый Массив;
	КонецЕсли;
	
	ОбновитьДанныеПоОшибкамНаСервере(ДанныеДокументовЭПД);
	
КонецПроцедуры

&НаСервере
Процедура ОбновитьДанныеПоОшибкамНаСервере(Знач ДанныеДокументовЭПД, Знач Ошибки = Неопределено)
	
	ПредставленияТитула = МодульКода("ESD_ТНОбщегоНазначенияСервер").РезультатФункцииТитулов(
		ТранспортнаяНакладная(), "ПредставленияТитула", ЭтаФорма.ТипТитула);
	
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("Этап",                    ПредставленияТитула.Представление);
	...
	Если Ошибки = Неопределено Тогда
		...
		ДополнительныеПараметры.ДанныеДокумента.Вставить("ДокументыЭПД", ДанныеДокументовЭПД);
	КонецЕсли;
...
Процедура СформироватьНадписьВодителейСтрокой()
	...
	Если Массив.Количество() Тогда
		ЭтаФорма.ВодителиСтрокой = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").СтрСоединитьЭПД(Массив, ", ");
	Иначе
...
&НаКлиенте
Процедура ДобавитьВодителяВТаблицу()
	
	Модуль_ТНКлиент = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент");
	
	НоваяСтрока = ЭтаФорма.ОтветственныеЛица.Добавить();
	ИдентификаторСтроки = НоваяСтрока.ПолучитьИдентификатор();
	
	НоваяСтрока.КлючСтроки = Модуль_ТНКлиент.НовыйКлючСтроки(ЭтаФорма, "ОтветственныеЛица");
	...
	СтруктураОтветственного = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").СтруктураОтветственного(
		ЭтаФорма.ОтветственныеЛица, "", ИдентификаторСтроки);
	
	Модуль_ТНКлиент.НачатьРедактированиеОтветственногоЛица(
		ЭтаФорма, СтруктураОтветственного, "Водитель");
...
Процедура РедактированиеОтметок(ТаблицаИмя, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		Если Не Копирование Тогда
			ТекДанные.ДатаВремя = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ДатаСеанса();
			ТекДанные.Этап      = ЗначениеМетаданныхКлиент("ESD_ТипТитулаЭтрН." + Этап);
		КонецЕсли;
...
Функция РеквизитыБанковскогоСчета(Знач БанковскийСчет)
	...
	Если ЕстьСправочникБанкСчета Тогда 
		...
		РеквизитыСчета = МодульКода("ESD_ТНОбщегоНазначенияСервер").ЗначенияРеквизитовОбъекта(
...
Процедура ТипПлательщикПриИзмененииНаСервере()
	...
	МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").УдалитьСтрокиПоРоли(ЭтаФорма.ДанныеЮрЛиц, "ПустаяСсылка");
	ЭтаФорма.ДругойПлательщикСтрокой = "Заполнить";
	
	Если ПустаяСтрока(ЭтаФорма.ТипПлательщик) Тогда
		
		СтруктураПлательщика = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеЮрЛицаПоРоли(
	...
	Если Не ДругойПлательщик Тогда
...
Процедура ПослеРедактированияОтветственногоЛицаНаФорме(СтруктураОтветственногоЛица) Экспорт
	...
	ОбновитьОформившего(СтруктураОтветственногоЛица);
...
Процедура УстановитьТипыРеквизитов()
	...
	ПараметрыВыбора.Добавить(Новый ПараметрВыбора(ИмяОтбораТип, ЗначениеМетаданных("ESD_ТипыДокумента.АктКоммерческий")));
	МодульТНОбщегоНазначенияСервер.УстановитьПараметрыВыбора(Элементы.АктыДокумент, ПараметрыВыбора);
	
	ПараметрыВыбора.Очистить();
	ПараметрыВыбора.Добавить(Новый ПараметрВыбора(ИмяОтбораТип, ЗначениеМетаданных("ESD_ТипыДокумента.КТранспортнойНакладной")));
	МодульТНОбщегоНазначенияСервер.УстановитьПараметрыВыбора(Элементы.ДокументЗамены, ПараметрыВыбора);
	
	ПараметрыВыбора.Очистить();
	ПараметрыВыбора.Добавить(Новый ПараметрВыбора(ИмяОтбораТип, ЗначениеМетаданных("ESD_ТипыДокумента.ОснованиеОплатыПеревозки")));
	МодульТНОбщегоНазначенияСервер.УстановитьПараметрыВыбора(Элементы.ОснованиеОплаты, ПараметрыВыбора);
	
	ПараметрыВыбора.Очистить();
	ПараметрыВыбора.Добавить(Новый ПараметрВыбора(ИмяОтбораТип, ЗначениеМетаданных("ESD_ТипыДокумента.ПередачиЦенностей")));
	...
	ПараметрыВыбора.Очистить();
	ПараметрыВыбора.Добавить(Новый ПараметрВыбора(ИмяОтбораТип, ЗначениеМетаданных("ESD_ТипыДокумента.ПутевойЛист")));
...
Процедура ЗаполнитьСписокВыбораСтавокНДС()
	Элементы.Ставка.СписокВыбора.Очистить();
	
	СтавкиНДС_ = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").СтавкиНДС_СписокВыбора();
	Для Каждого СтавкаНДС_ Из СтавкиНДС_ Цикл
		Элементы.Ставка.СписокВыбора.Добавить(СтавкаНДС_.Представление, СтавкаНДС_.Представление);	
	КонецЦикла;
КонецПроцедуры
