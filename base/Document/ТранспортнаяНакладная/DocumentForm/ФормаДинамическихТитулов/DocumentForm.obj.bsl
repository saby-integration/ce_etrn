...
Процедура ОформилНажатие(Элемент, СтандартнаяОбработка)
	...
	ИмяМетаданных = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ИмяМетаданныхПоФорме(ЭтаФорма);
...
Процедура ВодителиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	СтруктураОтветственного = ВыполнитьНаСервере(
		"ESD_ТНОбщегоНазначенияСервер.СтруктураОтветственного", ЭтаФорма.ОтветственныеЛица, "", ВыбраннаяСтрока);
...
Процедура ОбработатьОповещениеЗавершениеВыполнитьДействиеНаФорме(Параметр) Экспорт
	...
	Если Параметр.status = "error" Тогда
		МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ОбработатьОшибкиИзФормыВыполнитьДействие(
			ЭтаФорма, ЭтаФорма.СсылкаНаЭТрН, Параметр);
		ЭтаФорма.ТаблицаОшибок.Добавить();
	КонецЕсли;
