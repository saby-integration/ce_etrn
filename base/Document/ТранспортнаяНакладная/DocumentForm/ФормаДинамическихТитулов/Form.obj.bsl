...
Процедура ИзменениеТелефонаОформителя()
	...
	Отбор = Новый Структура;
	Отбор.Вставить("Роль", "ОформительДокумента");
	
	СтрокаОформителя = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").НайтиСтрокиУниверсально(
...
Процедура ПрицепыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ТаблицаПрицеповПриНачалеРедактирования(
		ЭтаФорма,
		ПроцедураПослеОбновленияДанныхТранспортныхСредств);
...
Процедура ЗаполнитьДанныеТитула_Переадресовка()
	...
	Если Не ЭтаФорма.ЭтоРасшифровкаИзменений Тогда
		...
		Если ЭтаФорма.ЭтоУведомление Тогда
			Пользователь = МодульКода("ESD_ТНОбщегоНазначенияСервер").АвторизованныйПользователь();
...
Функция ДанныеТитула_Переадресовка()
	...
	ЭлектроннаяПочта = Новый Массив; 	
	Если ЗначениеЗаполнено(ЭтаФорма.Оформил_ЭлектроннаяПочта) Тогда
		ЭлектроннаяПочта.Добавить(СокрЛП(ЭтаФорма.Оформил_ЭлектроннаяПочта));	
	КонецЕсли;	
	
	СтруктураОтветственного.Вставить("ЭлектроннаяПочта", ЭлектроннаяПочта);
...
Процедура ОтказатьВСогласованииПослеВводаПричины(Результат, ДопПараметры) Экспорт
	...
	Иначе
		...
		ВыполнитьНаСервере("ESD_ТНОбщегоНазначенияСервер.ОбновитьВнешнийВидЭлементовОшибок", ЭтаФорма);
		Возврат;
	КонецЕсли;
...
Процедура ВодителиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока И Не Копирование Тогда
		...
		СтруктураОтветственного = ВыполнитьНаСервере(
			"ESD_ТНОбщегоНазначенияСервер.СтруктураОтветственного", ЭтаФорма.ОтветственныеЛица, "", Элемент.ТекущаяСтрока);
