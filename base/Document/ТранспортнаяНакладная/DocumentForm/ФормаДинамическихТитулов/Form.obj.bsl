...
Процедура Получатель_ТелефонПриИзменении(Элемент)
	...
	РолиЮрЛиц = МодульКодаКлиент("Saby_ТНОбщегоНазначенияКлиентСервер").РолиЮрЛиц(ЭтаФорма, "Получатель");	
	МодульКодаКлиент("Saby_ТНОбщегоНазначенияКлиент").ТелефонПриИзменении(
...
Процедура ОформилНажатие(Элемент, СтандартнаяОбработка)
	...
	ИмяМетаданных = МодульКодаКлиент("Saby_ТНОбщегоНазначенияКлиентСервер").ИмяМетаданныхПоФорме(ЭтаФорма);
	
	СтруктураОтветственного = ВыполнитьНаСервере("Saby_ТНОбщегоНазначенияСервер.СтруктураОтветственного",
...
Процедура ИзменениеТелефонаОформителя()
	...
	СтрокаОформителя = МодульКодаКлиент("Saby_ТНОбщегоНазначенияКлиент").НайтиСтрокиУниверсально(
...
Процедура ДополнитьПараметрыВыгрузки(ПараметрыВыгрузки, ДополнительныеПараметры)
	...
	Если ЭтаФорма.ТипТитула = ЗначениеМетаданныхКлиент("Saby_ТипТитулаЭтрН.СогласованиеСтоимости") Тогда
...
Процедура ОбновитьДанныеПоОшибкамНаСервере(Знач Ошибки = Неопределено)
	...
	ПредставленияТитула = МодульКода("Saby_ТНОбщегоНазначенияСервер").РезультатФункцииТитулов(
		"Saby_ТранспортнаяНакладная", "ПредставленияТитула", ЭтаФорма.ТипТитула);
...
Процедура РедактированиеОтметок(ТаблицаИмя, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		Если Не Копирование Тогда
			...
			ТекДанные.Этап      = ЗначениеМетаданныхКлиент("Saby_ТипТитулаЭтрН." + Этап);
		КонецЕсли;
