...
Функция ПараметрыПроверкиРеквизитов(ДанныеДокумента, ПараметрыВыгрузки) Экспорт
	...
	ОшибкиПоРеквизитам = МодульТНОбщегоНазначенияСервер.СоответствиеРеквизитовОшибок(ПараметрыВыгрузки);
...
Процедура СформироватьНовыеОснования(СтруктураРезультата, ДанныеИзИНИ) Экспорт
	...
	Если ЗначениеЗаполнено(ГрузоотправительСсылка) Тогда
		...
		СтруктураОснования = МодульТНОбщегоНазначенияСервер.СтруктураОснования(
			ГрузоотправительСсылка, ЗначениеМетаданных("Saby_РолиКонтрагентов.Отправитель"));
	...
	Если ЗначениеЗаполнено(ГрузополучательСсылка) Тогда
		СтруктураОснования = МодульТНОбщегоНазначенияСервер.СтруктураОснования(
			ГрузополучательСсылка, ЗначениеМетаданных("Saby_РолиКонтрагентов.Получатель"));
		СтруктураРезультата.НовыеОснования.Добавить(СтруктураОснования);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ГрузоперевозчикСсылка) Тогда
		СтруктураОснования = МодульТНОбщегоНазначенияСервер.СтруктураОснования(
			ГрузоперевозчикСсылка, ЗначениеМетаданных("Saby_РолиКонтрагентов.Перевозчик"));
		СтруктураРезультата.НовыеОснования.Добавить(СтруктураОснования);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ЗаказчикСсылка) Тогда
		СтруктураОснования = МодульТНОбщегоНазначенияСервер.СтруктураОснования(
			ЗаказчикСсылка, ЗначениеМетаданных("Saby_РолиКонтрагентов.Заказчик"));
		СтруктураРезультата.НовыеОснования.Добавить(СтруктураОснования);
	КонецЕсли;
...
Процедура ДополнитьПодстановки(ДанныеИзИНИ, ПараметрыЗаполнения) Экспорт
	...
	Если ПараметрыЗаполнения.РольКонтрагента = ЗначениеМетаданных("Saby_РолиКонтрагентов.Отправитель") Тогда
		ДополнитьПодстановкиГрузоотправителя(ДанныеИзИНИ, ПараметрыЗаполнения);
	ИначеЕсли ПараметрыЗаполнения.РольКонтрагента = ЗначениеМетаданных("Saby_РолиКонтрагентов.Получатель") Тогда
		ДополнитьПодстановкиГрузополучателя(ДанныеИзИНИ, ПараметрыЗаполнения);
	ИначеЕсли ПараметрыЗаполнения.РольКонтрагента = ЗначениеМетаданных("Saby_РолиКонтрагентов.Перевозчик") Тогда
		ДополнитьПодстановкиГрузоперевозчика(ДанныеИзИНИ, ПараметрыЗаполнения);
	ИначеЕсли ПараметрыЗаполнения.РольКонтрагента = ЗначениеМетаданных("Saby_РолиКонтрагентов.Заказчик") Тогда
...
Процедура ДополнитьПроверяемыеРеквизитыЭтапаПогрузка(ДанныеДокумента, ПроверяемыеРеквизиты, ПараметрыВыгрузки)
	...
	Если РеквизитыОбъекта.Владелец_Тип = ЗначениеМетаданных("Saby_ВладелецОбъектаОтгрузки.Неизвестен") Тогда
	...
	Если ПараметрыВыгрузки.ТипТитула = ЗначениеМетаданных("Saby_ТипТитулаЭтрН.ПустаяСсылка")
		И РеквизитыОбъекта.Владелец_Тип = ЗначениеМетаданных("Saby_ВладелецОбъектаОтгрузки.ДругоеЮрЛицо") Тогда
...
Процедура ДопПроверкиФЛКПогрузка(ДанныеДокумента, ПараметрыВыгрузки, ПараметрыПроверкиРеквизитов, Ошибки)
	...
	МодульТНОбщегоНазначенияСервер.ПроверитьЗаполнениеТранспортногоСредства(
		ДанныеДокумента, Ошибки, ПараметрыПроверкиРеквизитов.ОшибкиПоРеквизитам);
	
	МодульТНОбщегоНазначенияСервер.ПроверитьЗаполнениеТипаВладения(
		ДанныеДокумента, Ошибки, ПараметрыПроверкиРеквизитов.ОшибкиПоРеквизитам);
...
Процедура ДопПроверкиФЛКУведомлениеОПереадресовке(ДанныеДокумента, ПараметрыПроверкиРеквизитов, Ошибки)
	...
	Структура = МодульТНОбщегоНазначенияСервер.ДесериализоватьЗначение(ДанныеДокумента.Реквизиты.ОформилСтруктура);
...
Функция ТитулПогрузка(ДанныеДокумента)
	...
	Дата = МодульТНВыгрузкаСервер.ЗначениеДатыВремяДляВыгрузки(РеквизитыОбъекта.Дата, Истина);
...
Функция ТитулПолучениеГруза(ДанныеДокумента)
	...
	ПриемкаДата = МодульТНВыгрузкаСервер.СтруктураДатаВремя(РеквизитыОбъекта.Перевозчик_ПриемкаДатаВремя);
	ДобавитьСвойствоВСтруктуруСКонтролем(Замечания, "ДатаВремяПостановки", ПриемкаДата, Истина);
	
	ПрибылДата = МодульТНВыгрузкаСервер.СтруктураДатаВремя(РеквизитыОбъекта.Перевозчик_ПриемкаДатаВремяПрибыл);
	ДобавитьСвойствоВСтруктуруСКонтролем(Замечания, "ДатаВремяПрибытия", ПрибылДата, Истина);
	
	УбылДата = МодульТНВыгрузкаСервер.СтруктураДатаВремя(РеквизитыОбъекта.Перевозчик_ПриемкаДатаВремяУбыл);
...
Функция ТитулПриемкаГруза(ДанныеДокумента)
	...
	ПрибылДата = МодульКода("Saby_ТНВыгрузкаСервер").СтруктураДатаВремя(РеквизитыОбъекта.Выгрузка_ДатаВремяПрибыл);
	ДобавитьСвойствоВСтруктуруСКонтролем(ОбщаяСтруктура, "ДатаВремяПрибытия", ПрибылДата, Истина);
	
	УбылДата = МодульКода("Saby_ТНВыгрузкаСервер").СтруктураДатаВремя(РеквизитыОбъекта.Выгрузка_ДатаВремяУбыл);
	...
	Масса.Вставить(
		"МетодОпределения",
		МодульКода("Перечисления.Saby_МетодОпределенияМассы").КодПоЗначению(РеквизитыОбъекта.Выгрузка_МассаМетодРасчета));
...
Функция ТитулВыдачаГруза(ДанныеДокумента)
	...
	ПостановкаДата = МодульТНВыгрузкаСервер.СтруктураДатаВремя(РеквизитыОбъекта.Перевозчик_ВыгрузкаДатаВремя);
	ДобавитьСвойствоВСтруктуруСКонтролем(Замечания, "ДатаВремяПостановки", ПостановкаДата, Истина);
	
	ПрибылДата = МодульТНВыгрузкаСервер.СтруктураДатаВремя(РеквизитыОбъекта.Перевозчик_ВыгрузкаДатаВремяПрибыл);
	ДобавитьСвойствоВСтруктуруСКонтролем(Замечания, "ДатаВремяПрибытия", ПрибылДата, Истина); 
	
	УбылДата = МодульТНВыгрузкаСервер.СтруктураДатаВремя(РеквизитыОбъекта.Перевозчик_ВыгрузкаДатаВремяУбыл);
	ДобавитьСвойствоВСтруктуруСКонтролем(Замечания, "ДатаВремяУбытия", УбылДата, Истина);
...
Функция ТитулПереадресовка(ДанныеТитула)
	...
	Переадресовка.Вставить(
		"ДатаВремя",
		МодульТНВыгрузкаСервер.СтруктураДатаВремя(РеквизитыОбъекта.Переадресовка_ДатаВремя, , Истина));
...
Функция ТитулЗаменаВодителя(ДанныеТитула)
	...
	Если ЗначениеЗаполнено(ОснованиеЗамены) Тогда  
		...
		ЗаменаВодителяТС.Вставить(
			"ДатаВремя",
			МодульТНВыгрузкаСервер.СтруктураДатаВремя(РеквизитыОбъекта.ДатаВремяЗамены, , Истина));
...
Функция ТитулИзменениеСтоимости(ДанныеТитула)
	...
	СтруктураСтоимости.Налог.Вставить(
		"Ставка", МодульКода("Saby_ТНОбщегоНазначенияСервер").ЗначениеСтавкиНДС(РеквизитыОбъекта.Ставка));
...
Функция СвязанныеДанныеСГрузом(ДанныеДокумента, КлючСтроки)
	...
	МодульКода("Saby_ТНВыгрузкаСервер").ЗаполнитьОпасныйГруз(ОпасныеГрузыИзДокумента, ОпасныеГрузы);
...
Функция УсловияПеревозки(ДанныеДокумента, ИнициаторПодтверждение)
	...
	Переадресовка.Вставить(
		"КтоПереадресует", МодульКода("Перечисления.Saby_РолиКонтрагентов").КодПоЗначению(РеквизитыОбъекта.ИнициаторПереадресации));
...
Функция ДанныеПогрузки(ДанныеДокумента, ОснованиеОтгрузки, ОснованиеПолномочий) 
	...
	Если РеквизитыОбъекта.Владелец_Тип = ЗначениеМетаданных("Saby_ВладелецОбъектаОтгрузки.Отправитель") Тогда
	...
	ВладелецПункта.Вставить(
		"Тип", МодульКода("Перечисления.Saby_ВладелецОбъектаОтгрузки").КодПоЗначению(РеквизитыОбъекта.Владелец_Тип));
	...
	Груз.Масса.Вставить(
		"МетодОпределения",
		МодульКода("Перечисления.Saby_МетодОпределенияМассы").КодПоЗначению(РеквизитыОбъекта.Погрузка_МассаМетодРасчета));
	...
	Структура.Вставить(
		"ДатаВремяПрибытия",  МодульТНВыгрузкаСервер.СтруктураДатаВремя(РеквизитыОбъекта.Погрузка_ДатаВремяПрибыл));
	Структура.Вставить(
		"ДатаВремяУбытия",    МодульТНВыгрузкаСервер.СтруктураДатаВремя(РеквизитыОбъекта.Погрузка_ДатаВремяУбыл));
...
Процедура ЗагрузкаТитулПогрузка(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Если Не ДопПараметры.ТолькоАктивныйЭтап Тогда
		...
		МодульТНЗагрузкаСервер.ЗаполнитьДанныеПоГрузам(ОбъектДок, ДанныеТитула.ДляДокумента);
...
Процедура ЗагрузкаТитулЗаменаВодителяТС(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Для Каждого ДанныеТитула Из ДанныеЗаменаВодителяТС Цикл
		...
		МодульКода("Saby_ТНЗагрузкаСервер").ЗаполнитьДанныеТСИПрицеповДокумента(ОбъектДок, ДанныеТитула.ДляДокумента);
		
		МодульКода("Saby_ТНЗагрузкаСервер").ЗаполнитьОтветственныхЛиц(
			ОбъектДок,
			ДанныеТитула.ДляДокумента.Водители,
			ЗначениеМетаданных("Saby_РолиОтветственных.Водитель"));
	...
	Для Каждого Вложение Из ВложенияДляЗагрузки Цикл
		...
		Если ЗначениеЗаполнено(ЗаявкаНаПеревозку) Тогда		
			...
			ДляДокумента.Вставить(
				"Отправитель_ЗаявкаДата", МодульКода("Saby_ТНОбщегоНазначенияСервер").ПреобразоватьСтрокуВДатуЭПД(ЗаявкаНаПеревозку["Дата"]));
		КонецЕсли;
		...
		ДляДокумента.Вставить("Водители", МодульТНЗагрузкаСервер.ДанныеВодителей(Вложение["Водители"], ДопПараметры));
...
Функция ДанныеПриемкаГруза(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Для Каждого Вложение Из ВложенияДляЗагрузки Цикл
		...
		Если ЗначениеЗаполнено(Масса) Тогда 
			...
			МетодОпределения = МодульКода("Перечисления.Saby_МетодОпределенияМассы").ЗначениеПоКоду(Масса["МетодОпределения"]);
...
Функция ДанныеПереадресовка(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Для Каждого Вложение Из ВложенияДляЗагрузки Цикл
		...
		Если ЗначениеЗаполнено(Основание) Тогда 
			...
			ОснованиеСсылка = МодульТНЗагрузкаСервер.ЗагрузитьОдиночныйДокументЭПД(Основание, Тип);				
...
Функция ДанныеЗаменаВодителяТС(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Для Каждого Вложение Из ВложенияДляЗагрузки Цикл
		...
		Если ЗначениеЗаполнено(Основание) Тогда 
			...
			ДокументЗамены = МодульТНЗагрузкаСервер.ЗагрузитьОдиночныйДокументЭПД(Основание, Тип);
...
Функция ДанныеСогласованиеСтоимости(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Для Каждого Вложение Из ВложенияДляЗагрузки Цикл
		...
		Если ЗначениеЗаполнено(ИнойПлательщик) Тогда
			...
			СтруктураДанныхЮрЛица = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").СтруктураДанныхЮрЛица();
			...
			Если ЗначениеЗаполнено(Основание) Тогда 
				...
				ОснованиеСсылка = МодульКода("Saby_ТНЗагрузкаСервер").ЗагрузитьОдиночныйДокументЭПД(Основание, Тип);				
...
Функция СтруктураДанныхДляРегистра(ОбъектДок, Вложение, Титул, ИмяМетаданных, Просмотрено = Истина)
	...
	СтруктураДанныхДляРегистра = МодульКода("Saby_ТНЗагрузкаСервер").СтруктураДанныхДляРегистра(ОбъектДок.Ссылка, Титул, Основной);
...
Процедура ЗагрузитьДанныеПогрузки(ДляДокумента, Погрузка)
	...
	МассаМетодРасчета = МодульКода("Перечисления.Saby_МетодОпределенияМассы").ЗначениеПоКоду(МассаМетодРасчета);
...
Процедура ЗагрузитьУсловияПеревозки(ДляДокумента, УсловияПеревозки)
	...
	ДляДокумента.Вставить(
		...
		МодульКода("Перечисления.Saby_РолиКонтрагентов").ЗначениеПоКоду(КтоПереадресует));
	...
	Если ЗначениеЗаполнено(ПодтверждениеПолномочий) Тогда 
		...
		ПодтверждениеСсылка = МодульКода("Saby_ТНЗагрузкаСервер").ЗагрузитьОдиночныйДокументЭПД(ПодтверждениеПолномочий, Тип);
...
Процедура ЗаполнитьДокументыКтоПогружает(ДляДокумента, КтоПогружает)
	...
	// Документы владения
	ТЗ = МодульТНЗагрузкаСервер.ТаблицаДокументовЭПД();
	НомерСтроки = 0;
	...
	Если ЗначениеЗаполнено(ДокументОснование) Тогда 			
		НС = ТЗ.Добавить();
		Тип = ЗначениеМетаданных("Saby_ТипыДокумента.ПередачиЦенностей");
		МодульТНЗагрузкаСервер.ЗаполнитьДанныеСтрокиТаблицыДокументовЭПД(НС, ДокументОснование, Тип, НомерСтроки);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ДокПолномочий) Тогда
		НС = ТЗ.Добавить();
		Тип = ЗначениеМетаданных("Saby_ТипыДокумента.ОтветственныйНаОсновании");
		МодульТНЗагрузкаСервер.ЗаполнитьДанныеСтрокиТаблицыДокументовЭПД(НС, ДокПолномочий, Тип, НомерСтроки);
	Иначе
	...
	СоответствиеДокументов = МодульТНЗагрузкаСервер.НайтиСоздатьДокументЭПД(ТЗ);
		
	// Заполняем данные
	Для Каждого СтрокаДок Из ТЗ Цикл 
		
		ДокументОснования = СоответствиеДокументов.Получить(СтрокаДок.НомерСтроки);
		Если СтрокаДок.ТипДок = ЗначениеМетаданных("Saby_ТипыДокумента.ПередачиЦенностей") Тогда 
			ДляДокумента.Вставить("Отгрузка_НаОснованииДокумент", ДокументОснования);
		ИначеЕсли СтрокаДок.ТипДок = ЗначениеМетаданных("Saby_ТипыДокумента.ОтветственныйНаОсновании") Тогда
...
Процедура ЗаполнитьДанныеВладельца(ДляДокумента, ВладелецПунктаСОнлайна)
	...
	Тип = МодульКода("Перечисления.Saby_ВладелецОбъектаОтгрузки").ЗначениеПоКоду(ВладелецПунктаСОнлайна["Тип"]);
...
Функция ДанныеПоОтметкам(ОтметкиСОнлайна, Титул)
	...
	Для Каждого Отметка Из ОтметкиСОнлайна["Отметка"] Цикл
		...
		СтруктураОтметки.Вставить(
			"ДатаВремя",
			МодульКода("Saby_ТНОбщегоНазначенияСервер").ПреобразоватьСтрокуВДатуЭПД(Отметка["ДатаВремя"]["Значение"]));
...
Функция ДанныеАктовПоОтметкам(АктыВзвешиванияСОнлайна, АктыКоммерческиеСОнлайна)
	...
	ТаблицаАктов = МодульТНЗагрузкаСервер.ТаблицаДокументовЭПД();
	
	МодульТНЗагрузкаСервер.ДокументыЭПД(
		АктыВзвешиванияСОнлайна, 
		ТаблицаАктов, 
		ЗначениеМетаданных("Saby_ТипыДокумента.АктВзвешивания"),
	    НомерСтроки);
	
	МодульТНЗагрузкаСервер.ДокументыЭПД(
		АктыКоммерческиеСОнлайна, 
		ТаблицаАктов, 
		ЗначениеМетаданных("Saby_ТипыДокумента.АктКоммерческий"),
	    НомерСтроки);
	
	// Получим ссылки на существующие и новые документы СправочникСсылка.Saby_ДокументыЭПД
	СоответствиеДокументов = МодульТНЗагрузкаСервер.НайтиСоздатьДокументЭПД(ТаблицаАктов);
...
Процедура ЗаполнитьДанныеПоОтметкамДокумента(ОбъектДок, Отметки, Титул)
	...
	Для Каждого Отметка Из Отметки["Отметка"] Цикл
		...
		НС.ДатаВремя  = МодульКода("Saby_ТНОбщегоНазначенияСервер").ПреобразоватьСтрокуВДатуЭПД(Отметка["ДатаВремя"]["Значение"]);
...
Процедура ЗагрузитьДанныеДокументовЭПД(ДляДокумента, ДанныеДокументов)
	...
	Если ЗначениеЗаполнено(СопроводительнаяВедомость) Тогда 
		НС = ТЗ.Добавить();
		Тип = ЗначениеМетаданных("Saby_ТипыДокумента.СопроводительнаяВедомость");
		МодульТНЗагрузкаСервер.ЗаполнитьДанныеСтрокиТаблицыДокументовЭПД(НС, СопроводительнаяВедомость, Тип, НомерСтроки);
	КонецЕсли;
	
	// УслугиПеревозки
	Если ЗначениеЗаполнено(ДоговорПеревозки) Тогда 
		НС = ТЗ.Добавить();	
		Тип = ЗначениеМетаданных("Saby_ТипыДокумента.ДоговорПеревозки");
		МодульТНЗагрузкаСервер.ЗаполнитьДанныеСтрокиТаблицыДокументовЭПД(НС, ДоговорПеревозки, Тип, НомерСтроки);
	КонецЕсли;
	...
	Для Каждого СтрокаДок Из ТЗ Цикл
		...
		Если СтрокаДок.ТипДок = ЗначениеМетаданных("Saby_ТипыДокумента.СопроводительнаяВедомость") Тогда
			
			ДляДокумента.Вставить("Отправитель_СопроводительнаяВедомость", СсылкаДок);
			
		ИначеЕсли СтрокаДок.ТипДок = ЗначениеМетаданных("Saby_ТипыДокумента.ДоговорПеревозки") Тогда
...
Функция ФлагЭкспедитор(ДляДокумента)
	...
	Иначе
		СтрокаОтправителя = МодульТНОбщегоНазначенияКлиентСервер.СтруктураДанныхЮрЛица();
	КонецЕсли;
