...
Процедура УстановитьЗапретЗаполненияЭТрН(ПараметрыЗаполнения) Экспорт
	...
	Если ТипЗнч(ПараметрыЗаполнения.Объект.Ссылка) = Тип("Структура") Тогда
		ПогрузкаАдрес            = ПараметрыЗаполнения.Объект.Ссылка.Погрузка_Адрес;
		ОтправительАдресДоставки = ПараметрыЗаполнения.Объект.Ссылка.Отправитель_АдресДоставки;
	Иначе
...
Функция ТитулЗаменаВодителя(ДанныеТитула)
	...
	// Прицепы
	Прицепы = МодульТНВыгрузкаСервер.ПодстановкиПрицепов(ДанныеТитула.ТранспортныеСредства);
...
Функция ПодстановкиОснованияСоставителя(ДанныеДокумента, ОснованиеСоставителя)
	...
	Если ДанныеДокумента.ЭтоВО Тогда 
		...
		Отбор = Новый Структура;
		Отбор.Вставить("Тип", "ОформительОснование"); 
		
		НайденныеСтроки = НайтиСтрокиУниверсально(ДанныеДокумента.ДокументыЭПД, Отбор);
...
Процедура ЗагрузкаТитулПогрузка(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Если Не ДопПараметры.ТолькоАктивныйЭтап Тогда
		...
		Исключения = "Отметки,ОтветственныеЛица,ТранспортныеСредства,СпецРазрешения,ДанныеЮрЛиц,КонтактныеДанные";	
		Если ТипЗнч(ОбъектДок) = Тип("Структура") Тогда
			Исключения = "ДокументыЭПД," + Исключения;
		КонецЕсли;	
...
Процедура ЗагрузкаТитулПолучениеГруза(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Если Не ДопПараметры.ТолькоАктивныйЭтап Тогда
		...
		КолонкиИсключения = "Отметки,ДанныеЮрЛиц,КонтактныеДанные,ОтветственныеЛица";
...
Процедура ЗагрузкаТитулПриемкаГруза(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Если Не ДопПараметры.ТолькоАктивныйЭтап Тогда
		...
		ЗаполнитьЗначенияСвойств(ОбъектДок, ДанныеДляДокументаБезПустыхЗначений, , "ДокументыЭПД,Отметки,ДанныеЮрЛиц,КонтактныеДанные");
...
Процедура ЗагрузкаТитулВыдачаГруза(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Если Не ДопПараметры.ТолькоАктивныйЭтап Тогда
		...
		ЗаполнитьЗначенияСвойств(ОбъектДок, ДанныеТитула.ДляДокумента, , "ДокументыЭПД,Отметки,ДанныеЮрЛиц,КонтактныеДанные");
