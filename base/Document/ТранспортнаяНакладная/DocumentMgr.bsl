...
Функция ПроверятьРеквизитТЧДокумента(СтрокаТЧ, ИмяТЧ, ИмяРеквизита, ДанныеДокумента) Экспорт
	...
	Если ИмяТЧ = "Водители" Тогда
		Возврат МодульКода("ESD_ТНОбщегоНазначенияСервер").ПроверятьРеквизитВодителя(СтрокаТЧ, ИмяРеквизита);
	ИначеЕсли ИмяТЧ = "КонтактныеДанные" Тогда
		Возврат СтрокаТЧ.Роль <> ЗначениеМетаданных("ESD_РолиКонтрагентов.Отгрузчик") Или ДанныеДокумента.ПроверятьПоляОтгрузчика;
	Иначе
		Возврат Истина;
	КонецЕсли;
...
Функция ТитулыДляЗагрузки(ОбъектДок, ПараметрыЗагрузки) Экспорт
	...
	ЭтоДинамическийТитул = МодульТНОбщегоНазначенияСервер.РезультатФункцииТитулов(
		ТранспортнаяНакладная(), "ЭтоДинамическийТитул", Этап);
...
Процедура ДопПроверкиФЛКПереадресовка(ДанныеДокумента, Ошибки)
	...
	Если СтруктураДанныхЮрЛица.Заполнена Тогда
		СтруктураОшибки = ФЛК("Получатель_Телефон", "Получатель_Телефон", "Не заполнен телефон получателя");
		МодульКода("ESD_ТНОбщегоНазначенияСервер").ПроверитьКонтактныеДанные(
...
Функция ТипыПоУмолчаниюДляЗаполненияНаОсновании()
	...
	Если Не ЭтоДоступнаяКонфигурация
		Или (СинонимКонфигурации = ДоступныеКонфигурации.УАТ 
	...
	Если СинонимКонфигурации = ДоступныеКонфигурации.УТ10 
		Или СинонимКонфигурации = ДоступныеКонфигурации.УПП Тогда
		Возврат ДокументыОснования;
	КонецЕсли;
	...
	Если СинонимКонфигурации <> ДоступныеКонфигурации.Бух3 Тогда
		...
		Если СинонимКонфигурации <> ДоступныеКонфигурации.УТ Тогда
...
Функция ДанныеПогрузки(ДанныеДокумента, ПодстановкиДокументов)
	...
	Если ДанныеДокумента.Владелец_Тип = ЗначениеМетаданных("ESD_ВладелецОбъектаОтгрузки.Отправитель") Тогда
		...
		ВладелецПункта.Грузоотправитель = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСтруктуры(
...
Функция ЛицоОтгрузки(ДанныеДокумента, ПодстановкиДокументов)
	...
	Если Тип = ТипДругогоОтправителя Тогда 
		...
		Если ЗначениеЗаполнено(Ответственный) Тогда
			...
			ДанныеОтветственного = МодульКода("ESD_ТНОбщегоНазначенияСервер").СтруктураОтветственного(
...
Функция ДанныеПереадресовка(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Для Каждого Вложение Из ВложенияДляЗагрузки Цикл
		...
		Основание = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(
...
Функция ДанныеУведомлениеОПереадресовке(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Для Каждого Вложение Из ВложенияДляЗагрузки Цикл
		...
		Причина = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(Вложение, "Переадресовка.Уведомление.Причина");
		Если ЗначениеЗаполнено(Причина) Тогда
			ДляДокумента.Вставить("Причина", Причина);
		КонецЕсли;
		
		Оформитель = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(
...
Процедура ЗагрузитьДанныеДокумента(ОбъектДок, ДляДокумента, Вложение, ДопПараметры)
	...
	// Загружать номер и дату только для ВО
	
	Если ТипЗнч(ОбъектДок) <> Тип("Структура") Тогда
		Возврат;
	КонецЕсли;
	
	ДатаДокумента = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(Вложение, "Документ.Дата");
	ДляДокумента.Вставить(
		"Дата", МодульКода("ESD_ТНОбщегоНазначенияСервер").ПреобразоватьСтрокуВДатуЭПД(ДатаДокумента));
	
	НомерДокумента = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(Вложение, "Документ.Номер");
...
Процедура ЗаполнитьДанныеВладельца(ДляДокумента, ВладелецПунктаСОнлайна)
	...
	ИначеЕсли Тип = ЗначениеМетаданных("ESD_ВладелецОбъектаОтгрузки.ДругоеЮрЛицо") Тогда
		...
		ДругойКонтрагент = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(
