...
Функция ТитулПереадресовка(ДанныеТитула)
	...
	Если ЗначениеЗаполнено(ДанныеТитула.ПереадресовкаНаОсновании) Тогда
		...
		ПодстановкаДокумента = МодульКода("Saby_ТНВыгрузкаСервер").ПодстановкаДокументаПоСсылке(
			ДанныеТитула.ПереадресовкаНаОсновании);
...
Функция ТитулЗаменаВодителя(ДанныеТитула)
	...
	Если Не ЗначениеЗаполнено(ОснованиеЗамены) Тогда
		ОснованиеЗамены = МодульТНВыгрузкаСервер.ПодстановкаДокументаПоСсылке(ДанныеТитула.ДокументЗамены);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ДанныеТитула.Водители) Тогда 
		Водители = МодульТНВыгрузкаСервер.ПодстановкиВодителя(ДанныеТитула.Водители, ДанныеТитула.ПутевыеЛисты);
	...
	Если ЗначениеЗаполнено(ДанныеТитула.ОснованиеПередачиГрузаТС) Тогда
		...
		ПодстановкаДокумента = МодульТНВыгрузкаСервер.ПодстановкаДокументаПоСсылке(
			ДанныеТитула.ОснованиеПередачиГрузаТС);
...
Функция АктыОтметки(ДанныеДокумента, МодульКлиентСервер, Отбор)
	...
	Для Каждого Строка Из НайденныеСтроки Цикл
		...
		Если ДанныеДокумента.ЭтоВО Тогда
			...
			ОтборЭПД.Вставить("КлючСтроки", Строка.КлючСтроки_ДокументыЭПД);
			
			НайденныеСтрокиЭПД = МодульКлиентСервер.НайтиСтрокиУниверсально(ДанныеДокумента.ДокументыЭПД, ОтборЭПД);
			Если НайденныеСтрокиЭПД.Количество() Тогда
				МассивАктовРасчет.Добавить(НайденныеСтрокиЭПД[0]);
			КонецЕсли;
...
Процедура ЗагрузкаТитулПогрузка(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Если Не ДопПараметры.ТолькоАктивныйЭтап Тогда
		...
		Исключения = ДополнитьСписокИсключений(ОбъектДок, 
						...
						"ОтветственныеЛица,ТранспортныеСредства,СпецРазрешения");
...
Функция ДополнитьСписокИсключений(ОбъектДок, ДанныеТитула, СписокИсключений = "")
	...
	Для Каждого ЭлементИсключения Из МассивИсключений Цикл
		...
		Если ЕстьВДокументе И ЕстьВТитуле Тогда			
