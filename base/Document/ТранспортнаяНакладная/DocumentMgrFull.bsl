...
#Область include_etrn_base_Document_ОбщийФункционал_МодульМенеджера_ОбработчикиСобытий_Представление
#КонецОбласти // include_etrn_base_Document_ОбщийФункционал_МодульМенеджера_ОбработчикиСобытий_Представление 
...
Процедура СохранитьВыбранныеТипыДляЗаполненияНаОсновании(МассивТипов) Экспорт
	МодульКода("ESD_ТНОбщегоНазначенияСервер").ESD_СохранитьНастройкиОбъекта("ТипыДляЗаполненияНаОснованииДокументаТРН", 
		МассивТипов, , "");
КонецПроцедуры
...
Функция ИзменитьРеквизитТЧ(СтрокаТЧ, ИмяТЧ, Реквизит, ДанныеДокумента) Экспорт
	...
	ИначеЕсли ИмяТЧ = "КонтактныеДанные" Тогда
		НовыйРеквизит = НовыйРеквизитКонтактныеДанные(СтрокаТЧ, Реквизит, ДанныеДокумента);
	Иначе
...
Функция ТекущиеЭтапыДокумента(ОбъектДокумента, ИндексКартинки) Экспорт
	...
	Если РезультатФункции.Количество() = 0 И ИндексКартинки = 0 Тогда
		ПервыйТитул_ = МодульКода("ESD_ТНОбщегоНазначенияСервер")
			.РезультатФункцииТитулов(ТранспортнаяНакладная(), "ПервыйТитул");
...
#Область include_etrn_base_Document_ТранспортнаяНакладная_МодульМенеджера_СлужебныйПрограммныйИнтерфейс_Инициализация
#КонецОбласти // include_etrn_base_Document_ТранспортнаяНакладная_МодульМенеджера_СлужебныйПрограммныйИнтерфейс_Инициализация
...
Процедура ДополнитьПроверяемыеРеквизитыЭтапаПогрузка(ДанныеДокумента, ПроверяемыеРеквизиты, ПараметрыВыгрузки)
	...
	ПроверяемыеРеквизиты.Добавить(ФЛК("Погрузка_Адрес", "ДекорацияЗаполнитьАдресПогрузки", 
		"Не заполнен адрес погрузки"));

	ПроверяемыеРеквизиты.Добавить(ФЛК("Отправитель_ЗаявкаДата", "Заявка_Дата", "Не заполнена дата заявки"));
	ПроверяемыеРеквизиты.Добавить(ФЛК("Отправитель_АдресДоставки", "ДекорацияЗаполнитьАдресДоставки", 
		"Не заполнен адрес доставки"));
	...
	ПроверяемыеРеквизиты.Добавить(
		ФЛК("ОпасныеГрузы.ОпасныйГрузНаименование", ОпасныеГрузыОпасныйГруз, "Не заполнено наименование опасного груза"));
	ПроверяемыеРеквизиты.Добавить(
...
Процедура ДопПроверкиФЛКПогрузка(ДанныеДокумента, ПараметрыВыгрузки, Ошибки)
	...
	Если ДанныеДокумента.Отправитель_Экспедитор И ЗначениеЗаполнено(ДанныеДокумента.ЗаказчикСтрокой) Тогда
	...
	ИмяРеквизита_ = "Погрузка_Адрес_GLN";
	АдресПогрузки_ = ДанныеДокумента[ИмяРеквизита_];
	Если ЗначениеЗаполнено(АдресПогрузки_) И СтрДлина(СокрЛП(АдресПогрузки_)) <> 13 Тогда
		СтруктураОшибки = ФЛК(ИмяРеквизита_, ИмяРеквизита_, "Адрес погрузки. GLN должен состоять из 13 символов");
		МодульТНОбщегоНазначенияСервер.ДобавитьОшибкуПользователю(Ошибки, СтруктураОшибки.РеквизитОбъекта, 
			СтруктураОшибки.Ошибка,
			СтруктураОшибки.РеквизитОбъекта,
			СтруктураОшибки.ЭлементФормы);
	КонецЕсли;

	ИмяРеквизита_ = "Отправитель_АдресДоставки_GLN";
	АдресДоставки_ = ДанныеДокумента[ИмяРеквизита_];
	Если ЗначениеЗаполнено(АдресДоставки_) И СтрДлина(СокрЛП(АдресДоставки_)) <> 13 Тогда
		СтруктураОшибки = ФЛК(ИмяРеквизита_, ИмяРеквизита_, "Адрес доставки. GLN должен состоять из 13 символов");
		МодульТНОбщегоНазначенияСервер.ДобавитьОшибкуПользователю(Ошибки, СтруктураОшибки.РеквизитОбъекта, 
			СтруктураОшибки.Ошибка,
			СтруктураОшибки.РеквизитОбъекта,
			СтруктураОшибки.ЭлементФормы);
...
Функция НовыйРеквизитКонтактныеДанные(СтрокаТЧ, Реквизит, ДанныеДокумента)
	Если СтрокаТЧ.Роль = ЗначениеМетаданных("ESD_РолиКонтрагентов.Отправитель")
		Или СтрокаТЧ.Роль = ЗначениеМетаданных("ESD_РолиКонтрагентов.Получатель")
		Или СтрокаТЧ.Роль = ЗначениеМетаданных("ESD_РолиКонтрагентов.Перевозчик") 
	Тогда
		РеквизитОбъекта = Реквизит.РеквизитОбъекта;
		ЭлементФормы = МодульКода("Перечисления.ESD_РолиКонтрагентов").ИмяРоли(СтрокаТЧ.Роль) 
			+ "КонтактныеДанные" 
			+ Реквизит.РеквизитОбъекта;
		ПрефиксОшибки = Строка(СтрокаТЧ.Роль);
	ИначеЕсли СтрокаТЧ.Роль = ЗначениеМетаданных("ESD_РолиКонтрагентов.Заказчик") Тогда
		РеквизитОбъекта = "ЗаказчикСтрокой";
		ЭлементФормы = РеквизитОбъекта;
		ПрефиксОшибки = "Заказчик";
	ИначеЕсли СтрокаТЧ.Роль = ЗначениеМетаданных("ESD_РолиКонтрагентов.Отгрузчик") Тогда
		Если ЗначениеЗаполнено(ДанныеДокумента["СоставительСтрокой"]) Тогда
			РеквизитОбъекта = "ОформительСтрокой";
			ЭлементФормы = РеквизитОбъекта;
			ПрефиксОшибки = "СОХ";
		Иначе
			РеквизитОбъекта = Реквизит.РеквизитОбъекта;
			ЭлементФормы = МодульКода("Перечисления.ESD_РолиКонтрагентов").ИмяРоли(СтрокаТЧ.Роль) 
				+ "КонтактныеДанные" 
				+ Реквизит.РеквизитОбъекта;
			ПрефиксОшибки = "Отгрузчик";
		КонецЕсли;
	Иначе
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат ФЛК(РеквизитОбъекта, ЭлементФормы, ПрефиксОшибки + ": " + Реквизит.Ошибка);
КонецФункции
...
Функция ПроверятьДанныеЮрЛица(СтрокаТЧ, Реквизит, ДанныеДокумента)
	...
	Если СтрокаТЧ.Роль = ЗначениеМетаданных("ESD_РолиКонтрагентов.ВладелецОбъекта") Тогда
		РезультатФункции = ДанныеДокумента.Владелец_Тип = ЗначениеМетаданных("ESD_ВладелецОбъектаОтгрузки.ДругоеЮрЛицо")
			И Реквизит.РеквизитОбъекта <> "Адрес"
			И ПроверятьРеквизит;
	ИначеЕсли СтрокаТЧ.Роль = ЗначениеМетаданных("ESD_РолиКонтрагентов.Отгрузчик") Тогда
...
Функция ТипыПоУмолчаниюДляЗаполненияНаОсновании()
	...
	Если Не ДоступнаяКонфигурация.Поддержана Или ИмяКонфигурации = "УАТ" Тогда
		Возврат ДокументыОснования;
	КонецЕсли;
	
	ДокументыОснования.Добавить("Документы.РеализацияТоваровУслуг");
	Если ИмяКонфигурации = "УТ10" Или ИмяКонфигурации = "УПП" Тогда
	...
	Если ИмяКонфигурации <> "Бух3" Тогда
		...
		ДокументыОснования.Добавить("Документы.ЗаданиеНаПеревозку");
		
		Если ИмяКонфигурации <> "УТ11" Тогда
			ДокументыОснования.Добавить("Документы.ПриобретениеТоваровУслуг");
		КонецЕсли;
		
		ДокументыОснования.Добавить("Документы.ПоручениеЭкспедитору");
...
Функция ПользовательскиеТипыДляЗаполненияНаОсновании()
	РезультатФункции = МодульКода("ESD_ТНОбщегоНазначенияСервер")
		.ESD_ЗагрузитьНастройкиОбъекта("ТипыДляЗаполненияНаОснованииДокументаТРН", , "");
	Возврат РезультатФункции;
КонецФункции
...
Функция ТекстЗапросаРеквизитов()
	...
	|	ВЫБОР
	|		КОГДА ESD_ТН.Выгрузка_Адрес = """" ТОГДА
	|			ESD_ТН.Отправитель_АдресДоставки
	|		ИНАЧЕ
	|			ESD_ТН.Выгрузка_Адрес
	|	КОНЕЦ КАК Выгрузка_Адрес,
	|	ВЫБОР
	|		КОГДА ESD_ТН.Выгрузка_Адрес = """" ТОГДА
	|			ESD_ТН.Отправитель_АдресДоставки_GLN
	|		ИНАЧЕ
	|			ESD_ТН.Выгрузка_Адрес_GLN
	|	КОНЕЦ КАК Выгрузка_Адрес_GLN,
	|	ESD_ТН.Отправитель_АдресДоставки КАК Отправитель_АдресДоставки,
	|	ESD_ТН.Отправитель_АдресДоставки_GLN КАК Отправитель_АдресДоставки_GLN,
	...
	|	ESD_ТН.Погрузка_Адрес_GLN КАК Погрузка_Адрес_GLN,
...
Функция НестандартныеЗапросыТаблиц(Таблицы)
	...
	Для Каждого ИмяТаблицы Из Таблицы Цикл
		...
		ИначеЕсли ИмяТаблицы = "ОпасныеГрузы" Тогда
			...
			|	ЕСТЬNULL(ESD_ОпасныеГрузы.Наименование, """") КАК ОпасныйГрузНаименование,
			...
			|	ЕСТЬNULL(ESD_ОпасныеГрузы.Комментарий, """") КАК Описание,
			...
			|	ЕСТЬNULL(ESD_ОпасныеГрузы.НазваниеРадионуклида, """") КАК РадиоактивныйРадионуклид,
			...
			|	ЕСТЬNULL(ESD_ОпасныеГрузы.КатегорияУпаковки, """") КАК УпаковкаКатегория,
			...
			|	ЕСТЬNULL(ESD_ОпасныеГрузы.МассаНеттоВзрывчатый, """") КАК ВзрывчатыйМассаНетто,
			|	ЕСТЬNULL(ESD_ОпасныеГрузы.МассаНеттоВзрывчатыйВсего, """") КАК ВзрывчатыйМассаНеттоДляВсех,
			|	ЕСТЬNULL(ESD_ОпасныеГрузы.ПроцентСмеси, """") КАК ПроцентныйСоставСмеси,
...
Функция ТитулПогрузка(ДанныеДокумента)
	...
	СтруктураТитула.Вставить(
		"СоставительЧерновика",
		МодульТНВыгрузкаСервер.ДанныеЮрЛица("Составитель",  ДанныеДокумента));
	...
	ДополнительныеПараметры_ = ПунктПогрузкиВыгрузки_ДополнительныеПараметры(, Истина);
	СтруктураТитула.Вставить("ПунктВыгрузки", ПунктВыгрузки(ДанныеДокумента, ДополнительныеПараметры_));
	СтруктураТитула.Вставить("ПунктПогрузки", ПунктПогрузки(ДанныеДокумента, ДополнительныеПараметры_));
...
Функция ТитулПриемкаГруза(ДанныеДокумента)
	...
	ДополнительныеПараметры_ = ПунктПогрузкиВыгрузки_ДополнительныеПараметры(Истина, Истина);
	СтруктураТитула.Вставить("ПунктВыгрузки", ПунктВыгрузки(ДанныеДокумента, ДополнительныеПараметры_));
...
Функция ЛицоОтгрузки(ДанныеДокумента, ПодстановкиДокументов)
	...
	Если Тип = ТипГрузоотправитель Тогда
		Структура.Вставить("Грузоотправитель", МодульКода("ESD_ТНВыгрузкаСервер").ДанныеЮрЛица("Отправитель", ДанныеДокумента));
...
Функция ПунктПогрузки(РеквизитыОбъекта, ДополнительныеПараметры = Неопределено)
	Перем GLN;
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") Тогда
		ДополнительныеПараметры.Свойство("GLN", GLN);
	КонецЕсли;
	
	СтруктураПункта = ПунктПогрузкиВыгрузки_Шаблон(РеквизитыОбъекта.Погрузка_ДатаВремя);
	
	// ToDo: структуру из АдресПогрузкиЗначение 
	СтруктураПункта.Адрес.Фактический.Текст = РеквизитыОбъекта.Погрузка_Адрес;
	Если GLN = Истина Тогда
		СтруктураПункта.Адрес.Фактический.Вставить("GLN", РеквизитыОбъекта.Погрузка_Адрес_GLN);
	КонецЕсли;
...
Функция ПунктВыгрузки(РеквизитыОбъекта, ДополнительныеПараметры = Неопределено)
	Перем ДляПриемки, GLN;
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") Тогда
		ДополнительныеПараметры.Свойство("ДляПриемки", ДляПриемки);
		ДополнительныеПараметры.Свойство("GLN", GLN);
	КонецЕсли;

	Если ДляПриемки = Истина Тогда
		Адрес_ = "Выгрузка_Адрес";
		GLN_   = "Выгрузка_Адрес_GLN";
		Дата_  = "Выгрузка_ДатаВремя";
	Иначе
		Адрес_ = "Отправитель_АдресДоставки";
		GLN_   = "Отправитель_АдресДоставки_GLN";
		Дата_  = "Отправитель_ДоставитьДо";
	КонецЕсли;
	
	СтруктураПункта = ПунктПогрузкиВыгрузки_Шаблон(РеквизитыОбъекта[Дата_]);
	
	// падает на незаполненых составляющих фиас
	СтруктураПункта.Адрес.Фактический.АдресТекст = РеквизитыОбъекта[Адрес_]; 
	СтруктураПункта.Адрес.Фактический.Текст      = РеквизитыОбъекта[Адрес_];

	Если GLN = Истина Тогда
		СтруктураПункта.Адрес.Фактический.Вставить("GLN", РеквизитыОбъекта[GLN_]);
	КонецЕсли;
...
Функция ПунктПогрузкиВыгрузки_Шаблон(ДатаВремя)
	РезультатФункции = Новый Структура;
	
	СтруктураДатаВремя = МодульКода("ESD_ТНВыгрузкаСервер").СтруктураДатаВремя(ДатаВремя);
	РезультатФункции.Вставить("ДатаВремя", СтруктураДатаВремя);
	РезультатФункции.Вставить("Адрес",     МодульКода("ESD_ТНВыгрузкаСервер").ШаблонФактАдрес());
	
	Возврат РезультатФункции;
КонецФункции

Функция ПунктПогрузкиВыгрузки_ДополнительныеПараметры(ДляПриемки = Ложь, GLN = Ложь)
	РезультатФункции = Новый Структура("ДляПриемки, GLN", ДляПриемки, GLN);
	Возврат РезультатФункции;
КонецФункции
...
Процедура ЗагрузкаТитулПогрузка(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Если Не ДанныеПогрузка.Количество() Тогда 
		Возврат;
	КонецЕсли;
	...
	Если ДопПараметры.ЗагрузкаСОнлайна И Не ДопПараметры.ЭтоЧерновик Тогда
		МодульКода("РегистрыСведений.ESD_ДанныеТитулов").ЗаписатьДанные(
			МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ЗначениеВМассиве(ДанныеТитула), 
			ОбъектДок, 
...
Процедура ЗагрузкаТитулПереадресовка(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Если СтрокиПолучателя.Количество() > 0 Тогда
		...
		// При переадресовки очистим GLN предыдущего получателя
		ОбъектДок.Отправитель_АдресДоставки_GLN = "";
	КонецЕсли;
...
Процедура ЗагрузкаТитулУведомленияОУточнении(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	// Если пришло несколько вложений в "УведУточ", то берем последний в списке
	// Вложение "УведУточ" формируется на каждое вложение - XML и каждый приложенный файл (картинка, документ и пр.)
...
Функция ДанныеПогрузка(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Для Каждого Вложение Из ВложенияДляЗагрузки Цикл	
		// Установим порядковый номер титула
		Вложение.Вставить("ТипТитулаПорядок", 0);
		...
		ЗагрузитьИдентификаторыЭДОУчастников(ДляДокумента, ДопПараметры);
...
Функция ДанныеПолучениеГруза(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Для Каждого Вложение Из ВложенияДляЗагрузки Цикл
		// Установим порядковый номер титула
		Вложение.Вставить("ТипТитулаПорядок", 1);
...
Функция ДанныеПриемкаГруза(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	ВидОперации = ВидОперацииПоКодуПеревозки(Вложения, ДопПараметры);
			
	Для Каждого Вложение Из ВложенияДляЗагрузки Цикл
		// Установим порядковый номер титула
		Вложение.Вставить("ТипТитулаПорядок", 2);
		
		ДляДокумента = ПараметрыДляДокумента(ДопПараметры.КлючСтроки);
		
		SabyПриемкаГруза_Загрузка_ПунктВыгрузки(ДляДокумента, Вложение);	
		
		МодульТНОбщегоНазначенияСервер = МодульКода("ESD_ТНОбщегоНазначенияСервер");
		МодульТНОбщегоНазначенияКлиентСервер = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер");
			
		ДатаПрибытия = МодульТНОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(Вложение, 
			"Выгрузка.ДатаВремяПрибытия.Значение");
		ДляДокумента.Вставить("Выгрузка_ДатаВремяПрибыл",
			МодульТНОбщегоНазначенияСервер.ПреобразоватьСтрокуВДатуЭПД(ДатаПрибытия));
		
		ДатаУбытия = МодульТНОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(Вложение, 
			"Выгрузка.ДатаВремяУбытия.Значение");
		ДляДокумента.Вставить("Выгрузка_ДатаВремяУбыл",
		...
		Если ЗначениеЗаполнено(Масса) Тогда 
			...
			ДляДокумента.Вставить("Выгрузка_МассаБрутто", МодульТНОбщегоНазначенияКлиентСервер.ЧислоИзСтроки(Масса["Брутто"]));
			ДляДокумента.Вставить("Выгрузка_МассаНетто", МодульТНОбщегоНазначенияКлиентСервер.ЧислоИзСтроки(Масса["Нетто"]));
		...
		Если ЗначениеЗаполнено(ГрузВыгрузка) Тогда
			ДляДокумента.Вставить(
				"Выгрузка_КоличествоМест", МодульТНОбщегоНазначенияКлиентСервер.ЧислоИзСтроки(ГрузВыгрузка["КоличествоМест"]));
			ДляДокумента.Вставить("Выгрузка_Объем", МодульТНОбщегоНазначенияКлиентСервер.ЧислоИзСтроки(ГрузВыгрузка["Объем"]));
		КонецЕсли;
		...
		КлючПредставление = "Представление";
		Если ВидОперации.Частичная Тогда
			ДляДокумента.Вставить(КлючПредставление, "Получен частично");
			ОбъектДок.Комментарий = "Возможно, требуется переадресация";
		ИначеЕсли ВидОперации.Отменена Тогда
			ДляДокумента.Вставить(КлючПредставление, ПриемкаГруза_НеПринят());	
			ДляДокумента.КомментарийУведомление = Вложение["Комментарий"];	
		Иначе
			ДляДокумента.Удалить(КлючПредставление);
		КонецЕсли;
...
Функция ДанныеВыдачаГруза(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Для Каждого Вложение Из ВложенияДляЗагрузки Цикл
		// Установим порядковый номер титула
		Вложение.Вставить("ТипТитулаПорядок", 3);
		...
		Если ВидОперации.Частичная Тогда
			...
			ОбъектДок.Комментарий = "Возможно, требуется переадресация";
		ИначеЕсли ВидОперации.Отменена Тогда
			ДляДокумента.Вставить("Представление", ВыдачаГруза_НеВыдан());
			ДляДокумента.КомментарийУведомление = "Комментарий: " + Вложение["Комментарий"];	
		Иначе		
...
Функция ПараметрыДляДокумента(КлючСтроки)
	...
	ДляДокумента.Вставить("КомментарийУведомление", "");
...
Функция ДанныеПереадресовка(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Для Каждого Вложение Из ВложенияДляЗагрузки Цикл
		// Установим порядковый номер титула
		Вложение.Вставить("ТипТитулаПорядок", 4);
...
Функция ДанныеУведомлениеОПереадресовке(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Для Каждого Вложение Из ВложенияДляЗагрузки Цикл
		// Установим порядковый номер титула
		Вложение.Вставить("ТипТитулаПорядок", 8);
...
Функция ДанныеЗаменаВодителяТС(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Для Каждого Вложение Из ВложенияДляЗагрузки Цикл
		// Установим порядковый номер титула
		Вложение.Вставить("ТипТитулаПорядок", 5);
...
Функция ДанныеИзменениеСтоимости(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Для Каждого Вложение Из ВложенияДляЗагрузки Цикл
		// Установим порядковый номер титула
		Вложение.Вставить("ТипТитулаПорядок", 6);
...
Функция ДанныеСогласованиеСтоимости(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Для Каждого Вложение Из ВложенияДляЗагрузки Цикл
		// Установим порядковый номер титула
		Вложение.Вставить("ТипТитулаПорядок", 7);
...
Процедура ЗаполнитьПрошлыйТитулДляВложения(ОбъектДок, Вложение, Титул)
	...
	Период        = Вложение["ДатаВложенияТитула"];
	Идентификатор = Вложение["ИдентификаторВложенияТитула"];
...
Процедура ЗагрузитьДанныеПогрузки(ОбъектДок, ДляДокумента, Погрузка, ДопПараметры)
	...
	КоличествоМест = МодульТНОбщегоНазначенияКлиентСервер.ЧислоИзСтроки(КоличествоМест);
...
Процедура ДанныеПунктаПогрузки(ДанныеПогрузкиСОнлайна, СтруктураДляЗаполнения, ДопПараметры)
	...
	Если ЗначениеЗаполнено(АдресФактПогрузка) Тогда
		...
		Если ЗначениеЗаполнено(ДанныеАдресаПогрузка) Тогда
			...
			СтруктураДляЗаполнения.Вставить("Погрузка_Адрес_GLN",    ДанныеАдресаПогрузка.GLN);
		КонецЕсли;
...
Процедура ДанныеПунктаВыгрузки(ДанныеВыгрузкиСОнлайна, СтруктураДляЗаполнения)
	...
	Если ЗначениеЗаполнено(АдресФактВыгрузка) Тогда	
		...
		Если ЗначениеЗаполнено(ДанныеАдресаВыгрузка) Тогда
			...
			СтруктураДляЗаполнения.Вставить("Отправитель_АдресДоставки_GLN", ДанныеАдресаВыгрузка.GLN);
		КонецЕсли;
	КонецЕсли;
...
#Область SabyПриемкаГруза

Процедура SabyПриемкаГруза_Загрузка_ПунктВыгрузки(СтруктураЭПД, ДанныеSaby)
	// Определим используемые модули
	МодульТНОбщегоНазначенияКлиентСервер = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер");
	
	// Получим и проверим "ПунктВыгрузки"
	ПунктВыгрузки_ = МодульТНОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(ДанныеSaby, "ПунктВыгрузки");
	Если Не ЗначениеЗаполнено(ПунктВыгрузки_) Тогда
		Возврат;
	КонецЕсли;
	
	// Получим и обработаем "Дату выгрузки" 
	ДатаВремяВыгрузки = МодульТНОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(ПунктВыгрузки_, "ДатаВремя.Значение");
	Если ДатаВремяВыгрузки <> Неопределено Тогда
		ДатаВремяВыгрузки = МодульКода("ESD_ТНОбщегоНазначенияСервер").ПреобразоватьСтрокуВДатуЭПД(ДатаВремяВыгрузки);
		СтруктураЭПД.Вставить("Выгрузка_ДатаВремя", ДатаВремяВыгрузки);
	КонецЕсли;
	
	// Получим и обработаем "Адрес выгрузки"
	Адрес_ = МодульТНОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(ПунктВыгрузки_, "Адрес.Фактический");
	Если ЗначениеЗаполнено(Адрес_) Тогда	
		ДанныеАдресаВыгрузка = МодульКода("ESD_ТНЗагрузкаСервер").ПреобразоватьСтруктуруАдреса(Адрес_);
		Если ЗначениеЗаполнено(ДанныеАдресаВыгрузка) Тогда
			СтруктураЭПД.Вставить("Выгрузка_Адрес",          ДанныеАдресаВыгрузка.АдресСтрокой);
			СтруктураЭПД.Вставить("Выгрузка_Адрес_Значение", ДанныеАдресаВыгрузка.АдресСтруктурой);
			СтруктураЭПД.Вставить("Выгрузка_Адрес_GLN",      ДанныеАдресаВыгрузка.GLN);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

#КонецОбласти // SabyПриемкаГруза
...
Процедура ЗагрузитьУсловияПеревозки(ОбъектДок, ДляДокумента, УсловияПеревозки)
	...
	ДляДокумента.Вставить(
		...
		МодульТНОбщегоНазначенияКлиентСервер.ЧислоИзСтроки(ТемператураМинимальная));
	...
	ДляДокумента.Вставить(
		...
		МодульТНОбщегоНазначенияКлиентСервер.ЧислоИзСтроки(ТемператураМаксимальная));
	...
	ДляДокумента.Вставить(
		...
		МодульТНОбщегоНазначенияКлиентСервер.ЧислоИзСтроки(ВлажностьМинимальная));
	...
	ДляДокумента.Вставить(
		...
		МодульТНОбщегоНазначенияКлиентСервер.ЧислоИзСтроки(ВлажностьМаксимальная));
...
Функция ВидОперацииПоКодуПеревозки(Вложения, ДопПараметры)
	// имеется ли частиная приемка и выдача
	Результат = Новый Структура("Частичная,Отменена", Ложь, Ложь);
	
	// если приемок/выдач несколько то они частичные
	Если Вложения.Количество() > 1 Тогда
		Результат.Частичная = Истина;
		Возврат Результат;
	КонецЕсли;
	
	МодульТНОбщегоНазначенияКлиентСервер = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер");
	
	Если ТипЗнч(МодульТНОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(Вложения[0], "Operation.Rejected")) = Тип("Соответствие") Тогда
		Результат.Отменена = Истина;
		Возврат Результат;
	КонецЕсли;
	...
	ПолученЧастично = КодПеревозки_ПолученЧастично();
	ВыданЧастично   = КодПеревозки_ВыданЧастично();
	НеВыдан			= КодПеревозки_НеВыдан();
	НеПринят		= КодПеревозки_НеПринят();
	
	// определяем по кодам перевозки
	Для Каждого Элемент Из ДопПараметры.КодПеревозки Цикл
		Код = Элемент["Код"];
		Если Код = ПолученЧастично ИЛИ Код = ВыданЧастично Тогда
			Результат.Частичная = Истина;
			Прервать;
		КонецЕсли;
		Если Код = НеВыдан ИЛИ Код = НеПринят Тогда
			Результат.Отменена = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Функция КодПеревозки_ПолученЧастично()
	Возврат "5";	
КонецФункции

Функция КодПеревозки_ВыданЧастично()
	Возврат "8";
КонецФункции

Функция КодПеревозки_НеВыдан()
	Возврат "9";
КонецФункции

Функция КодПеревозки_НеПринят()
	Возврат "3";
КонецФункции

// Возвращает представление выбора "Не принят" этапа/титула "Приемка груза"
//
// Возвращаемое значение:
//   Строка - представление выбора "Не принят"
//
Функция ПриемкаГруза_НеПринят() Экспорт
	Возврат "Не принят";
КонецФункции

// Возвращает представление выбора "Не выдан" этапа/титула "Выдача груза"
//
// Возвращаемое значение:
//   Строка - представление выбора "Не выдан"
//
Функция ВыдачаГруза_НеВыдан() Экспорт
	Возврат "Не выдан";
КонецФункции
...
Процедура ЗаполнитьСоставителя(ОбъектДок, Вложение, ДляДокумента, ДопПараметры)
	...
	// Если присутствует "Основание" составителя, тогда "Отгрузчик" = "Оформитель"
	Если ЗначениеЗаполнено(Основание) Тогда 
...
Функция ДополнитьСписокИсключений(ОбъектДок, ДанныеТитула, СписокИсключений = "")
	...
	МассивИсключений.Добавить("Грузы");
...
Процедура ЗагрузитьИдентификаторыЭДОУчастников(ДляДокумента, ДопПараметры)
	ДанныеСторон_ = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ЗначениеИзКоллекции(ДопПараметры, "Стороны");
	
	СоответствиеРолей = Новый Соответствие;
	СоответствиеРолей.Вставить("Отправитель", ЗначениеМетаданных("ESD_РолиКонтрагентов.Отправитель"));
	СоответствиеРолей.Вставить("Получатель",  ЗначениеМетаданных("ESD_РолиКонтрагентов.Получатель"));
	СоответствиеРолей.Вставить("Перевозчик",  ЗначениеМетаданных("ESD_РолиКонтрагентов.Перевозчик"));
	
	МодульКода("ESD_ТНЗагрузкаСервер")
		.ДанныеЮрЛиц_ЗаполнитьИдентификаторыЭДО(ДляДокумента.ДанныеЮрЛиц, СоответствиеРолей, ДанныеСторон_);
КонецПроцедуры
