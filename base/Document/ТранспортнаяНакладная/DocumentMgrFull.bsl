...
Функция ПунктВыгрузки(РеквизитыОбъекта, ДополнительныеПараметры = Неопределено)
	...
	Если ДляПриемки = Истина Тогда
		СтруктураПункта = ПунктПогрузкиВыгрузки_Шаблон(РеквизитыОбъекта.Выгрузка_ДатаВремя);
	Иначе
		СтруктураПункта = ПунктПогрузкиВыгрузки_Шаблон(РеквизитыОбъекта.Отправитель_ДоставитьДо);
	КонецЕсли;
	...
	Если GLN = Истина Тогда
		СтруктураПункта.Адрес.Фактический.Вставить("GLN", РеквизитыОбъекта.Отправитель_АдресДоставки_GLN);
	КонецЕсли;
...
Процедура ЗагрузитьИдентификаторыЭДОУчастников(ДляДокумента, ДопПараметры)
	...
	Если ДопПараметры.Стороны = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого КлючЗначение Из ДопПараметры.Стороны Цикл
		
		РольЮрЛица = СоответствиеРолей[КлючЗначение.Ключ];
		Если РольЮрЛица = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ОтборПоРоли = Новый Структура("Роль", РольЮрЛица);
		
		НайденныеСтрокиЮрЛица = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").НайтиСтрокиУниверсально(
			ДляДокумента.ДанныеЮрЛиц, ОтборПоРоли);
		
		Для Каждого СтрокаЮрЛица Из НайденныеСтрокиЮрЛица Цикл
			СтрокаЮрЛица.ИдентификаторЭДО = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ЗначениеИзКоллекции(
				КлючЗначение.Значение, "Идентификатор", "");
		КонецЦикла;
