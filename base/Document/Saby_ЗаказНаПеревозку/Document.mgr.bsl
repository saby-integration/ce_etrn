...
Функция НестандартныеЗапросыТаблиц(Таблицы)
	...
	Для Каждого ИмяТаблицы Из Таблицы Цикл
		...
		Если ИмяТаблицы = "Грузы" Тогда
			...
			|	Saby_ЗаказНаПеревозкуГрузы.ОпасныйГруз.Наименование КАК ОпасныйГрузНаименование,
...
Функция Груз(ДанныеДокумента)
	...
	Для Каждого СтрокаГруза Из ДанныеДокумента.Грузы Цикл
		...
		Если ЗначениеЗаполнено(СтрокаГруза.ОпасныйГруз) Тогда
			...
			Saby_ТНВыгрузкаСервер.ЗаполнитьОпасныйГрузДляВыгрузки(ОпасныеГрузыИзДокумента, ОпасныйГруз, ДанныеДокумента.ЭтоВО);
...
Функция ТитулУтверждение(ДанныеДокумента)
	...
	СтруктураТитула.Вставить(
		...
		Saby_ТНВыгрузкаСервер.ПодстановкиТранспортногоСредства(ДанныеДокумента));
	
	СтруктураТитула.ТранспортноеСредство.Вставить(
		"Прицеп", Saby_ТНВыгрузкаСервер.ПодстановкиПрицепов(ДанныеДокумента));
...
Функция ДанныеУтверждение(ОбъектДок, Вложения, Титул, ДопПараметры)
	...
	Для Каждого Вложение Из ВложенияДляЗагрузки Цикл
		...
		Saby_ТНЗагрузкаСервер.ЗагрузитьОтветственного(
			...
			Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Вложение, "Грузоперевозчик.Ответственный"));
		...
		УполномоченноеЛицо = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
...
Процедура ЗагрузитьМаршрут(ДляДокумента, Маршрут)
	...
	ДляДокумента.Вставить(
		"КонечнаяТочкаАдрес",
		Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Маршрут, "КонечныйПункт.Название"));
	
	// Отправление
	Отправление = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Маршрут, "Отправление");
	
	ДляДокумента.Вставить(
		"ПодачаАдрес",
		Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Отправление, "АдресТекст"));
	
	// Подача ТС
	ПодачаДата = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Отправление, "ПодачаТС.ДатаВремя");
	...
	Интервал = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Отправление, "ПодачаТС.ПредельныйИнтервалВремени");
...
Процедура ЗагрузитьПунктыВыгрузки(ДляДокумента, Маршрут)
	...
	Для Каждого Пункт Из Пункты Цикл
		...
		// Операция
		ДатаВремяСтрокой = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Пункт, "Операция.ДатаВремя");		
		СтруктураПункта.Вставить(
			"ДатаВремя",
			Saby_ТНОбщегоНазначенияСервер.ПреобразоватьСтрокуВДатуЭПД(ДатаВремяСтрокой));
		
		ИнтервалСтрокой = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Пункт, "Операция.ПредельныйИнтервалВремени");		
		СтруктураПункта.Вставить(
			"ДопустимоеВремя", 
			Saby_ТНОбщегоНазначенияСервер.ПреобразоватьСтрокуВоВремя(ИнтервалСтрокой));
		 			
		Тип = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Пункт, "Операция.Тип");
		СтруктураПункта.Вставить("ЭтоПогрузка", Тип = "Погрузка");
		
		Если СтруктураПункта.ЭтоПогрузка Тогда
			СтруктураПункта.Вставить(
				"ОрганизацияСтрока", 
				Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Пункт, "Организация.Название"));
			
			СтруктураПункта.Вставить(
				"ОрганизацияИНН",
				Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Пункт, "Организация.ИНН"));
...
Процедура ЗагрузитьУсловияПеревозки(ДляДокумента, УсловияПеревозки)
	...
	// УсловияПеревозки
	СпецУсловия    = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(УсловияПеревозки, "УказанияНорм");
	ПредельныйСрок = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(УсловияПеревозки, "ПредельныйСрок");
	ИныеУсловия    = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(УсловияПеревозки, "Иные");
	ТребованияКТС  = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(УсловияПеревозки, 
	...
	ВлажностьОт   = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(КлиматическийРежим, "Влажность.Минимальная");
	ВлажностьДо   = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(КлиматическийРежим, "Влажность.Максимальная");
	ТемператураОт = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(КлиматическийРежим, "Температура.Минимальная");
	ТемператураДо = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(КлиматическийРежим, "Температура.Максимальная");	
...
Процедура ЗагрузитьПараметрыТС(ДляДокумента, ПараметрыТС)
	...
	ТипТС      = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
					ПараметрыТС, "Тип");
				
	КузовТС    = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
					ПараметрыТС, "ТипКузова");
				
	ПогрузкаТС = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
	...
	ГрузоподъемностьТС = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
							ПараметрыТС, "Грузоподъемность");
	ВместимостьТС      = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
...
Процедура ЗагрузитьДанныеДокументовЭПД(ДляДокумента, ДанныеДокументов)
	...
	Договор = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(ДанныеДокументов, "Договор");
	...
	СоставительОснование = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(ДанныеДокументов, "Файл.Составитель.Основание");
...
Процедура ЗаполнитьДанныеЮрЛиц(ДляДокумента, Вложение)
	...
	УполномоченныеЛица = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
	...
	// Составитель
	Основание = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Вложение, "Файл.Составитель.Основание.Документ");
	Если ЗначениеЗаполнено(Основание) Тогда 
	
		Составитель = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Вложение, "Файл.Составитель.Наименование");
...
Процедура ЗагрузитьДанныеПоСтоимости(ДляДокумента, СтоимостьПеревозки)
	...
	ДляДокумента.Вставить(
		"ВалютаКод",          Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(СтоимостьПеревозки, "Валюта.Код"));
	ДляДокумента.Вставить(
		"ВалютаНаименование", Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(СтоимостьПеревозки, "Валюта.Наименование"));
	
	СтавкаНДС = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(СтоимостьПеревозки, "Налог.Ставка");
...
Процедура ЗагрузитьКомментарийОтрицательногоЗавершения(ДляДокумента, Вложение)
	...
	ОтветПеревозчика = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Вложение, "Документ.Ответ.Значение");
	ОтказаноВПриемке = ОтветПеревозчика = "2";
	
	Если ОтказаноВПриемке Тогда
		ДляДокумента.Вставить(
			"КомментарийУведомление",
			Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Вложение, "Документ.Ответ.Причина"));
	КонецЕсли;
