	...
	Если ЭтоНоваяГруппа Тогда
		...
		Если Параметры.Свойство("Родитель")
			...
			ИначеЕсли Параметры.Родитель.Родитель <> Неопределено
				...
				// В качестве первого родителя была передана ссылка на элемент.
				ОбъектЗначение.Родитель = Параметры.Родитель.Родитель;
			КонецЕсли;
		...
		ОбъектЗначение.Автор         = Saby_ТНОбщегоНазначенияСервер.АвторизованныйПользователь();
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		Отказ = Истина;
		ОповещениеОбОтвете = Новый ОписаниеОповещения("ЗакрытьФормуПослеОтветаНаВопрос", ЭтаФорма);
...
Процедура НастроитьОбъектФормы(Знач НовыйОбъект)
	...
	СтатусСоздана = СтроковыеФункции.ФорматированнаяСтрока(НСтр("ru = '<a href=""%1"">%2</a>';
																|en = '<a href=""%1"">%2</a>'"),
		ПолучитьНавигационнуюСсылку(НовыйОбъект.Автор), Строка(НовыйОбъект.Автор));
...
Функция ОбработатьКомандуЗаписиФайла()
	...
	Оповестить("Запись_Файл", Новый Структура("ЭтоНовый", ФайлБылСоздан), ЭтаФорма.Объект.Ссылка);
...
Функция ЗаписатьФайл(Знач ПараметрОбъект = Неопределено)
	...
	НачатьТранзакцию();
	Попытка
		ЗаписываемыйОбъект.Изменил                      = Saby_ТНОбщегоНазначенияСервер.АвторизованныйПользователь();
...
Процедура ОбновитьИнформациюОбИзменении()
	...
	СтатусИзменена = СтроковыеФункции.ФорматированнаяСтрока(НСтр("ru = '<a href=""%1"">%2</a>';
																|en = '<a href=""%1"">%2</a>'"),
		ПолучитьНавигационнуюСсылку(ЭтаФорма["Объект"].Изменил), Строка(ЭтаФорма["Объект"].Изменил));
