...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.РежимВыбора Тогда
		СтандартныеПодсистемыСервер.УстановитьКлючНазначенияФормы(ЭтаФорма, "ВыборПодбор");
	...
	ПравоДляПроверки = "Чтение";
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда 
		МодульУправлениеДоступом = ОбщегоНазначения.ОбщийМодуль("УправлениеДоступом");
		ЕстьПравоИспользоватьШаблоны = МодульУправлениеДоступом.ЕстьПраво(ПравоДляПроверки, Справочники.ПапкиФайлов.Шаблоны);
	Иначе
		ЕстьПравоИспользоватьШаблоны = ПравоДоступа(ПравоДляПроверки, Метаданные.Справочники.Файлы)
			И ПравоДоступа(ПравоДляПроверки, Метаданные.Справочники.ПапкиФайлов);
	КонецЕсли;
	
	МетаданныеДляПоиска = Метаданные.НайтиПоТипу(ТипЗнч(ВладелецФайла));
	Если НЕ ЕстьПравоИспользоватьШаблоны
		Или НастройкиФайлов.НеСоздаватьФайлыПоШаблону.Найти(МетаданныеДляПоиска) <> Неопределено Тогда
	...
	Если ТипЗнч(Saby_ТНОбщегоНазначенияСервер.АвторизованныйПользователь()) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
		РаботаСФайламиСлужебный.ИзменитьФормуДляВнешнегоПользователя(ЭтаФорма, Истина);
	КонецЕсли;
	...
	ПравоДляПроверки = "Редактирование";
	Если Пользователи.ЭтоСеансВнешнегоПользователя() Тогда
		ТолькоПросмотр = Не ПравоДоступа(ПравоДляПроверки, МетаданныеСправочникаСФайлами);
	Иначе
		ТолькоПросмотр = Не ПравоДоступа(ПравоДляПроверки, МетаданныеСправочникаСФайлами)
			Или Не ПравоДоступа(ПравоДляПроверки, Параметры.ВладелецФайла.Метаданные());
	КонецЕсли;
	...
	Элементы.НастроитьСинхронизацию.Видимость = ПравоДоступа(
		"Редактирование", Метаданные.Справочники.УчетныеЗаписиСинхронизацииФайлов);
	...
	РаботаСФайламиПереопределяемый.ПриСозданииФормыСпискаФайлов(ЭтаФорма);
...
&НаКлиенте
Процедура ДекорацияДатаСинхронизацииОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки,
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	// удаление временного файла происходит после закрытия файла, обработчиком оповещения		
...
Процедура СписокПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если ТипЗнч(ПараметрыПеретаскивания.Значение) = Тип("Массив")
		...
		Если ТипЗнч(ПеретаскиваемыйФайл) <> ТипФайл Тогда
		...
		Для Каждого ПеретаскиваемыйФайл Из ПараметрыПеретаскивания.Значение Цикл
			...
			Если ТипЗнч(ПеретаскиваемыйФайл) = ТипФайл
	...
	ИначеЕсли ТипЗнч(ПараметрыПеретаскивания.Значение) = ТипФайл
...
Процедура УстановитьПометкуУдаления(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ТекстВопроса = Saby_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонВопроса, ТекущиеДанные.Наименование);
		ДополнительныеПараметры = Новый Структура("ФайлСсылка", ТекущиеДанные.Ссылка);
		Оповещение = Новый ОписаниеОповещения("ПослеВопросОПометкиНаУдаление", ЭтаФорма, ДополнительныеПараметры);
...
Процедура Печать(Команда)
	...
	Если ВыделенныеСтроки.Количество() > 0 Тогда
		РаботаСФайламиКлиент.НапечататьФайлы(ВыделенныеСтроки, ЭтаФорма.УникальныйИдентификатор);
	КонецЕсли;
...
Процедура ПечатьСоШтампом(Команда)
	...
	ДокументСоШтампом = РаботаСФайламиСлужебныйВызовСервера.ТабличныйДокументСоШтампом(
		ТекущиеДанные.Ссылка, ТекущиеДанные.Ссылка);
...
Процедура НастроитьСинхронизацию(Команда)
	...
	ОткрытьФорму(
		"РегистрСведений.НастройкиСинхронизацииФайлов.Форма.ПростаяФормаЗаписиНастройки", ПараметрыЗаписи, ЭтаФорма);
...
Процедура СоздатьГруппу(Команда)
	...
	ОткрытьФорму("Обработка.Saby_РаботаСФайлами.Форма.ГруппаФайлов", ПараметрыФормы);
...
Процедура СравнитьФайлы(Команда)
	...
	ДвеСтроки = 2;
	Если ЧислоВыделенныхСтрок = ДвеСтроки Тогда
...
Процедура ПеренестиВГруппу(Команда)
	Если Элементы.Список.ВыделенныеСтроки.Количество() > 0 Тогда
		...
		ОткрытьФорму("Обработка.РаботаСФайлами.Форма.ВыборГруппы", ПараметрыОткрытия, ЭтаФорма);
	КонецЕсли;
КонецПроцедуры
...
Процедура Подписать(Команда)
	...
	ОписаниеОповещения      = Новый ОписаниеОповещения("ДобавлениеПодписейЗавершить", ЭтаФорма);
...
Процедура ДобавитьЭПИзФайла(Команда)
	...
	РаботаСФайламиСлужебныйКлиент.ДобавитьПодписьИзФайла(
		...
		Новый ОписаниеОповещения("ДобавлениеПодписейЗавершить", ЭтаФорма));
...
Процедура Зашифровать(Команда)
	...
	Обработчик = Новый ОписаниеОповещения("ЗашифроватьПослеШифрованияНаКлиенте", ЭтаФорма, ПараметрыОбработчика);
...
Процедура Расшифровать(Команда)
	...
	Обработчик = Новый ОписаниеОповещения("РасшифроватьПослеРасшифровкиНаКлиенте", ЭтаФорма, ПараметрыОбработчика);
...
Процедура ЗакончитьРедактирование(Команда)
	...
	Если МассивФайлов.Количество() > 1 Тогда
		...
		ОткрытьФорму("Обработка.РаботаСФайлами.Форма.ФормаЗавершениеРедактирования", ПараметрыФормы, ЭтаФорма);
	ИначеЕсли МассивФайлов.Количество() = 1 Тогда 
		Обработчик = Новый ОписаниеОповещения("УстановитьДоступностьФайловыхКоманд", ЭтаФорма);
...
Процедура Занять(Команда)
	...
	ИначеЕсли КоличествоФайлов > 1 Тогда
		...
		Обработчик = Новый ОписаниеОповещения("УстановитьДоступностьФайловыхКоманд", ЭтаФорма, МассивФайлов);
...
Процедура Удалить(Команда)
	...
	РаботаСФайламиСлужебныйКлиент.УдалитьДанные(
		Новый ОписаниеОповещения("ПослеУдаленияДанных", ЭтаФорма),
...
Процедура УстановитьУсловноеОформление(Знач ЕстьРеквизитСлужебный = Ложь)
	...
	Если ЕстьРеквизитСлужебный Тогда
		...
		ОтборЭлемента = ГруппаОтбора.Элементы.Добавить(ТипЭлементОтбораКомпоновкиДанных);
	...
	УстановитьЦветТекста(Элемент.Оформление, ЦветаСтиля.ТекстЗапрещеннойЯчейкиЦвет);
	...
	УстановитьЦветТекста(Элемент.Оформление, ЦветаСтиля.ФайлЗанятыйТекущимПользователем);
	...
	Отбор = Элемент.Отбор.Элементы.Добавить(ТипЭлементОтбораКомпоновкиДанных);
...
&НаСервере
Процедура УстановитьЦветТекста(Оформление, Цвет)
	
	Оформление.УстановитьЗначениеПараметра("ЦветТекста", Цвет);
...
Процедура ДобавитьФайл()
	...
	ТекущиеДанные = Элементы.Список.ТекущиеДанные;
	ГруппаФайлов = Неопределено;
	Если ТекущиеДанные <> Неопределено И ТекущиеДанные.ЭтоГруппа
		И ТекущиеДанные.ВладелецФайла = Параметры.ВладелецФайла Тогда
		ГруппаФайлов = ТекущиеДанные.Ссылка;
	КонецЕсли;
	Если ЭтоВладелецЭлементовСправочникаФайлы Тогда
		РаботаСФайламиСлужебныйКлиент.ДобавитьФайлИзФайловойСистемы(Параметры.ВладелецФайла, ЭтаФорма);
	Иначе
...
Процедура ОткрытьФайл()
	...
	Если ЗапрещенныеРасширения.НайтиПоЗначению(ТекущиеДанные.Расширение) <> Неопределено Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ОткрытьФайлПослеПодтверждения", ЭтаФорма, ДополнительныеПараметры);
...
Процедура ОткрытьКарточкуФайла()
	...
	ПараметрыФормы.Вставить("ВладелецФайла",     Параметры.ВладелецФайла);
	...
	Иначе 
		...
		ОткрытьФорму(
			"Обработка.Saby_РаботаСФайлами.Форма.ПрисоединенныйФайл", 
			ПараметрыФормы, , 
...
Процедура СписокВыборПослеВыбораРежимаРедактирования(Результат, ПараметрыВыполнения) Экспорт
	...
	Если Результат = "Редактировать" Тогда
		Обработчик = Новый ОписаниеОповещения("СписокВыборПослеРедактированияФайла", ЭтаФорма, ПараметрыВыполнения);
...
Процедура НастроитьДинамическийСписок(ИмяСправочникаХранилищаФайлов, Знач ЕстьРеквизитСлужебный = Ложь)
	...
	Список.Параметры.УстановитьЗначениеПараметра("ТекущийПользователь", Saby_ТНОбщегоНазначенияСервер.АвторизованныйПользователь());
...
&НаКлиентеНаСервереБезКонтекста
Функция ПолучитьДоступныеКоманды(ТекущиеДанныеФайла, ФайлыРедактируютсяВОблачномСервисе,
		ВозможностьОсвободитьФайл, АвторизованныйПользователь)
	...
	Если ФайлРедактируется Тогда
		...
		Если ФайлРедактируетТекущийПользователь Тогда
			УдалитьКомандыИзМассива(ИменаКоманд, "ОбновитьИзФайлаНаДиске");
		Иначе
			УдалитьКомандыИзМассива(ИменаКоманд, "ЗакончитьРедактирование");
			Если Не ВозможностьОсвободитьФайл Тогда
				УдалитьКомандыИзМассива(ИменаКоманд, "Освободить");
			КонецЕсли;
			УдалитьКомандыИзМассива(ИменаКоманд, "Редактировать");
		КонецЕсли;
		
		УдалитьКомандыЭП(ИменаКоманд);
		
		УдалитьКомандыИзМассива(
			ИменаКоманд,
			"Занять,УстановитьПометкуУдаления,КонтекстноеМенюУстановитьПометкуУдаления,
			|ОбновитьИзФайлаНаДиске,СохранитьКак,
			|Зашифровать,Расшифровать,
			|Удалить");
		
	Иначе
		УдалитьКомандыИзМассива(
			ИменаКоманд, "ЗакончитьРедактирование,Освободить");
	КонецЕсли;
	
	Если ТекущиеДанныеФайла.ЭтоГруппа Тогда
		УдалитьКомандыИзМассива(
			ИменаКоманд,
			"Редактировать,Подписать,ДобавитьЭПИзФайла,СохранитьВместеСЭП,Зашифровать,Расшифровать,
			|ОбновитьИзФайлаНаДиске,Скопировать,ОткрытьКаталогФайла,ОткрытьФайлДляПросмотра,СохранитьКак,
			|Занять,Отправить,ПечатьСоШтампом,Печать,Удалить");
	КонецЕсли;
	
	Если ФайлПодписан Тогда
		УдалитьКомандыИзМассива(
			ИменаКоманд,
			"ЗакончитьРедактирование,Освободить,Редактировать,ОбновитьИзФайлаНаДиске,Занять");
	КонецЕсли;
	...
	Если ФайлЗашифрован Тогда
		...
		УдалитьКомандыИзМассива(
			ИменаКоманд,
			"ЗакончитьРедактирование,Освободить,Редактировать,Занять,
			|ОбновитьИзФайлаНаДиске,
			|Зашифровать,
			|ОткрытьКаталогФайла,ОткрытьФайлДляПросмотра,СохранитьКак");
	Иначе
		УдалитьКомандыИзМассива(ИменаКоманд, "Расшифровать");
	КонецЕсли;
	...
	Если ФайлыРедактируютсяВОблачномСервисе Тогда
		...
		УдалитьКомандыИзМассива(
			ИменаКоманд,
			"Добавить,ДобавитьИзФайлаНаДиске,ДобавитьФайлПоШаблону,ДобавитьФайлСоСканера,Скопировать,
			|СоздатьГруппу,ПереместитьВГруппу,УстановитьПометкуУдаления,КонтекстноеМенюУстановитьПометкуУдаления,Занять,
			|ЗагрузитьФайлы,ЗагрузитьПапку,
			|Удалить");
		
	КонецЕсли;
	
	Если Не АвторТекущийПользователь Тогда
		УдалитьКомандыИзМассива(ИменаКоманд, "Удалить");
	КонецЕсли;
...
Процедура УдалитьКомандыЭП(ИменаКоманд)
	...
	УдалитьКомандыИзМассива(ИменаКоманд, "Подписать,ДобавитьЭПИзФайла,СохранитьВместеСЭП");
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура УдалитьКомандыИзМассива(Массив, ИменаКоманд)
	
	МассивКомандДляУдаления = СтрРазделить(ИменаКоманд, ",");
	
	Для Каждого ИмяКоманды Из МассивКомандДляУдаления Цикл
		ИмяКоманды = СокрЛП(ИмяКоманды);
...
Процедура ПриИзмененииИспользованияПодписанияИлиШифрованияНаСервере()
	...
	РаботаСФайламиСлужебный.КриптографияПриСозданииФормыНаСервере(ЭтаФорма);
...
Процедура ПриОтправкеФайловПочтой(ПараметрыОтправки, Знач ФайлыДляОтправки, ВладелецФайлов, УникальныйИдентификатор)
	РаботаСФайламиПереопределяемый.ПриОтправкеФайловПочтой(
		ПараметрыОтправки, ФайлыДляОтправки, ВладелецФайлов, УникальныйИдентификатор);
КонецПроцедуры
...
Процедура ОбновитьПредпросмотр()
	...
	Если ТекущиеДанные <> Неопределено
		...
		Исключение
			...
			ТекстНевыбраннойКартинки = ТекстНевыбраннойКартинки + Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		КонецПопытки;
...
Процедура ОбновитьПояснениеОблачногоСервиса()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюФайлов") Тогда
		...
		Если СведенияОСинхронизации.Количество() > 0  Тогда
			...
			Элементы.ДекорацияДатаСинхронизации.ОтображениеПодсказки = 
				?(СведенияОСинхронизации.Синхронизирован, ОтображениеПодсказки.Нет, ОтображениеПодсказки.Кнопка);
			...
			Элементы.ДекорацияДатаСинхронизации.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
				НСтр("ru = 'Синхронизировано: <a href=""OpenJournal"">%1</a>';
					|en = 'Synchronized on: <a href=""OpenJournal"">%1</a>'"),
