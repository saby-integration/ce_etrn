...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекущийПользователь = Saby_ТНОбщегоНазначенияСервер.ТекущийПользователь();
	...
	РаботаСФайламиСлужебный.ФормаЭлементаПриСозданииНаСервере(
		ЭтаФорма, Отказ, СтандартнаяОбработка, Параметры, ТолькоПросмотр, Истина);
	
	Элементы.ВладелецФайла0.Заголовок = ТипВладельца;
	
	УстановитьДоступностьКнопок(ЭтаФорма, Элементы);
	...
	ИнтеграцияПодсистемБСП.ПриСозданииФормыЭлементаФайлов(ЭтаФорма);
	РаботаСФайламиПереопределяемый.ПриСозданииФормыЭлементаФайлов(ЭтаФорма);
...
Процедура ПриОткрытии(Отказ)
	...
	РаботаСФайламиСлужебныйКлиент.ПрочитатьСертификатыПодписей(ЭтаФорма);
...
Процедура ОбновитьИзФайлаНаДиске(Команда)
	...
	Если ЭтоНовый()
		...
		Или ЭтаФорма.Объект.ПодписанЭП
		Или ЗначениеЗаполнено(ЭтаФорма.Объект.Редактирует) Тогда
		
		Возврат;
	КонецЕсли;
	
	ДанныеФайла = ДанныеФайла(ТекущаяСсылкаНаФайл(), УникальныйИдентификатор, "ВызовСервера");
	Обработчик = Новый ОписаниеОповещения("ОбновитьИзФайлаНаДискеЗавершение", ЭтаФорма);
...
Процедура СтандартныеУстановитьПометкуУдаления(Команда)
	...
	ТекстВопроса = Saby_ТНОбщегоНазначенияКлиентСервер.СтрШаблонЭПД(ТекстВопроса, ЭтаФорма.Объект.Ссылка);
		
	ОписаниеОповещения = Новый ОписаниеОповещения("СтандартныеУстановитьПометкуУдаленияОтветПолучен", ЭтаФорма);
...
Процедура СтандартнаяПеречитать(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("СтандартнаяПеречитатьОтветПолучен", ЭтаФорма);
	ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
...
Процедура Печать(Команда)
	...
	Если ЗначениеЗаполнено(ТекущаяСсылкаНаФайл()) Или ОбработатьКомандуЗаписиФайла() Тогда
		Файлы = Saby_ТНОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТекущаяСсылкаНаФайл());
		РаботаСФайламиКлиент.НапечататьФайлы(Файлы, ЭтаФорма.УникальныйИдентификатор);
	КонецЕсли;
...
Процедура Отправить(Команда)
	...
	Если ЗначениеЗаполнено(СсылкаНаТекущийОбъект)
		Или ОбработатьКомандуЗаписиФайла() Тогда
		Файлы = Saby_ТНОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(СсылкаНаТекущийОбъект);
...
Процедура Подключаемый_СвойстваВыполнитьКоманду(ЭлементИлиКоманда, НавигационнаяСсылка = Неопределено,
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.Свойства") Тогда
		...
		МодульУправлениеСвойствамиКлиент.ВыполнитьКоманду(ЭтаФорма, ЭлементИлиКоманда, СтандартнаяОбработка);
	КонецЕсли;
...
Процедура Подписать(Команда)
	...
	ОписаниеОповещения      = Новый ОписаниеОповещения("ПриПолученииПодписи", ЭтаФорма);
...
Процедура ДобавитьЭПИзФайла(Команда)
	...
	РаботаСФайламиСлужебныйКлиент.ДобавитьПодписьИзФайла(
		...
		Новый ОписаниеОповещения("ПриПолученииПодписей", ЭтаФорма));
...
Процедура Зашифровать(Команда)
	...
	Если ЭтоНовый() Или ЗначениеЗаполнено(ЭтаФорма.Объект.Редактирует) Или ЭтаФорма.Объект.Зашифрован Тогда
		Возврат;
	КонецЕсли;
...
Процедура Расшифровать(Команда)
	...
	Если ЭтоНовый() Или Не ЭтаФорма.Объект.Зашифрован Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСписокШифрованияНаСервере()
	РаботаСФайламиСлужебный.ЗаполнитьСписокШифрования(ЭтаФорма);
КонецПроцедуры
...
Процедура ПроверитьВсе(Команда)
	...
	ДанныеФайла = ДанныеФайла(ТекущаяСсылкаНаФайл(), УникальныйИдентификатор);
	РаботаСФайламиСлужебныйКлиент.ПроверитьПодписи(ЭтаФорма, ДанныеФайла.СсылкаНаДвоичныеДанныеФайла);
...
Процедура УдалитьЭП(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("УдалитьЭПОтветПолучен", ЭтаФорма);
...
Процедура УстановитьДоступностьКомандСпискаЭП()
	...
	РаботаСФайламиСлужебныйКлиент.УстановитьДоступностьКомандСпискаЭлектронныхПодписей(ЭтаФорма);
...
Процедура УстановитьДоступностьКомандСпискаШифрования()
	...
	РаботаСФайламиСлужебныйКлиент.УстановитьДоступностьКомандСпискаСертификатовШифрования(ЭтаФорма);
...
Процедура Занять(Команда)
	...
	Обработчик = Новый ОписаниеОповещения("ПрочитатьИУстановитьДоступностьЭлементовФормы", ЭтаФорма);
...
Процедура ЗакончитьРедактирование(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗакончитьРедактированиеВыполненоПомещение", ЭтаФорма);
...
Процедура Освободить(Команда)
	...
	Если ЭтоНовый()
		Или Не ЗначениеЗаполнено(ЭтаФорма.Объект.Редактирует)
		Или ЭтаФорма.Объект.Редактирует <> ТекущийПользователь И НЕ ИзменениеФайлов Тогда
...
Процедура СвойстваВыполнитьОтложеннуюИнициализацию()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Свойства") Тогда
		...
		МодульУправлениеСвойствами.ЗаполнитьДополнительныеРеквизитыВФорме(ЭтаФорма);
	КонецЕсли;
...
Процедура ОбновитьЗависимостиДополнительныхРеквизитов()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.Свойства") Тогда
		МодульУправлениеСвойствамиКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("УправлениеСвойствамиКлиент");
		МодульУправлениеСвойствамиКлиент.ОбновитьЗависимостиДополнительныхРеквизитов(ЭтаФорма);
	КонецЕсли;
...
Процедура УстановитьВидимостьКомандФормы()
	...
	Элементы.ФормаУдалить.Видимость = ЭтаФорма.Объект.Автор = ТекущийПользователь
...
Процедура ОбновитьЗаголовок()
	...
	Если ЗначениеЗаполнено(СсылкаНаТекущийОбъект) Тогда
		Заголовок = Saby_ТНОбщегоНазначенияКлиентСервер.СтрШаблонЭПД(
...
Процедура ОтображатьВкладкиДополнительныхДанных()
	...
	ИспользоватьВкладки = ВидимостьДополнительныхРеквизитов
		Или Элементы.ГруппаЭлектронныеПодписи.Видимость
		Или Элементы.ГруппаСертификатыШифрования.Видимость;
...
Процедура ОткрытьФайлДляПросмотра()
	...
	Если ЗапрещенныеРасширения.НайтиПоЗначению(ЭтаФорма.Объект.Расширение) <> Неопределено Тогда
		Оповещение = Новый ОписаниеОповещения("ОткрытьФайлПослеПодтверждения", ЭтаФорма);
...
Процедура СохранитьКак()
	...
	Если ЭтоНовый() Или ЭтаФорма.Объект.Зашифрован Тогда
		Возврат;
	КонецЕсли;
...
Процедура УдалитьИзСпискаПодписейИЗаписатьФайл()
	...
	РаботаСФайламиСлужебный.ЗаполнитьСписокПодписей(ЭтаФорма);
	УстановитьДоступностьКнопок(ЭтаФорма, Элементы);
...
Процедура УстановитьДоступностьКнопок(Форма, Элементы)
	...
	Если Форма.ЭлектронныеПодписи.Количество() = 0 Тогда
		СделатьКомандыНедоступными(ИменаКоманд, "ОткрытьПодпись");
	КонецЕсли;
...
Функция ИменаКомандИзмененияДанныхФайла()
	...
	ИменаКоманд.Вставить("ЗакончитьРедактирование", ЗакончитьРедактированиеМассив());
...
&НаКлиентеНаСервереБезКонтекста
Функция ЗакончитьРедактированиеМассив()
	
	РезультатФункции = Новый Массив;
	РезультатФункции.Добавить("ФормаЗакончитьРедактирование");
	РезультатФункции.Добавить("ЗакончитьРедактирование");
...
Функция ДоступныеКомандыФормы(Форма)
	...
	Если ФайлРедактируется Тогда
		Если ФайлРедактируетТекущийПользователь Тогда
			СделатьКомандыНедоступными(ИменаКоманд, "ОбновитьИзФайлаНаДиске");
		Иначе
			СделатьКомандыНедоступными(
				ИменаКоманд,
				"ЗакончитьРедактирование");
			СделатьКомандыНедоступными(ИменаКоманд, "Редактировать");
			СделатьНедоступнымОсвободить(Не Форма.ИзменениеФайлов, ИменаКоманд);
		КонецЕсли;
		СделатьКомандыЭПНедоступными(ИменаКоманд);
		
		СделатьКомандыНедоступными(
			ИменаКоманд,
			"Занять,ОбновитьИзФайлаНаДиске,СохранитьКак,Зашифровать,Расшифровать");
	Иначе
		СделатьКомандыНедоступными(
			ИменаКоманд,
			"ЗакончитьРедактирование,СохранитьИзменения");
		СделатьНедоступнымОсвободить(Не Форма.ИзменениеФайлов, ИменаКоманд);
	КонецЕсли;
	...
	Если ФайлПодписан Тогда
		...
		СделатьКомандыНедоступными(
			ИменаКоманд,
			"ЗакончитьРедактирование,Редактировать,ОбновитьИзФайлаНаДиске");
	Иначе
		СделатьКомандыНедоступными(
			ИменаКоманд,
			"ОткрытьСертификат,ОткрытьПодпись,ПроверитьЭП,ПроверитьВсе,СохранитьПодпись,УдалитьЭП,СохранитьВместеСЭП");
	КонецЕсли;
	...
	Если ФайлЗашифрован Тогда
		...
		СделатьКомандыНедоступными(
			ИменаКоманд,
			"ЗакончитьРедактирование,Редактировать,ОбновитьИзФайлаНаДиске,Зашифровать,
			|ОткрытьКаталогФайла,ОткрытьФайлДляПросмотра,СохранитьКак,
			|Подписать");
		
	Иначе
		СделатьКомандыНедоступными(ИменаКоманд, "Расшифровать");
	КонецЕсли;
	
	Если ФайлРедактируетсяВОблаке Тогда
		СделатьКомандыНедоступными(
			ИменаКоманд,
			"СтандартныеСкопировать,СтандартныеУстановитьПометкуУдаления,
			|СтандартнаяЗаписать,СтандартнаяЗаписатьИЗакрыть,СохранитьИзменения");
...
&НаКлиентеНаСервереБезКонтекста
Процедура СделатьНедоступнымОсвободить(СделатьНедоступной, МассивКоманд)
	
	Если СделатьНедоступной Тогда
		СделатьКомандыНедоступными(МассивКоманд, "Освободить");
	КонецЕсли;
...
Процедура СделатьКомандыЭПНедоступными(Знач ИменаКоманд)
	...
	СделатьКомандыНедоступными(
		ИменаКоманд,
		"Подписать,ДобавитьЭПИзФайла,СохранитьВместеСЭП");
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура СделатьКомандыНедоступными(ИменаКоманд, ИменаНедоступныхКоманд)
	
	МассивИменКоманд = СтрРазделить(ИменаНедоступныхКоманд, ",");
...
Функция ОбработатьКомандуЗаписиФайла()
	...
	Попытка
		РаботаСФайламиСлужебныйКлиент.КорректноеИмяФайла(ЭтаФорма.Объект.Наименование);
	Исключение
		ОбщегоНазначенияКлиент.СообщитьПользователю(
	...
	ОповеститьОбИзменении(ЭтаФорма.Объект.ВладелецФайла);
	
	Оповестить("Запись_Файл", Новый Структура("ЭтоНовый, Событие", ФайлБылСоздан, "Запись"), ЭтаФорма.Объект.Ссылка);
	
	УстановитьДоступностьКомандСпискаЭП();
	УстановитьДоступностьКомандСпискаШифрования();
	
	Если НаименованиеДоЗаписи <> ЭтаФорма.Объект.Наименование Тогда
		
		// в кэше обновить файл
		РаботаСФайламиСлужебныйКлиент.ОбновитьИнформациюВРабочемКаталоге(
			ЭтаФорма.Объект.Ссылка, ЭтаФорма.Объект.Наименование);
		
		НаименованиеДоЗаписи = ЭтаФорма.Объект.Наименование;
...
Функция ЗаписатьФайл(Знач ПараметрОбъект = Неопределено)
	...
	Если ЗначениеЗаполнено(ЗначениеКопирования) Тогда
		...
		Попытка
			...
			Если ДоступнаЭлектроннаяПодпись
...
Процедура ПриИзмененияИспользованияПодписанияИлиШифрованияНаСервере()
	...
	РаботаСФайламиСлужебный.КриптографияПриСозданииФормыНаСервере(ЭтаФорма, Ложь);
...
Процедура ГруппаДополнительныеДанныеСтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.Свойства")
		...
		И Не ЭтаФорма.ПараметрыСвойств.ВыполненаОтложеннаяИнициализация Тогда
		
		СвойстваВыполнитьОтложеннуюИнициализацию();
		МодульУправлениеСвойствамиКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("УправлениеСвойствамиКлиент");
		МодульУправлениеСвойствамиКлиент.ПослеЗагрузкиДополнительныхРеквизитов(ЭтаФорма);
	КонецЕсли;
...
Процедура ОбновитьПояснениеОблачногоСервиса(ПрисоединенныйФайл)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюФайлов") Тогда
		...
		СведенияОСинхронизации = РаботаСФайламиСлужебный.СведенияОСинхронизации(ЭтаФорма.Объект.ВладелецФайла);
...
Функция ТекущееНаименованиеФайла()
	...
	ОбъектФормы = ЭтаФорма.Объект; // СправочникОбъект
