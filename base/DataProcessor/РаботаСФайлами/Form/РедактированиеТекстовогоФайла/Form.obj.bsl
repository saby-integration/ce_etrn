...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы Тогда
		ТекстПредупреждения = Saby_ТНОбщегоНазначенияКлиентСервер.СтрШаблонЭПД(
	...
	ОбработчикРезультата = Новый ОписаниеОповещения(
		"ПередЗакрытиемПослеОтветаНаВопросПриВыходеИзТекстовогоРедактора", ЭтаФорма);
	
	ТекстНапоминания = Saby_ТНОбщегоНазначенияКлиентСервер.СтрШаблонЭПД(
...
Процедура СохранитьКак(Команда)
	...
	Фильтр = Saby_ТНОбщегоНазначенияКлиентСервер.СтрШаблонЭПД(
	...
	Если ВыборФайла.Выбрать() Тогда	
		...
		ESD_ТНОбщегоНазначенияКлиент.ПоказатьИнформативноеОповещениеПользователя(
...
Процедура Записать(Команда)
	...
	Обработчик = Новый ОписаниеОповещения("ЗакончитьРедактированиеЗавершение", ЭтаФорма);
	ПараметрыОбновленияФайла = РаботаСФайламиСлужебныйКлиент.ПараметрыОбновленияФайла(
		Обработчик, Файл, ИдентификаторВладельца);
...
Процедура ЗакончитьРедактирование(Команда)
	...
	Обработчик = Новый ОписаниеОповещения("ЗакончитьРедактированиеЗавершение", ЭтаФорма, ПараметрыОбработчика);
...
#Иначе                            
	...
	ПараметрыВыполнения.Вставить("ПолноеИмяФайлаСлева", ИмяФайла);
	...
	Исключение 
		  ...
		  ДобавитьОшибкуВЖурналРегистрации(Описание);
	КонецПопытки;
...
Процедура ДобавитьОшибкуВЖурналРегистрации(Описание)
	
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'При удалении файла произошла ошибка'"),
...
Процедура ВыбратьКодировку(Команда)
	...
	Обработчик = Новый ОписаниеОповещения("ВыбратьКодировкуЗавершение", ЭтаФорма);
	ОткрытьФорму("Обработка.РаботаСФайлами.Форма.ВыборКодировки", ПараметрыФормы, ЭтаФорма, , , , Обработчик);
КонецПроцедуры
...
Процедура ВыполнитьСравнениеФайлов(Результат, ПараметрыВыполнения) Экспорт
	Если ПараметрыВыполнения.ТекущийШаг = 1 Тогда
		...
		Если ПараметрыВыполнения.СпособСравненияВерсийФайлов = Неопределено Тогда
			Обработчик = Новый ОписаниеОповещения("ВыполнитьСравнениеФайлов", ЭтаФорма, ПараметрыВыполнения);
			ОткрытьФорму("Обработка.РаботаСФайлами.Форма.ВыборСпособаСравненияВерсий", , ЭтаФорма, , , , Обработчик);
	...
	Если ПараметрыВыполнения.ТекущийШаг = 2 Тогда
		...
		ПередаваемыеФайлы.Добавить(
			Новый ОписаниеПередаваемогоФайла(ПараметрыВыполнения.ПолноеИмяФайлаСлева, АдресФайлаСлева));
