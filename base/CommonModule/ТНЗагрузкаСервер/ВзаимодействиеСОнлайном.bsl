...
Функция ИдентификаторВложения(ДанныеТитула, Титул, СтруктураВложения = Неопределено) Экспорт
	...
	ИначеЕсли ПредставленияТитула.КодТитула = "1115113" Тогда
		ИдФайл = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(
...
Процедура ДобавитьВложения(Вложения, Событие, ИмяМетаданных)
	...
	Для Каждого Вложение Из ВложенияСОнлайна Цикл
		...
		Если ИмяМетаданных = ТранспортнаяНакладнаяКлиент() Тогда
			ИмяФайлаСРасширением     = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Вложение, "Файл.Имя", "");
		...
		ДатаВремя = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Событие, "ДатаВремя");
...
Функция КлючСоответствияВложения(ДанныеТитула, ПредставленияТитула)
	...
	РезультатФункции = МодульВыполнения.ДанныеСоответствия(ДанныеТитула, "Файл.ИмяФайлаБезРасширения");
		
	Если РезультатФункции = Неопределено Тогда
		РезультатФункции = МодульВыполнения.ДанныеСоответствия(ДанныеТитула, "Документ.Дополнительная информация.Имя файла");
	КонецЕсли;
	
	Если РезультатФункции = Неопределено Тогда
		РезультатФункции = МодульВыполнения.ДанныеСоответствия(ДанныеТитула, "Документ.Данные.НаимДок");
	КонецЕсли;		
