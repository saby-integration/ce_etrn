...
Функция КлючСоответствияВложения(ДанныеТитула, ПредставленияТитула)
	...
	МодульВыполнения = МодульКодаКлиент("Saby_ТНОбщегоНазначенияКлиентСервер");
...
Процедура ЗаписатьДополнительнуюСтатистику(ПараметрыЗагрузки, ПараметрыМетода, ОбщиеПараметрыЗагрузки)
	...
	Если ПараметрыЗагрузки.ИмяМетаданных = "Saby_ПутевойЛист" И ПустаяСтрока(ПараметрыЗагрузки.ТекущийТитул) Тогда
		...
		// Несколько водителей
		ВодителиСтрокой = МодульКода("Saby_ТНОбщегоНазначенияСервер").ЗначениеРеквизитаОбъекта(
			ПараметрыЗагрузки.Ссылка, "Водитель"); 
			
		МассивВодителей = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").СтрРазделитьЭПД(ВодителиСтрокой, ",");	
