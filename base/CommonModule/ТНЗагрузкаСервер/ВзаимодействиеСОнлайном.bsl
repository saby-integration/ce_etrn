...
Процедура ДанныеДокументаИзСБИС(ПараметрыЗагрузки, ОбщиеПараметрыЗагрузки, Результат) Экспорт
	...
	ИДСбис = ИдСбисДокумента(ПараметрыЗагрузки.Ссылка);
	
	// документа нет в сбис. данные не загрузить
	Если Не ЗначениеЗаполнено(ИДСбис) Тогда 
		ТекстОшибки = "У документа нет идентификатора Saby, обновление данных не возможно!";
...
Функция ИдСбисДокумента(Ссылка)

	Если ТипЗнч(Ссылка) = Тип("Структура") Тогда
		ИДСбис = Ссылка.Идентификатор;
	Иначе
		
		ДанныеСостояния = ВыполнитьНаСервере("ESD_ТНОбщегоНазначенияСервер.ДанныеСостоянияОбъекта", Ссылка);
...
Функция ПараметрыМетодаЗаписиСтатистики(ТекущийТитул, ДокументСсылка) Экспорт
...
Процедура ЗаписатьСтатистикуРедактора(Ссылка, Событие) Экспорт
	...
	ПараметрыМетода = ПараметрыМетодаЗаписиСтатистики("", Ссылка);
		
	ПараметрыМетода.data.action 	= Событие;
	ПараметрыМетода.data.functional = "1C MappingEditor";
