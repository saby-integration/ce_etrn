...
Процедура ДанныеДокументаИзСБИС(ПараметрыЗагрузки, ОбщиеПараметрыЗагрузки, Результат) Экспорт
	...
	ИДСбис = ИдСбисДокумента(ПараметрыЗагрузки);
	
	// документа нет в сбис. данные не загрузить
	Если Не ЗначениеЗаполнено(ИДСбис) Тогда 
		ТекстОшибки = "У документа нет идентификатора Saby, обновление данных невозможно!";
	...
	ПараметрыМетодаЗагрузки.Вставить(
		"ДопПоля",
		"ДопДействия,ТекущиеЭтапы,Подстановки,Расширение,Стороны,ИдентификаторУчастника");
	...
	Если ПараметрыЗагрузки.ИмяМетаданных = ПутевойЛистКлиент() Тогда 
		ПараметрыМетодаЗагрузки.ДопПоля = ПараметрыМетодаЗагрузки.ДопПоля + ",ЗаменаВодителя";
	КонецЕсли;
...
Функция ИдСбисДокумента(ПараметрыЗагрузки)
	...
	Иначе
		...
		Если ДанныеСостояния <> Неопределено Тогда 
			...
			ПараметрыЗагрузки.ТипИсточника           = ДанныеСостояния.ТипИсточника;
			ПараметрыЗагрузки.ИдентификаторИсточника = ДанныеСостояния.ИдентификаторИсточника;
			ПараметрыЗагрузки.Ссылка                 = ДанныеСостояния.Ссылка;
...
Процедура ДобавитьПодстановкуТитула(ИзмененияПоДокументу, КодТитулаОнлайна, ЗначенияПодстановок, ИмяМетаданных)
	...
	МассивПодстановок = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ЗначениеВМассив(ЗначенияПодстановок);
...
Функция ПараметрыМетодаЗаписиСтатистики(ТекущийТитул, ИмяДействия, ДокументСсылка = Неопределено, ИмяФункционала = "", 
																							ИмяКонтекста = "") Экспорт
	...
	Если ДокументСсылка <> Неопределено Тогда
		...
		ИмяФункционала = ИмяПродуктаЭПДСтатистика() + ПараметрыAPI.Тип;
...
Процедура ЗаписатьСтатистикуРедактора(Ссылка, Событие) Экспорт
	...
	ПараметрыМетода = ПараметрыМетодаЗаписиСтатистики("", Событие, Ссылка, "1C MappingEditor");
...
Процедура ЗаписатьСтатистикуВызоваВОблако(Событие, Функционал, Форма = Неопределено) Экспорт
	...
	Попытка
		...
		Если Форма <> Неопределено И Форма.ЭтоВО Тогда
			context = "ВО2";
			ТранспортВО = МодульКодаКлиент("ESD_ТНОбщегоНазначенияСервер").ТранспортВО(Форма, context_params);
		...
		ПараметрыМетода = ПараметрыМетодаЗаписиСтатистики("", Событие, , Функционал, context);
...
Функция СсылкаНаПечатнуюФорму(ПараметрыЗагрузки, ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.ЭтоПечатьQRКода Тогда
		ПараметрыМетода.ДопПоля = "Расширение,QRFrameLinks";
	КонецЕсли;
