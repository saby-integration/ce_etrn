...
Процедура ЗагрузитьДанныеДинамическогоТитула(Форма, ИдентификаторТитула) Экспорт
	...
	Если ВыборкаДанных.Следующий() Тогда
		...
		ИмяМетаданных = МодульКода("Saby_ТНОбщегоНазначенияСервер").ИмяМетаданныхДокумента(Форма.СсылкаНаДокумент);
		...
		МассивСсылокНаДокумент.Добавить(Форма.СсылкаНаДокумент);
...
Процедура ДанныеТСПрицеповСпецРазрешений(ДляДокумента, СтруктураТССОнлайна) Экспорт
	...
	ЗаполнитьСтруктуруТранспортногоСредства(СтруктураТС, СтруктураТССОнлайна, ДляДокумента);
	...
	Если ЗначениеЗаполнено(Прицепы) Тогда
		...
		Для Каждого Прицеп Из Прицепы Цикл
			...
			СтруктураТС.КлючСтроки = МодульТНОбщегоНазначенияКлиентСервер.НовыйКлючОсновнойСтроки(
				ДляДокумента, ИмяТаблицыТранспортныеСредства);
			ЗаполнитьСтруктуруТранспортногоСредства(СтруктураТС, Прицеп, ДляДокумента);
			Если ЗначениеЗаполнено(СтруктураТС.РегистрационныйНомер) Тогда
				ДляДокумента.ТранспортныеСредства.Добавить(СтруктураТС);
			КонецЕсли;
		КонецЦикла;
...
Процедура ДанныеВодителей(ДляДокумента, ВодителиСОнлайна, ДопПараметры) Экспорт
	...
	Для Каждого Водитель Из ВодителиСОнлайна["Водитель"] Цикл 
		...
		ЗагрузитьДокументыЭПД(
			...
			ЗначениеМетаданных("Saby_ТипыДокумента.ПутевойЛист"));
		...
		Для ИндексДокументаЭПД = НачальныйИндекс По ДляДокумента.ДокументыЭПД.ВГраница() Цикл
			...
			СтруктураПутевогоЛиста.Вставить("КлючСтроки_ОтветственныеЛица", СтруктураВодителя.КлючСтроки);
...
Процедура ЗаполнитьСсылкиНаДокументЭПД(ДляДокумента) Экспорт
	...
	Если Метаданные.Справочники.Найти("Saby_ДокументыЭПД") = Неопределено Тогда
...
Процедура ЗаполнитьДокументыЭПД(ОбъектДок, ДляДокумента, ТипДокумента) Экспорт
	...
	Для Каждого СтруктураДокументаЭПД Из ДляДокумента.ДокументыЭПД Цикл
		...
		Если СтруктураДокументаЭПД.Тип <> ТипДокумента Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьОтветственныхЛиц(ОбъектДок, ДанныеОтветственныхЛиц, РольОтветственного) Экспорт
	...
	ЕстьПутевыеЛисты = МодульТНОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойство(ОбъектДок, "ПутевыеЛисты");
	
	ОтборОтветственных = Новый Структура;
	ОтборОтветственных.Вставить("Роль", РольОтветственного);
	СтрокиДляЗагрузки = МодульТНОбщегоНазначенияКлиентСервер.НайтиСтрокиУниверсально(
		ДанныеОтветственныхЛиц, ОтборОтветственных);
	
	Если ЗначениеЗаполнено(ДанныеОтветственныхЛиц) И СтрокиДляЗагрузки.Количество() > 0 Тогда
		
		ОтборОтветственных = Новый Структура;
		ОтборОтветственных.Вставить("Роль", РольОтветственного);
		МассивСтрокДляУдаления = МодульТНОбщегоНазначенияКлиентСервер.НайтиСтрокиУниверсально(
	...
	КлючСтроки = МодульТНОбщегоНазначенияКлиентСервер.НовыйКлючОсновнойСтроки(ОбъектДок, "ОтветственныеЛица");
...
Процедура ЗагрузитьДанныеПоГрузам(ДляДокумента, ДанныеГруз, ЗагрузкаСОнлайна, ЭтоЭТрН) Экспорт
	...
	Если ЭтоЭТрН Тогда
		...
		ДляДокумента.Вставить("ВидыУпаковки",      Новый Массив);
	...
	Для Каждого Груз Из Позиции Цикл 
		...
		Если ЭтоЭТрН Тогда
			Если СтруктураВидаУпаковки <> Неопределено Тогда
				СтруктураВидаУпаковки.КлючСтроки = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").НовыйКлючОсновнойСтроки(
					ДляДокумента, "ВидыУпаковки");
				ДляДокумента.ВидыУпаковки.Добавить(СтруктураВидаУпаковки);
				СтруктураГруза.Вставить("КлючСтроки_ВидыУпаковки", СтруктураВидаУпаковки.КлючСтроки);
			КонецЕсли;
...
Процедура ЗаполнитьСтруктуруТранспортногоСредства(СтруктураТС, СтруктураТССОнлайна, ДляДокумента)
	...
	Для ИндексДокументаЭПД = НачальныйИндекс По ДляДокумента.ДокументыЭПД.ВГраница() Цикл
		...
		МассивОснованийВладения.Добавить(СтруктураДокументаЭПД.Ссылка);
	КонецЦикла;
...
Функция СтруктураВодителя(Водитель)
	...
	СтруктураВодителя.Фамилия  = МодульТНОбщегоНазначенияСервер.ДанныеСоответствия(Водитель, "Фамилия", "");
	СтруктураВодителя.Имя      = МодульТНОбщегоНазначенияСервер.ДанныеСоответствия(Водитель, "Имя", "");
	СтруктураВодителя.Отчество = МодульТНОбщегоНазначенияСервер.ДанныеСоответствия(Водитель, "Отчество", "");
	СтруктураВодителя.ИНН      = МодульТНОбщегоНазначенияСервер.ДанныеСоответствия(Водитель, "ИНН", "");
	
	СтруктураВодителя.Серия = МодульТНОбщегоНазначенияСервер.ДанныеСоответствия(Водитель, "ВУ.Серия", "");
	СтруктураВодителя.Номер = МодульТНОбщегоНазначенияСервер.ДанныеСоответствия(Водитель, "ВУ.Номер", "");
	
	СтруктураВодителя.ДатаВыдачи = МодульТНОбщегоНазначенияСервер.ДанныеСоответствия(Водитель, "ВУ.ДатаВыдачи", "");
	СтруктураВодителя.ДатаВыдачи = МодульТНОбщегоНазначенияСервер.ПреобразоватьСтрокуВДатуЭПД(СтруктураВодителя.ДатаВыдачи);
	...
	Если ЗначениеЗаполнено(Водитель["ФИО"]) Тогда
		...
		Если ЗаполнитьФИО Тогда
			ФИО = МодульТНОбщегоНазначенияСервер.ФИОИзСтроки(Наименование);
	...
	Телефоны = МодульТНОбщегоНазначенияСервер.ДанныеСоответствия(Водитель, "Телефон", Новый Массив);
	Если ЗначениеЗаполнено(Телефоны) Тогда
