...
//
Процедура ЗаписатьДанныеДокумента(ПараметрыЗагрузки, Результат = Неопределено) Экспорт
	...
	ПараметрыЗагрузки.Ссылка = СохранитьДокумент(ОбъектДок, ПараметрыЗагрузки, ДополнительныеПараметры);
КонецПроцедуры
...
Функция ПреобразоватьСтруктуруАдреса(ДанныеАдреса) Экспорт
	СтруктураАдреса = Новый Структура("АдресСтруктурой, АдресСтрокой", Новый Структура, "");
...
Процедура SabyПослеЗагрузкиДанныхДокумента(ПараметрыЗагрузки, ОбщиеПараметрыЗагрузки, Результат)
	...
	Попытка
		...
		ЗаписатьДанныеДокумента(ПараметрыЗагрузки, Результат);
...
Функция ЗагрузитьАрхивВложениеВоВложенныеФайлы(ДокументСсылка, СсылкаНаАрхив = Неопределено) Экспорт
	Результат = Новый Структура;
		
	СостояниеОбъекта = ВыполнитьНаСервере("ESD_ТНОбщегоНазначенияСервер.ДанныеСостоянияОбъекта", ДокументСсылка);
	Если Не ЗначениеЗаполнено(СостояниеОбъекта) Или Не ЗначениеЗаполнено(СостояниеОбъекта.Состояние.ИдентификаторСБИС) Тогда
		Результат.Вставить("ОписаниеОшибки", "Документооборот не начат. Архив документа недоступен.");
		Возврат Результат;
	КонецЕсли;
	
	ДополнительныеПараметры = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДополнительныеПараметрыПечати(Ложь,
	ДокументСсылка.УникальныйИдентификатор(), "", Ложь, Истина, Ложь);
	
	ПараметрыЗагрузки = ПараметрыЗагрузкиИзСбис(ДокументСсылка, ДополнительныеПараметры, , Ложь);
	Если ТипЗнч(ПараметрыЗагрузки.context_params) = Тип("Структура") Тогда
		ПараметрыЗагрузки.context_params.Вставить("Timeout", 120);
	КонецЕсли;
	
	// Вручную укажем признак необходимости загрузки
	ПараметрыЗагрузки.ЗагрузкаТребуется = Истина;
	АдресДвоичныхДанных = АдресДвоичныхДанныхФайла(ПараметрыЗагрузки, ДополнительныеПараметры, СсылкаНаАрхив);
	Если Не ЗначениеЗаполнено(АдресДвоичныхДанных) Тогда
		Результат.Вставить("ОписаниеОшибки", "Не удалось получить архив документа из Saby.");
		Возврат Результат;				
	КонецЕсли;
	
	ESDОбщегоНазначенияСервер = МодульКода("ESD_ТНОбщегоНазначенияСервер");

	// Выполним поиск созданного ранее присоединенного файла
	ОписаниеФайла = "Автоматически Saby";
	СсылкаНаПрисоединенныйФайл = ESDОбщегоНазначенияСервер.ПрисоединенныйФайлПоВладельцуИОписанию(
		ДокументСсылка, ОписаниеФайла);
	...
	Иначе
		СтруктураДокумента = ESDОбщегоНазначенияСервер.ЗначенияРеквизитовОбъекта(ДокументСсылка, "Ссылка,Номер,Дата,НомерСбис", Истина);
		...
		ПараметрыФайла.Вставить("ИмяБезРасширения", ESDОбщегоНазначенияСервер.ПредставлениеДокумента(СтруктураДокумента));
...
//  ЗагрузкаТребуется - Булево - признак необходимости загрузки
//
// Возвращаемое значение:
//   Структура - структура параметров.
//
Функция ПараметрыЗагрузкиИзСбис(Знач СсылкаНаДокумент, ДополнительныеПараметры,
		context_params = Неопределено, ЗагрузкаТребуется = Истина) Экспорт
	...
	РезультатФункции.Вставить("ОбъектОбработки",              ОбъектОбработки());
	РезультатФункции.Вставить("УникальныйИдентификаторФормы", ДополнительныеПараметры.УникальныйИдентификаторФормы);
	РезультатФункции.Вставить("СписокДокументов",             Неопределено);
	РезультатФункции.Вставить("ЭтоЧерновик",                  Истина);
	РезультатФункции.Вставить("ЗагрузкаТребуется",            ЗагрузкаТребуется);
	...
	Если СсылкаНаДокумент <> Неопределено Тогда
		ЗаполнитьПараметрыЗагрузкиПоСсылке(РезультатФункции, Ложь, ДополнительныеПараметры);
	КонецЕсли;
	
	Возврат РезультатФункции;
КонецФункции

Процедура ЗаполнитьПараметрыЗагрузкиПоСсылке(Параметры, ПовторноеПолучение, ДополнительныеПараметры)
	ДанныеСостояния = МодульКода("ESD_ТНОбщегоНазначенияСервер").ДанныеСостоянияОбъекта(Параметры.СсылкаНаДокумент);
	...
	НеобходимоОбновитьДанные = Параметры.ЗагрузкаТребуется
		И Не ДополнительныеПараметры.ЭтоПечатьQRКода
		Или Не ЗначениеЗаполнено(Параметры.ИдентификаторСБИС);

	РезультатВыгрузки = Неопределено;
	Если НеобходимоОбновитьДанные Тогда
		РезультатВыгрузки = ВыгрузитьДокументSaby(Параметры, ДополнительныеПараметры);
	ИначеЕсли Не Подключаемый_ЗаполнитьПараметрыЗагрузки(Параметры, ДополнительныеПараметры, РезультатВыгрузки) Тогда
		Возврат;
	Иначе
		Возврат;
	КонецЕсли;
	
	Если РезультатВыгрузки.Итог.Успех Тогда
		ЗаполнитьПараметрыЗагрузкиПоСсылке(Параметры, Истина, ДополнительныеПараметры);
	Иначе
		Параметры.СписокДокументов = РезультатВыгрузки.Ошибки;
		Параметры.ОшибкиОбщие = РезультатВыгрузки.ОшибкиОбщие;
	КонецЕсли;	
КонецПроцедуры

Процедура ЗаполнитьПараметрыЗагрузкиПоСсылкеДляРасширения(Параметры, ДанныеСостояния)
	ИмяМетаданных = МодульКода("ESD_ТНОбщегоНазначенияСервер").ИмяМетаданныхДокумента(Параметры.СсылкаНаДокумент);
	...
	ЗаполнитьЗначенияСвойств(Параметры, ВыборкаДанных, "Номер, Дата");
	...
	Параметры.ИдентификаторСБИС = ИдентификаторСБИС;
	Если ЗначениеЗаполнено(ДанныеСостояния.Состояние) Тогда
		Параметры.ЗагрузкаТребуется = Параметры.ЗагрузкаТребуется 
			И Не МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДокументооборотЗавершен(ДанныеСостояния.Состояние.ИндексКартинки);
		Параметры.ЭтоЧерновик = Ложь;
	Иначе
		Параметры.ЭтоЧерновик = Истина;
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнитьПараметрыЗагрузкиПоСсылкеДляВО(Параметры, ДанныеСостояния)
	...
	Если ДанныеСостояния <> Неопределено Тогда
		Параметры.ИдентификаторСБИС = ДанныеСостояния.UID;
...
Функция ВыгрузитьДокументSaby(Параметры, ДополнительныеПараметры)
	...
	ПараметрыВыгрузки.ВыполнитьФЛК = Не ЗначениеЗаполнено(Параметры.ИдентификаторСБИС);
	...
	РезультатВыгрузки = МодульКода("ESD_ТНВыгрузкаСервер")
		.РезультатВыгрузкиДокументов(ДанныеДляВыгрузки, Неопределено, Новый Структура);	
	// Вернем результат выгрузки
	Возврат РезультатВыгрузки;
КонецФункции

Функция ПараметрыВыгрузкиДокумента(Параметры, ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.Свойство("ТипДокумента") Тогда
		ПараметрыВыгрузки.ИмяМетаданных = ДополнительныеПараметры.ТипДокумента;
	КонецЕсли;
...
#Область include_etrn_base_CommonModule_ТНЗагрузкаСервер_ПодключаемыйФункционал_СлужебныеПроцедурыИФункции
#КонецОбласти // include_etrn_base_CommonModule_ТНЗагрузкаСервер_ПодключаемыйФункционал_СлужебныеПроцедурыИФункции
