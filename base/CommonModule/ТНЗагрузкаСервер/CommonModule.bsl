...
Функция РезультатЗагрузкиДокументов(ДанныеДляЗагрузки, context_params, ДополнительныеПараметры) Экспорт
	...
	РезультатФункции = МодульТНОбщегоНазначенияКлиентСервер.РезультатВзаимодействияСоСБИС(
	...
	Для Каждого ПараметрыЗагрузки Из ДанныеДляЗагрузки Цикл	
		...
		ПараметрыЗагрузки.Вставить("ПолнаяЗагрузка", Истина);
	...
	МодульТНОбщегоНазначенияСервер.ЗаписатьОшибкиВЖурналРегистрации(РезультатФункции.Ошибки, 
		МодульТНОбщегоНазначенияКлиентСервер.ИмяСобытияЖурналаРегистрации_ЗагрузкаИзSaby());
...
Функция ЗагрузкаДокументовПоИзменениям(СтруктураПараметров) Экспорт
	...
	Результат = МодульТНОбщегоНазначенияКлиентСервер.РезультатВзаимодействияСоСБИС(СписокИзменений.Количество());
	...
	МодульТНОбщегоНазначенияСервер.ЗаписатьОшибкиВЖурналРегистрации(Результат.Ошибки, 
		МодульТНОбщегоНазначенияКлиентСервер.ИмяСобытияЖурналаРегистрации_ЗагрузкаИзSaby());
...
Процедура SabyПослеЗагрузкиДанныхДокумента(ПараметрыЗагрузки, ОбщиеПараметрыЗагрузки, Результат)
	...
	Исключение
		...
		ТекстОшибки = МодульТНОбщегоНазначенияСервер.ПричинаОшибкиИзЯдра(ИнфоОшибки.Описание);
		...
		КлючСоответствия = МодульТНОбщегоНазначенияКлиентСервер.КлючСсылка(
...
Процедура ЗагрузитьТекущиеЭтапыДокумента(ОбъектДок, ПараметрыЗагрузки)
	...
	Для Каждого ТекущийЭтап Из ТекущиеЭтапы Цикл
		Наименование_ = ТекущийЭтап["Наименование"];
		Если Не ЗначениеЗаполнено(Наименование_) Тогда
			Продолжить;
		КонецЕсли;

		СтрокаТекущегоЭтапа = Новый Структура("Наименование,ТипТитула,Доступен");
		СтрокаТекущегоЭтапа.Наименование  = Наименование_;
		СтрокаТекущегоЭтапа.ТипТитула     = МодульESD_ОбщегоНазначенияСервер.РезультатФункцииТитулов(
			ПараметрыЗагрузки.ИмяМетаданных, "ЗначениеПоЭтапу", Наименование_);
...
Функция SabyСписокИзменений_Загрузить(ПараметрыПолученияИзменений)
	...
	Исключение
		...
		ТекстОшибки = МодульТНОбщегоНазначенияСервер.ПричинаОшибкиИзЯдра(ИнформацияОбОшибке().Описание);
		
		МодульТНОбщегоНазначенияСервер.ЗаписатьОшибкуВЖурналРегистрации(Неопределено, 
			МодульТНОбщегоНазначенияКлиентСервер.ИмяСобытияЖурналаРегистрации_ЗагрузкаИзSaby(), 
			ТекстОшибки);	
	КонецПопытки;
