...
Процедура ЗагрузитьДокумент(ПараметрыЗагрузки, ОбщиеПараметрыЗагрузки, Результат)
	...
	Попытка
		...
		Если Не ПараметрыЗагрузки.ЗагрузкаНаКлиенте И ПараметрыЗагрузки.ТолькоАктивныйЭтап Тогда
...
Функция СохранитьДокумент(ОбъектДок, ИД, АктивныйЭтап, Состояние, ИзДокумента)
	...
	Попытка
		...
		ПараметрыСтатус = МодульКода("ESD_ТНОбщегоНазначенияСервер").ПараметрыЗаписиРегистраСостояний();
		...
		МодульКода("ESD_ТНОбщегоНазначенияСервер").ОбновитьДанныеСостояния(ПараметрыСтатус);
