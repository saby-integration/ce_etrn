...
Процедура ЗаписатьДанныеДокумента(ПараметрыЗагрузки, Результат = Неопределено) Экспорт
	...
	КодСостояния = МодульТНОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(ПараметрыЗагрузки.ДанныеДокумента, "Состояние.Код");
	...
	Если КодСостояния = КодСостоянияЗавершеноСПроблемами Тогда
		ОбъектДок.КомментарийУведомление = МодульТНОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(
	...
	Если ВложенияЗагруженыБезОшибок Тогда
		...
		ПараметрыЗагрузки.Ссылка = СохранитьДокумент(
			ОбъектДок, ТекущийЭтапДокумента, Состояние, ПараметрыЗагрузки);
			
	ИначеЕсли ЗначениеЗаполнено(ЗагруженныйТитул) Тогда
		ПараметрыЗагрузки.Ссылка = СохранитьДокумент(
			ОбъектДок, ЗагруженныйТитул, Состояние, ПараметрыЗагрузки);
	Иначе
...
Функция СохранитьДокумент(ОбъектДок, АктивныйЭтап, Состояние, ПараметрыЗагрузки)
	...
	Попытка
		...
		// Запись Справочника "Присоединенные файлы"  
		ДОЗавершен = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДокументооборотЗавершен(Состояние);
		ВыгружатьАрхивВложений = ХранилищеОбщихНастроек.Загрузить("Saby", "ВыгружатьАрхивВложений", , "ESD_ДляВсехПользователей");
		
		Если ДОЗавершен И ЗначениеЗаполнено(ВыгружатьАрхивВложений) И ВыгружатьАрхивВложений Тогда
			ЗагрузитьАрхивВложениеВоВложенныеФайлы(ОбъектДок.Ссылка, ПараметрыЗагрузки.ДанныеДокумента["СсылкаНаАрхив"]);
		КонецЕсли;
		
		МодульКода("РегистрыСведений.ESD_ДанныеСобытий").ДобавитьНабор(Ссылка, ПараметрыЗагрузки.ДанныеДокумента["Событие"], Истина);
	Исключение
...
Функция ЗагрузитьАрхивВложениеВоВложенныеФайлы(ДокументСсылка, СсылкаНаАрхив = Неопределено) Экспорт
	...
	ПараметрыЗагрузки = ПараметрыЗагрузкиИзСбис(ДокументСсылка, ДополнительныеПараметры);
	...
	СсылкаНаПрисоединенныйФайл = МодульКода("ESD_ТНОбщегоНазначенияСервер").ПрисоединенныйФайлПоВладельцуИОписанию(
	ДокументСсылка, ОписаниеФайла);
	// Сравним файлы по ДД, если совпадают, то пропустим загрузку
	Если СсылкаНаПрисоединенныйФайл <> Неопределено И Строка(СсылкаНаПрисоединенныйФайл.ДвоичныеДанныеФайла.Получить()) 
														= Строка(ПолучитьИзВременногоХранилища(АдресДвоичныхДанных)) Тогда
		УдалитьИзВременногоХранилища(АдресДвоичныхДанных);
	Иначе
		ПараметрыФайла = Новый Структура;
		ПараметрыФайла.Вставить("ВладелецФайлов", ДокументСсылка);
		ПараметрыФайла.Вставить("ИмяБезРасширения", Строка(ДокументСсылка));
		...
		МодульКода("ESD_ТНЗагрузкаСервер").ЗаписатьСтатистикуВызоваВОблако("СкачиваниеВложений", "Расширение 1С ESD");
	КонецЕсли;
...
Функция ЭтапДляПодписания(ОбъектДок, ПараметрыЗагрузки)
	...
	ДоступныеЭтапы = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").НайтиСтрокиУниверсально(ОбъектДок.ТекущиеЭтапы, ОтборЭтапов);
	...
	ДоступныеЭтапыПереадресовки = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").НайтиСтрокиУниверсально(
		ОбъектДок.ТекущиеЭтапы, ОтборЭтапов);
	Если ДоступныеЭтапыПереадресовки.Количество() > 0 Тогда
		РезультатФункции = ДоступныеЭтапыПереадресовки[0].Наименование;
	КонецЕсли;
...
Процедура SabyСписокИзменений_ОбработатьДанные(Изменения, ДанныеSaby, ПараметрыПолученияИзменений, ДопПараметры)
	...
	Если ЗначениеЗаполнено(СобытияДокументаSaby) Тогда
		...
		Если ЗначениеЗаполнено(Комментарий) Тогда
			КодСостояния = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ЗначениеИзКоллекции(ДанныеSaby, "Состояние.Код");
			КодСостояния = МодульКода("ESD_ТНОбщегоНазначенияСервер").ЧислоИзСтроки(КодСостояния);
			КодСостоянияЗавершеноСПроблемами = 9;
			Если КодСостояния = КодСостоянияЗавершеноСПроблемами Тогда
...
Процедура ЗаполнитьПараметрыЗагрузкиПоСсылке(Параметры, ПовторноеПолучение, ДополнительныеПараметры)
	...
	НеобходимоОбновитьДанные = Параметры.ЗагрузкаТребуется
		...
		И (Параметры.УИДСбис = "" Или Параметры.ЭтоЧерновик);
...
Процедура ЗаполнитьПараметрыЗагрузкиПоСсылкеДляРасширения(Параметры, ДанныеСостояния)
	...
	ИначеЕсли МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ЕстьРеквизитИлиСвойство(ДанныеСостояния, "UID") Тогда
		ИД = ДанныеСостояния.UID;
	Иначе
		ИД = ДанныеСостояния.ИД;
	КонецЕсли;	
...
Функция ВыгрузитьДокументSaby(Параметры, ДополнительныеПараметры)
	...
	РезультатЗаписи = МодульКода("ESD_ТНВыгрузкаСервер").РезультатВыгрузкиДокументов(
		ДанныеДляВыгрузки, Параметры.context_params, ДопПараметры);
...
Процедура ЗагрузитьПараметрыГруза(СтруктураГруза, ПараметрыГруза)
	...
	МассаБрутто = МодульТНОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(ПараметрыГруза, "Масса.Брутто");
	МассаБрутто = МодульТНОбщегоНазначенияСервер.ЧислоИзСтроки(МассаБрутто);
	МассаНетто  = МодульТНОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(ПараметрыГруза, "Масса.Нетто");
	...
	Для Каждого Контейнер Из Контейнеры Цикл
		...
		Для Каждого СтрокаКонт Из Контейнер["Номер"] Цикл
			...
			Если ЗначениеЗаполнено(КоличествоКонтейнеров) Тогда 
				СтруктураКонтейнера.Количество = МодульКода("ESD_ТНОбщегоНазначенияСервер").ЧислоИзСтроки(КоличествоКонтейнеров);
			КонецЕсли;
...
Процедура ЗагрузитьДанныеГруза_ТранспортнаяНакладная(Груз, СтруктураГруза, ДляДокумента,
	...
	Стоимость = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ЗначениеИзКоллекции(Груз, "Стоимость.ВключаяНДС");
	Если ЗначениеЗаполнено(Стоимость) Тогда
		Стоимость = МодульКода("ESD_ТНОбщегоНазначенияСервер").ЧислоИзСтроки(Стоимость);
	Иначе
		Стоимость = 0;
	КонецЕсли;
