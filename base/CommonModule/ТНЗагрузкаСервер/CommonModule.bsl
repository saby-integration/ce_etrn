...
Функция ЗагрузкаДокументовПоИзменениям(СтруктураПараметров) Экспорт
	...
	МодульКода("ESD_ТНОбщегоНазначенияСервер").ЗаписьОшибокВЖурналРегистрации(Результат.Ошибки);
...
Процедура ЗаписатьДанныеДокумента(ПараметрыЗагрузки, Результат = Неопределено) Экспорт
	...
	Если ТипЗнч(ОбъектДок) <> Тип("Структура") Тогда
		ОбъектДок.ГИС_УИД = ПараметрыЗагрузки.ДанныеДокумента["ГИС_УИД"];
	КонецЕсли;
...
Процедура ЗаполнитьОснованияВладения(ОбъектДок, СтрокаТС, СтруктураТС) Экспорт
	...
	// только для во
	ЭтоВО = ТипЗнч(ОбъектДок) = Тип("Структура")
		Или (ТипЗнч(ОбъектДок) = Тип("ФормаКлиентскогоПриложения") И ОбъектДок.ЭтоВО);
	Если Не ЭтоВО Тогда
		Возврат;
	КонецЕсли;
	
	НовыйКлючСтрокиЭПД = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").НовыйКлючОсновнойСтроки(ОбъектДок, "ДокументыЭПД");
	НовыйКлючСтрокиЮрЛица = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").НовыйКлючОсновнойСтроки(ОбъектДок, "ДанныеЮрЛиц");
...
Функция НормализованныеДанныеЮрЛица(ПодстановкиЮрЛица) Экспорт
	...
	РеквизитыЮрЛица.Вставить("ФИО",      ДанныеОрганизации["ФИО"]);
...
Процедура ПередСохранениемДокумента(ОбъектДок, ПараметрыЗагрузки, ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.Состояние.ИдентификаторСБИС = КодСостоянияЗавершеноСПроблемами Тогда
		ОбъектДок.КомментарийУведомление = ESDОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(
...
Функция ОбъектЗагружаемогоДокумента(ПараметрыЗагрузки)
	...
	// поищем существующий документ
	Если Не ЗначениеЗаполнено(ПараметрыЗагрузки.Ссылка) Тогда
		ПараметрыЗагрузки.Ссылка = МодульКода("ESD_ТНОбщегоНазначенияСервер").ДокументПоИдентификаторуСбис(ПараметрыЗагрузки.ИдСбис);
	КонецЕсли;
...
Функция ЗагрузитьСписокИзмененийДокументов(СтруктураПараметров)
	...
	Пока ЕстьИзменения Цикл
		...
		Для Каждого ДанныеSaby Из СтруктураИзменений.ИзмененияПоДокументам Цикл
			SabyСписокИзменений_ОбработатьДанные(РезультатФункции, ДанныеSaby, ПараметрыПолученияИзменений, СтруктураПараметров.ДопПараметры);
		КонецЦикла;
	...
	Для Каждого КлючЗначение Из РезультатФункции Цикл
		...
		Если Не ЗначениеЗаполнено(ДанныеДокумента["Подстановки"]) Тогда
			РезультатФункции.Удалить(КлючЗначение.Ключ);
		КонецЕсли;
	КонецЦикла;
...
Функция SabyСписокИзменений_Загрузить(ПараметрыПолученияИзменений)
	...
	Исключение
		...
		Описание    = "Ошибка загрузки из SABY";
		
		МодульКода("ESD_ТНОбщегоНазначенияСервер").ЗаписатьОшибкуВЖурналРегистрации(Неопределено, Описание, ИнфОбОшибке);
...
Процедура SabyСписокИзменений_ОбработатьДанные(Изменения, ДанныеSaby, ПараметрыПолученияИзменений, ДопПараметры)
	...
	Если ЗначениеЗаполнено(СобытияДокументаSaby) Тогда
		...
		ПараметрыПолученияИзменений.Параметры.ИдентификаторСобытия = ПоследнееСобытие["Идентификатор"];
		ПараметрыПолученияИзменений.Параметры.ДатаВремяС           = ПоследнееСобытие["ДатаВремя"];	
...
Процедура ЗаполнитьПараметрыЗагрузкиПоСсылке(Параметры, ПовторноеПолучение, ДополнительныеПараметры)
	...
	Если НеобходимоОбновитьДанные Тогда
		РезультатЗаписи = ВыгрузитьДокументSaby(Параметры, ДополнительныеПараметры);
	ИначеЕсли ДополнительныеПараметры.Свойство("ПечатнаяФорма") Тогда
		РезультатЗаписи = ИзменитьПечатнуюФорму(Параметры, ДополнительныеПараметры);
	Иначе
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		Параметры.ОшибкиОбщие = РезультатЗаписи.ОшибкиОбщие;
	КонецЕсли;	
КонецПроцедуры
...
Процедура ЗаполнитьПараметрыЗагрузкиПоСсылкеДляРасширения(Параметры, ДанныеСостояния)
	...
	Если ЗначениеЗаполнено(ДанныеСостояния.Состояние) Тогда
		Параметры.ЗагрузкаТребуется = Параметры.ЗагрузкаТребуется 
			И Не МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДокументооборотЗавершен(ДанныеСостояния.Состояние);
...
Функция ВыгрузитьДокументSaby(Параметры, ДополнительныеПараметры)
	...
	МодульКода("ESD_ТНВыгрузкаСервер").ВыгрузитьДокумент(ПараметрыВыгрузки, , РезультатВыгрузки);	
	Возврат РезультатВыгрузки;
КонецФункции

Функция ИзменитьПечатнуюФорму(Параметры, ДополнительныеПараметры)
	ПараметрыВыгрузки = ПараметрыВыгрузкиДокумента(Параметры, ДополнительныеПараметры);
	РезультатЗаписи = МодульКода("ESD_ТНВыгрузкаСервер").ВыгрузитьПечатнуюФорму(
...
Функция ПараметрыВыгрузкиДокумента(Параметры, ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.Свойство("ПечатнаяФорма") Тогда
		ПараметрыВыгрузки.Вставить("ПечатнаяФорма", ДополнительныеПараметры.ПечатнаяФорма);	
	КонецЕсли;
