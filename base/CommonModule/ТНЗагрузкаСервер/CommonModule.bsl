...
Процедура ЗагрузкаДанныхДокумента(ПараметрыЗагрузки, Результат = Неопределено) Экспорт
	...
	КодСостояния = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(ПараметрыЗагрузки.ДанныеДокумента, "Состояние.Код");
	...
	Если КодСостояния = КодСостоянияЗавершеноСПроблемами Тогда
		ОбъектДок.КомментарийУведомление = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
...
Функция ОбъектЗагружаемогоДокумента(ПараметрыЗагрузки)
	...
	Если Не ПараметрыЗагрузки.ЭтоЗаполнениеСПомощьюINI Тогда
		...
		ДатаВремяСоздания = МодульКода("ESD_ТНОбщегоНазначенияСервер").ПреобразоватьСтрокуВДатуЭПД(
...
Процедура ЗаполнитьКомментарийОтрицательногоЗавершенияПоСобытию(ИзмененияПоДокументу)
	...
	КодСостояния = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(ИзмененияПоДокументу, "Состояние.Код");
...
Функция ДанныеСпецРазрешений(СпецУсловияСОнлайна)
	...
	Для Каждого Условие Из СпецУсловияСОнлайна Цикл   
		...
		Если ЗначениеЗаполнено(СпецРазрешение) Тогда
			...
			НаименованиеОрганаВласти = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(Условие, "ОИВ.Наименование");
...
Функция СтруктураВодителя(Водитель)
	...
	СтруктураВодителя.Фамилия        = МодульКлиентСервер.ДанныеСоответствия(Водитель, "Фамилия",        "");
	СтруктураВодителя.Имя            = МодульКлиентСервер.ДанныеСоответствия(Водитель, "Имя",            "");
	СтруктураВодителя.Отчество       = МодульКлиентСервер.ДанныеСоответствия(Водитель, "Отчество",       "");
	СтруктураВодителя.ИНН            = МодульКлиентСервер.ДанныеСоответствия(Водитель, "ИНН",            "");
	СтруктураВодителя.СНИЛС          = МодульКлиентСервер.ДанныеСоответствия(Водитель, "СНИЛС",          "");
	СтруктураВодителя.ТабельныйНомер = МодульКлиентСервер.ДанныеСоответствия(Водитель, "ТабельныйНомер", "");
	
	СтруктураВодителя.Серия = МодульКлиентСервер.ДанныеСоответствия(Водитель, "ВУ.Серия", "");
	СтруктураВодителя.Номер = МодульКлиентСервер.ДанныеСоответствия(Водитель, "ВУ.Номер", "");
	
	СтруктураВодителя.ДатаВыдачи = МодульКлиентСервер.ДанныеСоответствия(Водитель, "ВУ.ДатаВыдачи", "");
	...
	Телефоны = МодульКлиентСервер.ДанныеСоответствия(Водитель, "Телефон", Новый Массив);
	Если ЗначениеЗаполнено(Телефоны) Тогда
...
Процедура ЗаполнитьРеквизитыЮрЛица(Роль, СтруктураДанныхЮрЛица, ДанныеЮрЛицаОнлайн)
	...
	ИначеЕсли ЗначениеЗаполнено(Реквизиты["ИНН"]) Тогда
		СтруктураДанныхЮрЛица.ИНН        = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Реквизиты, "ИНН", "");
		СтруктураДанныхЮрЛица.КодФилиала = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Реквизиты, "КодФилиала", "");
	Иначе
		// Неизвестный узел данных, ничего не заполняем.
		СтруктураДанныхЮрЛица.ИНН = "";
	КонецЕсли;
	
	СтруктураДанныхЮрЛица.ИдентификаторЭДО = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
...
Процедура ЗаполнитьРеквизитыЮЛ(СтруктураДанныхЮрЛица, Реквизиты)
	...
	СтруктураДанныхЮрЛица.ИНН = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Реквизиты, "ЮЛ.ИНН", "");
	СтруктураДанныхЮрЛица.КПП = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Реквизиты, "ЮЛ.КПП", "");
	
	СтруктураДанныхЮрЛица.КодФилиала   = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Реквизиты, "ЮЛ.КодФилиала", "");
	
	НаименованиеОрганизации = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Реквизиты, "ЮЛ.Название");
	Если Не ЗначениеЗаполнено(НаименованиеОрганизации) Тогда
		НаименованиеОрганизации = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Реквизиты, "ЮЛ.Наименование");
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыФЛ(СтруктураДанныхЮрЛица, Реквизиты)
	...
	СтруктураДанныхЮрЛица.ИНН        = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Реквизиты, "ФЛ.ИНН", "");
	СтруктураДанныхЮрЛица.КодФилиала = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Реквизиты, "ФЛ.КодФилиала", "");
...
Процедура ЗаполнитьРеквизитыИП(СтруктураДанныхЮрЛица, Реквизиты)
	...
	СтруктураДанныхЮрЛица.ИНН        = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Реквизиты, "ИП.ИНН", "");
	СтруктураДанныхЮрЛица.ОГРН       = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Реквизиты, "ИП.ОГРН", "");
	СтруктураДанныхЮрЛица.КодФилиала = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Реквизиты, "ИП.КодФилиала", "");
...
Процедура ЗаполнитьРеквизитыИностранногоЛица(СтруктураДанныхЮрЛица, Реквизиты)
	...
	СтруктураДанныхЮрЛица.НаименованиеОрганизации = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(
		Реквизиты, "ИностранноеЛицо.Наименование", "");
	
	Страна = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(Реквизиты, "ИностранноеЛицо.Страна");
...
Процедура ЗаполнитьРеквизитыАдресаЮрЛица(СтруктураДанныхЮрЛица, ДанныеЮрЛицаОнлайн)
	...
	ЮрАдрес = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(ДанныеЮрЛицаОнлайн, "Адрес.Юридический");
	Если Не ЗначениеЗаполнено(ЮрАдрес) Тогда
		ЮрАдрес = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(ДанныеЮрЛицаОнлайн, "Адрес");
    КонецЕсли;
...
Функция ВалютаПоДаннымОнлайна(ДанныеГруз, ЗагрузкаСОнлайна, Знач ЗначениеПоУмолчанию = Неопределено) Экспорт
	...
	Валюта = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(ДанныеГруз, "Стоимость.Валюта");
...
Функция СтруктураОпасногоГруза(СтрокаОпасн)
	...
	// Наименование
	СтруктураОпасногоГруза.Наименование = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
		СтрокаОпасн, "Наименование.Отгрузочное", "");
		
	СтруктураОпасногоГруза.ПолноеНаименование = СтруктураОпасногоГруза.Наименование;
	
	СтруктураОпасногоГруза.ТехническоеНаименование = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
	...
	// Упаковка	
	СтруктураОпасногоГруза.ГруппаУпаковки = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
		СтрокаОпасн, "Упаковка.Группа", "");
		
	СтруктураОпасногоГруза.КатегорияУпаковки = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
	...
	// Температура	
	СтруктураОпасногоГруза.КонтрольнаяТемпература = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
		СтрокаОпасн, "Температура.Контрольная", "");
		
	СтруктураОпасногоГруза.АварийнаяТемпература = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
		СтрокаОпасн, "Температура.Аварийная", "");
		
	// Радиоактивный	
	СтруктураОпасногоГруза.НазваниеРадионуклида = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
		СтрокаОпасн, "Радиоактивный.Радионуклид", "");
		
	СтруктураОпасногоГруза.ПолнаяАктивность = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
		СтрокаОпасн, "Радиоактивный.ПолнаяАктивность", "");
		
	СтруктураОпасногоГруза.МаксАктивность = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
		СтрокаОпасн, "Радиоактивный.МаксАктивность", "");
		
	// Взрывчатый
	СтруктураОпасногоГруза.МассаНеттоВзрывчатый = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
		СтрокаОпасн, "Взрывчатый.МассаНетто", "");
		
	СтруктураОпасногоГруза.МассаНеттоВзрывчатыйВсего = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
...
Процедура ЗаполнитьОбщиеПараметрыОпасногоГруза(СтруктураОпасногоГруза, СтрокаОпасн)
	...
	СтруктураОпасногоГруза.НомерООН = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
		СтрокаОпасн, "НомерООН", "");
	СтруктураОпасногоГруза.ФизИХимФорма = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
		СтрокаОпасн, "ФизХимФорма", "");
	СтруктураОпасногоГруза.ТранспортныйИндекс = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
		СтрокаОпасн, "ТранспортныйИндекс", "");
	СтруктураОпасногоГруза.ЗнакОпасности = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
		СтрокаОпасн, "ЗнакОпасности", "");
	СтруктураОпасногоГруза.ИндексБезопасности = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
		СтрокаОпасн, "ИндексБезопасности", "");
	СтруктураОпасногоГруза.ОпознавательныйЗнак = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
		СтрокаОпасн, "ОпознавательныйЗнак", "");
	СтруктураОпасногоГруза.ПроцентСмеси = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
		СтрокаОпасн, "ПроцентныйСоставСмеси", "");
	СтруктураОпасногоГруза.Комментарий = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
		СтрокаОпасн, "Описание", "");
	СтруктураОпасногоГруза.КлассификационныйКод = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
		СтрокаОпасн, "КлассификационныйКод", "");
	
	КодОграниченияПроездТуннеля = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
...
Процедура ЗагрузитьПараметрыГруза(СтруктураГруза, ПараметрыГруза)
	...
	МассаБрутто = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(ПараметрыГруза, "Масса.Брутто");
	МассаБрутто = МодульТНОбщегоНазначенияСервер.ЧислоИзСтроки(МассаБрутто);
	МассаНетто  = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(ПараметрыГруза, "Масса.Нетто");
...
Процедура ЗагрузитьДанныеГруза_ТранспортнаяНакладная(Груз, СтруктураГруза, ДляДокумента,
	...
	Стоимость = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(Груз, "Стоимость.ВключаяНДС");
...
Процедура ЗагрузитьДанныеГруза_ЗаказНаПеревозку(ДляДокумента, Груз, СтруктураГруза)
	...
	ОпасныйГруз = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Груз, "ОпасныйГруз");
	...
	// Заказчик и договор
	Заказчик = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Груз, "Заказчик");
	Договор  = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Груз, "Договор");
