...
Функция РезультатЗагрузкиДокументов(ДанныеДляЗагрузки, context_params, ДополнительныеПараметры) Экспорт
	...
	МодульКода("Справочники.Saby_ОпасныеГрузы").ДозаполнитьНовыеОпасныеГрузы(РезультатФункции, ОбщиеПараметрыЗагрузки);
	МодульКода("Справочники.Saby_ВидыУпаковки").ДозаполнитьНовыеВидыУпаковки(РезультатФункции, ОбщиеПараметрыЗагрузки);
...
Функция ЗагрузкаДокументовПоИзменениям(СтруктураПараметров) Экспорт
	...
	Результат = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").РезультатВзаимодействияСоСБИС(СписокИзменений.Количество());
...
Процедура ДанныеВодителей(ОбъектДок, ДляДокумента, ВодителиСОнлайна, ДопПараметры) Экспорт
	...
	Для Каждого Водитель Из ВодителиСОнлайна["Водитель"] Цикл 
		...
		Если МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").ЕстьРеквизитИлиСвойство(ОбъектДок, ИмяТаблицыВодителей) Тогда
		...
		Для ИндексДокументаЭПД = НачальныйИндекс По ДляДокумента.ДокументыЭПД.ВГраница() Цикл
			...
			СтруктураПутевогоЛиста.Вставить("КлючСтроки_ДокументыЭПД", СтруктураДокументаЭПД.КлючСтроки); 
...
Процедура ЗагрузитьДанныеЮрЛиц(ДляДокумента, ПодстановкиЮрЛиц, Роль, СтруктураЮрЛица = Неопределено) Экспорт
	...
	Для Каждого ПодстановкиЮрЛица Из МассивПодстановокЮрЛиц Цикл
		...
		ДанныеЮрЛица.КлючСтроки = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").НовыйКлючОсновнойСтроки(
...
Функция ПреобразоватьСтруктуруАдреса(ДанныеАдреса) Экспорт
	...
	Если ЗначениеЗаполнено(Текст) 
		...
		Иначе 
			Если ТипЗнч(Адрес) = Тип("Строка") Тогда
				Представление = МодульКода("УправлениеКонтактнойИнформациейСлужебный").ПредставлениеКонтактнойИнформации(Адрес);
			Иначе
...
Функция ДобавитьИЗаполнитьСтроку(ТаблицаДляДобавления, ИсточникДанныхДляЗаполнения, ДополнительныеПоля = "") Экспорт
	...
	Если ТипЗнч(ТаблицаДляДобавления) = Тип("Массив") Тогда
		...
		Поля = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").СтрРазделитьЭПД(ДополнительныеПоля, ",", Ложь);
...
Процедура ЗагрузитьДокумент(ПараметрыЗагрузки, ОбщиеПараметрыЗагрузки, Результат)
	...
	Исключение
		...
		ТекстОшибки = МодульКода("Saby_ТНОбщегоНазначенияСервер").ПричинаОшибкиИзЯдра(ИнфоОшибки.Описание);
...
Функция ДопПараметрыЗагрузкиВложения(ПараметрыЗагрузки = Неопределено, Результат = Неопределено)
	...
	Если ПараметрыЗагрузки <> Неопределено Тогда  
		...
		Иначе
			РезультатФункции.КлючСтроки = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").СтруктураКлючСтроки();
		КонецЕсли;
...
Процедура ЗаполнитьКомментарийОтрицательногоЗавершенияПоСобытию(ИзмененияПоДокументу)
	...
	КодСостояния = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(ИзмененияПоДокументу, "Состояние.Код");
...
Процедура ОбработатьИзмененияДокументов(СписокИзменений, Результат, СтруктураПараметров)
	...
	МодульКода("Справочники.Saby_ОпасныеГрузы").ДозаполнитьНовыеОпасныеГрузы(Результат, ОбщиеПараметрыЗагрузки);
	МодульКода("Справочники.Saby_ВидыУпаковки").ДозаполнитьНовыеВидыУпаковки(Результат, ОбщиеПараметрыЗагрузки);
...
Процедура ЗаполнитьПараметрыЗагрузкиПоСсылкеДляВО(Параметры, ДанныеСостояния)
	...
	Если ДанныеСостояния <> Неопределено Тогда
		Параметры.УИДСбис     = ДанныеСостояния.UID;
		Параметры.ЭтоЧерновик = Не ЗначениеЗаполнено(ДанныеСостояния.Состояние.ИдентификаторСБИС);
	КонецЕсли;
...
Функция ДанныеСпецРазрешений(СпецУсловияСОнлайна)
	...
	Для Каждого Условие Из СпецУсловияСОнлайна Цикл   
		...
		Если ЗначениеЗаполнено(СпецРазрешение) Тогда
			...
			НаименованиеОрганаВласти = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(Условие, "ОИВ.Наименование");
...
Процедура ЗаполнитьСтруктуруДокументаЭПД(ДляДокумента, СтруктураДокументаЭПД, ДокументЭПД)
	...
	Если Не ЗначениеЗаполнено(СтруктураДокументаЭПД.Наименование) Тогда
		...
		СтруктураДокументаЭПД.Наименование = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").СтрШаблонЭПД(
...
Процедура УдалитьДокументыЭПДПоКлючу(ОбъектДок, ИмяКлюча, ЗначениеКлюча)
	...
	Отбор.Вставить(ИмяКлюча, ЗначениеКлюча);
		 
	СтрокиДокументовЭПД = МодульКода("Saby_ОбщегоНазначенияКлиентСервер").НайтиСтрокиУниверсально(ОбъектДок.ДокументыЭПД, Отбор);
	Для Каждого СтрокаЭПД Из СтрокиДокументовЭПД Цикл
			
		// вычистим данные связанных юр лиц
		УдалитьЮрЛицПоКлючуДокументаЭПД(ОбъектДок, СтрокаЭПД.КлючСтроки);
		
		Индекс = ОбъектДок.ДокументыЭПД.Найти(СтрокаЭПД);
...
Процедура УдалитьЮрЛицПоКлючуДокументаЭПД(ОбъектДок, КлючСтрокиЭПД)
	  ...
	  Отбор = Новый Структура("КлючСтроки_ДокументыЭПД", КлючСтрокиЭПД);
	  
	  СтрокиЮрЛиц = МодульКода("Saby_ОбщегоНазначенияКлиентСервер").НайтиСтрокиУниверсально(ОбъектДок.ДанныеЮрЛиц, Отбор);
	  Для Каждого СтрокаЮрЛиц Из СтрокиЮрЛиц Цикл
		  
		  Индекс = ОбъектДок.ДанныеЮрЛиц.Найти(СтрокаЮрЛиц);		  
...
Процедура ЗаполнитьРеквизитыЮрЛица(Роль, СтруктураДанныхЮрЛица, ДанныеЮрЛицаОнлайн)
	...
	ВсеРоли = МодульКода("Saby_ТНОбщегоНазначенияСервер").ВсеЗначенияПеречисления("Saby_РолиКонтрагентов", Истина);
...
Процедура ЗаполнитьРеквизитыИностранногоЛица(СтруктураДанныхЮрЛица, Реквизиты)
	...
	СтруктураДанныхЮрЛица.НаименованиеОрганизации = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(
		Реквизиты, "ИностранноеЛицо.Наименование", "");
	
	Страна = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(Реквизиты, "ИностранноеЛицо.Страна");
...
Процедура ЗаполнитьРеквизитыАдресаЮрЛица(СтруктураДанныхЮрЛица, ДанныеЮрЛицаОнлайн)
	...
	ЮрАдрес = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(ДанныеЮрЛицаОнлайн, "Адрес.Юридический");
	Если Не ЗначениеЗаполнено(ЮрАдрес) Тогда
		ЮрАдрес = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(ДанныеЮрЛицаОнлайн, "Адрес");
    КонецЕсли;
...
Функция ПредставлениеАдресаИзСоответствия(Соответствие)
	...
	Представление = МодульКода("УправлениеКонтактнойИнформациейСлужебный").ПредставлениеКонтактнойИнформации(Структура);
...
Функция НеобходимоУдалитьСвязаннуюСЮрЛицомИнформацию(ОбъектДок, ДанныеЮрЛиц, Роль, ДопПараметры)
	...
	Иначе
		...
		Если Роль <> ЗначениеМетаданных("Saby_РолиКонтрагентов.СторонаДокумента") 
			И Не ДопПараметры.ЭтоПервоеОснование Тогда
			
			Отбор = Новый Структура;
			Отбор.Вставить("Роль", Роль);
			
			Строки = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").НайтиСтрокиУниверсально(ДанныеЮрЛиц, Отбор);
...
Функция ВалютаПоДаннымОнлайна(ДанныеГруз, ЗагрузкаСОнлайна, Знач ЗначениеПоУмолчанию = Неопределено) Экспорт
	...
	Валюта = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(ДанныеГруз, "Стоимость.Валюта");
...
Процедура ЗаполнитьОбщиеПараметрыОпасногоГруза(СтруктураОпасногоГруза, СтрокаОпасн)
	...
	СтруктураОпасногоГруза.ДатаИстеченияУдержания = МодульКода("Saby_ТНОбщегоНазначенияСервер").ПреобразоватьСтрокуВДатуЭПД(
...
Процедура ЗагрузитьДанныеГруза_ТранспортнаяНакладная(Груз, СтруктураГруза, ДляДокумента,
	...
	Стоимость = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(Груз, "Стоимость.ВключаяНДС");
	Если ЗначениеЗаполнено(Стоимость) Тогда
		Стоимость = МодульКода("Saby_ТНОбщегоНазначенияСервер").ЧислоИзСтроки(Стоимость);
	Иначе
		Стоимость = 0;
	КонецЕсли;
