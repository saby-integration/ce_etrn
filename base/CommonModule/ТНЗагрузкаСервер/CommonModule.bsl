...
//
// Возвращаемое значение:
//	Структура - описание результата загрузки
//
Функция ЗагрузитьПоРегламенту(ТипДокумента, Начало = Неопределено, Окончание = Неопределено) Экспорт
    ...
    Возврат ЗагрузкаДокументовПоИзменениям(СтруктураПараметров);
...
Процедура ДанныеЮрЛиц_ЗаполнитьИдентификаторыЭДО(ДанныеЮрЛиц, СоответствиеРолей, ДанныеСторон) Экспорт
	...
	Для Каждого КлючЗначение Из ДанныеСторон Цикл
		...
		НайденныеСтрокиЮрЛица = МодульESD_ОбщегоНазначенияКлиентСервер.НайтиСтрокиУниверсально(ДанныеЮрЛиц, ОтборПоРоли);
		Для Каждого СтрокаЮрЛица Из НайденныеСтрокиЮрЛица Цикл
			СтрокаЮрЛица.ИдентификаторЭДО = МодульESD_ОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(
...
Процедура ЗагрузитьОтветственного(СтруктураДляЗаполнения, ОтветственныйСОнлайна) Экспорт
	...
	Если ЗначениеЗаполнено(Телефоны) Тогда		
		...
		МаксИндекс = Телефоны.Количество() - 1; 
		МинИндекс = Макс(МаксИндекс - 9, 0); 	
		Пока МаксИндекс >= МинИндекс Цикл
			Телефон = Телефоны[МаксИндекс]["Значение"];
			МаксИндекс = МаксИндекс - 1;
			
			Если Не ЗначениеЗаполнено(Телефон) Тогда
				МинИндекс = Макс(МинИндекс - 1, 0);
				Продолжить;	
			КонецЕсли; 
...
Процедура SabyПослеЗагрузкиДанныхДокумента(ПараметрыЗагрузки, ОбщиеПараметрыЗагрузки, Результат)
	...
	Исключение
		...
		ТекстОшибки = МодульESD_ТНОбщегоНазначенияСервер.ПричинаОшибкиИзЯдра(ИнфоОшибки.Описание);
		
		Ошибки = МодульESD_ТНОбщегоНазначенияКлиентСервер.СтруктураОшибок(ПараметрыЗагрузки.ИмяМетаданных);
		МодульESD_ТНОбщегоНазначенияСервер.ДобавитьОшибкуПользователю(Ошибки, Неопределено, ТекстОшибки);
		КлючСоответствия = МодульESD_ТНОбщегоНазначенияКлиентСервер.КлючСсылка(
...
Процедура ЗагрузитьТекущиеЭтапыДокумента(ОбъектДок, ПараметрыЗагрузки)
	...
	Для Каждого ТекущийЭтап Из ТекущиеЭтапы Цикл
		...
		СтрокаТекущегоЭтапа.Наименование  = ТекущийЭтап["Наименование"];
		СтрокаТекущегоЭтапа.ТипТитула     = МодульESD_ОбщегоНазначенияСервер.РезультатФункцииТитулов(
			ПараметрыЗагрузки.ИмяМетаданных, "ЗначениеПоЭтапу", СтрокаТекущегоЭтапа.Наименование);
			
		Если ЭтоУниверсальноеРасширение() Тогда
			СтрокаТекущегоЭтапа.Вставить("Идентификатор", ТекущийЭтап["Идентификатор"]);
			ЗаполнитьЗначенияСвойств(ОбъектДок.ТекущиеЭтапы.Добавить(), СтрокаТекущегоЭтапа);
		Иначе
...
Процедура УстановитьДоступностьТекущихЭтапов(ОбъектДок, ПараметрыЗагрузки)
	...
	МодульЭПД_ = МодульКода("Документы." + ПараметрыЗагрузки.ИмяМетаданных);
	
	ДоступныеЭтапы = МодульЭПД_.ДоступныеОрганизацииЭтапы(
	...
	Для Каждого СтрокаТекущегоЭтапа Из ОбъектДок.ТекущиеЭтапы Цикл 
		...
		Попытка
			МодульЭПД_.ТекущиеЭтапы_ОпределитьДоступность(СтрокаТекущегоЭтапа, ОбъектДок);
		Исключение
			Продолжить;
		КонецПопытки;
	КонецЦикла;	
...
Функция SabyСписокИзменений_Загрузить(ПараметрыПолученияИзменений)
	...
	Попытка
		...
		РезультатФункции.ЕстьИзменения = SabyСписокИзменений_ЕстьЕще(СоответствиеИзменений["Навигация"]["ЕстьЕще"]);
	Исключение
		МодульESD_ТНОбщегоНазначенияСервер = МодульКода("ESD_ТНОбщегоНазначенияСервер");

		РезультатФункции.ЕстьИзменения = Ложь;
		ТекстОшибки = МодульESD_ТНОбщегоНазначенияСервер.ПричинаОшибкиИзЯдра(ИнформацияОбОшибке().Описание);
...
Функция SabyСписокИзменений_ЕстьЕще(Знач Значение)
...
Процедура ЗаполнитьРеквизитыИностранногоЛица(СтруктураДанныхЮрЛица, Реквизиты)
	...
	МодульESD_ОбщегоНазначенияКлиентСервер = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер");
	ИностранноеЛицо = МодульESD_ОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(Реквизиты, 
		"ИностранноеЛицо");
	Если Не ЗначениеЗаполнено(ИностранноеЛицо) Тогда
		Возврат;
	КонецЕсли;
	
	СтруктураДанныхЮрЛица.НаименованиеОрганизации = МодульESD_ОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(ИностранноеЛицо, 
		"Наименование", "");
	
	Идентификатор_ = МодульESD_ОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(ИностранноеЛицо, "Идентификатор");
	СтруктураДанныхЮрЛица.Вставить("ИностранноеЛицо_Идентификатор", Идентификатор_); 
	
	ЗаполнитьРеквизитыИностранногоЛица_Страна(СтруктураДанныхЮрЛица, ИностранноеЛицо);
КонецПроцедуры

Процедура ЗаполнитьРеквизитыИностранногоЛица_Страна(СтруктураДанныхЮрЛица, Данные)
	МодульESD_ОбщегоНазначенияКлиентСервер = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер");
	
	Страна = МодульESD_ОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(Данные, 
		"Страна");
	Если Не ЗначениеЗаполнено(Страна) Тогда
		Возврат;
	КонецЕсли;
	
	Если ТипЗнч(Страна) = Тип("Соответствие") Тогда
		Название_ = МодульESD_ОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(Страна, 
			"Название", "");
		СтруктураДанныхЮрЛица.СтранаРегистрации = СокрЛП(Название_);
		
		Код_ = МодульESD_ОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(Страна, 
			"Код", "");
		СтруктураДанныхЮрЛица.Вставить("СтранаРегистрации_Код", СокрЛП(Код_));
	Иначе
