...
Процедура ЗагрузитьТекущиеЭтапыДокумента(ОбъектДок, ПараметрыЗагрузки)
	...
	Для Каждого ТекущийЭтап Из ТекущиеЭтапы Цикл
		...
		Доступен = ДоступныеЭтапы[НаименованиеЭтапа] <> Неопределено
			Или ДоступныеЭтапыПоУзлуЭтап[НаименованиеЭтапа] <> Неопределено;
		
		Если НаименованиеЭтапа = "Выпуск в рейс" Тогда
			Доступен = Доступен И ТекущиеЭтапы.Количество() = 1;
		КонецЕсли;
