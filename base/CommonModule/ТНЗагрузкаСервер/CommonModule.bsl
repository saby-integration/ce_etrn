...
Процедура ЗагрузитьДокумент(ПараметрыЗагрузки, ОбщиеПараметрыЗагрузки, Результат)
	...
	ДанныеДокументаИзСБИС(ПараметрыЗагрузки, ОбщиеПараметрыЗагрузки, Результат);
...
Функция ДопПараметрыЗагрузкиВложения(ПараметрыЗагрузки = Неопределено, Результат = Неопределено)
	...
	Если ПараметрыЗагрузки <> Неопределено Тогда  
		...
		Иначе
			РезультатФункции.КлючСтроки = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").СтруктураКлючСтроки();
		КонецЕсли;
...
Функция СписокИзмененийДокументов(СтруктураПараметров)
	...
	Пока ЕстьИзменения Цикл
		...
		СтруктураИзменений = СледующиеИзмененияДокументов(ПараметрыПолученияИзменений);
		
		Для Каждого Изменение Из СтруктураИзменений.ИзмененияПоДокументам Цикл
...
Функция СледующиеИзмененияДокументов(ПараметрыПолученияИзменений)
...
Процедура ДобавитьИзменение(Изменения, ИзменениеСОнлайна, ПараметрыПолученияИзменений, ДопПараметры)
	...
	Если ИзмененияПоДокументу = Неопределено Тогда
		...
		ИзмененияПоДокументу.Вставить("Регламент",         ИзменениеСОнлайна["Регламент"]);
		...
		ИзмененияПоДокументу.Вставить("СсылкаНаАрхив",     ИзменениеСОнлайна["СсылкаНаАрхив"]);
		...
		ИзмененияПоДокументу.Вставить("КодПеревозки",      ИзменениеСОнлайна["КодПеревозки"]);
...
Процедура ДобавитьСобытия(СобытияОнлайна, ПараметрыИзменений, ИзмененияПоДокументу = Неопределено)
	...
	Для Каждого Событие Из СобытияОнлайна Цикл
		...
		МассивВложений = МодульТНОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(Событие, "Вложение", Новый Массив);
		Для Каждого Вложение Из МассивВложений Цикл
			...
			ИмяФайла = МодульТНОбщегоНазначенияКлиентСервер.СтрРазделитьЭПД(ИмяФайла, ".")[0];
			СобытиеСоответствие.Вставить("ИмяФайла", ИмяФайла);
		КонецЦикла;
...
Процедура ЗаполнитьКомментарийОтрицательногоЗавершенияПоСобытию(ИзмененияПоДокументу)
	...
	КодСостояния = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ЗначениеИзКоллекции(ИзмененияПоДокументу, "Состояние.Код");
