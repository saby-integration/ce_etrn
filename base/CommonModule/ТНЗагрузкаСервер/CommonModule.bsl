...
//
Функция ПараметрыЗагрузкиИзСбис(Знач СсылкаНаДокумент, context_params = Неопределено,
		УникальныйИдентификаторФормы = Неопределено, ЗагрузкаТребуется = Истина) Экспорт
...
Процедура ЗаполнитьПараметрыЗагрузкиПоСсылке(Параметры, ПовторноеПолучение)
	...
	НеобходимоОбновитьДанные = Параметры.ЗагрузкаТребуется И (Параметры.УИДСбис = "" Или Параметры.ЭтоЧерновик);
...
Функция СтруктураПрисоединенногоФайла(ПараметрыЗагрузки)
	...
	ИмяБезРасширения = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").СтрШаблонЭПД(
		ШаблонИмениФайла,
		ИмяМетаданных,
...
Функция НеобходимоУдалитьСвязаннуюСЮрЛицомИнформацию(ОбъектДок, ДанныеЮрЛиц, Роль, ДопПараметры)
	...
	Иначе
		...
		Если Роль <> ЗначениеМетаданных("Saby_РолиКонтрагентов.СторонаДокумента") 
			...
			Строки = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").НайтиСтрокиУниверсально(ОбъектДок.ДанныеЮрЛиц, Отбор);
