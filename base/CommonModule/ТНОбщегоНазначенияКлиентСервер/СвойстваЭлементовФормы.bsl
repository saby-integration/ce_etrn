...
Процедура ГрупповаяУстановкаСвойствЭлементовФормы(Форма, СвойстваЭлементовФормы) Экспорт
	...
	Для Каждого МассивСвойств Из СвойстваЭлементовФормы Цикл
		...
		#Если Сервер Тогда
			МассивИмен = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").СтрРазделитьЭПД(МассивСвойств.Ключ, "_");
		#Иначе			
			МассивИмен = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").СтрРазделитьЭПД(МассивСвойств.Ключ, "_");
		#КонецЕсли
...
//	ЗначениеОтмены - Произвольно - значение, при котором отменяется изменение значения свойства
//
Процедура ИзменитьСвойствоЭлементовФормы(Форма, Свойство, Значение, МассивЭлементов, ЗначениеОтмены = Неопределено) Экспорт
	Для Каждого Элемент Из МассивЭлементов Цикл
		НайденныйЭлемент = Форма.Элементы.Найти(Элемент);
		Если НайденныйЭлемент = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ТекущееЗначение = Форма.Элементы[Элемент][Свойство];
		Если ТекущееЗначение = ЗначениеОтмены Тогда
			Продолжить;
		КонецЕсли;
