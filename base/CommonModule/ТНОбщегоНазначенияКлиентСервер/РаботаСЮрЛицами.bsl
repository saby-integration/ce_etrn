...
// Обновляет контактные данные юр лица в объекте
//
// Параметры:
//  Объект - ДокументОбъект - объект для обновления данных
//  СтруктураДанныхЮрЛиц - Структура - структура данных о юр лице
//  СтрокаЮрЛиц - СтрокаТабличнойЧасти - Созданная или найденая строка табличной части объекта
//
//DynamicDirective
Процедура ОбновитьКонтактныеДанныеОбъекта(Объект, СтруктураДанныхЮрЛиц, СтрокаЮрЛиц) Экспорт
	
	МодульТНОбщегоНазначенияКлиентСервер = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер");

	Если Не МодульТНОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойство(Объект, "КонтактныеДанные") Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого СтрокаДляУдаления Из НайденныеСтроки Цикл
		Объект.КонтактныеДанные.Удалить(СтрокаДляУдаления);
	КонецЦикла;
	...
	Для Каждого СтруктураКонтактныхДанных Из СтруктураДанныхЮрЛиц.КонтактныеДанные Цикл
		// Не переносим контакт, если не заполнен "тип" и "значение"
		Если Не ЗначениеЗаполнено(СтруктураКонтактныхДанных.Тип) И Не ЗначениеЗаполнено(СтруктураКонтактныхДанных.Значение) Тогда
			Продолжить;
		КонецЕсли;
...
// Добавляет данные юр лица в объект
//
// Параметры:
//  Объект - ДокументОбъект - объект для обновления данных
//  СтруктураДанныхЮрЛиц - Структура - структура данных о юр лице
//  ПараметрыРеквизитаФормы - Структура - реквизит "ЭлементФормы" из СтруктураДанныхЮрЛиц 
//  СтрокаЮрЛиц - СтрокаТабличнойЧасти - Созданная или найденая строка табличной части объекта
//
//DynamicDirective
Процедура ДобавитьДанныеЮрЛицОбъекта(Объект, СтруктураДанныхЮрЛиц, ПараметрыРеквизитаФормы, СтрокаЮрЛиц) Экспорт
	
	МодульТНОбщегоНазначенияКлиентСервер = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер");
	
	КлючСтроки = СтрокаЮрЛиц.КлючСтроки;
	Если СтруктураДанныхЮрЛиц.Удалить Тогда
		Объект.ДанныеЮрЛиц.Удалить(СтрокаЮрЛиц);
		КлючСтроки = 0;
	КонецЕсли;
	
	Если ПараметрыРеквизитаФормы.ЭтоСтрокаТаблицы Тогда
		ИсходнаяСтрока = Объект[ПараметрыРеквизитаФормы.ИмяИсходнойТаблицы].НайтиПоИдентификатору(
...
// Находит или создает строку в ТЧ "ДанныеЮрЛиц" юр лица в объекте
//
// Параметры:
//  ПараметрыРеквизитаФормы - Структура - реквизит "ЭлементФормы" из СтруктураДанныхЮрЛиц 
//  Объект - ДокументОбъект - объект для обновления данных
//  Роль - ПеречислениеСсылка.ESD_РолиКонтрагентов - роль контрагента в ДО
//
// Возвращаемое значение:
//   СтрокаТабличнойЧасти - СтрокаТЧ "ДанныеЮрЛиц" существующая или созданная 
//
//DynamicDirective
Функция СтрокаЮрЛиц(ПараметрыРеквизитаФормы, Объект, Роль) Экспорт
	
	Если ПараметрыРеквизитаФормы.ЭтоСтрокаТаблицы Тогда
		Если ПараметрыРеквизитаФормы.ИдентификаторСтроки = Неопределено Тогда
			РезультатФункции = Объект.ДанныеЮрЛиц.Добавить();
			РезультатФункции.КлючСтроки = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").НовыйКлючОсновнойСтроки(
				Объект, "ДанныеЮрЛиц");
		Иначе
			РезультатФункции = Объект.ДанныеЮрЛиц.НайтиПоИдентификатору(ПараметрыРеквизитаФормы.ИдентификаторСтроки);
			Если РезультатФункции = Неопределено Тогда
				РезультатФункции = Объект.ДанныеЮрЛиц[ПараметрыРеквизитаФормы.ИдентификаторСтроки];				
			КонецЕсли;	
		КонецЕсли;
	Иначе
		Отбор = Новый Структура;
...
Процедура ОбновитьАдресаПогрузкиДоставки(Форма, ДопПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ДопПараметры.Значение) Тогда
		Если ТипЗнч(ДопПараметры.Значение) = Тип("Строка") Тогда
			Представление = ДопПараметры.Значение;
		Иначе
			Представление = ДопПараметры.Значение.Представление;
			Если ДопПараметры.Значение.Свойство("КонтактнаяИнформация") Тогда
				Значение  = ДопПараметры.Значение.КонтактнаяИнформация;
			Иначе
				#Если Сервер Или ТолстыйКлиентОбычноеПриложение Тогда
					Значение  = ЗначениеВСтрокуВнутр(ДопПараметры.Значение);
				#КонецЕсли
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если Найти(Форма.ИмяФормы, ОрганизацииЭПДКлиент()) > 0 Тогда
	...
	ОбновитьАдресаСторон(Форма, ФормаОбъект, ДопПараметры);
КонецПроцедуры
...
Процедура ОбновитьОсновнойНомерТелефона(Форма, ДопПараметры) Экспорт
	...
	ТипКИТелефон = ЗначениеМетаданных(ТипыКонтактнойИнформацииКлиент() + ".Телефон");
...
Функция РолиЮрЛиц(Форма, РолиСтрокой) Экспорт
	...
	Для Каждого РольСтрокой Из МассивРолей Цикл
		...
		Роль = ЗначениеМетаданных("ESD_РолиКонтрагентов." + РольСтрокой_);
		...
		Для Каждого СтрокаЮрЛиц Из НайденныеСтроки Цикл
			...
			Если РольСтрокой_ = "УполномоченноеЛицо" Тогда
		...
		Если НайденныеСтроки.Количество() = 0 И РольСтрокой_ <> "УполномоченноеЛицо" Тогда
			СтруктураДанных = СтруктураДанныхРолей();
...
Функция ДопПараметрыОбновленияКИ(Роли, ТаблицаЮрЛиц = Неопределено,
	...
	РезультатФункции.Вставить("ИгнорироватьЗаполненноеЗначение", Ложь);
...
//  ПараметрыРеквизита - Структура - см. функцию Saby_ТНОбщегоНазначенияКлиентСервер.ПараметрыРеквизитаФормы
//
// Возвращаемое значение:
//   Структура - структура данных юрлица
//
//DynamicDirective
Функция ДанныеЮрЛицаПоРоли(ДанныеЮрЛиц, РольСтрокой, ПараметрыРеквизита = Неопределено) Экспорт
	
	Если ПараметрыРеквизита = Неопределено Тогда
		ПараметрыРеквизитаФормы = ПараметрыРеквизитаФормы("", "");
	Иначе
		ПараметрыРеквизитаФормы = ПараметрыРеквизита;
	КонецЕсли;
	...
	#Иначе
		Роль = ЗначениеМетаданныхКлиент("ESD_РолиКонтрагентов." + РольСтрокой);
	#КонецЕсли
	
	НайденныеСтроки = НайтиСтрокиТаблицыЮрЛиц(ДанныеЮрЛиц, Роль, ПараметрыРеквизитаФормы);
	Если НайденныеСтроки.Количество() > 0 Тогда
		...
		Если ЗначениеЗаполнено(РезультатФункции.ЮрФизЛицо)
			И Не ЗначениеЗаполнено(РезультатФункции.ЮрФизЛицоСтрокой) 
		Тогда	
			РезультатФункции.ЮрФизЛицоСтрокой = ЮрФизЛицо_Имя(РезультатФункции.ЮрФизЛицо);		
		КонецЕсли;	
	Иначе
...
Функция ЮрФизЛицо_Имя(Значение) Экспорт
	Перем РезультатФункции;
	
	Если Значение = ЗначениеМетаданных("ESD_ЮрФизЛицо.ИндивидуальныйПредприниматель") Тогда
		РезультатФункции = "ИндивидуальныйПредприниматель";
	ИначеЕсли Значение = ЗначениеМетаданных("ESD_ЮрФизЛицо.ФизЛицо") Тогда
		РезультатФункции = "ФизЛицо";		
	ИначеЕсли Значение = ЗначениеМетаданных("ESD_ЮрФизЛицо.ЮрЛицоНеРезидент") Тогда
...
// Возвращает массив строк по роли
//
// Параметры:
//  ДанныеЮрЛиц - ТабличнаяЧасть - таблица с данными организаций
//  Роль - Перечисление.ESD_РолиКонтрагентов - роль, используемая для поиска
//  ПараметрыРеквизитаФормы - Структура - см. функцию Saby_ТНОбщегоНазначенияКлиентСервер.ПараметрыРеквизитаФормы
//
// Возвращаемое значение:
//   Массив - строки, найденные согласно параметру "Роль"
//
//DynamicDirective
Функция НайтиСтрокиТаблицыЮрЛиц(ДанныеЮрЛиц, Роль, ПараметрыРеквизитаФормы) Экспорт
	
	Если ПараметрыРеквизитаФормы.ЭтоСтрокаТаблицы Тогда
		НайденныеСтроки = Новый Массив;
		Если ПараметрыРеквизитаФормы.ИдентификаторСтроки <> Неопределено Тогда
			
			НужнаяСтрока = ДанныеЮрЛиц.НайтиПоИдентификатору(ПараметрыРеквизитаФормы.ИдентификаторСтроки);
			Если НужнаяСтрока = Неопределено Тогда
				НужнаяСтрока = ДанныеЮрЛиц[ПараметрыРеквизитаФормы.ИдентификаторСтроки];
			КонецЕсли;	
			
			НайденныеСтроки.Добавить(НужнаяСтрока);
...
//	ЗначениеПриОтсутствии - Строка - Значение, возвращаемое при отсутствии найденного наименования организации
//
// Возвращаемое значение:
//   Строка - представление юрлица
//
//DynamicDirective
Функция ПредставлениеЮрЛицаПоРоли(ДанныеЮрЛиц, РольСтрокой = "", ЗначениеПриОтсутствии = "Заполнить") Экспорт
	...
	Если Не ЗначениеЗаполнено(НаименованиеОрганизации) Тогда
		НаименованиеОрганизации = ЗначениеПриОтсутствии;
	КонецЕсли;
...
// Удаление "юр. лиц" согласно установленному отбору по роли
//
// Параметры:
//	ДанныеЮрЛиц - Таблица, ДанныеФормыКоллекция - таблица данных юр. лиц
//	КонтактныеДанные - Таблица, ДанныеФормыКоллекция, Неопределено - таблица контактных данных
//  Роль - Строка - имя роли для очистки
//       - ПеречислениеСсылка - значение перечисления
//
//DynamicDirective
Процедура УдалитьЮрЛицоПоРоли(ДанныеЮрЛиц, Знач КонтактныеДанные, Знач Роль) Экспорт
	Если ДанныеЮрЛиц.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Если ТипЗнч(Роль) = Тип("Строка") Тогда
		Роль = ЗначениеМетаданных("ESD_РолиКонтрагентов." + Роль);
	КонецЕсли;
	...
	СтруктураОтбора = Новый Структура("Роль", Роль);
	Для Каждого СтрокаДляУдаления Из НайтиСтрокиУниверсально(ДанныеЮрЛиц, СтруктураОтбора) Цикл
		УдалитьКонтактныеДанныеПоРоли(КонтактныеДанные, Роль, СтрокаДляУдаления.КлючСтроки);
		ДанныеЮрЛиц.Удалить(СтрокаДляУдаления);
	КонецЦикла;
КонецПроцедуры

// Удаление "контактных данных" согласно установленному отбору
//
// Параметры:
//	Таблица - Таблица, ДанныеФормыКоллекция - таблица контактных данных
//  Роль - Строка - имя роли для очистки
//       - ПеречислениеСсылка - значение перечисления
//	КлючСтроки - Число - ключ строки/связи с таблицей "ДанныеЮрЛиц"
//
//DynamicDirective
Процедура УдалитьКонтактныеДанныеПоРоли(Таблица, Знач Роль, КлючСтроки = 0) Экспорт
	...
	СтруктураОтбора_ = Новый Структура("Роль", Роль);
	Если КлючСтроки > 0 Тогда
		СтруктураОтбора_.Вставить("КлючСтроки_ДанныеЮрЛиц", КлючСтроки);
	КонецЕсли;

	УдалитьСтрокиПоОтбору(Таблица, СтруктураОтбора_);
КонецПроцедуры

// Удаление "ответственных лиц" согласно установленному отбору по роли
//
// Параметры:
//	Таблица - ТаблицаЗначений, ДанныеФормыКоллекция - таблица данных "ответственных лиц" 
//  Роль - Строка - имя роли для очистки
//       - ПеречислениеСсылка - значение перечисления
//
//DynamicDirective
Процедура УдалитьОтветственноеЛицоПоРоли(Таблица, Знач Роль) Экспорт
	Если Таблица.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
//  СтруктураОтбора - Структура - структура отбора данных таблицы
//
//DynamicDirective
Процедура УдалитьСтрокиПоОтбору(Таблица, СтруктураОтбора) Экспорт
	Для Каждого СтрокаДляУдаления Из НайтиСтрокиУниверсально(Таблица, СтруктураОтбора) Цикл
