...
Функция ДанныеЮрЛицаПоРоли(ДанныеЮрЛиц, РольСтрокой, ПараметрыРеквизита = Неопределено) Экспорт
	...
	Если НайденныеСтроки.Количество() > 0 Тогда
		...
		Если ЗначениеЗаполнено(РезультатФункции.ЮрФизЛицо)
			...
			Если РезультатФункции.ЮрФизЛицо = ЗначениеМетаданных("ESD_ЮрФизЛицо.ИндивидуальныйПредприниматель") Тогда
...
Процедура УдалитьСтрокиПоРоли(Таблица, РольСтрокой, ЭтоОтветственныеЛица = Ложь) Экспорт
	...
	Если ТипЗнч(РольСтрокой) = Тип("Строка") Тогда
		Если ЭтоОтветственныеЛица Тогда
			Роль = ЗначениеМетаданных("ESD_РолиОтветственных." + РольСтрокой);
		Иначе
			Роль = ЗначениеМетаданных("ESD_РолиКонтрагентов." + РольСтрокой);
		КонецЕсли;
	Иначе 
