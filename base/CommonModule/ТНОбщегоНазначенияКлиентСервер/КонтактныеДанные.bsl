...
Процедура ЗаполнитьПолеОсновногоТелефона(Форма, СтрокаЮрЛица, СтрокаТелефона)
	...
	ИначеЕсли СтрокаТелефона <> Неопределено И СтрокаТелефона.Основной Тогда
		...
		Форма.Элементы[СтрокаЮрЛица.ПолеОсновногоТелефона].Маска = "";
...
Процедура ОбновитьАдресаСторон(Форма, ФормаОбъект, ДопПараметры)
	...
	АдресЗначение = АдресЗначение(ДанныеКИ);	
	Если РольСтрокой = "Отправитель" Тогда	
		ОбновитьАдресОтправителя(ФормаОбъект, АдресЗначение, ДопПараметры);		
	ИначеЕсли РольСтрокой = "Получатель" Тогда	
		ОбновитьАдресПолучателя(ФормаОбъект, АдресЗначение, ДопПараметры);		
	Иначе 
		Возврат;			
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбновитьАдресОтправителя(ОбъектДокумента, АдресЗначение, ДопПараметры)
	Если Не ЕстьРеквизитИлиСвойство(ОбъектДокумента, "Погрузка_Адрес") Тогда
		Возврат;
	КонецЕсли;
	
	Если ДопПараметры.ИгнорироватьЗаполненноеЗначение И ЗначениеЗаполнено(ОбъектДокумента.Погрузка_Адрес) Тогда
		Возврат;
	КонецЕсли;
	
	ОбъектДокумента.Погрузка_Адрес        = АдресЗначение.Адрес;
	ОбъектДокумента.АдресПогрузкиЗначение = АдресЗначение.Значение;
КонецПроцедуры

//DynamicDirective

Процедура ОбновитьАдресПолучателя(ОбъектДокумента, АдресЗначение, ДопПараметры)
	Если Не ЕстьРеквизитИлиСвойство(ОбъектДокумента, "Отправитель_АдресДоставки") Тогда
		Возврат;
	КонецЕсли;
	                                 
	Если ДопПараметры.ИгнорироватьЗаполненноеЗначение И ЗначениеЗаполнено(ОбъектДокумента.Отправитель_АдресДоставки) Тогда
		Возврат;
	КонецЕсли;
	
	ОбъектДокумента.Отправитель_АдресДоставки = АдресЗначение.Адрес;
	ОбъектДокумента.АдресДоставкиЗначение     = АдресЗначение.Значение;	
КонецПроцедуры

//DynamicDirective

Функция АдресЗначение(ДанныеКИ)
	...
	ИначеЕсли ТипЗнч(ДанныеКИ) = Тип("Структура") Тогда
		РезультатФункции.Адрес    = ДанныеКИ.Представление;
		Если ДанныеКИ.Свойство("КонтактнаяИнформация") Тогда
			РезультатФункции.Значение = ДанныеКИ.КонтактнаяИнформация;
		Иначе
			#Если Сервер Или ТолстыйКлиентОбычноеПриложение Тогда
				РезультатФункции.Значение = ЗначениеВСтрокуВнутр(ДанныеКИ);
			#КонецЕсли
		КонецЕсли;	
	ИначеЕсли ТипЗнч(ДанныеКИ) = Тип("Строка") Тогда
		РезультатФункции.Адрес    = ДанныеКИ;
		РезультатФункции.Значение = "";
	Иначе
		РезультатФункции.Адрес    = ДанныеКИ.Адрес;
		РезультатФункции.Значение = ДанныеКИ.АдресСтруктура;
	КонецЕсли;
...
Функция СтруктураКИ(Ссылка, Вид) Экспорт
	...
	ПустойТип = ЗначениеМетаданных(ТипыКонтактнойИнформацииКлиент() + ".ПустаяСсылка");
