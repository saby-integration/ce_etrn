...
Функция ПараметрыЗаполненияНаОснованииINI(Объект = Неопределено, ИмяМетаданных = "") Экспорт
	...
	ШаблонИмениИНИ = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ШаблонИмениINI(ИмяМетаданных);
	
	КлючСтроки = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").СтруктураКлючСтроки();
	...
	РезультатФункции.Вставить("РольКонтрагента",           ЗначениеМетаданных("ESD_РолиКонтрагентов.ПустаяСсылка"));
