...
//
Процедура УдалитьСтрокиПоРоли(Таблица, РольСтрокой) Экспорт
	...
	Если ТипЗнч(РольСтрокой) = Тип("Строка") Тогда  
		Если ТипЗнч(Таблица[0].Роль) = Тип("ПеречислениеСсылка.Saby_РолиКонтрагентов") Тогда
			Роль = ПредопределенноеЗначение("Перечисление.Saby_РолиКонтрагентов." + РольСтрокой);
		Иначе
			Роль = ПредопределенноеЗначение("Перечисление.Saby_РолиОтветственных." + РольСтрокой);
		КонецЕсли;
	Иначе 
		Роль = РольСтрокой;
	КонецЕсли;
	...
	Отбор = Новый Структура;
	Отбор.Вставить("Роль", Роль);
	НайденныеСтроки = Saby_ТНОбщегоНазначенияКлиентСервер.НайтиСтрокиУниверсально(Таблица, Отбор);
...
Процедура СкопироватьДанныеЮрЛица(Объект, СтруктураПриемник, СтруктураИсточник) Экспорт
	...
	Если ТипЗнч(Объект.ДанныеЮрЛиц) = Тип("Массив") Тогда
		...
		СтрокаПриемник = СтруктураДанныхЮрЛица();
