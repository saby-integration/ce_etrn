...
Процедура ОбновитьАдресаПогрузкиДоставки(Форма, ДопПараметры) Экспорт
	...
	Если ДопПараметры.Роли = "Оформитель" Тогда 
		ФормаОбъект.МестоОтправления         = ДопПараметры.Значение.Представление;
		ФормаОбъект.МестоОтправленияЗначение = Saby_ТНОбщегоНазначенияСервер.АдресВФорматеОбмена(
			ДопПараметры.Значение.Значение);
		Возврат;
	КонецЕсли;
	
	Если ДопПараметры.Роли = "ПодачаТС" Тогда
		ФормаОбъект.ПодачаАдрес          = ДопПараметры.Значение.Представление;
		ФормаОбъект.ПодачаАдресСтруктура = Saby_ТНОбщегоНазначенияСервер.АдресВФорматеОбмена(
			ДопПараметры.Значение.Значение);
		Возврат;
	КонецЕсли;
	
	Если ДопПараметры.Роли = "КонечнаяТочка" Тогда
		ФормаОбъект.КонечнаяТочкаАдрес          = ДопПараметры.Значение.Представление;
		ФормаОбъект.КонечнаяТочкаАдресСтруктура = Saby_ТНОбщегоНазначенияСервер.АдресВФорматеОбмена(
			ДопПараметры.Значение.Значение);
		Возврат;
	КонецЕсли;
	...
	Если ДопПараметры.Роли = "АдресаПунктовВыгрузки" Или ДопПараметры.Роли = "АдресаПромежуточныхПунктов" Тогда
		...
		СтрокаАдреса.Значение  = ДопПараметры.Значение.Представление;
		СтрокаАдреса.Структура = Saby_ТНОбщегоНазначенияСервер.АдресВФорматеОбмена(
			ДопПараметры.Значение.Значение);
		Возврат;
	КонецЕсли;
	// - рефакторинг
	...
	Если СтрокаОтправителя <> Неопределено Тогда
		...
		Если ЕстьРеквизитИлиСвойство("Погрузка_Адрес", ФормаОбъект) Тогда
	...
	Для Каждого СтруктураРоли Из МассивРолей Цикл
		...
		Если ПустаяСтрока(Реквизит) Тогда
			...
			Если ЗначениеЗаполнено(СтруктураРоли.РольСтрокой)
				И ЕстьРеквизитИлиСвойство(СтруктураРоли.РольСтрокой, ФормаОбъект) Тогда
...
Процедура ИзменитьВидимостьДоступностьЭлементовФормы(Форма, Свойство, Значение, Массив) Экспорт
...
Процедура СформироватьНадписьПрицепыСтрокой(Форма)
	...
	Если Не ЕстьРеквизитИлиСвойство("ПрицепыСтрокой", Форма) Тогда
		Возврат;
	КонецЕсли;
