...
Процедура ЗагрузитьДанныеДинамическогоТитула(Форма, ИдентификаторТитула) Экспорт
	...
	Если ВыборкаДанных.Следующий() Тогда
		...
		ИмяМетаданных = Saby_ТНОбщегоНазначенияСервер.ИмяМетаданныхДокумента(Форма.СсылкаНаДокумент);
...
Процедура ЗагрузкаДанныхДокумента(ПараметрыЗагрузки) Экспорт
	...
	Для Каждого Титул Из МассивТитулов Цикл
		...
		ПредставленияТитула = Saby_ТНОбщегоНазначенияСервер.РезультатФункцииТитулов(
...
Функция ТекущиеЗначенияДокумента(ОбъектДок, ПараметрыЗагрузки) Экспорт
	...
	Если ПараметрыЗагрузки.ТекущийТитул <> Неопределено Тогда
		Если ТипЗнч(ПараметрыЗагрузки.ТекущийТитул) = Тип("Строка") Тогда
			РезультатФункции.Титул = Saby_ТНОбщегоНазначенияСервер.РезультатФункцииТитулов(
...
Функция ДанныеДляРегистраТитулов(Вложение, ИмяМетаданных, СтруктураДанныхДляРегистра) Экспорт
	...
	Иначе
		...
		Если ДляРегистра.ТипТитулаОтказа = Неопределено Тогда
			ТипТитулаОтказа = Saby_ТНОбщегоНазначенияСервер.РезультатФункцииТитулов(ИмяМетаданных, "ПустаяСсылка");
...
Функция ТаблицаДокументовЭПД() Экспорт
	...
	ТЗ.Колонки.Добавить("Номер",        Saby_ТНОбщегоНазначенияКлиентСервер.ОписаниеТипаСтрока(255));
	...
	ТЗ.Колонки.Добавить("Наименование", Saby_ТНОбщегоНазначенияКлиентСервер.ОписаниеТипаСтрока(255)); 
	ТЗ.Колонки.Добавить("Стороны",      Saby_ТНОбщегоНазначенияКлиентСервер.ОписаниеТипаСтрока(0));
...
Процедура ЗаполнитьДанныеСтрокиТаблицыДокументовЭПД(Строка, Данные, Тип, НомерСтроки = 0) Экспорт
	...
	Строка.Дата         = Saby_ТНОбщегоНазначенияСервер.ПреобразоватьСтрокуВДатуЭПД(Данные["Дата"]);
	Строка.ТипДок       = Тип;
...
Функция ДатаВложения(ДанныеТитула, Титул, СтруктураВложения = Неопределено) Экспорт
	...
	ИначеЕсли СтруктураВложения <> Неопределено Тогда
		...
		РезультатФункции = Saby_ТНОбщегоНазначенияСервер.ПреобразоватьСтрокуВДатуЭПД(СтруктураВложения["ДатаВремя"]);
	...
	ИначеЕсли Титул = Перечисления.Saby_ТипТитулаЭтрН.Погрузка Тогда
		...
		РезультатФункции = Saby_ТНОбщегоНазначенияСервер.ПреобразоватьСтрокуВДатуЭПД(ДатаСтрокой);
...
Процедура ЗагрузитьДанныеПоГрузам(ДляДокумента, ДанныеГруз, ЗагрузкаСОнлайна, ЭтоЭТрН) Экспорт
	...
	Для Каждого Груз Из Позиции Цикл 
		...
		СтруктураГруза.Вставить("ВидТары", НайтиСоздатьВидУпаковки(Груз["ТараКод"]));
...
Функция ОбъектЗагружаемогоДокумента(ПараметрыЗагрузки)
	...
	Иначе
		...
		ДатаВремяСоздания = Saby_ТНОбщегоНазначенияСервер.ПреобразоватьСтрокуВДатуЭПД(
...
Функция ДатаСобытияПоДокументу(Параметры, Знач ДатаПоУмолчанию = Неопределено)
	...
	Возврат Saby_ТНОбщегоНазначенияСервер.ПреобразоватьСтрокуВДатуЭПД(События[0]["ДатаВремя"]);
...
Функция ТекущийЭтапДокумента(ОбъектДок, ПараметрыЗагрузки)
	...
	Если РезультатФункции = Неопределено Тогда
		...
		Для Каждого ТекущийЭтап Из ТекущиеЭтапы Цикл
			ДатаСобытия = Saby_ТНОбщегоНазначенияСервер.ПреобразоватьСтрокуВДатуЭПД(ТекущийЭтап["ДатаСобытия"]);
...
Процедура ЗагрузитьТекущиеЭтапыДокумента(ОбъектДок, ПараметрыЗагрузки)
	...
	Для Каждого ТекущийЭтап Из ТекущиеЭтапы Цикл
		...
		ТипТитула = Saby_ТНОбщегоНазначенияСервер.РезультатФункцииТитулов(
...
Процедура ДобавитьПодстановкуТитула(ИзмененияПоДокументу, КодТитулаОнлайна, ЗначенияПодстановок, ИмяМетаданных)
	...
	Титул = Saby_ТНОбщегоНазначенияСервер.РезультатФункцииТитулов(ИмяМетаданных, "ЗначениеПоИмениПодстановки", КодТитулаОнлайна);
	Если Не ЗначениеЗаполнено(Титул) Тогда
		Возврат;
	КонецЕсли;
		
	ПредставленияТитула = Saby_ТНОбщегоНазначенияСервер.РезультатФункцииТитулов(ИмяМетаданных, "ПредставленияТитула", Титул);
...
Функция КлючСоответствияВложения(ДанныеТитула, ПредставленияТитула)
	...
	РезультатФункции = Saby_ТНОбщегоНазначенияСервер.ДанныеСоответствия(ДанныеТитула, "Файл.ИмяФайлаБезРасширения");
	Если РезультатФункции = Неопределено Тогда
		РезультатФункции = Saby_ТНОбщегоНазначенияСервер.ДанныеСоответствия(
...
Процедура ДобавитьВложения(Вложения, Событие, ИмяМетаданных)
	...
	Для Каждого Вложение Из ВложенияСОнлайна Цикл
		...
		Если ИмяМетаданных = "Saby_ТранспортнаяНакладная" Тогда
			ИмяФайлаСРасширением     = Saby_ТНОбщегоНазначенияСервер.ДанныеСоответствия(Вложение, "Файл.Имя", "");
	...
	Для Каждого Условие Из СпецУсловияСОнлайна Цикл   
		...
		Если ЗначениеЗаполнено(СпецРазрешение) Тогда
			...
			СтруктураСпецРазрешения.Вставить(
				"Дата",
				Saby_ТНОбщегоНазначенияСервер.ПреобразоватьСтрокуВДатуЭПД(СпецРазрешение["Дата"]));
...
Функция ДанныеВодителяДляЗагрузки(Водитель)
	...
	Если ЗначениеЗаполнено(ВУ) Тогда 
		...
		Если ЗначениеЗаполнено(ДанныеВодителя.ДатаВыдачи) Тогда 
			ДанныеВодителя.ДатаВыдачи = Saby_ТНОбщегоНазначенияСервер.ПреобразоватьСтрокуВДатуЭПД(ДанныеВодителя.ДатаВыдачи);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыЮрЛица(Роль, СтруктураДанныхЮрЛица, ДанныеЮрЛицаОнлайн)
	...
	ВсеРоли = Saby_ТНОбщегоНазначенияСервер.ВсеЗначенияПеречисления("Saby_РолиКонтрагентов", Истина);
...
Процедура ЗаполнитьОснованиеУполномоченногоЛица(СтруктураДанныхЮрЛица, ДанныеЮрЛицаОнлайн)
	...
	Если ЗначениеЗаполнено(Основание) И ЗначениеЗаполнено(Основание["Дата"]) Тогда
		...
		ЗаполнитьДанныеСтрокиТаблицыДокументовЭПД(НС, Основание, Тип, НомерСтроки);
	КонецЕсли;
	...
	СоответствиеОснований = НайтиСоздатьДокументЭПД(ТЗ);
...
Функция СтруктураФИО(Реквизиты)
	...
	Если Реквизиты["ФИО"] <> Неопределено Тогда
		Реквизиты = Saby_ТНОбщегоНазначенияСервер.ФИОИзСтроки(Реквизиты["ФИО"]);		
	КонецЕсли;	
...
Процедура ЗагрузитьОпасныеГрузы(ОпасныйГруз, ДляДокумента, КлючСтроки_Грузы)
	...
	Для Каждого СтрокаОпасн Из ОпасныйГруз Цикл
		...
		ОпасныйГруз = НайтиСоздатьОпасныйГруз(СтрокаОпасн);
		Если Не ЗначениеЗаполнено(ОпасныйГруз) Тогда
			Продолжить;
		КонецЕсли;
