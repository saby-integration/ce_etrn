...
Процедура ЗагрузитьДанныеОрганизации(
	...
	Если ТипЗнч(ДляДокумента.ДанныеЮрЛиц) = Тип("Массив") Тогда		
		ДляДокумента.ДанныеЮрЛиц.Добавить(СтруктураДанныхЮрЛица);
	Иначе 
		
		// прямая запись в таблицу
		НС = ДляДокумента.ДанныеЮрЛиц.Добавить();
...
Процедура ПолучитьДанныеДокументаИзСБИС(ПараметрыЗагрузки, ОбщиеПараметрыЗагрузки, Результат)
	...
	Параметры.Вставить("ДопПоля",       "ДопДействия,ТекущиеЭтапы,Подстановки,Расширение,Стороны");
...
Функция ЭтапДляПодписания(ОбъектДок, ПараметрыЗагрузки)
	...
	Если Saby_ТНОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойство(ОбъектДок, "ТекущиеЭтапы") Тогда
...
Процедура ЗагрузитьТекущиеЭтапыДокумента(ОбъектДок, ПараметрыЗагрузки)
	...
	Если Не Saby_ТНОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойство(ОбъектДок, "ТекущиеЭтапы") Тогда
		Возврат;
	КонецЕсли;
...
Функция ПараметрыПолученияИзменений(СтруктураПараметров)
	...
	РезультатФункции.Параметры.Вставить("ДопПоля",            "ДопДействия,ТекущиеЭтапы,Подстановки,Стороны");
...
Функция ОсновныеДанныеЛица(ДанныеЮрЛица, ЭтоСтронаДокумента)
	...
	РезультатФункции.ЭтоИП = Реквизиты["ИП"] <> Неопределено;
	РезультатФункции.ЭтоЮЛ = Реквизиты["ЮЛ"] <> Неопределено;
	РезультатФункции.ЭтоФЛ = Реквизиты["ФЛ"] <> Неопределено;
	
	РезультатФункции.ЭтоИнЛицо = Реквизиты["ИностранноеЛицо"] <> Неопределено;
...
Функция НовыйКонтрагент(ДанныеЮрЛица)
	...
	Если Saby_ТНОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойство(НовоеЮрЛицо, "ЮрФизЛицо") Тогда
	...
	Если ЭтоЮЛ Тогда 
		...
		ЕстьНаименование = Saby_ТНОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойство(РеквизитыЮЛ, "Наименование");
		
		НазваниеПолное = РеквизитыЮЛ["Название"];
		Если ТипЗнч(НазваниеПолное) = Тип("Соответствие") Тогда
			НазваниеПолное = НазваниеПолное["Полное"];
		Иначе 
			Если ЕстьНаименование Тогда
				НазваниеПолное = РеквизитыЮЛ["Наименование"];
			КонецЕсли;	
		КонецЕсли;
	...
	ИначеЕсли ЭтоИП Тогда 
		...
		Если Saby_ТНОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойство(НовоеЮрЛицо, "РегистрационныйНомер") Тогда
...
Процедура ЗаполнитьДополнительнуюИнформациюКонтрагента(НовоеЮрЛицо, ДанныеРеквизитов)
	...
	Если Saby_ТНОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойство(НовоеЮрЛицо, "ДополнительнаяИнформация") Тогда
		НовоеЮрЛицо.ДополнительнаяИнформация = Информация;
	ИначеЕсли Saby_ТНОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойство(НовоеЮрЛицо, "ДополнительноеОписание") Тогда
...
Процедура ЗаполнитьКонтактыЮрЛица(НовоеЮрЛицо, КонтактыСОнлайна)
	...
	Если ЗначениеЗаполнено(Почты) Тогда
		Для Каждого ЭлПочта Из Почты Цикл
			...
			ЭлПочтаКИ.Вид           = ВидыКИ.ЭлектроннаяПочта;
...
Функция ЕстьРеквизитДействуетС(КонтактнаяИнформация)
	...
	Возврат Saby_ТНОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойство(КонтактнаяИнформация, "ДействуетС");
...
Процедура ЗаполнитьДанныеПоГрузам(ОбъектДок, ДанныеГруз, ЗагрузкаСОнлайна) Экспорт
	...
	Для Каждого Груз Из Позиции Цикл 
		...
		Иначе 				
			// Заказ на перевозку
			ЗаполнитьДанныеГруза_ЗаказНаПеревозку(ОбъектДок, Груз, НС);			
		КонецЕсли;	
...
Процедура ЗаполнитьДанныеГруза_ЗаказНаПеревозку(ОбъектДок, Груз, НС)
	...
	НС.РаспределениеПоДлинеПлатформы = ?(РаспределениеПоДлине = "0", Истина, Ложь);
	...
	НС.ПогрузкаВодителем = ?(ПогрузкаВодителем = "0", Истина, Ложь);
	...
	Если ЗначениеЗаполнено(Заказчик) Тогда
		...
		РольЗаказчик = Перечисления.Saby_РолиКонтрагентов.Заказчик;		
		НовыйКлюч    = Saby_ТНОбщегоНазначенияКлиентСервер.НовыйКлючОсновнойСтроки(ОбъектДок, "ДанныеЮрЛиц");
				
		ЗагрузитьДанныеОрганизации(ОбъектДок, Заказчик, РольЗаказчик, НовыйКлюч);
		
		КИЗаказчик = Saby_ТНЗагрузкаСервер.КонтактнаяИнформация(Заказчик, РольЗаказчик);
		Для Каждого ЭлементКИ Из КИЗаказчик Цикл		
			
			НоваяСрокаКИ = ОбъектДок.КонтактныеДанные.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСрокаКИ, ЭлементКИ);
			НоваяСрокаКИ.КлючСтроки_ДанныеЮрЛиц = НовыйКлюч;
			
		КонецЦикла;
		
		// Строка данных юр лиц
		ДанныеЗаказчика = Saby_ТНОбщегоНазначенияКлиентСервер.ДанныеЮрЛицаПоРоли(ОбъектДок.ДанныеЮрЛиц, "Заказчик");
				
		НС.ЗаказчикСтрока         = ДанныеЗаказчика.НаименованиеОрганизации;
		НС.КлючСтроки_ДанныеЮрЛиц = НовыйКлюч;
		
		Если ЗначениеЗаполнено(Договор) Тогда 
			НС.Договор = ЗагрузитьОдиночныйДокументЭПД(Договор, Перечисления.Saby_ТипыДокумента.ДоговорПеревозки);
		КонецЕсли;
