...
Функция ТекстСообщенияКоманды(Ключ)
	...
	ЗагрузитьВСбис.Успешно            = "В SABY выгружено %1 документов";
	ЗагрузитьВСбис.ВыполненоСОшибками = "В SABY выгружено документов %1 из %2";
	...
	ТекстСообщения.Вставить("ВыгрузитьВСбис",                  ЗагрузитьВСбис);
	...
	ОбновитьДокументИзСбис.Успешно            = "Из SABY загружено %1 документов";
	ОбновитьДокументИзСбис.ВыполненоСОшибками = "Из SABY загружено документов %1 из %2";
...
Процедура ОповеститьПользователяОРезультатах(Форма, ИмяКоманды, РезультатОбработки) Экспорт
	...
	ИначеЕсли РезультатОбработки.Итог.Всего = 0 Тогда
		...
		Картинка = КартинкаИнтерфейса(Форма, "ESD_СостояниеОшибка");
		
	ИначеЕсли РезультатОбработки.Итог.Успех Тогда
		Картинка = КартинкаИнтерфейса(Форма, "ESD_Успешно32");
	...
	Если Картинка = Неопределено Тогда
		Картинка = КартинкаИнтерфейса(Форма, "ESD_Ошибка32");
	КонецЕсли;
	
	ПоказатьИнформативноеОповещениеПользователя(ТекстЗаголовка,,
		Текст,
		Картинка);
КонецПроцедуры
...
Процедура УспешноеВыполнениеПоУмолчанию(РезультатОбработки, ТекстСообщения, Текст, Картинка)
	...
	Если Найти(ТекстСообщения.Успешно, "%") > 0 Тогда
		Текст = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").СтрШаблонЭПД(
			ТекстСообщения.Успешно, Формат(РезультатОбработки.Итог.Количество, "ЧН=; ЧГ="));
	Иначе
		Текст = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").СтрШаблонЭПД(ТекстСообщения.Успешно);
	КонецЕсли;
