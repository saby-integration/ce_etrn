...
Процедура ОбработатьОшибкиИзФормыВыполнитьДействие(Форма, Ссылка, Параметр) Экспорт
	...
	Если ТипЗнч(Параметр.data) = Тип("Структура") Тогда
		Сообщение       = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСтруктуры(Параметр.data, "message");
		Детали          = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСтруктуры(Параметр.data, "detail");
		МассивСообщений = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСтруктуры(Параметр.data, "Результат");
	ИначеЕсли ТипЗнч(Параметр.data) = Тип("Соответствие") Тогда
		Сообщение       = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Параметр.data, "message");
		Детали          = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Параметр.data, "detail");
		МассивСообщений = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(Параметр.data, "Результат");
	Иначе
		Возврат;
	КонецЕсли;
...
Функция ДанныеОшибки(Строка, ЭтоСтруктура, ИндексОшибки)
	...
	Если ТипЗнч(ДокументСБИС) = Тип("Структура") Тогда
		СтруктураОшибки.ТекстДляОднойОшибки = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСтруктуры(
		...
		КонецЕсли;
	ИначеЕсли ТипЗнч(ДокументСБИС) = Тип("Соответствие") Тогда
		СтруктураОшибки.ТекстДляОднойОшибки = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
			ДокументСБИС, "detail.error.details", Неопределено);
		Если СтруктураОшибки.ТекстДляОднойОшибки = Неопределено Тогда
			СтруктураОшибки.ТекстДляОднойОшибки = МодульТНОбщегоНазначенияКлиентСервер.ДанныеСоответствия(
				ДокументСБИС, "message", "");
