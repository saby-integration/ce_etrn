...
Процедура ОбработатьОповещенияДО(Форма, ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Saby_ЗавершениеВыполнитьДействие" Тогда
		...
		ПослеВыполненияПереходаПоДокументам(Форма, Параметр);
...
Функция ПроверкаСобытияФормы(ИмяСобытия, ЭтоНужнаяФорма, ЭтоФормаСписка)
	...
	Возврат (ИмяСобытия = "Saby_ВыполнитьПереход" И ЭтоНужнаяФорма)
		Или (ИмяСобытия = "ESD_ВыполнитьПереходыПоДокументам" И ЭтоНужнаяФорма)
...
&НаКлиенте
Процедура ВыполнитьПереходыПоДокументам(Форма, МассивДанныхДокументов, ПроцедураПослеЗакрытия = Неопределено) Экспорт
	
	СгруппированныеПоТитулуДокументы = СгруппированныеПоТитулуДокументы(МассивДанныхДокументов);
	
	Для Каждого КлючЗначение Из СгруппированныеПоТитулуДокументы Цикл
		
		СписокДокументовСБИС = Новый Массив;
		СписокДокументов1С   = Новый Массив;
		
		Для Каждого ДанныеДокумента Из КлючЗначение.Значение Цикл
...
Функция СгруппированныеПоТитулуДокументы(МассивДанныхДокументов)
	...
	Для Каждого ДанныеДокумента Из МассивДанныхДокументов Цикл
		МассивДокументовТитула = РезультатФункции[ДанныеДокумента.ТипТитула];
		Если МассивДокументовТитула = Неопределено Тогда
			МассивДокументовТитула = Новый Массив;
			РезультатФункции.Вставить(ДанныеДокумента.ТипТитула, МассивДокументовТитула);
		КонецЕсли;
...
&НаКлиенте
Процедура ПослеВыполненияПереходаПоДокументам(Форма, РезультатВыполненияПерехода)
	
	Если ЭтоФормаСписка(Форма) Тогда
		ПослеВыполненияПереходаВФормеСписка(Форма, РезультатВыполненияПерехода);
	Иначе
		ПослеВыполненияПереходаВФорме(Форма, РезультатВыполненияПерехода);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеВыполненияПереходаВФорме(Форма, РезультатВыполненияПерехода)
	
	Если РезультатВыполненияПерехода.status = "error" Тогда
		ОбработатьОшибкиФормыВыполнитьДействие(Форма, РезультатВыполненияПерехода);
...
&НаКлиенте
Процедура ПослеВыполненияПереходаВФормеСписка(Форма, РезультатВыполненияПерехода)
...
&НаКлиенте
Процедура ОбработатьОшибкиФормыВыполнитьДействие(Форма, РезультатВыполненияПерехода)
	
	МодульТНОбщегоНазначенияКлиентСервер = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер");
	
	МассивСообщений = МодульТНОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(
		РезультатВыполненияПерехода, "data.Результат", Новый Массив);
	
	ВыполнитьНаСервере("ESD_ТНОбщегоНазначенияСервер.ЗаписатьОшибкиВыполненияВРегистрСостояний", МассивСообщений);
	
	Если Не ЭтоФормаСписка(Форма) Тогда
		
		Сообщение = МодульТНОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(РезультатВыполненияПерехода, "data.message");
		Детали    = МодульТНОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(РезультатВыполненияПерехода, "data.detail");
