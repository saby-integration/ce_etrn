...
Процедура ДобавитьДанныеЮрЛиц(Форма, СтруктураДанныхЮрЛиц, ПараметрыРеквизитаФормы)
	...
	// Переходим на простые типы для ВО
    ДозаполнитьДанныеЮрЛиц(СтруктураДанныхЮрЛиц);
...
Процедура ОбновитьКонтактныеДанные(Форма, СтруктураДанныхЮрЛиц, СтрокаЮрЛиц, ПараметрыРеквизитаФормы)
	...
	Для Каждого СтруктураКонтактныхДанных Из СтруктураДанныхЮрЛиц.КонтактныеДанные Цикл
		...
		// переход на простые типы
		ДозаполнитьТипыКонтактнойИнформации(СтруктураКонтактныхДанных);
...
#Область ПереходФормыОрганизацииЭПД_на_ВО3

&НаКлиенте
Процедура ДозаполнитьДанныеЮрЛиц(СтруктураДанныхЮрЛиц)
	
	Если СтруктураДанныхЮрЛиц.Свойство("РольСтрокой") Тогда
		Роль = ЗначениеМетаданных("Saby_РолиКонтрагентов." + СтруктураДанныхЮрЛиц.РольСтрокой);
		СтруктураДанныхЮрЛиц.Вставить("Роль", Роль);
	КонецЕсли;
	
	Если СтруктураДанныхЮрЛиц.Свойство("ЮрФизЛицоСтрокой")
		И ЗначениеЗаполнено(СтруктураДанныхЮрЛиц.ЮрФизЛицоСтрокой) Тогда 
		
		ЮрФизЛицо = ЗначениеМетаданных("Saby_ЮрФизЛицо." + СтруктураДанныхЮрЛиц.ЮрФизЛицоСтрокой); 
		СтруктураДанныхЮрЛиц.Вставить("ЮрФизЛицо", ЮрФизЛицо);
...
&НаКлиенте
Процедура ДозаполнениеПараметровФормыОрганизацииЭПД(ПараметрыФормы)
	
	Если ЗначениеЗаполнено(ПараметрыФормы.ЮрФизЛицо)
		И Не ЗначениеЗаполнено(ПараметрыФормы.ЮрФизЛицоСтрокой) Тогда
		
		ИменаПеречисления = ВыполнитьНаСервере("Saby_ТНОбщегоНазначенияСервер.ВсеЗначенияПеречисления", "Saby_ЮрФизЛицо", Истина);
		ПараметрыФормы.ЮрФизЛицоСтрокой = ИменаПеречисления[ПараметрыФормы.ЮрФизЛицо];
...
&НаКлиенте
Процедура ДозаполнитьТипыКонтактнойИнформации(КонтактныеДанные)
	
	Если ЗначениеЗаполнено(КонтактныеДанные.Тип) 
		И ТипЗнч(КонтактныеДанные.Тип) = Тип("Строка") Тогда  		
		КонтактныеДанные.Тип = ЗначениеМетаданных("ТипыКонтактнойИнформации." + КонтактныеДанные.Тип);		
	КонецЕсли;	
	
КонецПроцедуры

#КонецОбласти // ПереходФормыОрганизацииЭПД_на_ВО3 
