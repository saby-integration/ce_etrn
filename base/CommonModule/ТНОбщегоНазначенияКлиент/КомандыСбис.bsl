...
Функция НужноЗагружатьОпасныеГрузы(ПараметрыЗагрузки)
	...
	ПозицииГруза = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(ПервыйТитул, "Груз.Позиция");
	
	Если ЗначениеЗаполнено(ПозицииГруза) Тогда
		Для Каждого ПозицияГруза Из ПозицииГруза Цикл
			ОпасныйГруз = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(ПозицияГруза, "ОпасныйГруз");
...
Функция НужноЗагружатьВидыУпаковки(ПараметрыЗагрузки)
	...
	Если ЗначениеЗаполнено(ПозицииГруза) Тогда
		Для Каждого ПозицияГруза Из ПозицииГруза Цикл
			ТараКод = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(ПозицияГруза, "ТараКод");
...
Функция ПодстановкаПервогоТитула(ПараметрыЗагрузки)
	...
	ПервыйТитул = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(
...
Процедура ОповеститьПослеОбработкиРезультатаКоманды(ИмяКоманды, РезультатОбработки, Источник)
	...
	Для Каждого Элемент Из МассивСсылок Цикл
		...
		ИначеЕсли ИмяКоманды = "ВыгрузитьВСбис" Или ИмяКоманды = "ЗагрузитьВСбисДинамическийТитул" Тогда
