...
Процедура ОбменДаннымиСБИСИзФормыСписка(Форма, ИмяКоманды, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ЭтоВыгрузка Или ЭтоОбновитьЭтап Тогда
		МассивДокументов = МодульТНОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(ДополнительныеПараметры, "МассивДокументов");
		Если МассивДокументов = Неопределено Тогда
			МассивДокументов = ДокументыДляОбработки(Форма, ЭтоВыгрузка);
		КонецЕсли;
...
//	ДополнительныеПараметры - Структура, Неопределено - дополнительные параметры открытия документа
//
&НаКлиенте
Процедура ОткрытьДокументВSABY(СсылкаНаДокумент, ТипДокумента = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	Результат = ВыполнитьНаСервере("ESD_ТНОбщегоНазначенияСервер.ДанныеДляОткрытияДокументаВСБИС",
		СсылкаНаДокумент, ТипДокумента, ДополнительныеПараметры);
...
#Область include_etrn_base_CommonModule_ТНОбщегоНазначенияКлиент_ПрограммныйИнтерфейс
#КонецОбласти // include_etrn_base_CommonModule_ТНОбщегоНазначенияКлиент_ПрограммныйИнтерфейс
...
Процедура ОшибкиПриАктивизацииСтроки(Форма, СоответствиеТаблицФормыИОбъекта = Неопределено) Экспорт
	...
	Если Форма.ТекущийЭлемент.Имя <> "ТаблицаОшибок" Или Не ЗначениеЗаполнено(ТекущиеДанные.РеквизитОбъекта) Тогда
		Возврат;
	КонецЕсли;
	...
	Если Не ЭтоТаблицаФормы(Форма, ИмяТаблицыФормы, ИмяТаблицыОбъекта)
		Или ТипЗнч(ЭлементФормы.Родитель) <> Тип("ТаблицаФормы") 
	Тогда	
...
//	ЭтоВыгрузка - Булево - признак обработки выгрузки
// Возвращаемое значение:
//	Массив - ссылки на документы для обработки
//
&НаКлиенте
Функция ДокументыДляОбработки(Форма, ЭтоВыгрузка = Ложь) Экспорт
	МодульESD_ОбщегоНазначенияКлиентСервер = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер");
	
	Список_ = Форма.Элементы.Список;
	ВыделенныеСтроки = Список_.ВыделенныеСтроки;
	
	Массив = Новый Массив;
	Для Каждого Строка Из ВыделенныеСтроки Цикл
		Если ЭтоВыгрузка Тогда
			ДанныеСтроки_ = Список_.ДанныеСтроки(Строка);
			Если МодульESD_ОбщегоНазначенияКлиентСервер.ДокументооборотЗавершен(ДанныеСтроки_.СостояниеИндексКартинки) Тогда
				Продолжить;
			КонецЕсли;
			
			Если Форма.АктивныйЭтапСтрокой = "" Тогда
...
#Область include_etrn_base_CommonModule_ТНОбщегоНазначенияКлиент_ОсновнойФункционал_ЗаполнениеНаОсновании_СлужебныйПрограммныйИнтерфейс
#КонецОбласти // include_etrn_base_CommonModule_ТНОбщегоНазначенияКлиент_ОсновнойФункционал_ЗаполнениеНаОсновании_СлужебныйПрограммныйИнтерфейс

#Область include_etrn_base_CommonModule_ТНОбщегоНазначенияКлиент_СлужебныйПрограммныйИнтерфейс
#КонецОбласти // include_etrn_base_CommonModule_ТНОбщегоНазначенияКлиент_СлужебныйПрограммныйИнтерфейс
...
&НаКлиенте
Процедура ОткрытьДокументРасширенияЭПД(СсылкаНаДокумент, ТипДокумента) Экспорт
...
&НаКлиенте
Процедура ОткрытьСсылкуИзСпискаДокументов(Результат)
...
Процедура ОткрытьПервыйДокументИзСоответствия(СоответствиеДокументов) Экспорт
	Для Каждого КлючЗначение Из СоответствиеДокументов Цикл	
		...
		Если ЕстьСсылкаНаДокумент(СсылкаНаДокумент, СтруктураДанныхОшибок) Тогда
...
Процедура УстановитьАктивнуюСтрокуВОсновнойТаблице(Форма, ИмяТаблицы, ИндексСтроки)	
	// TODO Переписать установку текущей строки при проверки ФЛК
	Если ИмяТаблицы = "ПартииГруза" Или ИмяТаблицы = "Услуги" Тогда
		ЭлементТаблицы = Форма.Элементы.Найти(ИмяТаблицы);
		ФормаОбъект    = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ФормаОбъект(Форма);
		Если ЭлементТаблицы <> Неопределено И ИндексСтроки < ФормаОбъект[ИмяТаблицы].Количество() Тогда
			ЭлементТаблицы.ТекущаяСтрока = ФормаОбъект[ИмяТаблицы][ИндексСтроки].ПолучитьИдентификатор();
		КонецЕсли;
	КонецЕсли;
...
#Область include_etrn_base_CommonModule_ТНОбщегоНазначенияКлиент_СлужебныеПроцедурыИФункции
#КонецОбласти // include_etrn_base_CommonModule_ТНОбщегоНазначенияКлиент_СлужебныеПроцедурыИФункции
