...
//
Процедура ОткрытьДокументВSABY(СсылкаНаДокумент, ВыгрузкаНаКлиенте, ТипДокумента = Неопределено) Экспорт
	
	Результат = ВыполнитьНаСервере(
		"ESD_ТНОбщегоНазначенияСервер.ДанныеДляОткрытияДокументаВСБИС", СсылкаНаДокумент, Ложь, ТипДокумента);
...
Процедура ЗагрузитьАрхивДокумента(Форма, ЭтоФормаСписка) Экспорт
	...
	Если Не ЭтоФормаСписка И Не ДополнительныеПараметры.Свойство("ОшибкаЗагрузкиАрхива") И ДополнительныеПараметры.Свойство("СсылкаНаДокумент") Тогда	
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ВладелецФайла", Форма.Объект.Ссылка);
		ПараметрыФормы.Вставить("ЗаголовокФормы", МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").СтрШаблонЭПД("Присоединенные файлы: %1", Форма.Объект.Ссылка));
...
Процедура ВыбратьТипДокументаОснования(Форма, ПараметрыФормы = Неопределено) Экспорт
	...
	ИспользоватьНовыйРедактор = ВыполнитьНаСервере(
		"ESD_ТНОбщегоНазначенияСервер.ЗначениеКонстанты", "ESD_ИспользоватьНовыйРедактор", Ложь);
	
	ПараметрыАвторизации = АвторизацияSaby();
	Если Не ЗначениеЗаполнено(ПараметрыАвторизации) И Не ИспользоватьНовыйРедактор Тогда
		Возврат;
	КонецЕсли;
	
	Если Форма.ТипыОснований.Количество() = 0 Тогда
		
		Если ИспользоватьНовыйРедактор Тогда
			Сообщение = Новый СообщениеПользователю;
		...
		Если ЭтоФормаСписка(Форма) Тогда 
			ДопПараметры.Вставить("Ссылка", ПараметрыФормы.ТипДокумента);
		Иначе
			ДопПараметры.Вставить("Ссылка", Форма.Объект.Ссылка);
		КонецЕсли;	
			
		ОткрытьФорму("ОбщаяФорма." + ВыборТиповЭПДКлиент(), ДопПараметры, Форма);
	...
	Иначе
		...
		ДопПараметры.Вставить("ИспользоватьНовыйРедактор", ИспользоватьНовыйРедактор);
...
Процедура ПослеВыбораЗначенияДокументаОснования(Значение, ДопПараметры) Экспорт
	...
	Если ДопПараметры.ИспользоватьНовыйРедактор 
		И ЭтоУниверсальноеРасширение() Тогда
		...
		Если РезультатЗаполненияНаОсновании.ОК Тогда
			ПоказатьОповещениеПользователя("Заполнение выполнено", , , КартинкаИнтерфейса(Форма, "ESD_Успешно32"));
		Иначе
			
			ДополнительныеПараметры = Новый Структура;
			ДополнительныеПараметры.Вставить("РезультатЗаполненияНаОсновании", РезультатЗаполненияНаОсновании);
...
//
Процедура ОткрытьИныхПолучателей(Форма, ТолькоПросмотр, ЗаписыватьФорму) Экспорт
	...
	ПараметрыОткрытияФормыОбработки = ПараметрыОткрытияФормыОбработки(
		"ОбщаяФорма." + ИныеПолучателиДокументаЭПДКлиент(), ПараметрыФормы, Форма);
...
&НаКлиенте
Процедура ESD_ПриНачалеРаботы() Экспорт
	...
	Если Кеш["ПройденаПервичнаяИнициализация"] = Истина Тогда
		ESD_ПриАвторизации(Кеш);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ESD_ПриАвторизации(Кеш = Неопределено) Экспорт
	Если Кеш = Неопределено Тогда
		Кеш = МодульКода("ESD_ТНОбщегоНазначенияСервер").ESD_ПараметрыРаботыФункционала();	
	КонецЕсли;
	
	Если Кеш["ЕстьДоступКЯдру"] = Истина Тогда
...
Функция СоздатьПечатнуюФорму(СсылкаНаДокумент, Форма, ДополнительныеПараметры) Экспорт
	Если Форма.ЭтоВО Тогда
		...
		context_params = ДополнительныеПараметры.context_params;
