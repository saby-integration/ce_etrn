...
Процедура ОткрытьДокументВСБИС(СсылкаНаДокумент, ВыгрузкаНаКлиенте) Экспорт
	...
	Результат = ВыполнитьНаСервере("ESD_ТНОбщегоНазначенияСервер.ДанныеДляОткрытияДокументаВСБИС", СсылкаНаДокумент);
...
Процедура ESD_ПриНачалеРаботы() Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыАвторизации) Тогда
		...
		Модуль_ESD_Сервер.УстановитьЗначениеКонстанты(
			"ESD_ПройденаПервичнаяИнициализация", Истина);
...
Процедура ОткрытьПервыйДокументИзСоответствия(СоответствиеДокументов) Экспорт
	...
	Для Каждого КлючЗначение Из СоответствиеДокументов Цикл
		...
		ИмяФормы = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").СтрШаблонЭПД(
