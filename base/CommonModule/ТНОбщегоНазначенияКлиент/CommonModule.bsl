...
Процедура ЗагрузитьАрхивДокумента(Форма, ЭтоФормаСписка) Экспорт
	...
	ПроцедураОткрытьПечатнуюФорму = Новый ОписаниеОповещения(
		"ПередВыгрузкойАрхива",
		МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент"),
		ДополнительныеПараметры);
	
	ПроверитьМодифицированностьИПродолжитьВыполнение(ПроцедураОткрытьПечатнуюФорму);
	
	Если НЕ ЭтоФормаСписка И НЕ ДополнительныеПараметры.Свойство("ОшибкаЗагрузкиАрхива") Тогда
...
&НаКлиенте
Процедура ESD_ПриАвторизации() Экспорт
...
Процедура ПередВыгрузкойАрхива(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ЭтоФормаСписка Тогда
		МассивСсылокНаДокумент = ДополнительныеПараметры.Форма.Элементы.Список.ВыделенныеСтроки;
