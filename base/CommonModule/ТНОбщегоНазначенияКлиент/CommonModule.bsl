...
//  ОбменНаКлиенте - Булево - признак определяющий источник запросов на внешний сервис
//
Процедура СоздатьИОткрытьПечатнуюФорму(СсылкаНаДокумент, УникальныйИдентификаторФормы, ОбменНаКлиенте) Экспорт
	
	Если ОбменНаКлиенте Тогда
		
		context_params = ВыполнитьНаСервере("Saby_Core.ПроверитьНаличиеПараметровПодключения", Неопределено);
		ВыгрузитьЧерновикНаКлиенте(СсылкаНаДокумент, УникальныйИдентификаторФормы, context_params);
		
		Результат = ВыполнитьНаСервере("Saby_ТНОбщегоНазначенияСервер.ДанныеПечатнойФормыДляОткрытия",
			СсылкаНаДокумент, Не ОбменНаКлиенте, УникальныйИдентификаторФормы);
		...
		Результат.ПараметрыЗагрузки.ОбъектОбработки = ТранспортВО.Транспорт;
		Результат.АдресДвоичныхДанных = АдресДвоичныхДанныхФайла(Результат.ПараметрыЗагрузки);
...
&НаКлиенте
Процедура ВыгрузитьЧерновикНаКлиенте(СсылкаНаДокумент, УникальныйИдентификатор, context_params)
	...
	Если Не ЗначениеЗаполнено(СсылкаНаДокумент.Состояние.ИдентификаторСБИС) Тогда
		...
		Если Не РезультатВыгрузки.Итог.Успех Тогда			
			 ОбработатьРезультатКоманды(РезультатВыгрузки, "ВыгрузитьЧерновик", УникальныйИдентификатор);
			 Возврат;			 
		КонецЕсли;
		
		РезультатВыгрузки.Выполнение.Параметры[0].ПараметрыВыгрузки.Ссылка = СсылкаНаДокумент;
		
		ПродолжитьВыполнениеНаКлиенте(РезультатВыгрузки, "ВыгрузитьВСбис");
