	...
	Если ЭтоВыгрузка Или ЭтоОбновитьЭтап Тогда
		...
		Для Каждого СсылкаНаДокумент Из МассивДокументов Цикл
			...
			Иначе
				...
				ПараметрыКоманды.ИзРасширения = Форма.Открыта();
...
Процедура ПослеВыбораЗначенияДокументаОснования(Значение, ДопПараметры) Экспорт
	...
	Иначе
		// если ни чего не выбрали то выходим
		Возврат;		
	КонецЕсли;
...
Процедура ESD_ПриНачалеРаботы() Экспорт
	...
	Модуль_ESD_Сервер = МодульКода("ESD_ТНОбщегоНазначенияСервер");
	
	ПройденаПервичнаяИнициализация = Модуль_ESD_Сервер.ЗначениеКонстанты("ESD_ПройденаПервичнаяИнициализация", Истина);
	
	Если Не ПройденаПервичнаяИнициализация Тогда
		// ранее была сделана авторизация, считаем что инициализация пройдена
		Модуль_ESD_Сервер.УстановитьЗначениеКонстанты(
			"ESD_ПройденаПервичнаяИнициализация", ЗначениеЗаполнено(ПараметрыАвторизации));
		
		// включаем функционал
		Модуль_ESD_Сервер.УстановитьЗначениеКонстанты(
			"ESD_ИспользоватьТранспортнаяНакладная", Истина);
		
		Модуль_ESD_Сервер.УстановитьЗначениеКонстанты(
			"ESD_ИспользоватьПутевойЛист", Истина);
		
		Модуль_ESD_Сервер.УстановитьЗначениеКонстанты(
			"ESD_ИспользоватьЗаказНаПеревозку", Истина);
	КонецЕсли;
	
	Модуль_ESD_Сервер.ESD_ПриНачалеРаботыСервер();
	
	МодульКода("ESD_ТНОбщегоНазначенияСервер").ОбновитьНастройкиДоТекущегоРелиза();
...
Процедура ОбновитьДокументыСБИС(Форма, ИмяКоманды, ИмяМетаданных) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДатаОбновления) Тогда
		МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ОбновитьДокументыЗаПериод(Форма, ИмяКоманды);
	Иначе
