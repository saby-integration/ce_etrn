...
Функция АвторизацияSaby(Команда, ЭтаФорма) Экспорт
	...
	context_params	= МодульКодаКлиент("Saby_Core").ПроверитьНаличиеПараметровПодключения();
	...
	Если ЭтоЗагрузкаЧтениеИлиОбновление(ИмяКоманды) Тогда
		...
		Если context_params <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(МассивДокументов) Тогда 
				МассивДокументов = МодульКодаКлиент("ОбщегоНазначенияКлиентСервер").ЗначениеВМассиве(МассивДокументов[0]);
			КонецЕсли;
...
Процедура ВыполнитьКомандуВФоне(Форма, СтруктураПараметров) Экспорт
	...
	ПараметрыОжидания = МодульКодаКлиент("ДлительныеОперацииКлиент").ПараметрыОжидания(Форма); 
	...
	МодульКодаКлиент("ДлительныеОперацииКлиент").ОжидатьЗавершение(ДлительнаяОперация, Оповещение, ПараметрыОжидания);
...
Функция ЕстьСсылкаНаДокумент(СсылкаНаДокумент, СтруктураДанныхОшибок)
	...
	Возврат ЗначениеЗаполнено(СсылкаНаДокумент)
		И ТипЗнч(СсылкаНаДокумент) = Тип("ДокументСсылка." + СтруктураДанныхОшибок.ИмяМетаданных);
