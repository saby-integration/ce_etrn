...
Процедура ПроверитьМодифицированностьИПродолжитьВыполнение(ПроцедураДляВыполнения, ЗаписыватьБезВопроса = Ложь) Экспорт
	...
	Если Не НеобходимоЗаписатьФорму(Форма) ИЛИ ЗаписыватьБезВопроса Тогда		
		ПослеВопросаОЗаписиФормы(КодВозвратаДиалога.ОК, ДополнительныеПараметры);		
	Иначе
...
Процедура ПослеВопросаОЗаписиФормы(Результат, ДополнительныеПараметры) Экспорт
	...
	Если НеобходимоЗаписатьФорму(Форма) И Результат = КодВозвратаДиалога.ОК Тогда
		Попытка
			Если Не Форма.Записать() Тогда
				Возврат;
			КонецЕсли;
		Исключение
			ИнформацияОбОшибке_ = ИнформацияОбОшибке();
			СообщитьПользователю(КраткоеПредставлениеОшибки(ИнформацияОбОшибке_));
			Возврат;
		КонецПопытки;
	КонецЕсли;
...
Функция НеобходимоЗаписатьФорму(Форма)
	...
	Если МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ЕстьРеквизитИлиСвойство(Форма, "ЭтоВО") И Форма.ЭтоВО Тогда
