...
Функция СформироватьСтруктуруДанныхДокумента(ПараметрыВыгрузки, Результат) Экспорт
	...
	Если Результат.Ошибки.Количество() > 0 И Результат.Ошибки[КлючСоответствия] <> Неопределено Тогда
		...
		ТекстОшибки = "";
		Для Каждого СтруктураОшибки Из ДанныеОшибки.СписокОшибок Цикл
			ТекстОшибки = ТекстОшибки + СтруктураОшибки.ТекстДляОднойОшибки;
		КонецЦикла;
...
Функция ДанныеДляВыгрузки(ДанныеДокумента, ПараметрыВыгрузки, ОбщиеПараметрыВыгрузки = Неопределено) Экспорт
	...
	ДобавитьИнформационныеПоля(ДокументСтруктура, ПараметрыВыгрузки);	
...
Функция ВыгрузитьПечатнуюФорму(ПараметрыВыгрузки, context_params = Неопределено) Экспорт
	...
	ДокументВыгрузки = Новый Структура("ИнформационноеПоле", СформироватьИнформационныеПоля(ПараметрыВыгрузки));
...
Процедура ЗаполнитьНашуОрганизацию(Вложение, ПараметрыВыгрузки, ОбщиеПараметрыВыгрузки = Неопределено) Экспорт
	...
	Составитель     = Модуль_ESDКлиентСервер.ЗначениеИзКоллекции(ДанныеТитула, "Файл.Составитель.ДанныеЮрЛица");
	ЕстьСоставитель = ЗначениеЗаполнено(Составитель);
	
	Оформитель      = Модуль_ESDКлиентСервер.ЗначениеИзКоллекции(ДанныеТитула, "Документ.Оформитель");
...
Функция ШаблонФактАдрес() Экспорт
	...
	Структура = Новый Структура; 
	Структура.Вставить("Фактический", МодульКода("ESD_ТНОбщегоНазначенияСервер").ОбщийШаблонАдреса(Истина));
...
Функция ВыгрузитьДанныеЭПД(ПараметрыВыгрузки, ОбщиеПараметрыВыгрузки = Неопределено, Результат = Неопределено) Экспорт
	...
	МодульESDОбщегоНазначенияСервер.ЗаполнитьПараметрыВыгрузки(ПараметрыВыгрузки, Результат);
...
#Область ИнформационноеПоле

Процедура ДобавитьИнформационныеПоля(СтруктураДокумента, ПараметрыВыгрузки)
	ИнформационныеПоля = СформироватьИнформационныеПоля(ПараметрыВыгрузки);
	Если НЕ ИнформационныеПоля.Количество() Тогда
		Возврат;
	КонецЕсли;
	...
	СтруктураДокумента.Документ.Вставить("ИнформационноеПоле", ИнформационныеПоля);
КонецПроцедуры

Функция СформироватьИнформационныеПоля(ПараметрыВыгрузки)
	ИнформационныеПоля = Новый Массив;
	ДобавитьИнформационныеПоляТекста(ИнформационныеПоля, ПараметрыВыгрузки);
	Возврат ИнформационныеПоля;
КонецФункции

Процедура ДобавитьИнформационныеПоляТекста(ИнформационныеПоля, ПараметрыВыгрузки)
	ОписаниеТекста = Новый Массив;
	
	Если ПараметрыВыгрузки.Свойство("ПечатнаяФорма") Тогда
		ОписаниеТекста.Добавить(ИнформационноеПоле_ПечатнаяФорма(ПараметрыВыгрузки.ПечатнаяФорма));
	КонецЕсли;
	
	Если ОписаниеТекста.Количество() Тогда
		ИнформационныеПоля.Добавить(Новый Структура("Текст", ОписаниеТекста));
	КонецЕсли;
КонецПроцедуры

Функция ИнформационноеПоле_ПечатнаяФорма(ПечатнаяФорма)
	Возврат Новый Структура("Ид,Значение", "VisualForm", ПечатнаяФорма);
КонецФункции

#КонецОбласти // ИнформационноеПоле
...
//  ЭтоВО - Булево - признак что это продукт ВО
//
Процедура ЗаписатьДополнительныеСведенияОснования(Документ, ДанныеДляЗаписи, ЭтоВО = Ложь) Экспорт
	
	ВыгружатьВДопСведения = ЭтоВО
		Или (ЭтоУниверсальноеРасширение() И Константы["ESD_ХранитьСтатусыВДопСведениях"].Получить());
								
	Если Не ВыгружатьВДопСведения Тогда
		Возврат;
	КонецЕсли;
	...
	Иначе
		ЗаписатьДопСведенияВРасширении(Модуль_ESDЯдро, ДокументОснование, ДанныеДляЗаписи);
	КонецЕсли;
...
Процедура ЗаписатьДопСведенияВРасширении(Модуль_ESDЯдро, ДокументОснование, ДанныеДляЗаписи)	
		
	Регламент = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ЗначениеИзКоллекции(
			ДанныеДляЗаписи, "ДанныеДокумента.Регламент.Название");

	Состояние = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ЗначениеИзКоллекции(
