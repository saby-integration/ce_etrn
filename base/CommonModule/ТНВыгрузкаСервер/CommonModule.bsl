...
Функция РезультатВыгрузкиДокументов(ДанныеДляВыгрузки, context_params, ДополнительныеПараметры) Экспорт
	...
	РезультатФункции = МодульТНОбщегоНазначенияСервер.РезультатВзаимодействияСоСБИС(ДанныеДляВыгрузки.Количество());
...
//
Процедура ЗаписатьСтатистикуНапрямую(ДокументСсылка, ТекущийТитул, ИмяМетаданных) Экспорт
	...
	ЗаписатьДополнительнуюСтатистику(ДокументСсылка, ТекущийТитул, ПараметрыМетодаДопВызова, ИмяМетаданных);
...
Процедура ЗаполнитьНашуОрганизацию(Вложение, ПараметрыВыгрузки) Экспорт
	...
	Составитель     = МодульТНОбщегоНазначенияСервер.ДанныеСтруктуры(ДанныеТитула, "Файл.Составитель.ДанныеЮрЛица");
	ЕстьСоставитель = ЗначениеЗаполнено(Составитель);
	
	Оформитель      = МодульТНОбщегоНазначенияСервер.ДанныеСтруктуры(ДанныеТитула, "Документ.Оформитель");
...
Функция ВодителиНаВыгрузку(Водители, ПутевыеЛисты, Исключения = Неопределено) Экспорт
	...
	Для Каждого Строка Из Водители Цикл 
		...
		ДанныеВодителя.ВУ.ДатаВыдачи = МодульКода("Saby_ТНВыгрузкаСервер").ЗначениеДатыВремяДляВыгрузки(Строка.ДатаВыдачи, Истина);
		...
		Если ЗначениеЗаполнено(Исключения) Тогда
			МодульКода("Saby_ТНОбщегоНазначенияСервер").УдалитьСвойстваИзСтруктуры(ДанныеВодителя, Исключения);
		КонецЕсли;		
...
Процедура ЗаполнитьОпасныйГрузДляВыгрузки(ОпасныеГрузыИзДокумента, ОпасныеГрузы) Экспорт
	...
	Для Каждого Опасный Из ОпасныеГрузыИзДокумента Цикл 
		...
		ОпасныйСтруктура.Вставить(
			"УпаковкаГруппа", МодульКода("Перечисления.Saby_ГруппыУпаковки").КодПоГруппеУпаковки(Опасный.ГруппаУпаковки));
		ОпасныйСтруктура.Вставить(
			"КодОграниченияПроездТуннеля",
			МодульКода("Перечисления.Saby_КодыОграниченийПроездаЧерезТуннели").КодПоОграничениюПроезда(
				Опасный.КодОграниченияПроездТуннеля));
...
Процедура ВыполнитьДействиеВСбис(ПараметрыВыгрузки, ОбщиеПараметрыВыгрузки, ПараметрыДействия)
	...
	ПараметрыЗагрузки = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").ПараметрыЗагрузки(ПараметрыВыгрузки.Ссылка);
	...
	МодульКода("Saby_ТНЗагрузкаСервер").РезультатЗагрузкиДокументов(
...
Функция ЗаполненоОснованиеПередачиГруза(ДанныеДляПроверки)
	...
	Возврат МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").ЕстьРеквизитИлиСвойство(ДанныеДляПроверки, "ОснованиеПередачиГруза")
...
Функция ДанныеДокументовВодителей(Водители, ПутевыеЛисты)
	...
	Если ДокументыВодителей.Количество() Тогда  
		ДанныеДокументов = МодульКода("Saby_ТНОбщегоНазначенияСервер").РеквизитыДокументовСбис(ДокументыВодителей);
	КонецЕсли;
...
Функция ПараметрыМетодаЗаписиСтатистики(ТекущийТитул, ДокументСсылка)
	...
	ПараметрыAPI   = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").ПараметрыДокументаДляАПИ(ИмяМетаданных);
...
Процедура ЗаписатьСтатистикуВОблако(ДокументСсылка, ПараметрыМетода)
	...
	Ответ = МодульКода("Saby_ТНОбщегоНазначенияСервер").ВыполнитьМетодAPI(ИмяМетода, ПараметрыМетода);	
...
Процедура ЗаписатьДополнительнуюСтатистику(ДокументСсылка, ТекущийТитул, ПараметрыМетода, ИмяМетаданных)
	...
	Если ИмяМетаданных = "Saby_ПутевойЛист" И ПустаяСтрока(ТекущийТитул) Тогда
		
		ЗначениеПЛПредыдущий = МодульКода("Saby_ТНОбщегоНазначенияСервер").ЗначениеРеквизитаОбъекта(ДокументСсылка, "ПЛПредыдущий");
