...
//
Процедура ЗаполнитьСтруктуруДокументаСПроверкойФЛК(ПараметрыВыгрузки, Результат, ТолькоВыполнитьФЛК = Ложь,
	ОбщиеПараметрыВыгрузки = Неопределено) Экспорт
	...
	Если Результат.Ошибки.Количество() > 0 И Результат.Ошибки[КлючСоответствия] <> Неопределено Тогда
		...
		ПараметрыСтатус.Ошибка                 = ТекстОшибки;
		
		МодульТНОбщегоНазначенияСервер.ОбновитьДанныеСостояния(ПараметрыСтатус);
		
		ДобавитьОбщуюОшибку(ПараметрыСтатус, Результат);
		Возврат;
	КонецЕсли;
	...
	Если РезультатФЛК.СписокОшибок.Количество() > 0 Тогда
		КлючСоответствия = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").КлючСсылка(ПараметрыВыгрузки.Ссылка);
	...
	Если Не ТолькоВыполнитьФЛК Тогда
		ПараметрыВыгрузки.СтруктураДокумента = ДанныеДляВыгрузки(ДанныеДокумента, ПараметрыВыгрузки, ОбщиеПараметрыВыгрузки);
	КонецЕсли;
...
Процедура ВыгрузитьДокумент(ПараметрыВыгрузки, ОбщиеПараметрыВыгрузки, Результат)
	...
	МодульТНОбщегоНазначенияСервер.ЗаполнитьПараметрыВыгрузки(ПараметрыВыгрузки, Результат);
	...
	Иначе
		ВыгрузкаВыполненаУспешно = ВыполнитьВыгрузку(ПараметрыВыгрузки, ОбщиеПараметрыВыгрузки, Результат);
		Если Не ВыгрузкаВыполненаУспешно Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОтправитьСтатистикуПоРегламенту(context_params)
	...
	Результат.Вставить("ИспользуетсяОсновнаяРоль", 0);
	...
	Для Каждого ПользовательИБ Из МассивПИБ Цикл 
		...
		ФлагСтатистика1 =  ПользовательИБ.Роли.Содержит(Метаданные.Роли.ESD_ОсновнаяРоль) 
							И Не ПользовательИБ.Роли.Содержит(Метаданные.Роли.ESD_ПолныеПрава);
		Если ФлагСтатистика1 Тогда
			Результат.ИспользуетсяОсновнаяРоль = Результат.ИспользуетсяОсновнаяРоль + 1;
		КонецЕсли;
		
		ФлагСтатистика2 = ПользовательИБ.Роли.Содержит(Метаданные.Роли.ESD_ЗаказНаПеревозку_ДобавлениеИзменение) 
						Или ПользовательИБ.Роли.Содержит(Метаданные.Роли.ESD_Запуск) 
...
Процедура ЗаписатьДопСведенияВРасширении(Модуль_ESDЯдро, ДокументОснование, ДанныеДляЗаписи)	
	...
	Данные.Вставить("Аккаунт",      context_params["session"]["account"]);
