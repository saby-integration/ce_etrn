...
// Выполняет ФЛК документа и возвращает его данные для выгрузки.
//
// Параметры:
...
//
// Возвращаемое значение:
//   Структура, Неопределено - данные документа; Неопределено, если ФЛК не пройден
//     или данные документа не сформированы.
//
Функция СформироватьСтруктуруДокументаСПроверкойФЛК(ПараметрыВыгрузки, Результат) Экспорт
...
Функция ДанныеДляВыгрузки(ДанныеДокумента, ПараметрыВыгрузки, ОбщиеПараметрыВыгрузки = Неопределено) Экспорт
	...
	Если ДокументСтруктура.Свойство("Вложение") Тогда
		...
		Если ЭтоПервыйТитул Тогда	
			Вложение_ЗаполнитьНашуОрганизацию(Вложение, ПараметрыВыгрузки, ОбщиеПараметрыВыгрузки);
	...
	Иначе
		...
		Если ПараметрыВыгрузки.ТипТитула = ПервыйТитул Тогда 	
			Регламент = Новый Структура("Название", ПараметрыВыгрузки.ПараметрыДляАПИ.Регламент);
			ДокументСтруктура.Вставить("Регламент", Регламент);
			
			Вложение_ЗаполнитьНашуОрганизацию(ДокументСтруктура, ПараметрыВыгрузки);
			Вложение_ЗаполнитьСтороны(ДокументСтруктура, ПараметрыВыгрузки);		
		КонецЕсли;	
	КонецЕсли;
...
Функция ЗначениеДатыВремяДляВыгрузки(ДатаВремя, БезВремени = Ложь, СоСмещением = Истина) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДатаВремя) 
		Или ТипЗнч(ДатаВремя) = Тип("Дата") И НачалоДня(ДатаВремя) = Дата(1, 1, 1) 
	Тогда
		Возврат Значение;
	КонецЕсли;
...
Процедура Вложение_ЗаполнитьСтороны(Вложение, ПараметрыВыгрузки) Экспорт
	Если ПараметрыВыгрузки.ЭтоВыгрузкаЧерновика Или ПараметрыВыгрузки.ВыгрузкаНаКлиенте Тогда
		Возврат;
	КонецЕсли;
	
	Стороны_ = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер")
		.ЗначениеИзКоллекции(ПараметрыВыгрузки.ДополнительныеПараметры, "Стороны");
	Если Не ЗначениеЗаполнено(Стороны_) Тогда
		Возврат;
	КонецЕсли;
	
	Вложение.Вставить("Стороны", Стороны_);
КонецПроцедуры

Процедура Вложение_ЗаполнитьНашуОрганизацию(Вложение, ПараметрыВыгрузки, ОбщиеПараметрыВыгрузки = Неопределено) Экспорт
...
Функция ПодстановкиТранспортногоСредства(ДанныеТитула, Строка = Неопределено, ДляПутевогоЛиста = Ложь,
	Знач ПараметрыЗаполнения = Неопределено) Экспорт
	
	Если ПараметрыЗаполнения = Неопределено Тогда
		ПараметрыЗаполнения = ПодстановкиТранспортногоСредства_ПараметрыЗаполнения();
	КонецЕсли;
	...
	// Из заполненной структуры удалим поля без значений
	Для Каждого КлючЗначение Из ПараметрыЗаполнения.УдалятьПриОтсутствииЗначения Цикл
		Если Не ЗначениеЗаполнено(СтруктураТС[КлючЗначение.Ключ]) Тогда
			СтруктураТС.Удалить(КлючЗначение.Ключ);
		КонецЕсли;
	КонецЦикла;
...
Функция ПодстановкиТранспортногоСредства_ПараметрыЗаполнения(УдалятьПриОтсутствииЗначения = "") Экспорт
	РезультатФункции = Новый Структура;

	УдалятьПриОтсутствииЗначения_ = Новый Структура(УдалятьПриОтсутствииЗначения);
	РезультатФункции.Вставить("УдалятьПриОтсутствииЗначения", УдалятьПриОтсутствииЗначения_);

	Возврат РезультатФункции;
КонецФункции

Функция ПодстановкиПрицепов(ДанныеТитула, ДляПутевогоЛиста = Ложь, ПараметрыЗаполнения = Неопределено) Экспорт
	...
	Для Каждого СтрокаТС Из ТранспортныеСредства Цикл
		...
		СтруктураТС = ПодстановкиТранспортногоСредства(ДанныеТитула, СтрокаТС, ДляПутевогоЛиста, ПараметрыЗаполнения);
...
Функция ДанныеЮрЛица(РольСтрокой, ДанныеДокумента, ЗаполнитьКодФилиала = Ложь, СтруктураЮрЛица = Неопределено) Экспорт
	...
	Если ДанныеДокумента.Свойство("КонтактныеДанные") Тогда
		ДанныеЮрЛица_ДобавитьКонтактныеДанные(СтруктураДанных, ДанныеДокумента.КонтактныеДанные, Роль);	
	КонецЕсли;
		
	Возврат СтруктураДанных;
КонецФункции

Процедура ДанныеЮрЛица_ДобавитьКонтактныеДанные(СтруктураДанных, КонтактныеДанные, Роль = Неопределено) Экспорт
	Контакты = МодульКода("ESD_ТНВыгрузкаСервер").КонтактыЮрЛица(КонтактныеДанные, Роль);
...
Процедура ОсновныеДанныеЮрЛицаИзДокумента(СтруктураДанных, СтрокаДанные, ЗаполнитьКодФилиала) Экспорт	
	Если СтрокаДанные.ЮрФизЛицо = ЗначениеМетаданных("ESD_ЮрФизЛицо.ЮрЛицоНеРезидент") Тогда	
		Ключ_ = "ИностранноеЛицо"; // Не резидент (не полный) 		
		ДанныеЛица_                     = ШаблонИностранногоЛица();		
		ДанныеЛица_.Наименование        = СтрокаДанные.НаименованиеОрганизации;
		ДанныеЛица_.Страна              = СтрокаДанные.СтранаРегистрации;
	...
	ИначеЕсли СтрокаДанные.ЮрФизЛицо = ЗначениеМетаданных("ESD_ЮрФизЛицо.ИндивидуальныйПредприниматель") Тогда 	
		Ключ_ = "ИП"; // Индивидуальный предприниматель
		ДанныеЛица_ = ШаблонЮрЛицаИП();				
		ДанныеЛица_.ИНН = СтрокаДанные.ИНН; 		
		
		ЗаполнитьФИО(СтруктураДанных, СтрокаДанные, ДанныеЛица_, Истина);
		ДобавитьКодФилиала(ДанныеЛица_, СтрокаДанные, ЗаполнитьКодФилиала); 	
	ИначеЕсли СтрокаДанные.ЮрФизЛицо = ЗначениеМетаданных("ESD_ЮрФизЛицо.ФизЛицо") Тогда 	
		Ключ_ = "ФЛ"; // Физ. лицо
		ДанныеЛица_ = ШаблонФизЛицо();
		ДанныеЛица_.ИНН = СтрокаДанные.ИНН;		
		ЗаполнитьФИО(СтруктураДанных, СтрокаДанные, ДанныеЛица_, Ложь);
		
		ДобавитьКодФилиала(ДанныеЛица_, СтрокаДанные, ЗаполнитьКодФилиала);	
	Иначе			
		Ключ_ = "ЮЛ"; // Юр. лицо		
		ДанныеЛица_ = ШаблонЮЛ();				
		ДанныеЛица_.ИНН = СтрокаДанные.ИНН;
		ДанныеЛица_.КПП = СтрокаДанные.КПП;
		
		Если Не ЗначениеЗаполнено(СтруктураДанных.Название.Полное) Тогда	
			ДанныеЛица_.Название.Полное     = СтрокаДанные.НаименованиеОрганизации;
	    	СтруктураДанных.Название.Полное = СтрокаДанные.НаименованиеОрганизации;	
		Иначе
			ДанныеЛица_.Название.Полное = СтруктураДанных.Название.Полное;
		КонецЕсли;
		
		ДанныеЛица_.Наименование = ДанныеЛица_.Название.Полное;
		
		ДобавитьКодФилиала(ДанныеЛица_, СтрокаДанные, ЗаполнитьКодФилиала);	
	КонецЕсли;		
	
	СтруктураДанных.Реквизиты.Вставить(Ключ_, ДанныеЛица_);
...
Функция КонтактыЮрЛица(КонтактныеДанные, Роль = Неопределено) Экспорт
	...
	УникТелефоны = Новый Соответствие;
	МассивТелефонов = Новый Массив;
	// Поиск телефонов по отбору
	Отбор.Вставить("Тип", ЗначениеМетаданных(ТипыКонтактнойИнформации() + ".Телефон"));
	НайденныеСтроки = МодульESD_ОбщегоНазначенияКлиентСервер.НайтиСтрокиУниверсально(КонтактныеДанные, Отбор);
	// Итеративная проверка и формирование списка телефонов
	Для Каждого Строка Из НайденныеСтроки Цикл
		Если Не ЗначениеЗаполнено(Строка.Значение) Или УникТелефоны[Строка.Значение] = Истина Тогда
			Продолжить;
		КонецЕсли;
		
		СтруктураТелефон = Новый Структура("Значение", Строка.Значение);
		МассивТелефонов.Добавить(СтруктураТелефон);
		УникТелефоны.Вставить(Строка.Значение, Истина);
	КонецЦикла;
	
	// Формирование основных переменных
	УникАдресаЭлПочты = Новый Соответствие;
	МассивЭлектроннаяПочта = Новый Массив;
	// Поиск адресов эл. почты по отбору
	Отбор.Вставить("Тип", ЗначениеМетаданных(ТипыКонтактнойИнформации() + ".АдресЭлектроннойПочты"));
	НайденныеСтроки = МодульESD_ОбщегоНазначенияКлиентСервер.НайтиСтрокиУниверсально(КонтактныеДанные, Отбор);
	// Итеративная проверка и формирование списка адресов эл. почты
	Для Каждого Строка Из НайденныеСтроки Цикл
		Если Не ЗначениеЗаполнено(Строка.Значение) Или УникАдресаЭлПочты[Строка.Значение] = Истина Тогда
			Продолжить;
		КонецЕсли;
		
		СтруктураЭлектроннаяПочта = Новый Структура("Значение", Строка.Значение);
		МассивЭлектроннаяПочта.Добавить(СтруктураЭлектроннаяПочта);
		УникАдресаЭлПочты.Вставить(Строка.Значение, Истина);
	КонецЦикла;
...
Функция ВыгрузитьДанныеЭПД(ПараметрыВыгрузки, ОбщиеПараметрыВыгрузки = Неопределено, Результат = Неопределено) Экспорт
	...
	СтруктураДанныхДокумента = СформироватьСтруктуруДокументаСПроверкойФЛК(ПараметрыВыгрузки, Результат);
	Если Не ЗначениеЗаполнено(СтруктураДанныхДокумента) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ПараметрыВыгрузки.СтруктураДокумента = ДанныеДляВыгрузки(СтруктураДанныхДокумента, ПараметрыВыгрузки,
		ОбщиеПараметрыВыгрузки);
...
//
// Возвращаемое значение:
//	Структура - описание результата выполнения выгрузки
//
Функция ВыгрузитьПоРегламенту(context_params, ТипДокумента, МассивДокументов = Неопределено) Экспорт
	РезультатФункции = Новый Структура("Успешно, Описание", Истина, Неопределено);
	Если Не ЗначениеЗаполнено(context_params) Тогда
		РезультатФункции.Успешно = Ложь;
		РезультатФункции.Описание = "Отсутствует подключение пользователя в Saby";
		Возврат РезультатФункции;
	КонецЕсли;
	...
	Если РезультатЗапроса.Пустой() Тогда
		РезультатФункции.Описание = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").РезультатВзаимодействияСоСБИС(0);
		Возврат РезультатФункции;
	КонецЕсли;
	...
	РезультатФункции.Описание = МодульКода("ESD_ТНВыгрузкаСервер")
		.РезультатВыгрузкиДокументов(ДанныеДляВыгрузки, context_params, ДополнительныеПараметры);
	Возврат РезультатФункции;
КонецФункции 

Процедура ОтправитьСтатистикуПоРегламенту(context_params)
	Результат = Новый Структура("ИспользованиеСистемыПрав", 0);
	...
	Для Каждого КлючЗначения Из Результат Цикл
		...
		Исключение			
			...
			ТекстОшибки = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").СтрШаблонЭПД(
				"Не записана статистика ""%1"": %2", КлючЗначения.Ключ, Информация.Описание);
