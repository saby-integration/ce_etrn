...
Функция ПодстановкиТранспортногоСредства(ДанныеТитула, СтрокаТС = Неопределено, ДляПутевогоЛиста = Ложь) Экспорт
...
//
Функция ПодстановкиВодителя(Водители, ПутевыеЛисты, Исключения = Неопределено, ЭПЛ = Ложь) Экспорт
	
	МассивДанных = Новый Массив;
	
	ПодстановкиПутевыхЛистов = ПодстановкиПутевыхЛистов(ПутевыеЛисты);
	...
	Для Каждого СтрокаВодителя Из Водители Цикл
		...
		СвойстваИсключения = "";
		Если СтрокаВодителя.Свойство("ВУ") Тогда  
...
Процедура ДобавитьПодстановкиОснованийВладения(ДанныеТитула, СтруктураТС, СтрокаТС)
	...
	Если ДанныеТитула.ЭтоВО Тогда 
		...
		НайденныеСтроки = НайтиСтрокиУниверсально(ДанныеТитула.ДокументыЭПД, Отбор);
	...
	Иначе
		...
		Если МассивДокументовЭПД.Количество() > 0
			И ТипЗнч(МассивДокументовЭПД[0].Тип) <> Тип("Строка") Тогда
			ЗаполнитьДанныеДокументовЭПД(МассивДокументовЭПД);
		КонецЕсли;
...
Функция ПодстановкиПутевыхЛистов(ПутевыеЛисты)
	...
	Для Каждого СтрокаЛисты Из ПутевыеЛисты Цикл
		Если ТипЗнч(СтрокаЛисты) <> Тип("Структура") Тогда
			СтруктураДокументаЭПД = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").СтруктураДокументаЭПД();
		Иначе
			СтруктураДокументаЭПД = СтрокаЛисты;
		КонецЕсли;
		СтруктураДокументаЭПД.Ссылка = СтрокаЛисты.Документ;
