...
Функция ПереходВыполнен(ПараметрыВыгрузки, ОбщиеПараметрыВыгрузки, Результат)
	...
	Исключение
		...
		Состояние = ВыполнитьНаСервере("Справочники.ESD_СостоянияОбъектов.ПолучитьСтатусДокументаСБИС", СостояниеКод);
...
Функция ПодписьДобавлена(ОбщиеПараметрыВыгрузки, РезультатПодготовки, ДанныеДляПодписания)
	...
	РезультатПодготовкиЭтап = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(
	...
	ВложенияДляДобавленияПодписи = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиентСервер").ДанныеСоответствия(
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		Сообщить("Не удалось выполнить подписание с типом " + ТипДанныхПодписания);
		РезультатФункции = Ложь;
	КонецПопытки;
...
Процедура ПодписатьНаСервереЭПД(Контекст, ВложенияДляДобавленияПодписи, ДанныеДляПодписания)
	...
	МодульКода("ESD_ТНОбщегоНазначенияСервер").ОбъектОбработки().ПодписатьНаСервере(
		Контекст,
		ВложенияДляДобавленияПодписи,
		ДанныеДляПодписания["СертификатДляПодписания"]);
