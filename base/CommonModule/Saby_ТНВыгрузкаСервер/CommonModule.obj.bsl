...
Функция ДанныеДляВыгрузки(ДанныеДокумента, ПараметрыВыгрузки) Экспорт
	...
	СторонняяОрганизация = Ложь;
	Если ДокументСтруктура.Свойство("СторонняяОрганизация") Тогда
		СторонняяОрганизация = ДокументСтруктура.СторонняяОрганизация;
	КонецЕсли;
...
Функция ОбщийШаблонЮрЛица(Ссылка) Экспорт
	...
	Иначе 	
		Структура.Вставить("Адрес", Saby_ТНОбщегоНазначенияСервер.ОбщийШаблонАдреса());
	КонецЕсли;	
