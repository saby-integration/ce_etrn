...
Функция ДанныеДляВыгрузки(ДанныеДокумента, ПараметрыВыгрузки) Экспорт
	...
	ДокументСтруктура = Документы[ПараметрыВыгрузки.ИмяМетаданных].ВложениеНаВыгрузку(
	...
	Иначе
		...
		ИмяПеречисленияТитула = ПараметрыВыгрузки.ПараметрыДляАПИ.ИмяПеречисления;
		
		ПервыйТитул = Saby_ТНОбщегоНазначенияСервер.РезультатФункцииТитулов(ПараметрыВыгрузки.ИмяМетаданных, "ПервыйТитул");
...
Процедура ЗаписатьСтатистикуНапрямую(ДокументСсылка, ТекущийТитул) Экспорт
    ...
    ПараметрыМетода = ПараметрыМетодаЗаписиСтатистики(ТекущийТитул, ДокументСсылка);
	
	// параметры обнуляются после вызова записи
	ПараметрыМетодаДопВызова = ЗначениеИзСтрокиВнутр(ЗначениеВСтрокуВнутр(ПараметрыМетода));
	
	ЗаписатьСтатистикуВОблако(ДокументСсылка, ПараметрыМетода);
	
	// Дополнительная запись стастики. Требуется в исключительных случаях
	ЗаписатьДополнительнуюСтатистику(ДокументСсылка, ТекущийТитул, ПараметрыМетодаДопВызова);
...
Процедура ЗаполнитьНашуОрганизацию(Вложение, ПараметрыВыгрузки) Экспорт
	...
	Составитель     = Saby_ТНОбщегоНазначенияСервер.ДанныеСтруктуры(ДанныеТитула, "Файл.Составитель.ДанныеЮрЛица");
	ЕстьСоставитель = ЗначениеЗаполнено(Составитель);
	
	Оформитель      = Saby_ТНОбщегоНазначенияСервер.ДанныеСтруктуры(ДанныеТитула, "Документ.Оформитель");
...
Функция СпецУсловия(ТЗ_СпецРазрешения) Экспорт
	...
	Для Каждого Строка Из ТЗ_СпецРазрешения Цикл
		...
		Если ТипЗнч(Строка) <> Тип("Структура") Тогда 
			ПараметрыСтроки = Saby_ТНОбщегоНазначенияСервер.СтрокаТаблицыЗначенийВСтруктуруЭПД(Строка, КолонкиИсключения);
		Иначе 
		...
		Если ПараметрыСтроки.Свойство("НаименованиеОрганаВласти") Тогда
			...
			Saby_ТНОбщегоНазначенияСервер.УдалитьСвойстваИзСтруктуры(ПараметрыСтроки, МассивУдалить);
...
//
Функция ВодителиНаВыгрузку(Водители, ПутевыеЛисты, Исключения = Неопределено) Экспорт
	...
	Для Каждого Строка Из Водители Цикл 
		...
		ЗаполнитьТелефоныОтветственного(Строка, ДанныеВодителя);
		...
		Отбор.Вставить("КлючСтроки_ОтветственныеЛица", Строка.КлючСтроки);
		...
		Если ЗначениеЗаполнено(Исключения) Тогда
			Saby_ТНОбщегоНазначенияСервер.УдалитьСвойстваИзСтруктуры(ДанныеВодителя, Исключения);
		КонецЕсли;		
...
Функция ДанныеЮрЛица(РольСтрокой, ДанныеДокумента, ЗаполнитьКодФилиала = Ложь) Экспорт
	...
	СтруктураДанных = ОбщийШаблонЮрЛица(ДанныеДокумента.Реквизиты.Ссылка);
...
Функция ОбщийШаблонЮрЛица(Ссылка) Экспорт
	...
	Иначе 	
		Структура.Вставить("Адрес", Saby_ТНОбщегоНазначенияСервер.ОбщийШаблонАдреса(Истина));
	КонецЕсли;	
...
Процедура ЗаполнитьОпасныйГруз(ОпасныеГрузыИзДокумента, ОпасныеГрузы) Экспорт
...
Функция ЗаписаныСотрудники(ПараметрыВыгрузки, ОбщиеПараметрыВыгрузки, Результат)
	...
	Для Каждого Сотрудник Из ПараметрыВыгрузки.СтруктураДокумента.ДанныеСотрудников Цикл
		...
		Попытка
			...
			ОбщиеПараметрыВыгрузки.ОбъектОбработки.local_helper_exec_method(
				ОбщиеПараметрыВыгрузки.context_params, "СБИС.ЗаписатьСотрудника", ПараметрыМетода, Истина);
...
Функция ЗаписатьНовыйДокументВСбис(ДанныеXML, ПараметрыВыгрузки, ОбщиеПараметрыВыгрузки)
	...
	Параметры.Вставить("Тип",      ПараметрыВыгрузки.ПараметрыДляАПИ.Тип);
	...
	Если ЗначениеЗаполнено(ДанныеXML.Грузоотправитель) Тогда
		Параметры.Вставить("Грузоотправитель", ДанныеXML.Грузоотправитель);
	КонецЕсли;	
...
Функция РезультатЗаписиНовогоДокументаВСБИС(ДанныеДокумента, ОбщиеПараметрыВыгрузки)
	...
	Аргументы.Вставить("timeout",          120);
...
Функция ЗаписатьВложениеДляДокументаСбис(Файл, ПараметрыВыгрузки, ОбщиеПараметрыВыгрузки)
	...
	Параметры = Новый Структура;		
	Параметры.Вставить("Идентификатор", ПараметрыВыгрузки.ИдентификаторСБИС);
	Параметры.Вставить("Вложение",      МассивВложений);
	...
	Если ПараметрыВыгрузки.ДинамическийТитул Тогда
		...
		Параметры.Вставить("Этап", СтруктураЭтапа);
		Параметры.Вставить("Тип",  ПараметрыВыгрузки.ПараметрыДляАПИ.Тип);
	КонецЕсли;
...
Функция РеквизитыТранспортногоСредства(ТС)
	...
	Если Saby_ТНОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойство(СсылкаТС, "РегистрационныйЗнак") Тогда
	...
	Результат = Saby_ТНОбщегоНазначенияСервер.ЗначенияРеквизитовОбъекта(ТС, Массив);
...
Функция ЗаполненоОснованиеПередачиГруза(ДанныеДляПроверки)
	...
	Возврат Saby_ТНОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойство(ДанныеДляПроверки, "ОснованиеПередачиГруза")
...
Процедура ЗаполнитьОснованияВладения(СтруктураТС, СтрокаТС)
	...
	Если Не Saby_ТНОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойство(СтрокаТС, "ОснованияВладения")
...
Функция ШаблонЮрАдрес()
	...
	Структура = Новый Структура;
	Структура.Вставить("Юридический", Saby_ТНОбщегоНазначенияСервер.ОбщийШаблонАдреса(Истина));
...
Процедура ВыгрузитьПоРегламенту(context_params, ТипДокумента, МассивДокументов = Неопределено) Экспорт
	...
	|ГДЕ
	|	ДокументЭПД.Направление = ЗНАЧЕНИЕ(Перечисление.Saby_Направление.Исходящий)
	...
	Пока ВыборкаДанных.Следующий() Цикл
		ПараметрыКоманды = Saby_ТНОбщегоНазначенияКлиентСервер.ПараметрыВыгрузки(ВыборкаДанных.Ссылка);
		ДанныеДляВыгрузки.Добавить(ПараметрыКоманды);
	КонецЦикла;
	
	ДополнительныеПараметры = Новый Структура;
	
	Saby_ТНВыгрузкаСервер.РезультатВыгрузкиДокументов(ДанныеДляВыгрузки, context_params, ДополнительныеПараметры);
...
Функция ПараметрыМетодаЗаписиСтатистики(ТекущийТитул, ДокументСсылка)
	...
	ЭтоДинамическийТитул = Saby_ТНОбщегоНазначенияСервер.РезультатФункцииТитулов(
...
Процедура ЗаписатьСтатистикуВОблако(ДокументСсылка, ПараметрыМетода)
...
Процедура ЗаписатьДополнительнуюСтатистику(ДокументСсылка, ТекущийТитул, ПараметрыМетода)
	...
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.Saby_ПутевойЛист") 
		И ПустаяСтрока(ТекущийТитул) Тогда
		
		ЭтоЗаменаВодителя = ЗначениеЗаполнено(Saby_ТНОбщегоНазначенияСервер.ЗначениеРеквизитаОбъекта(ДокументСсылка, "ПЛПредыдущий"));		
