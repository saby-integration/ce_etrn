...
Функция ДанныеДляВыгрузки(ДанныеДокумента, ПараметрыВыгрузки) Экспорт
	...
	Иначе
		...
		Если ПараметрыВыгрузки.ТипТитула = Перечисления[ИмяПеречисленияТитула].ПервыйТитул() Тогда 
...
Функция РезультатПолученияВложенияПоПодстановкам(ПараметрыВыгрузки, ОбщиеПараметрыВыгрузки)
	...
	Если МенеджерТитулов.ЭтоПервыйТитул(ПараметрыВыгрузки.ТипТитула) Тогда
		НашаОрганизация = ДанныеДокумента.Вложение[0].НашаОрганизация;
		ДанныеДокумента.Вложение[0].Удалить(ИмяКлючаНашаОрганизация);
		Если Не ДанныеДокумента.Вложение[0].Подстановка.Свойство(ИмяКлючаФайл) Тогда
...
Функция ПараметрыВыполненияДействия(ПараметрыВыгрузки, ОбщиеПараметрыВыгрузки, Результат)
	...
	Попытка
		РезультатПодготовки = ПодготовитьДействиеКПереходуСбис(ПараметрыВыгрузки, ОбщиеПараметрыВыгрузки, Неопределено);
	Исключение
