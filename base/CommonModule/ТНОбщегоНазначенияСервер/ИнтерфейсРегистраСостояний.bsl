...
//  ТипДокумента - Строка - имя метаданных докумета расширения
//  УчитыватьОбратнуюСовместимость - Булево - признак учета обратной совместимости
// 
// Возвращаемое значение:
//   Выборка - найденная из регистра запись или неопределенно
//
Функция ДанныеСостоянияОбъекта(Ссылка, ТипДокумента = Неопределено, УчитыватьОбратнуюСовместимость = Истина) Экспорт
	
	Данные = Новый Структура;
	Данные.Вставить("SbisId",                 "");
	Данные.Вставить("UID",                    "");
	Данные.Вставить("Аккаунт",                "");
	Данные.Вставить("АктивныйЭтап",           "");
	Данные.Вставить("Объект",                 Неопределено);
	Данные.Вставить("Сервис",                 "");
	Данные.Вставить("Состояние",              Неопределено);
	Данные.Вставить("ТипИсточника",           Неопределено);
	Данные.Вставить("ИдентификаторИсточника", Неопределено);
	Данные.Вставить("Ссылка",                 Неопределено);
	
	Если ТипЗнч(Ссылка) = Тип("Структура") Тогда
		Данные.UID          = Ссылка.Идентификатор;
		Данные.АктивныйЭтап = Ссылка.АктивныйЭтапСтрокой;
		Данные.Состояние    = Ссылка.Состояние;
	Иначе
		Аккаунт = АвторизованныйАккаунтСтрокой();
		ВыборкаРегистра = МодульКода("РегистрыСведений.ESD_Состояние").ПрочитатьПоОбъекту(Ссылка, , Аккаунт);
		Если ВыборкаРегистра <> Неопределено Тогда
			ЗаполнитьЗначенияСвойств(Данные, ВыборкаРегистра);
			Данные.Ссылка = ВыборкаРегистра.Объект;
		КонецЕсли;
	КонецЕсли;
...
// Возвращает структуру состояния документа по идентификатору
//
// Параметры:
//  ИдСбис - Строка - идентификатор документа
//
// Возвращаемое значение:
//   Структура - структура состояния
//
Функция СостояниеПоИдентификаторуСбис(ИдСбис) Экспорт
	
	СтруктураСостояния = МодульКода("РегистрыСведений.ESD_Состояние").СтатусыДокументовПрочитатьПоUID(ИдСбис);
	
	РезультатФункции = Новый Структура;
	РезультатФункции.Вставить("SbisId",                 "");
	РезультатФункции.Вставить("UID",                    ИдСбис);
	РезультатФункции.Вставить("Аккаунт",                АвторизованныйАккаунтСтрокой());
	РезультатФункции.Вставить("АктивныйЭтап",           "");
	РезультатФункции.Вставить("Объект",                 ИдСбис);
	РезультатФункции.Вставить("Сервис",                 "");
	РезультатФункции.Вставить("Состояние",              Неопределено);
	РезультатФункции.Вставить("ТипИсточника",           Неопределено);
	РезультатФункции.Вставить("ИдентификаторИсточника", Неопределено);
	РезультатФункции.Вставить("Ссылка",                 Неопределено);
	
	Если СтруктураСостояния.Следующий() Тогда
		ЗаполнитьЗначенияСвойств(РезультатФункции, СтруктураСостояния);
	КонецЕсли;
...
Функция ЗаписатьОшибкиВыполненияВРегистрСостояний(МассивСообщений) Экспорт
	...
	Для Каждого СтруктураСообщения Из МассивСообщений Цикл
		...
		ПараметрыСтатус = СостояниеПоИдентификаторуСбис(Идентификатор);
		...
		ЕстьОшибка = ЗначениеЗаполнено(Сообщение) Или ЗначениеЗаполнено(Детали);
		
		СтруктураСообщения.Вставить("СсылкаНаДокумент", ПараметрыСтатус.Объект);
		СтруктураСообщения.Вставить("ОшибокНет",        Не ЕстьОшибка);
