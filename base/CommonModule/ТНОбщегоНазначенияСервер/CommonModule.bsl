...
Функция ДлительнаяОперация(СтруктураПараметров, УникальныйИдентификаторФормы) Экспорт
	...
	ИначеЕсли ИмяКоманды = "РаспознаваниеВУ"
		...
		ИмяФункции = ИмяФункцииФоновогоЗадания(
			"Saby_ТНОбщегоНазначенияСервер." + ИмяКоманды,
...
Процедура ОчисткаРеквизитовОбъекта(ОбъектОчистки, РеквизитыДляОчистки) Экспорт
	...
	Для Каждого ИмяРеквизита Из РеквизитыДляОчистки Цикл
		...
		ЭтоТаблица = Найти(ТипЗнч(ОбъектОчистки[ИмяРеквизита]), "табличная часть");
...
//
Функция ВыполнитьМетодAPI(ИмяМетода, ПараметрыМетода) Экспорт
	...
	Попытка 
		...
		ДополнитьПараметрыМетодаAPI(ОбъектОбработки, ИмяМетода, ПараметрыМетода, context_params);
		
		Результат = ОбъектОбработки.local_helper_exec_method(
			context_params,
			ИмяМетода,
			ПараметрыМетода,
			Истина);
...
Функция ЗаполнитьНаОснованииINI(Объект, ДокументОснование) Экспорт
	...
	СвернутьГруз(Объект, ПараметрыЗаполнения.НоменклатураГруза, СтруктураПараметров);
	
	РезультатФункции.Вставить("ТекстЗаголовка", "");
	РезультатФункции.Вставить("Картинка",       КартинкаИнтерфейсаСервер(Объект, "Saby_Успешно32"));
	
	Если РезультатФункции.Ошибка Тогда
		РезультатФункции.ТекстЗаголовка = "Ошибка";
		РезультатФункции.Картинка = КартинкаИнтерфейсаСервер(Объект, "Saby_Ошибка32");
	ИначеЕсли СтруктураПараметров.ГруппироватьПоРеквизитуНоменклатуры
		И ПараметрыЗаполнения.НоменклатураГруза.Количество() = 0 Тогда
...
Функция РезультатРасчетаINI(ПараметрыЗаполнения) Экспорт
	...
	Если ПараметрыЗаполнения.context_params = Неопределено Тогда
		...
		ТекстЗаголовка = "Ошибка авторизации";
		ТекстСообщения = "Нет входа в учетную запись!";
		Ошибка = Истина;
	...
	Если Не (ТипЗнч(XMLИниФайл) = Тип("Строка") И Врег(Лев(XMLИниФайл, 4)) = "<XML") Тогда
		РезультатФункции.Результат.ТекстЗаголовка = "Ошибка поиска";
...
Функция ДанныеПечатнойФормыДляОткрытия(СсылкаНаДокумент, УникальныйИдентификаторФормы = Неопределено) Экспорт
	...
	Если Метаданные.Константы.Найти("Saby_ХранитьПечатныеФормыВБД") <> Неопределено Тогда
...
//     * СписокДокументов - Массив - данные документов с ошибкой.
//
Функция ДанныеДляОткрытияДокументаВСБИС(СсылкаНаДокумент) Экспорт
	...
	СсылкаНаДокументВСБИС = СтрШаблон(
...
Функция ФИОИзСтроки(СтрокаФИО) Экспорт
	...
	ЧастиФИО = СтрРазделить(ФИОБезИП, " ", Ложь);
...
Функция ОбщиеПараметрыВыгрузкиЗагрузки(context_params) Экспорт
	...
	Если Метаданные.Константы.Найти("Saby_ХранитьПечатныеФормыВБД") <> Неопределено Тогда
		Saby_ХранитьПечатныеФормыВБД = Константы.Saby_ХранитьПечатныеФормыВБД.Получить();
	Иначе
...
//   ОбщийМодуль
//
Функция ОбщийМодульЭПД(Имя) Экспорт
...
Функция РаспознаваниеВУ(ДопПараметры) Экспорт
	...
	Ответ = ВыполнитьМетодAPI(ИмяМетода, ПараметрыМетода);
	...
	ИначеЕсли Не ЗначениеЗаполнено(Ответ.Результат) 
		Или Не ЗначениеЗаполнено(Ответ.Результат["result"]) Тогда
...
Функция ЗагрузитьДанныеТСПоРегНомеру(ДопПараметры) Экспорт
	
	Параметры = Новый Структура;
	Параметры.Вставить("filter", Новый Структура("RegNumber", ДопПараметры.РегистрационныйНомер));
	Параметры.Вставить("limit",  1);
	Параметры.Вставить("page",   0);
	
	ПараметрыМетода = Новый Структура;
	ПараметрыМетода.Вставить("params", Параметры);
...
Функция ЗагрузитьДанныеТСПоВИН(ДопПараметры) Экспорт
...
Функция ПараметрыЗаполненияНаОснованииINI(Объект)
	...
	РезультатФункции.Вставить("КлючСтроки",                МодульКода("Saby_ТНЗагрузкаСервер").СтруктураКлючСтроки());
...
Функция ДанныеОснованийДляЗаполненияНаОснованииINI(ДокументОснование, ШаблонИмениINI)
	...
	ОписаниеТипаРолейКонтрагентов = Новый ОписаниеТипов("Строка");
	...
	ДобавитьТранспортнуюНакладную(РезультатФункции, ДокументОснование, ШаблонИмениINI);
	
	ДобавитьОснованияПоТранспортнойНакладной(РезультатФункции, ДокументОснование, ШаблонИмениINI);
	
	ДобавитьОснованияПоПоручениюЭкспедитору(РезультатФункции, ДокументОснование, ШаблонИмениINI);
	
	Возврат РезультатФункции;
	
КонецФункции

Процедура ДобавитьТранспортнуюНакладную(ТаблицаОснований, ДокументОснование, ШаблонИмениINI)
	...
	Если ВыборкаДанных.Следующий() Тогда
		...
		СтруктураОснования = ТаблицаОснований.Добавить();
...
Процедура ДобавитьОснованияПоТранспортнойНакладной(ТаблицаОснований, ДокументОснование, ШаблонИмениINI)
	...
	СтрокаОснования = ТаблицаОснований.Найти(ДокументОснование, "Основание");
	ИндексВставки   = ТаблицаОснований.Индекс(СтрокаОснования);
	...
	Пока ВыборкаДанных.Следующий() Цикл
		...
		СтруктураОснования = ТаблицаОснований.Вставить(ИндексВставки);
		СтруктураОснования.Основание = ВыборкаДанных.Ссылка;
		СтруктураОснования.ini_name  = СтрШаблон(ШаблонИмениINI, ИмяМетаданных);
	...
	Если ВыборкаДанных.Следующий() Тогда
		СтруктураОснования = ТаблицаОснований.Добавить();
		СтруктураОснования.Основание = ВыборкаДанных.ЗаданиеНаПеревозку;
		СтруктураОснования.ini_name  = СтрШаблон(ШаблонИмениINI, "ЗаданиеНаПеревозку");
	КонецЕсли;
	
КонецПроцедуры

Процедура ДобавитьОснованияПоПоручениюЭкспедитору(ТаблицаОснований, ДокументОснование, ШаблонИмениINI)
	...
	Пока ВыборкаДанных.Следующий() Цикл
		...
		СтруктураОснования = ТаблицаОснований.Вставить(ИндексВставки);
		СтруктураОснования.Основание = ВыборкаДанных.Основание;
		СтруктураОснования.ini_name  = СтрШаблон(ШаблонИмениINI, ИмяМетаданных);
	КонецЦикла;
...
Процедура СвернутьГруз(Объект, НоменклатураГруза, СтруктураПараметров)
	
	Если ТипЗнч(Объект.Ссылка) = Тип("Структура") Тогда
		ОбъектДок = Объект;
	Иначе
		ОбъектДок = Объект.Ссылка.ПолучитьОбъект();
	КонецЕсли;
	
	ЭтоЭТрН = МодульКода("Saby_ТНОбщегоНазначенияСервер").ИмяМетаданныхДокумента(Объект.Ссылка) = "Saby_ТранспортнаяНакладная";
	
	Если НеГруппироватьГруз(ОбъектДок, НоменклатураГруза, СтруктураПараметров) Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого Колонка Из Грузы.Колонки Цикл
		...
		КолонкиДляЗапроса.Добавить(СтрШаблон(СтрокаЗапроса, Колонка.Имя));
	КонецЦикла;
	...
	КолонкиПервойТаблицы = СтрСоединить(КолонкиДляЗапроса, РазделительСтрок);
	...
	Для Каждого ИмяКолонки Из СуммируемыеКолонки Цикл
		...
		КолонкиСуммыДляЗапроса.Добавить(СтрШаблон(СтрокаЗапроса, ИмяКолонки));
	КонецЦикла;
	КолонкиСуммыДляЗапроса = СтрСоединить(КолонкиСуммыДляЗапроса, РазделительСтрок);
	...
	Грузы.Очистить();
	СуммируемыеКолонки = СтрСоединить(СуммируемыеКолонки, ",");
...
Функция НеГруппироватьГруз(ОбъектДок, НоменклатураГруза, СтруктураПараметров)
	...
	Возврат СтруктураПараметров = Неопределено
		...
		Или (СтруктураПараметров.ГруппироватьПоРеквизитуНоменклатуры И НоменклатураГруза.Количество() = 0);
...
Процедура ЗаполнитьКритерийГруппировки(СтруктураПараметров, ЗапросДанных, Грузы)
	...
	Если СтруктураПараметров.ГруппироватьВОднуПозицию Тогда
		...
		НаименованиеОбщейПозиции = СтрШаблон(НаименованиеОбщейПозиции, Наименование);
...
Процедура ДополнитьПараметрыМетодаAPI(ОбъектОбработки, ИмяМетода, ПараметрыМетода, context_params)
	...
	Если ИмяМетода = "CloudStatistics.Send" Тогда 
		ПараметрыМетода.data.context = ОбъектОбработки.get_prop(context_params, "ExtSysSettingsId", "");
    КонецЕсли;
