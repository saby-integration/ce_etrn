...
Функция ЗначениеСтавкиНДС(Значение, Загрузка = Ложь) Экспорт
	...
	ДополнитьСоответствие(Параметры, "3",  "5%");
...
Процедура ОграничитьТипыДокументовОснований(Форма) Экспорт
	...
	Если МодульСправочникаСопоставлений <> Неопределено
		И Не Форма.ЭтоВО
		И МодульКода("ESD_ТНОбщегоНазначенияСервер").ЗначениеКонстанты("ESD_ИспользоватьНовыйРедактор", Ложь) Тогда
...
Функция ДанныеДляОткрытияДокументаВСБИС(СсылкаНаДокумент, ОбменНаКлиенте = Ложь) Экспорт
	...
	ПараметрыЗагрузки = МодульКода("ESD_ТНЗагрузкаСервер").ПараметрыЗагрузкиИзСбис(СсылкаНаДокумент, ДополнительныеПараметры);
...
Функция ЗаполнениеНаОснованииПоНовомуРедактору(Форма, СсылкаНаОснование) Экспорт
	...
	ОбъектЗаполнения.Заполнить(МассивОснований);
...
Процедура ЗаполнитьНаОснованииИспользуяНастройкиСопоставления(ОбъектЗаполнения, ДанныеЗаполнения) Экспорт
	...
	Для Каждого СсылкаНаОснование Из ДанныеЗаполнения Цикл
	...
	ПередЗаполнением(ОбъектЗаполнения);
	Для Каждого РезультатРасчетаПолей Из МассивРассчитанныхОснований Цикл
		ОбъектЗаполнения.ЗаполнитьНаОснованииРасчетаСопоставлений(РезультатРасчетаПолей);
	КонецЦикла;
...
Процедура ПередЗаполнением(ОбъектЗаполнения) Экспорт
	...
	Для Каждого СтандартныйРеквизит Из МетаданныеОбъекта.СтандартныеРеквизиты Цикл
		Если НеобходимыеРеквизиты.Получить(СтандартныйРеквизит.Имя) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ОбъектЗаполнения[СтандартныйРеквизит.Имя] = "";
	КонецЦикла;
	
	Для Каждого Реквизит Из МетаданныеОбъекта.Реквизиты Цикл
		ОбъектЗаполнения[Реквизит.Имя] = "";
	КонецЦикла;
	
	Для Каждого ТабличнаяЧасть Из МетаданныеОбъекта.ТабличныеЧасти Цикл
		ОбъектЗаполнения[ТабличнаяЧасть.Имя].Очистить();
	КонецЦикла;
	
КонецПроцедуры

Процедура ЗаполнитьРеквизитыОбъекта(ОбъектЗаполнения, РезультатРасчетаПолей) Экспорт
...
Процедура ЗаполнитьТранспортныеСредства(ОбъектЗаполнения, РезультатРасчетаПолей) Экспорт
	...
	Если Прицепы <> Неопределено Тогда
		Для Каждого Прицеп Из Прицепы Цикл
			Прицеп = Прицеп["Прицепы.Прицеп"];
			СтрокаТранспортногоСредства = ОбъектЗаполнения.ТранспортныеСредства.Добавить();
			СтрокаТранспортногоСредства.Вид = ЗначениеМетаданных("ESD_ВидыТС.Прицеп");
...
Процедура ЗаполнитьВодителей(ОбъектЗаполнения, РезультатРасчетаПолей) Экспорт
	...
	Пока ОбъектЗаполнения.ОтветственныеЛица.Количество() > 0
		И ОбъектЗаполнения.ОтветственныеЛица[ОбъектЗаполнения.ОтветственныеЛица.Количество() - 1].Роль = РольВодитель Цикл
	...
	Для Каждого Водитель Из Водители Цикл
		...
		Телефоны = МодульТНОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(Водитель, "Телефоны", Новый Массив);
		МассивТелефонов = Новый Массив;
...
Процедура ЗаполнитьСсылкиДокументовЭПД(РезультатРасчетаПолей) Экспорт
	...
	Для Каждого ДокументВладенияТС Из ДокументыВладенияТС Цикл
		МодульСправочникДокументыЭПД.ДобавитьДанныеДокументаЭПД(
			МассивДанныхДокументовЭПД,
			ДокументВладенияТС["ДокументВладения"],
	...
	Для Каждого Прицеп Из Прицепы Цикл
		ДокументыВладенияПрицепа = МодульТНОбщегоНазначенияКлиентСервер.ЗначениеИзКоллекции(
			Прицеп, "Прицеп.ДокументыВладения", Новый Массив);
		Для Каждого ДокументВладенияПрицепа Из ДокументыВладенияПрицепа Цикл
			МодульСправочникДокументыЭПД.ДобавитьДанныеДокументаЭПД(
				МассивДанныхДокументовЭПД,
				ДокументВладенияПрицепа["ДокументВладения"],
				ЗначениеМетаданных("ESD_ТипыДокумента.ПодтверждениеВладения"));
		КонецЦикла;
	КонецЦикла;
...
Функция СсылкаНаСкачиваниеУниверсальногоРасширения(ИсходнаяСсылка) Экспорт
	...
	Если СсылкаПоЧастям.Количество() Тогда
		...
		ПоследняяЧасть = СсылкаПоЧастям.Количество() - 1;
		СсылкаПоЧастям[ПоследняяЧасть] = ФайлПродукта;
...
Функция НеГруппироватьГруз(ОбъектДок, НоменклатураГруза, СтруктураПараметров, РезультатФункции)
	...
	Если ГруппировкаНовымРедактором Тогда 
	...
	ЕстьГруз = ОбъектДок.Грузы.Количество() 
				Или СтруктураПараметров.Свойство("ГруппировкаНовымРедактором");  
