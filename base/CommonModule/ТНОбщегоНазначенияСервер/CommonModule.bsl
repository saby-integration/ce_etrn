...
Функция ДоступныеКонфигурации() Экспорт
	...
	РезультатФункции.Вставить("УАТПроф", "Управление автотранспортом Проф, редакция 2.2");
	РезультатФункции.Вставить("УАТ",     "Управление автотранспортом Стандарт, редакция 2.2");
	РезультатФункции.Вставить("УТ10",    "Управление торговлей, редакция 10.3");
...
Процедура ЗаполнитьНаОснованииИспользуяНастройкиСопоставления(ОбъектЗаполнения, ДанныеЗаполнения) Экспорт
	...
	МодульМенеджераНастройкиСопоставлений = МодульКода("Справочники.ESD_НастройкиСопоставлений");
	Если МодульМенеджераНастройкиСопоставлений = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если Не Документы.ТипВсеСсылки().СодержитТип(ТипЗнч(ДанныеЗаполнения))
		И Справочники.ТипВсеСсылки().СодержитТип(ТипЗнч(ДанныеЗаполнения)) Тогда
		Возврат;
	КонецЕсли;
	
	МетаданныеОбъекта = ОбъектЗаполнения.Метаданные();
	Источник = ДанныеЗаполнения.Метаданные().Имя;
	Приемник = МетаданныеОбъекта.Имя;
	
	ОбъектНастроекСопоставлений = МодульМенеджераНастройкиСопоставлений.СсылкаПоИсточникуПриемнику(Источник, Приемник);
	Если ОбъектНастроекСопоставлений = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	РезультатРасчетаПолей = ОбъектНастроекСопоставлений.РезультатРасчетаПолей(ДанныеЗаполнения);
	...
	Для Каждого КлючЗначение Из РезультатРасчетаПолей Цикл
		Если МетаданныеОбъекта.Реквизиты.Найти(КлючЗначение.Ключ) <> Неопределено
			Или МетаданныеОбъекта.СтандартныеРеквизиты.Найти(КлючЗначение.Ключ) <> Неопределено Тогда
			ОбъектЗаполнения[КлючЗначение.Ключ] = КлючЗначение.Значение;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьДанныеЮрЛиц(ОбъектЗаполнения, РезультатРасчетаПолей, МассивЮрЛиц) Экспорт
	...
	Для Каждого ИмяЮрЛица Из МассивЮрЛиц Цикл
		...
		Результат = РезультатРасчетаПолей[ИмяЮрЛица];
		...
		ЗаполнитьЗначенияСвойств(СтрокаЮрЛица, Результат);
		СтрокаЮрЛица.КлючСтроки = КлючСтроки;
		СтрокаЮрЛица.Роль = МодульКода("Перечисления.ESD_РолиКонтрагентов").РольПоИмени(ИмяЮрЛица);
		
		Для Каждого СтруктураКонтактныхДанных Из Результат.КонтактныеДанные Цикл
...
Процедура ЗаполнитьТранспортныеСредства(ОбъектЗаполнения, РезультатРасчетаПолей) Экспорт
	...
	Если ТранспортноеСредство <> Неопределено Тогда
		...
		ЗаполнитьЗначенияСвойств(СтрокаТранспортногоСредства, ТранспортноеСредство);
	КонецЕсли;
	...
	Если Прицепы <> Неопределено Тогда
		Для Каждого Прицеп Из Прицепы Цикл
			СтрокаТранспортногоСредства = ОбъектЗаполнения.ТранспортныеСредства.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТранспортногоСредства, Прицеп);
		КонецЦикла;
	КонецЕсли;
...
Процедура ЗаполнитьВодителей(ОбъектЗаполнения, РезультатРасчетаПолей) Экспорт
	...
	Водители = РезультатРасчетаПолей["Водители"];
	Для Каждого Водитель Из Водители Цикл
		КлючСтроки = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").НовыйКлючОсновнойСтроки(ОбъектЗаполнения, "ОтветственныеЛица");
		СтрокаОтветственного = ОбъектЗаполнения.ОтветственныеЛица.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаОтветственного, Водитель);
		СтрокаОтветственного.КлючСтроки = КлючСтроки;
		СтрокаОтветственного.Роль = ЗначениеМетаданных("ESD_РолиОтветственных.Водитель");
		
		ПутевыеЛисты = Водитель["ПутевыеЛисты"];
		Если ПутевыеЛисты = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		Для Каждого ПутевойЛист Из ПутевыеЛисты Цикл
			СтрокаПутевогоЛиста = ОбъектЗаполнения.ПутевыеЛисты.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаПутевогоЛиста, ПутевойЛист);
...
Процедура ЗаполнитьГрузы(ОбъектЗаполнения, РезультатРасчетаПолей) Экспорт
	...
	Для Каждого Груз Из ПолеГрузы Цикл
		...
		КлючСтрокиЮрЛица = 0;
		ПолеЗаказчик = Груз["Заказчик"];
		Если ПолеЗаказчик <> Неопределено Тогда
			КлючСтрокиЮрЛица = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").НовыйКлючОсновнойСтроки(ОбъектЗаполнения, "ДанныеЮрЛиц");
			СтрокаЮрЛица = ОбъектЗаполнения.ДанныеЮрЛиц.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаЮрЛица, ПолеЗаказчик);
			СтрокаЮрЛица.КлючСтроки = КлючСтрокиЮрЛица;
			СтрокаЮрЛица.Роль = ЗначениеМетаданных("ESD_РолиКонтрагентов.Заказчик");
		КонецЕсли;
		
		СтрокаГруза = ОбъектЗаполнения.Грузы.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаГруза, Груз);
		...
		Для Каждого ИмяПодчиненнойТаблицы Из ПодчиненныеТаблицы Цикл
			ЗаполнитьПодчиненнуюТаблицу(ОбъектЗаполнения, Груз, СтрокаГруза.КлючСтроки, ИмяПодчиненнойТаблицы, "КлючСтроки_Грузы");
		КонецЦикла;
...
Процедура ЗаполнитьПодчиненнуюТаблицу(ОбъектЗаполнения, ИсточникДанных, КлючСтроки,
		ИмяПодчиненнойТаблицы, ИмяКолонкиКлюча) Экспорт
	
	ПолеПодчиненнойТаблицы = ИсточникДанных[ИмяПодчиненнойТаблицы];
	Если ПолеПодчиненнойТаблицы = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого СтруктураПодчиненнойТаблицы Из ПолеПодчиненнойТаблицы Цикл
		СтрокаПодчиненнойТаблицы = ОбъектЗаполнения[ИмяПодчиненнойТаблицы].Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаПодчиненнойТаблицы, СтруктураПодчиненнойТаблицы);
		СтрокаПодчиненнойТаблицы[ИмяКолонкиКлюча] = КлючСтроки;
	КонецЦикла;
...
Процедура ЗаполнитьОтветственных(ОбъектЗаполнения, РезультатРасчетаПолей, МассивОтветственных) Экспорт
	...
	Для Каждого ИмяОтветственного Из МассивОтветственных Цикл
		...
		Роль = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").СтрРазделитьЭПД(ИмяОтветственного, "_");
		Роль = Роль[Роль.ВГраница()];
		
		СтрокаОтветственногоЛица = ОбъектЗаполнения.ОтветственныеЛица.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаОтветственногоЛица, Ответственный);
...
Функция СсылкаНаСкачиваниеУниверсальногоРасширения(ИсходнаяСсылка) Экспорт
	...
	ЭтоCF = Метаданные.ФункциональныеОпции.Найти("ESD_ИспользоватьТранспортнаяНакладная") = Неопределено;	
	Если ЭтоCF Тогда
...
Процедура ПолучитьДанныеЮрЛицИзБазы(МассивЮрЛиц, МенеджерВременныхТаблиц)
	...
	Иначе
		...
		Если ИмяКонфигурации = ДоступныеКонфигурации.УАТ Или ИмяКонфигурации = ДоступныеКонфигурации.УАТПроф Тогда
...
Процедура СвернутьГруз(ОбъектСсылка, ПараметрыЗаполнения, РезультатФункции) Экспорт
	...
	ЭтоЭПЛ = МодульКода("ESD_ТНОбщегоНазначенияСервер").ИмяМетаданныхДокумента(ОбъектСсылка) = ПутевойЛист();
	Если ЭтоЭПЛ Тогда
		// в ЭПЛ нет груза
		Возврат;
	КонецЕсли;	
	
	ЭтоЭТрН = МодульКода("ESD_ТНОбщегоНазначенияСервер").ИмяМетаданныхДокумента(ОбъектСсылка) = ТранспортнаяНакладная();	
	...
	Для Каждого Колонка Из Грузы.Колонки Цикл
		...
		КолонкиДляЗапроса.Добавить(МодульТНОбщегоНазначенияКлиентСервер.СтрШаблонЭПД(СтрокаЗапроса, Колонка.Имя));
	КонецЦикла;
	...
	КолонкиПервойТаблицы = МодульТНОбщегоНазначенияКлиентСервер.СтрСоединитьЭПД(КолонкиДляЗапроса, РазделительСтрок);
	...
	Для Каждого ИмяКолонки Из СуммируемыеКолонки Цикл
		...
		КолонкиСуммыДляЗапроса.Добавить(МодульТНОбщегоНазначенияКлиентСервер.СтрШаблонЭПД(СтрокаЗапроса, ИмяКолонки));
	КонецЦикла;
	КолонкиСуммыДляЗапроса = МодульТНОбщегоНазначенияКлиентСервер.СтрСоединитьЭПД(КолонкиСуммыДляЗапроса, РазделительСтрок);
	...
	Грузы.Очистить();
	СуммируемыеКолонки = МодульТНОбщегоНазначенияКлиентСервер.СтрСоединитьЭПД(СуммируемыеКолонки, ",");
