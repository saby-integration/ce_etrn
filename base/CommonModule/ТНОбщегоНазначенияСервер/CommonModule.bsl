...
Функция ТранспортныеСредстваСтрокой(ОбъектДок) Экспорт
    ...
    СтрокиАвто = МодульКода("Saby_ТНОбщегоНазначенияКлиентСервер").СтрокиТС(
...
Функция ДанныеДляОткрытияДокументаВСБИС(СсылкаНаДокумент, ОбменНаКлиенте = Ложь) Экспорт
	...
	ПараметрыЗагрузки = МодульКода("Saby_ТНЗагрузкаСервер").ПараметрыЗагрузкиИзСбис(СсылкаНаДокумент, ДополнительныеПараметры);
