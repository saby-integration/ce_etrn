...
Функция ЗаполнитьНаОснованииINI(ЭтаФорма, ДокументОснование, ИмяМетаданных) Экспорт
	...
	ПараметрыЗаполнения = МодульКодаКлиент("Saby_ТНОбщегоНазначенияКлиентСервер").ПараметрыЗаполненияНаОснованииINI(
	...
	Если ПараметрыЗаполнения.НаКлиенте Тогда
		ТранспортВО = МодульКодаКлиент("Saby_ТНОбщегоНазначенияКлиент").ТранспортВО(ЭтаФорма, ПараметрыЗаполнения.context_params);
...
Функция РезультатРасчетаINI(ПараметрыЗаполнения, ТранспортБлокли) Экспорт
	...
	Если ПараметрыЗаполнения.context_params = Неопределено Тогда
		ПараметрыЗаполнения.context_params = ВыполнитьНаСервере("Saby_Core.ПроверитьНаличиеПараметровПодключения", Неопределено);
	КонецЕсли;
