...
Функция ЗаполнитьНаОснованииINI(ЭтаФорма, ДокументОснование, ИмяМетаданных) Экспорт
	...
	ОснованияДляЗаполнения = ВыполнитьНаСервере(
		"ESD_ТНОбщегоНазначенияСервер.ДанныеОснованийДляЗаполненияНаОснованииINI", ДокументОснование, ПараметрыЗаполнения);
	
	Если ПараметрыЗаполнения.НаКлиенте Тогда
		ТранспортВО = МодульКодаКлиент("ESD_ТНОбщегоНазначенияКлиент").ТранспортВО(ЭтаФорма, ПараметрыЗаполнения.context_params);
	...
	#Если НаСервере Тогда
		Картинка = МодульФормы.КартинкаИнтерфейсаСервер(ЭтаФорма, "ESD_Успешно32");
	#Иначе 	
		Картинка = МодульФормы.КартинкаИнтерфейса(ЭтаФорма, "ESD_Успешно32");
	#КонецЕсли
	
	РезультатФункции.Вставить("Картинка", Картинка);
	
	ВыполнитьНаСервере("ESD_ТНОбщегоНазначенияСервер.СвернутьГруз", ФормаОбъект.Ссылка, ПараметрыЗаполнения, РезультатФункции);
	
	Если РезультатФункции.Ошибка Тогда
		 		
		#Если НаСервере Тогда
			Картинка = МодульФормы.КартинкаИнтерфейсаСервер(ЭтаФорма, "ESD_Ошибка32");
		#Иначе 	
			Картинка = МодульФормы.КартинкаИнтерфейса(ЭтаФорма, "ESD_Ошибка32");
		#КонецЕсли
...
Функция РезультатРасчетаINI(ПараметрыЗаполнения, ТранспортБлокли) Экспорт
	...
	Если Результат.status <> "complete" Тогда
		Возврат СтруктураРезультатаINI(Истина, "Ошибка расчета", Результат.data.message);
	КонецЕсли;
