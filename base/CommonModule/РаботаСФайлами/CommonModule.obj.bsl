...
Функция Saby_ДобавитьФайлДляРасширения(
	...
	Если Не ЗначениеЗаполнено(ВладелецФайлов) Тогда
		ВызватьИсключение Saby_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	Если Не ЗначениеЗаполнено(ПрисоединенныйФайл.Изменил) Тогда
		ПрисоединенныйФайл.Изменил = Saby_ТНОбщегоНазначенияСервер.АвторизованныйПользователь();
	КонецЕсли;
...
Функция Saby_ИмяСправочника(СсылкаНаФайл, ВладелецФайлов)
	...
	Если СсылкаНаФайл = Неопределено Тогда
		...
		Описание = Saby_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура Saby_ЗаписатьДанныеФайла(ПрисоединенныйФайл, Контекст, 
	...
	Исключение
		...
		|%2'");
		КомментарийЖурналаРегистрации = Saby_ОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку(
		ШаблонСообщения,
