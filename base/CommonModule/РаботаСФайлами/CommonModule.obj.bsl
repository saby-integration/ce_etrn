...
Функция Saby_ХранилищеФайлаИзИнформационнойБазы(ФайлСсылка)
	...
	Если Saby_ТНОбщегоНазначенияКлиентСервер.ПрисоединенныйФайлВРасширении(ФайлСсылка) Тогда
...
Функция Saby_ДобавитьФайл(ПараметрыФайла, 
	...
	Если Saby_РаботаСПрисоединеннымиФайламиПовтИсп.ВладелецФайлаВРасширении(ПараметрыФайла.ВладелецФайлов) Тогда
...
Функция Saby_ДобавитьФайлДляРасширения(
	...
	Если Не ЗначениеЗаполнено(ВладелецФайлов) Тогда
		ВызватьИсключение Saby_ТНОбщегоНазначенияКлиентСервер.СтрШаблонЭПД(
	...
	ИмяИРасширение = Saby_ИмяИРасширениеФайла(РасширениеБезТочки, ИмяБезРасширения);
	...
	ДанныеФайла       = Saby_ИмяСправочника(НоваяСсылкаНаФайл, ВладелецФайлов);
	ИмяСправочника    = ДанныеФайла.ИмяСправочника;
	НоваяСсылкаНаФайл = ДанныеФайла.НоваяСсылкаНаФайл;
		
	ХранитьВерсии      = СтрСравнить(ИмяСправочника, Метаданные.Справочники.Файлы.Имя) = 0;	
	ПрисоединенныйФайл = Saby_СсылкаНаФайл(НоваяСсылкаНаФайл, ИмяСправочника);
	...
	ПрисоединенныйФайл.ТипХраненияФайла             = Saby_ТипФайлаХранения(ПрисоединенныйФайл);
	...
	Если Не ЗначениеЗаполнено(ПрисоединенныйФайл.Изменил) Тогда
		ПрисоединенныйФайл.Изменил = Saby_ТНОбщегоНазначенияСервер.ТекущийПользователь();
	КонецЕсли;
...
// Параметры:
//   СсылкаНаФайл - Неопределено - если у владельца файла только один справочник хранения файлов.
...
//
Функция Saby_ИмяСправочника(СсылкаНаФайл, ВладелецФайлов)
	...
	Если СсылкаНаФайл = Неопределено Тогда
		...
		Описание = Saby_ТНОбщегоНазначенияКлиентСервер.СтрШаблонЭПД(
	...
	Иначе
		...
		НоваяСсылкаНаФайл = СсылкаНаФайл;
		
		Если Не Справочники.ТипВсеСсылки().СодержитТип(ТипЗнч(НоваяСсылкаНаФайл))
			И Не Saby_ТНОбщегоНазначенияКлиентСервер.ПрисоединенныйФайлВРасширении(НоваяСсылкаНаФайл)
	...
	ДанныеФайла.Вставить("ИмяСправочника",    ИмяСправочника);
	ДанныеФайла.Вставить("НоваяСсылкаНаФайл", НоваяСсылкаНаФайл);
...
//
//
Процедура Saby_ЗаписатьДанныеФайла(ПрисоединенныйФайл, Контекст, 
	...
	Исключение
		...
		|%2'");
		КомментарийЖурналаРегистрации = Saby_ТНОбщегоНазначенияКлиентСервер.СтрШаблонЭПД(
		ШаблонСообщения,
...
//
Функция Saby_СсылкаНаФайл(НоваяСсылкаНаФайл, ИмяСправочника)
	
	ПрисоединенныйФайл = Справочники[ИмяСправочника].СоздатьЭлемент(); // ОпределяемыйТип.ПрисоединенныйФайлОбъект

	Если Saby_ТНОбщегоНазначенияСервер.СсылкаСуществует(НоваяСсылкаНаФайл)
		И Saby_ТНОбщегоНазначенияКлиентСервер.ПрисоединенныйФайлВРасширении(НоваяСсылкаНаФайл) Тогда
...
//
Функция Saby_ИмяИРасширениеФайла(РасширениеБезТочки, ИмяБезРасширения)
...
//
Функция Saby_ТипФайлаХранения(ПрисоединенныйФайл)
