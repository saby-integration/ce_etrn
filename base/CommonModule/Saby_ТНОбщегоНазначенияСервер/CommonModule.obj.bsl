...
// Параметры:
// 	Объект - Объект - объект для очистки, может быть документ или справочник  
//	ИмяТаблицы - Строка - таблица для очистки записей 
//  РольСтрокой - Строка - имя роли для очистки
//
Процедура УдалитьСтрокуПоРоли(Объект, ИмяТаблицы, РольСтрокой) Экспорт
			
	Если ИмяТаблицы = "ДанныеЮрЛиц" Тогда
		Роль = Перечисления.Saby_РолиКонтрагентов[РольСтрокой];
	Иначе 
		Роль = Перечисления.Saby_РолиОтветственных[РольСтрокой]; 
	КонецЕсли;
		
	Отбор = Новый Структура;
	Отбор.Вставить("Роль", Роль);
	НайденныеСтроки = Saby_ТНОбщегоНазначенияКлиентСервер.НайтиСтрокиУниверсально(Объект[ИмяТаблицы], Отбор);
	Если НайденныеСтроки.Количество() Тогда			
		Строка = НайденныеСтроки[0];
		Объект[ИмяТаблицы].Удалить(Строка);			
	КонецЕсли;	
...
Функция ВидыКонтактнойИнформации(Ссылка) Экспорт 
	...
	Иначе
		...
		Если ИменаПредопределенных.Найти("EmailКонтрагенты") <> Неопределено Тогда  
			ВидЕмейл = "EmailКонтрагенты"; // "Бухгалтерия предприятия, редакция 3.0"
		Иначе 
			ВидЕмейл = "EmailКонтрагента";
		КонецЕсли;
	...
	СтруктураКИ.Вставить("Емейл",     Справочники.ВидыКонтактнойИнформации[ВидЕмейл]);
...
Функция МассивВидовКонтактнойИнформации() Экспорт
	...
	РезультатФункции.Добавить(СтруктураВидовКИ.Емейл);
...
Функция ОрганизацияПрисутствуетВБазе(ИНН, КПП) Экспорт
	...
	Возврат Не ЗапросДанных.Выполнить().Пустой();
...
Процедура ЗачисткаОтметокШтрафовАктов(ОбъектДок, Титул, ЗначениеРоли) Экспорт
	...
	Если Saby_ТНОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойство("Ссылка", ОбъектДок)
...
//
Функция ОбщийШаблонАдреса() Экспорт
...
Процедура ЗаполнитьПараметрыВыгрузки(ПараметрыВыгрузки) Экспорт
	...
	ПараметрыВыгрузки.Тип = ПараметрыДокументаДляАПИ.Тип;
...
Функция ЗаполнитьНаОснованииINI(Объект, ДокументОснование) Экспорт
	...
	Для Каждого СтруктураОснования Из ОснованияДляЗаполнения Цикл
		...
		ПараметрыЗаполнения.МенеджерДокумента.ДобавитьОснованияДляЗаполнения(ОснованияДляЗаполнения, РезультатФункции);
...
//
// Возвращаемое значение:
//   Структура - структура всех титулов указанных в перечислении.
//     * КлючСтруктуры - Строка - имя типа титула как оно задано в конфигураторе
//     * ЗначениеСтруктуры - ПеречислениеСсылка - ссылка на соответствующий тип титула.
//
...
//
// Возвращаемое значение:
//   Структура - см. ФиоИВУФизЛиц + "ВУСтрокой"
//
...
//
Функция ДанныеСоответствия(Соответствие, Путь) Экспорт
...
Функция ДанныеПечатнойФормыДляОткрытия(СсылкаНаДокумент, УникальныйИдентификаторФормы = Неопределено) Экспорт
	...
	Иначе
		...
		АдресДвоичныхДанныхФайлаЭТрН = Saby_ТНЗагрузкаСервер.АдресДвоичныхДанныхФайла(ПараметрыЗагрузки);
...
Функция ФиоИВУФизЛиц(МассивФизлиц) Экспорт
	...
	Емейл   = Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты;
	
	ЗапросДанных.УстановитьПараметр("Телефон", Телефон);
	ЗапросДанных.УстановитьПараметр("Емейл",   Емейл);
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТелефоныЕмейлыФизлиц.ФизическоеЛицо КАК ФизическоеЛицо,
	|	ТелефоныЕмейлыФизлиц.Представление КАК Представление
	|ИЗ
	|	ТелефоныЕмейлыФизлиц КАК ТелефоныЕмейлыФизлиц
	|ГДЕ 
	|   ТелефоныЕмейлыФизлиц.Тип = &Телефон 
	|;
	|///////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТелефоныЕмейлыФизлиц.ФизическоеЛицо КАК ФизическоеЛицо,
	...
	|	ТелефоныЕмейлыФизлиц КАК ТелефоныЕмейлыФизлиц
	|ГДЕ
    |   ТелефоныЕмейлыФизлиц.Тип = &Емейл
	|
	|";
	...
	ВыборкаЕмейлы    = РезультатЗапроса[2].Выбрать();
	...
	Пока Выборка.Следующий() Цикл 
		...
		Пока ВыборкаЕмейлы.НайтиСледующий(Отбор) Цикл
			МассивЕмейлы.Добавить(ВыборкаЕмейлы.Представление);
		КонецЦикла;
		
		Структура.Вставить("Телефоны", ЗначениеВСтрокуВнутр(МассивТелефонов));
		Структура.Вставить("Емейлы",   ЗначениеВСтрокуВнутр(МассивЕмейлы));
...
Функция СтруктураДанныхОтветственногоЛица() Экспорт
	...
	РезультатФункции.Вставить("Емейлы",                "");
...
Функция РеквизитыДокументовСбис(МассивДокументов) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДокументыЭПДСтороны.Ссылка КАК Ссылка,
	|	ДокументыЭПДСтороны.Сторона КАК Сторона,
	|	ДокументыЭПДСтороны.Сторона.Наименование КАК Наименование,
	|	ДокументыЭПДСтороны.Сторона.ИНН КАК ИНН,
	|	ДокументыЭПДСтороны.Сторона.КПП КАК КПП,
	|	ДокументыЭПДСтороны.Сторона.СтранаРегистрации КАК Страна,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ДокументыЭПДСтороны.Сторона) = ТИП(Справочник.Организации)
	|			ТОГДА ВЫРАЗИТЬ(ДокументыЭПДСтороны.Сторона КАК Справочник.Организации).ОГРН
	|		КОГДА ТИПЗНАЧЕНИЯ(ДокументыЭПДСтороны.Сторона) = ТИП(Справочник.Контрагенты)
	|			ТОГДА ДокументыЭПДСтороны.Сторона.РегистрационныйНомер
	|		ИНАЧЕ &ПустаяСтрока
	|	КОНЕЦ КАК ОГРН,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ДокументыЭПДСтороны.Сторона) = ТИП(Справочник.Saby_СторонаДокумента)
	|			ТОГДА ДокументыЭПДСтороны.Сторона.ЮрФизЛицо
	|		ИНАЧЕ ЕСТЬNULL(ДокументыЭПДСтороны.Сторона.ЮрФизЛицо, ДокументыЭПДСтороны.Сторона.ЮридическоеФизическоеЛицо)
	|	КОНЕЦ КАК ЮрФизЛицо
	|ИЗ
	|	Справочник.Saby_ДокументыЭПД.Стороны КАК ДокументыЭПДСтороны
	|ГДЕ
	|	ДокументыЭПДСтороны.Ссылка В(&МассивСсылок)";
...
Процедура ПолучитьДанныеФизлиц(МассивФизлиц, МенеджерВременныхТаблиц)
	...
	ЕстьФИО = Saby_ТНОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойство("Фамилия", НоваяЗапись);
	
	ТелефонЕмейл = Новый Массив;
	ТелефонЕмейл.Добавить(Перечисления.ТипыКонтактнойИнформации.Телефон);
	ТелефонЕмейл.Добавить(Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты);
	
	ЗапросДанных = Новый Запрос;
	ЗапросДанных.УстановитьПараметр("МассивФизлиц", МассивФизлиц);
	ЗапросДанных.УстановитьПараметр("ТелефонЕмейл", ТелефонЕмейл);
	...
	|ПОМЕСТИТЬ ТелефоныЕмейлыФизлиц
	|ИЗ
	|	Справочник.ФизическиеЛица.КонтактнаяИнформация КАК ФизическиеЛицаКонтактнаяИнформация
	|ГДЕ
	|	ФизическиеЛицаКонтактнаяИнформация.Ссылка В (&МассивФизлиц)
	|	И ФизическиеЛицаКонтактнаяИнформация.Тип В (&ТелефонЕмейл)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Процедура ЗаполнитьКонтактныеДанные(ФормаОбъект, ТаблицаЮрЛиц, ТаблицаКИ)
	...
	Для Каждого СтрокаЮрЛица Из ТаблицаЮрЛиц Цикл
		...
		МассивКИЕмейлы = Saby_ТНОбщегоНазначенияКлиентСервер.ПолучитьКИ(ТаблицаКИ, СтрокаЮрЛица.Ссылка, ВидыКИ.Емейл, Ложь);
		
		Для Каждого СтруктураКИ Из МассивКИЕмейлы Цикл
	...
	Иначе
		...
		Если ИмяКонфигурации = ДоступныеКонфигурации.Бух3 Тогда
			ЗапросДанных.Текст = СтрЗаменить(
				...
				|		КОГДА Организации.ЮридическоеФизическоеЛицо = ЗНАЧЕНИЕ(Перечисление.ЮридическоеФизическоеЛицо.ФизическоеЛицо)
				|			ТОГДА ВЫБОР
				|					КОГДА Организации.СтатусФизическогоЛица = ЗНАЧЕНИЕ(Перечисление.СтатусыФизическихЛиц.ИндивидуальныйПредприниматель)
				|						ТОГДА ""ИндивидуальныйПредприниматель""
				|					ИНАЧЕ ""ФизЛицо""
				|				КОНЕЦ
				|	КОНЕЦ");
...
Процедура ЗачиститьШтрафы(ОбъектДок, КлючиОтметокДляУдаления)
	...
	Если Не Saby_ТНОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойство("Штрафы", ОбъектДок) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗачиститьАкты(ОбъектДок, КлючиОтметокДляУдаления)
	...
	Если Не Saby_ТНОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойство("Акты", ОбъектДок) Тогда
		Возврат;
	КонецЕсли;
...
Функция ДанныеТранспортныхСредствИзДругихКонфигураций(МассивСсылокТС, МенеджерВременныхТаблиц = Неопределено)
	...
	ЕстьВид = Saby_ТНОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойство(
		"Вид", Справочники.ТранспортныеСредства.ПустаяСсылка());
	
	Если Не ЕстьВид Тогда
		Возврат ДанныеТранспортныхСредствИзДругихКонфигурацийБезРасширения(МассивСсылокТС, МенеджерВременныхТаблиц);
	КонецЕсли;
	...
	ЕстьРегистрационныйЗнак = Saby_ТНОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойство(
		"РегистрационныйЗнак", Справочники.ТранспортныеСредства.ПустаяСсылка());
...
Функция ДанныеТранспортныхСредствИзДругихКонфигурацийБезРасширения(МассивСсылокТС, МенеджерВременныхТаблиц)
	...
	ЕстьТип = Saby_ТНОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойство(
		"Тип", Справочники.ТранспортныеСредства.ПустаяСсылка());
...
Процедура ОбновитьДанныеЛенты(Форма)
	...
	ДанныеДляЛенты = РегистрыСведений.Saby_ДанныеТитулов.ДанныеДляЛенты(Форма.Объект.Ссылка, ИмяМетаданных);
	...
	Для Каждого СтруктураДанных Из ДанныеДляЛенты Цикл
		...
		СтрокаЛенты.ТекущийТитул = ЭтоТекущийТитулЛенты(Форма, ИмяМетаданных, СтрокаЛенты.ТипТитула);
		
		Форма.ЗаполнитьНаименованиеТитула(СтрокаЛенты);
...
Функция ЭтоТекущийТитулЛенты(Форма, ИмяМетаданных, ТипТитула)
	...
	Если ТекущиеЭтапы.Количество() = 0 Тогда
		ЕстьТитулВТекущихЭтапах = МенеджерТитулов.ЭтоПервыйТитул(ТипТитула);
	Иначе
		ЕстьТитулВТекущихЭтапах = ТекущиеЭтапы.Найти(ТипТитула) <> Неопределено;
	КонецЕсли;
	
	Возврат Не ДокументооборотЗавершен И ЕстьТитулВТекущихЭтапах;
...
Процедура ДобавитьДекорациюТитул(Форма, СтрокаЛенты)
	...
	ИначеЕсли ЗначениеЗаполнено(СтрокаЛенты.Период) Тогда
	...
	ДекорацияФормы.Ширина = Форма.ШиринаПоТитулу(СтрокаЛенты.ТипТитула);
...
Функция АвторизованныйАккаунтСтрокой()
