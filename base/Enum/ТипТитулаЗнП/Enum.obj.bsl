...
Функция РезультатФункцииТитуловЗнП(ИмяФункции, Параметр1 = Неопределено,
	...
	ИначеЕсли ИмяФункции = "ПредставлениеПоИмениПодстановки" Тогда
		Возврат ПредставлениеПоИмениПодстановкиЗнП(Параметр1);
	...
	ИначеЕсли ИмяФункции = "ПустаяСсылка" Тогда
		Возврат ЗначениеМетаданных("ESD_ТипТитулаЗнП.ПустаяСсылка");
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
//
// Параметры:
//  ТипТитула - ПеречислениеСсылка.ESD_ТипТитулаЗнП - титул, для которого необходимо получить титул отказа.
//
// Возвращаемое значение:
//   Структура - данные титула отказа:
//     * Тип - ПеречислениеСсылка.ESD_ТипТитулаЗнП - титул, который был отменен.
...
//
// Возвращаемое значение:
//   ПеречислениеСсылка.ESD_ТипТитулаЗнП - ссылка на перечисление.
...
//     Если по коду ссылка не найдена, то возвращается пустая ссылка.
//
Функция ПредставлениеПоИмениПодстановкиЗнП(Знач ИмяПодстановки) Экспорт
	Перем Результат;
...
//
// Параметры:
//  СсылкаНаТитул - ПеречислениеСсылка.ESD_ТипТитулаЗнП - ссылка на титул,
...
Функция ПредставленияТитулаЗнП(СсылкаНаТитул) Экспорт
	...
	Если ЭтоПервыйТитулЗнП(СсылкаНаТитул) Тогда
		Возврат МодульТНОбщегоНазначенияСервер.СтруктураПредставленияТитула(
			Оформление, "1110361", "ИнформацияГрузоотправителя");
	ИначеЕсли СсылкаНаТитул = Утверждение Тогда
		Возврат МодульТНОбщегоНазначенияСервер.СтруктураПредставленияТитула(
			Утверждение, "1110362", "ИнформацияГрузоперевозчика");
	Иначе
		Возврат МодульТНОбщегоНазначенияСервер.СтруктураПредставленияТитула(ПустаяСсылка(), "", "");
	КонецЕсли;
...
//
// Параметры:
//  ТипТитула - ПеречислениеСсылка.ESD_ТипТитулаЗнП - титул для проверки
//
// Возвращаемое значение:
...
//
// Возвращаемое значение:
//   ПеречислениеСсылка.ESD_ТипТитулаЗнП - первый титул
//
...
Процедура СформироватьТаблицуДополненияЗнП(МенеджерВременныхТаблиц, ТекущиеЭтапыДокумента, СсылкаНаДокумент) Экспорт
	...
	|	&Оформление КАК ТипТитула,
	|	"""" КАК ИдентификаторТитула,
	|	ИСТИНА КАК Основной,
	|	"""" КАК Данные,
	|	&ПустаяСсылка КАК ТипТитулаОтказа,
	|	0 КАК ТипТитулаПорядок,
	|	&Оформление В (&ТекущиеЭтапыДокумента) КАК ЭтоТекущийЭтап
	...
	|	&ПустаяСсылка,
	|	1,
	|	&Утверждение В (&ТекущиеЭтапыДокумента)";
	
	ЗапросДанных.УстановитьПараметр("Оформление",   Оформление);
	ЗапросДанных.УстановитьПараметр("Утверждение",  Утверждение);
