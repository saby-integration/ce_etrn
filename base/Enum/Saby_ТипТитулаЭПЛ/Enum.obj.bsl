...
Функция РезультатФункцииТитуловЭПЛ(ИмяФункции, Параметр1 = Неопределено,
	...
	ИначеЕсли ИмяФункции = "ПустаяСсылка" Тогда
		Возврат ЗначениеМетаданных("Saby_ТипТитулаЭПЛ.ПустаяСсылка");
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура СформироватьТаблицуДополненияЭПЛ(МенеджерВременныхТаблиц, ТекущиеЭтапыДокумента, СсылкаНаДокумент) Экспорт
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		Saby_ПутевойЛист.ПЛПредыдущий <> """"
	|			ИЛИ Saby_ПутевойЛист.ДокументОснование_Идентификатор <> """"
	|				И Saby_ПутевойЛист.ДокументОснование_ОбъектМетаданных = ""Saby_ПутевойЛист"" КАК ЭтоЗаменаВодителя
	|	ИЗ
