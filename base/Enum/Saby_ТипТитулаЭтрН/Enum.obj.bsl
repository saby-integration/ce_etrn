...
Процедура СформироватьТаблицуДополнения(МенеджерВременныхТаблиц, ТекущиеЭтапыДокумента, СсылкаНаДокумент) Экспорт
	...
	|	ЗНАЧЕНИЕ(Перечисление.Saby_ТипТитулаЭтрН.Погрузка) В (&ТекущиеЭтапыДокумента) КАК ЭтоТекущийЭтап
	...
	|	2,
	|	ЗНАЧЕНИЕ(Перечисление.Saby_ТипТитулаЭтрН.ПолучениеГруза) В (&ТекущиеЭтапыДокумента)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ЗНАЧЕНИЕ(Перечисление.Saby_ТипТитулаЭтрН.ПустаяСсылка),
	|	3,
	|	ЗНАЧЕНИЕ(Перечисление.Saby_ТипТитулаЭтрН.ПриемкаГруза) В (&ТекущиеЭтапыДокумента)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ЗНАЧЕНИЕ(Перечисление.Saby_ТипТитулаЭтрН.ПустаяСсылка),
	|	4,
	|	ЗНАЧЕНИЕ(Перечисление.Saby_ТипТитулаЭтрН.ВыдачаГруза) В (&ТекущиеЭтапыДокумента)";
...
Функция СтруктураПредставленияТитула(ТипТитула, КодТитула, ПрефиксПодстановки = "ЭТрН_")
	...
	РезультатФункции.Вставить("НазваниеСобытияОтрицательногоПерехода", НазваниеСобытияОтрицательногоПерехода(ТипТитула));
	
	Возврат РезультатФункции;
	
КонецФункции

Функция НазваниеСобытияОтрицательногоПерехода(ТипТитула)
	
	Если ТипТитула = ПриемкаГруза Тогда
		РезультатФункции = "Приемка груза.Не принят";
	ИначеЕсли ТипТитула = ВыдачаГруза Тогда
