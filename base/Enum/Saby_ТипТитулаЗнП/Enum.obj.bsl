...
Функция ЗначениеПоИмениПодстановки(Знач ИмяПодстановки) Экспорт
	...
	Если ИмяПодстановки = "ИнформацияГрузоотправителя" Тогда
		Возврат Оформление;
	ИначеЕсли ИмяПодстановки = "ИнформацияПеревозчика" Тогда
		Возврат Подтверждение;
	Иначе
		Возврат ПустаяСсылка();
	КонецЕсли;
...
Функция ЗначениеПоПредставлению(Представление) Экспорт
	...
	Если Представление = "Информация грузоотправителя" Тогда
		Возврат Оформление;
	ИначеЕсли Представление = "Информация перевозчика" Тогда
		Возврат Подтверждение;
	Иначе
...
Функция ЗначениеПоЭтапу(НаименованиеЭтапа) Экспорт
	...
	Если НаименованиеЭтапа = "Заказ (заявка) информация грузоотправителя" Тогда
		Возврат Оформление;
	ИначеЕсли НаименованиеЭтапа = "Заказ (заявка) информация перевозчика" Тогда
		Возврат Подтверждение;
	Иначе
		Возврат ПустаяСсылка();
	КонецЕсли;
...
Функция ПредставленияТитула(СсылкаНаТитул) Экспорт
	...
	ИначеЕсли СсылкаНаТитул = Подтверждение Тогда
		Возврат Saby_ТНОбщегоНазначенияСервер.СтруктураПредставленияТитула(
			Подтверждение, "1110362", "ИнформацияПеревозчика");
	Иначе
...
Процедура СформироватьТаблицуДополнения(МенеджерВременныхТаблиц) Экспорт
	...
	|	ЗНАЧЕНИЕ(Перечисление.Saby_ТипТитулаЗнП.Подтверждение),
	|	"""",
	|	ИСТИНА,
	|	"""",
