...
//
// Параметры: 
// 	Титулы - Массив - данные по титулам в формате подстановок для записи 
//  ОбъектДок - ДокументОбъект.ESD_ТранспортнаяНакладная - объект исходного документа, по которому пишем данные
//  ИмяМетаданных - Строка - имя метаданных реквизита СсылкаНаДокумент
//
Процедура ЗаписатьДанные(Титулы, ОбъектДок, ИмяМетаданных) Экспорт
	
	Если Титулы = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого СтруктураТитула Из Титулы Цикл
		
		Если Не СтруктураТитула.Свойство("ДляРегистра")
			Или СтруктураТитула.ДляРегистра = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		СтруктураТитулаДляРегистра = СтруктураТитулаДляРегистра(СтруктураТитула, ИмяМетаданных);
...
//
// Параметры:
//  ОбъектДок - ДокументСсылка.ESD_ТранспортнаяНакладная - документ, по которому загружаются данные.
//  Титул - ПеречислениеСсылка.ESD_ТипТитулаЭтрН - загружаемый титул из СБИС.
...
//    ESD_ТНЗагрузкаСервер.ДопПараметрыЗагрузкиВложения
//
...
Функция ВложенияДляЗагрузки(ОбъектДок, Титул, Вложения, ДопПараметры) Экспорт
	...
	ТаблицаВложений.Колонки.Добавить("Идентификатор", МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ОписаниеТипаСтрока("36"));
	ТаблицаВложений.Колонки.Добавить(
		"Дата",
		МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ОписаниеТипаДата(ЧастиДаты.ДатаВремя));
	
	СоответствиеВложений = Новый Соответствие;
	Для Каждого СтруктураВложения Из Вложения Цикл
		Идентификатор = СтруктураВложения["ИдентификаторВложенияТитула"];
		...
		СтрокаВложения.Дата          = СтруктураВложения["ДатаВложенияТитула"];
	КонецЦикла;
	...
	|ИЗ
	|	ESD_ДанныеТитулов КАК ESD_ДанныеТитулов
	|ГДЕ
	|	ESD_ДанныеТитулов.ТипТитула = &ТипТитула
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МАКСИМУМ(ESD_ДанныеТитулов.Период) КАК Период,
	...
	|		ПО ESD_ДанныеТитулов.СсылкаНаДокумент = ДанныеТаблицыВложений.СсылкаНаДокумент
	|			И ESD_ДанныеТитулов.Период <= ДанныеТаблицыВложений.Дата
	|			И ESD_ДанныеТитулов.ИдентификаторТитула <> ДанныеТаблицыВложений.Идентификатор
	|ГДЕ
	|	&ЭтоУведомлениеОбУточнении
	|
	|СГРУППИРОВАТЬ ПО
	|	ESD_ДанныеТитулов.СсылкаНаДокумент,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ESD_ДанныеТитулов КАК ESD_ДанныеТитулов
	|		ПО ПериодПредыдущегоТитула.СсылкаНаДокумент = ESD_ДанныеТитулов.СсылкаНаДокумент
	|			И ПериодПредыдущегоТитула.Период = ESD_ДанныеТитулов.Период
	|			И ПериодПредыдущегоТитула.Идентификатор <> ESD_ДанныеТитулов.ИдентификаторТитула
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	ЗапросДанных.УстановитьПараметр(
		"ЭтоУведомлениеОбУточнении",
		ЗначениеМетаданных("ESD_ТипТитулаЭтрН.УведомлениеОУточнении") = Титул);
		
	ЗапросДанных.УстановитьПараметр(
		"ПустаяСсылкаНаТитул",
		МодульКода("ESD_ТНОбщегоНазначенияСервер").РезультатФункцииТитулов(ИмяМетаданных, "ПустаяСсылка"));
	...
	Пока ВыборкаДанных.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ВыборкаДанных.ТипПрошлогоТитула) Тогда
			СтруктураВложения.Вставить(
				"ПрошлыйТитул",
				МодульКода("ESD_ТНОбщегоНазначенияСервер").РезультатФункцииТитулов(
...
//
// Параметры:
//  СсылкаНаДокумент - ДокументСсылка.ESD_ТранспортнаяНакладная - документ для записи данных в регистр.
...
//
// Параметры:
//  ОбъектДок - ДокументСсылка.ESD_ТранспортнаяНакладная - документ, для которого нужны данные ленты.
//  ИмяМетаданных - Строка - имя метаданных реквизита СсылкаНаДокумент
...
Функция ДанныеДляЛенты(ОбъектДок, ИмяМетаданных, ТекущиеЭтапыДокумента) Экспорт
	...
	ЭтоВО = МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ЭтоФормаПриложения(ОбъектДок);
	Если ЭтоВО Тогда
		СформироватьДанныеТитулов(МенеджерВременныхТаблиц, ОбъектДок);
	Иначе
		СформироватьДанныеТитулов(МенеджерВременныхТаблиц, ОбъектДок.Ссылка);
	...
	|	ESD_ДанныеТитулов.ТипТитулаПорядок КАК ТипТитулаПорядок
	...
	|
	|ВЫБРАТЬ
	|	ДополнительныеДанныеТитулов.ТипТитула,
	|	ДополнительныеДанныеТитулов.Период,
	|	ДополнительныеДанныеТитулов.ИдентификаторТитула,
	|	ДополнительныеДанныеТитулов.Основной,
	|	ДополнительныеДанныеТитулов.Данные,
	|	ДополнительныеДанныеТитулов.ТипТитулаОтказа,
	|	ДополнительныеДанныеТитулов.ТипТитулаПорядок
	|ИЗ
	|	ДополнительныеДанныеТитулов КАК ДополнительныеДанныеТитулов
	|ГДЕ
	|	ДополнительныеДанныеТитулов.ТипТитулаПорядок > -1
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МАКСИМУМ(ТаблицаДополнения.ТипТитулаПорядок) КАК ТипТитулаПорядок
	|ПОМЕСТИТЬ НомерТекущегоЭтапа
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|	ГДЕ
	|		ТаблицаДополнения.ЭтоТекущийЭтап
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ -1
	|	) КАК ТаблицаДополнения
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ТаблицаДополнения.ТипТитулаПорядок КАК ТипТитулаПорядок
	|ПОМЕСТИТЬ СледующиеЭтапы
	|ИЗ
	|	ТаблицаДополнения КАК ТаблицаДополнения
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ НомерТекущегоЭтапа КАК НомерТекущегоЭтапа
	|		ПО ТаблицаДополнения.ТипТитулаПорядок > НомерТекущегоЭтапа.ТипТитулаПорядок
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеТитулов КАК ДанныеТитулов
	|		ПО ТаблицаДополнения.ТипТитула = ДанныеТитулов.ТипТитула
	|ГДЕ
	|	ВЫБОР 
	|		КОГДА НомерТекущегоЭтапа.ТипТитулаПорядок = -1 Тогда 
	|			ДанныеТитулов.ТипТитула ЕСТЬ NULL
	|		ИНАЧЕ
	|			ИСТИНА
	|	КОНЕЦ
	|;
	...
	|
	|ВЫБРАТЬ
	|	СледующиеЭтапы.Период,
	|	СледующиеЭтапы.ТипТитула,
	|	СледующиеЭтапы.ИдентификаторТитула,
	|	СледующиеЭтапы.Основной,
	|	СледующиеЭтапы.Данные,
	|	СледующиеЭтапы.ТипТитулаОтказа,
	|	СледующиеЭтапы.ТипТитулаПорядок,
	|	ЛОЖЬ
	|ИЗ
	|	СледующиеЭтапы КАК СледующиеЭтапы
	|
	|УПОРЯДОЧИТЬ ПО
	|	Период,
	|	ТипТитулаПорядок";
	...
	ПустаяСсылкаТитула = МодульКода("ESD_ТНОбщегоНазначенияСервер").РезультатФункцииТитулов(ИмяМетаданных, "ПустаяСсылка");
	
	Пока ВыборкаДанных.Следующий() Цикл
		
		ПериодТитулаОтказа = ТитулыОтказа.Получить(ВыборкаДанных.ТипТитула);
		Если ПериодТитулаОтказа <> Неопределено И (ВыборкаДанных.Основной Или ПериодТитулаОтказа >= ВыборкаДанных.Период) Тогда
			Продолжить;
		КонецЕсли;
		
		Если ВыборкаДанных.ТипТитулаОтказа <> ПустаяСсылкаТитула Тогда
			ТитулыОтказа.Вставить(ВыборкаДанных.ТипТитулаОтказа, ВыборкаДанных.Период);
		КонецЕсли;
...
// Возвращает данные последних титулов по списку переданных ссылок на документы
//
// Параметры:
//  СписокДокументов - Массив - массив ссылок на документы.
//
// Возвращаемое значение:
//  ТаблицаЗначений - таблица данных титулов
Функция ДанныеПоследнихТитулов(СписокДокументов) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = "
		|ВЫБРАТЬ
		|	АктуальныеДанныеТитулов.СсылкаНаДокумент КАК СсылкаНаДокумент,
		|	АктуальныеДанныеТитулов.ТипТитула КАК ТипТитула,
		|	АктуальныеДанныеТитулов.ИдентификаторТитула КАК ИдентификаторТитула,
		|	АктуальныеДанныеТитулов.Основной КАК Основной,
		|	АктуальныеДанныеТитулов.Данные КАК Данные,
		|	АктуальныеДанныеТитулов.Просмотрено КАК Просмотрено,
		|	АктуальныеДанныеТитулов.ТипТитулаОтказа КАК ТипТитулаОтказа,
		|	АктуальныеДанныеТитулов.Период КАК Период
		|ПОМЕСТИТЬ АктуальныеДанныеТитулов
		|ИЗ
		|	РегистрСведений.ESD_ДанныеТитулов.СрезПоследних(, 
		|   	СсылкаНаДокумент В(&СписокДокументов)
		|	) КАК АктуальныеДанныеТитулов
		|;/////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	АктуальныеДанныеТитулов.СсылкаНаДокумент КАК СсылкаНаДокумент,
		|	МАКСИМУМ(АктуальныеДанныеТитулов.Период) КАК Период
		|ПОМЕСТИТЬ МаксПериодДанных
		|ИЗ
		|	АктуальныеДанныеТитулов КАК АктуальныеДанныеТитулов
		|СГРУППИРОВАТЬ ПО
		|	АктуальныеДанныеТитулов.СсылкаНаДокумент
		|;////////////////////////
		|ВЫБРАТЬ
		|	АктуальныеДанныеТитулов.СсылкаНаДокумент КАК СсылкаНаДокумент,
		|	АктуальныеДанныеТитулов.ТипТитула КАК ТипТитула,
		...
		|	АктуальныеДанныеТитулов.Период КАК Период
		|ИЗ
		|	МаксПериодДанных КАК МаксПериодДанных
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ АктуальныеДанныеТитулов КАК АктуальныеДанныеТитулов
		|		ПО МаксПериодДанных.СсылкаНаДокумент = АктуальныеДанныеТитулов.СсылкаНаДокумент
		|			И МаксПериодДанных.Период = АктуальныеДанныеТитулов.Период
		|";
	Запрос.УстановитьПараметр("СписокДокументов", СписокДокументов);
	
	РезультатТЗ = Запрос.Выполнить().Выгрузить();
	Если РезультатТЗ.Количество() < 2 Тогда
		Возврат РезультатТЗ;
	КонецЕсли;
	
	Инд = РезультатТЗ.Количество() - 2;
	Пока Инд >= 0 Цикл
		Если РезультатТЗ[Инд].СсылкаНаДокумент = РезультатТЗ[Инд + 1].СсылкаНаДокумент Тогда
			РезультатТЗ.Удалить(Инд);
		КонецЕсли;
		Инд = Инд - 1;
	КонецЦикла;
...
Функция СтруктураДанных()
	...
	РезультатФункции.Вставить("ТипТитула",           ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ПустаяСсылка"));
...
Функция СтруктураТитулаДляРегистра(СтруктураТитула, ИмяМетаданных)
	...
	Иначе
		...
		ПустаяСсылкаТитула   = МодульКода("ESD_ТНОбщегоНазначенияСервер").РезультатФункцииТитулов(ИмяМетаданных, "ПустаяСсылка");
		ЭтоДинамическийТитул = МодульКода("ESD_ТНОбщегоНазначенияСервер").РезультатФункцииТитулов(
...
Процедура ЗаписатьДанныеТитулаПоОбъекту(ОбъектДок, СтруктураТитулаДляРегистра)
	...
	Если ЗначениеЗаполнено(ОбъектДок.Ссылка) И ТипЗнч(ОбъектДок.Ссылка) <> Тип("Структура") Тогда
		...
		ЗаполнитьЗначенияСвойств(МенеджерЗаписи, СтруктураТитулаДляРегистра);
		МенеджерЗаписи.Записать();
	ИначеЕсли МодульКода("ESD_ТНОбщегоНазначенияКлиентСервер").ЕстьРеквизитИлиСвойство(ОбъектДок, "ДополнительныеСвойства") Тогда
...
Процедура ЗаполнитьОписание(СтруктураДанных)
	...
	Если СтруктураДанных.ТипТитула = ЗначениеМетаданных("ESD_ТипТитулаЭтрН.УведомлениеОУточнении")
		Или СтруктураДанных.ТипТитула = ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ПриемкаГруза")
		Или СтруктураДанных.ТипТитула = ЗначениеМетаданных("ESD_ТипТитулаЭтрН.ВыдачаГруза") Тогда
