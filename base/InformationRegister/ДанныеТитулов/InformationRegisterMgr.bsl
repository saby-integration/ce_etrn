...
Процедура ЗаписатьДанные(ДинамическиеТитулы, ОбъектДок, ИмяМетаданных) Экспорт
	...
	Для Каждого СтруктураДинамическогоТитула Из ДинамическиеТитулы Цикл
		...
		Иначе // Для ВО пишем данные в реквизит формы
			СтрокаДанныхТитула = ОбъектДок.ДанныеТитулов.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаДанныхТитула, СтруктураТитулаДляРегистра);
		КонецЕсли;
...
Функция ВложенияДляЗагрузки(ОбъектДок, Титул, Вложения, ДопПараметры) Экспорт
	...
	Если ЭтоВО Тогда
		ИмяМетаданных = СтрРазделить(ОбъектДок.ИмяФормы, ".");
		ИмяМетаданных = ИмяМетаданных[ИмяМетаданных.ВГраница()];
...
Функция СтруктураТитулаДляРегистра(СтруктураТитула, ИмяМетаданных)
	...
	Иначе
		Если ЗначениеЗаполнено(СтруктураТитула.ДляРегистра.Данные) Тогда
			РезультатФункции.Данные = ЗначениеВСтрокуВнутр(СтруктураТитула.ДляРегистра.Данные);
		Иначе
...
Процедура СформироватьДанныеТитуловВО(МенеджерВременныхТаблиц, ФормаДокумента)
	...
	ЗапросДанных.УстановитьПараметр("ДанныеТитулов", ФормаДокумента.ДанныеТитулов.Выгрузить());
