...
Функция ВложенияДляЗагрузки(ОбъектДок, Титул, Вложения, ДопПараметры) Экспорт
	...
	Если ЭтоВО Тогда
		...
		СформироватьДанныеТитуловВО(МенеджерВременныхТаблиц, ОбъектДок);
	Иначе
		ИмяМетаданных = ОбъектДок.Метаданные().Имя;
		СформироватьДанныеТитуловРасширение(МенеджерВременныхТаблиц, ОбъектДок.Ссылка);
	КонецЕсли;
...
Функция ДанныеДляЛенты(ОбъектДок, ИмяМетаданных, ТекущиеЭтапыДокумента) Экспорт
	...
	ЗапросДанных.УстановитьПараметр("СсылкаНаДокумент", ОбъектДок.Ссылка);
	...
	Пока ВыборкаДанных.Следующий() Цикл
		...
		Если ТитулыОтказа.Получить(ВыборкаДанных.ТипТитула) <> Неопределено
			И ЗначениеЗаполнено(ВыборкаДанных.ИдентификаторТитула) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура СформироватьДанныеТитуловВО(МенеджерВременныхТаблиц, ОбъектДок)
	...
	ДанныеТитулов.Колонки.Добавить("ТипТитула",           МодульТНОбщегоНазначенияКлиентСервер.ОписаниеТипаСтрока(32));
	ДанныеТитулов.Колонки.Добавить("ИдентификаторТитула", МодульТНОбщегоНазначенияКлиентСервер.ОписаниеТипаСтрока(36));
	...
	ДанныеТитулов.Колонки.Добавить("ТипТитулаОтказа",     МодульТНОбщегоНазначенияКлиентСервер.ОписаниеТипаСтрока(32));
	ДанныеТитулов.Колонки.Добавить("ТипТитулаПорядок",    Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(2)));
...
Процедура СформироватьДанныеТитуловРасширение(МенеджерВременныхТаблиц, СсылкаНаДокумент)
