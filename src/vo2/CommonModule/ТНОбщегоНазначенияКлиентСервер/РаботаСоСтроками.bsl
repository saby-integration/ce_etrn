...
// 	 РазделительЦеликом - Булево - Признак составного разделителя 
//
// Возвращаемое значение:
//   Массив - Массив со строками, которые получились в результате разделения исходной строки.
//
Функция СтрРазделитьЭПД(Знач ИсходнаяСтрока, Разделитель, ВключатьПустые = Истина, РазделительЦеликом = Ложь) Экспорт
	
	ДлинаРазделителя = СтрДлина(Разделитель);	
	Если ДлинаРазделителя > 1 
		И Не РазделительЦеликом Тогда
		
		// делим посимвольно по составному разделителю
		Возврат РазделитьСтрокуПоСоставномуРазделителю(ИсходнаяСтрока, Разделитель, ВключатьПустые); 
...
Функция СтрШаблонЭПД(Знач Шаблон, Параметр1 = Неопределено, Параметр2 = Неопределено, Параметр3 = Неопределено,
	...
	СтрШаблонЭПДОбработатьПараметр(Результат, Параметр10,  НомерПараметра);
	НомерПараметра = НомерПараметра - 1;
	СтрШаблонЭПДОбработатьПараметр(Результат, Параметр9,   НомерПараметра);
	НомерПараметра = НомерПараметра - 1;
	СтрШаблонЭПДОбработатьПараметр(Результат, Параметр8,   НомерПараметра);
	НомерПараметра = НомерПараметра - 1;
	СтрШаблонЭПДОбработатьПараметр(Результат, Параметр7,   НомерПараметра);
	НомерПараметра = НомерПараметра - 1;
	СтрШаблонЭПДОбработатьПараметр(Результат, Параметр6,   НомерПараметра);
	НомерПараметра = НомерПараметра - 1;
	СтрШаблонЭПДОбработатьПараметр(Результат, Параметр5,   НомерПараметра);
	НомерПараметра = НомерПараметра - 1;
	СтрШаблонЭПДОбработатьПараметр(Результат, Параметр4,   НомерПараметра);
	НомерПараметра = НомерПараметра - 1;
	СтрШаблонЭПДОбработатьПараметр(Результат, Параметр3,   НомерПараметра);
	НомерПараметра = НомерПараметра - 1;
	СтрШаблонЭПДОбработатьПараметр(Результат, Параметр2,   НомерПараметра);
	НомерПараметра = НомерПараметра - 1;
	СтрШаблонЭПДОбработатьПараметр(Результат, Параметр1,   НомерПараметра);
...
//DynamicDirective
// Аналог функции СтрРазделить(), доступной с версии 8.3.6.
// Разделяет строку на части по указанным символам-разделителям посимвольно.
// Специализируется на делении по составному разделителю.
//
// Параметры:
//   ИсходнаяСтрока - Строка - Разделяемая строка.
//   Разделитель    - Строка - Строка символов, каждый из которых является индивидуальным разделителем.
...
//   Массив - Массив со строками, которые получились в результате разделения исходной строки.
//
Функция РазделитьСтрокуПоСоставномуРазделителю(ИсходнаяСтрока, Разделитель, ВключатьПустые) Экспорт
	
	КоличествоСимволов = СтрДлина(ИсходнаяСтрока);
	Подстроки = Новый Массив;
	
	СтрокаБезРазделителя = "";
	...
	Для Позиция = 1 По КоличествоСимволов Цикл
		...
		Если Найти(Разделитель, Символ) = 0 Тогда
			// не разделитель
			СтрокаБезРазделителя = СтрокаБезРазделителя + Символ;
		Иначе
			
			// разделитель
			Если ЗначениеЗаполнено(СтрокаБезРазделителя) Тогда
				Добавить = Истина;
			КонецЕсли;
			
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(СтрокаБезРазделителя) Тогда
			Продолжить;
		КонецЕсли;
		
		Если Добавить Или Позиция = КоличествоСимволов Тогда
			
			Подстроки.Добавить(СтрокаБезРазделителя);
			СтрокаБезРазделителя = "";
