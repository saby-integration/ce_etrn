...
Процедура ОткрытьДокументЭПД(ПараметрыДокумента, СтандартнаяОбработка, ИндексЭПД = Неопределено) Экспорт
	...
	Иначе
		...
		Если ПараметрыДокумента.ТипЭПД = "ДокументТС" Тогда
...
Процедура НачатьРедактированиеДокументаЭПД(Форма, СтруктураДокумента, Тип, ЗаголовокДляФормы)
	...
	ОповещениеПослеРедактирования = Новый ОписаниеОповещения(
		"ПослеРедактированияДокументаЭПД", МодульКодаКлиент("ЭтотОбъект"), ДополнительныеПараметры);
...
Процедура ЗаполнитьДанныеДокументаЭПД(Результат, ДополнительныеПараметры) Экспорт
	...
	ЗаполнитьДанныеАктов(ПараметрыСтроки, Результат, Строка);
...
Процедура ЗаполнитьДанныеАктов(ПараметрыСтроки, Результат, Строка)
	
	Если ПараметрыСтроки.ТипЭПД <> "Акты" Тогда
		Возврат;
	КонецЕсли;
		
	Если ЭтаФорма.ТекущийЭлемент.Имя = "Акты_Выгрузка" Тогда
		СтрокаАкта = Элементы.Акты_Выгрузка.ТекущиеДанные;
	Иначе	
		СтрокаАкта = Элементы.Акты.ТекущиеДанные;
	КонецЕсли;
	
	СтрокаАкта.Документ                = Результат.Наименование;
	СтрокаАкта.КлючСтроки_ДокументыЭПД = Строка.КлючСтроки;
...
Процедура АктыОткрыть(ИмяЭлемента, СтандартнаяОбработка) Экспорт
	...
	Если Не ЗначениеЗаполнено(ТекущиеДанные.КлючСтроки_ДокументыЭПД) Тогда
